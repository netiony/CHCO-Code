...
dir.data <- c("/Users/hhampson/Library/CloudStorage/OneDrive-UniversityofSouthernCalifornia/Chatzi Projects and Analyses/Env Chem SOL-CHS/Analysis/1_analysis_ready_data/metabolomics")

```

#2. Load & Clean Data
```{r load clean data, include=FALSE, echo=FALSE}
#Set digit options to avoid rounding
options(digits = 10)

#Load feature tables ----
ftrs <- readRDS(fs::path(dir.dat, "sol_chs_batch_cor_scaled_untargeted_fts.rds"))

#Select CHS features only 
ftrs <- ftrs$chs

## C18 Negative Features ----
# c18neg_ftrs_table <- data.frame(ftrs$c18neg)
c18neg_ftrs <- data.frame(ftrs$c18neg)
dim(c18neg_ftrs) # 3716 features
c18neg_ftrs <- data.frame(mz_rt=str_remove(colnames(c18neg_ftrs)[-1],"X"))
c18neg_ftrs <- c18neg_ftrs %>% 
  mutate(mz=str_remove(mz_rt,"_.*"))%>%
  mutate(rt=str_remove(mz_rt,".*_"))
c18neg_ftrs$mz <- round(as.numeric(c18neg_ftrs$mz),digit=5)
c18neg_ftrs$rt <- round(as.numeric(c18neg_ftrs$rt),digit=1)


## C18 Positive Features ----
c18pos_ftrs <- data.frame(ftrs$c18pos) 
dim(c18pos_ftrs) #5069 features
c18pos_ftrs <- data.frame(mz_rt=str_remove(colnames(c18pos_ftrs)[-1],"X"))
c18pos_ftrs <- c18pos_ftrs %>% 
  mutate(mz=str_remove(mz_rt,"_.*"))%>%
  mutate(rt=str_remove(mz_rt,".*_"))
c18pos_ftrs$mz <- round(as.numeric(c18pos_ftrs$mz),digit=5)
c18pos_ftrs$rt <- round(as.numeric(c18pos_ftrs$rt),digit=1)


## Hilic Negative Features ----
hilicneg_ftrs <- data.frame(ftrs$hilicneg)  
dim(hilicneg_ftrs) #7444 features
hilicneg_ftrs <- data.frame(mz_rt=str_remove(colnames(hilicneg_ftrs)[-1],"X"))
hilicneg_ftrs <- hilicneg_ftrs %>% 
  mutate(mz=str_remove(mz_rt,"_.*"))%>%
  mutate(rt=str_remove(mz_rt,".*_"))
hilicneg_ftrs$mz <- round(as.numeric(hilicneg_ftrs$mz),digit=5)
hilicneg_ftrs$rt <- round(as.numeric(hilicneg_ftrs$rt),digit=1)


## Hilic Positive Features ----
hilicpos_ftrs <- data.frame(ftrs$hilicpos) 
dim(hilicpos_ftrs) #6945 features
hilicpos_ftrs <- data.frame(mz_rt=str_remove(colnames(hilicpos_ftrs)[-1],"X"))
hilicpos_ftrs <- hilicpos_ftrs %>% 
  mutate(mz=str_remove(mz_rt,"_.*"))%>%
  mutate(rt=str_remove(mz_rt,".*_"))
hilicpos_ftrs$mz <- round(as.numeric(hilicpos_ftrs$mz),digit=5)
hilicpos_ftrs$rt <- round(as.numeric(hilicpos_ftrs$rt),digit=1)


#Load Confirmed Annotations ----
conf_ftrs <- fread(fs::path(dir.dat,"chs_batch_cor_scaled_confirmed_metabolites V3.csv")) %>% 
  janitor::clean_names() %>% 
  rename(rt=time) %>% 
  filter(!grepl("Exogenous",standard_grp)) %>% #filter to endogenous only
  filter(!grepl("Pesticides",standard_class)) %>% 
  filter(!grepl("PFAS",standard_class))
dim(conf_ftrs) #454 confirmed features, 377 endogenous
# rm(ftrs)

##C18 Negative Confirmed Annotations ----
c18neg_conf_ftrs <- conf_ftrs %>% 
  filter(mode=="c18neg")

##C18 Positive Confirmed Annotations ----
c18pos_conf_ftrs <- conf_ftrs %>% 
  filter(mode=="c18pos")

##Hilic Negative Confirmed Annotations ----
hilicneg_conf_ftrs <- conf_ftrs %>% 
  filter(mode=="hilicneg")

##Hilic Positive Confirmed Annotations ----
hilicpos_conf_ftrs <- conf_ftrs %>% 
...
# exposure <- c("ma_pfos","ma_pfoa","ma_pfda","ma_pfna","ma_pfhxs","ma_pfhps","ma_pfpes")
# outcome  <- c("eGFR_log","LCN2_log")
outcome_log_scaled
exposure_dat <- meta_filt[exposure_log_scaled]
# exposure_dat <- meta_filt[burden_score[1]]
# outcome_dat  <- meta_filt[outcome_log_scaled[c(1)]]
outcome_dat  <- meta_filt[outcome_log_scaled]


# outcome_dat <- outcome_dat %>% 
#   mutate(eGFR=exp(eGFR_log),
#          LCN2=exp(LCN2_log)) %>% 
#   select(-c(eGFR_log,LCN2_log))
# outcome_dat <- outcome_dat[[1]]
ids <- data.frame(id=meta_filt$id)
...
#Reduce metabolites first
# metab.data <- metab.data[c("id",sig.metabolites2)]
metabolite <- metabolites$key
# metab.data <- metab.data[c("id",metabolite_features)]
metab.data <- metab.data[c("id",metabolite)]
omics_df <- metab.data
omics_df$id <- as.numeric(omics_df$id)
omics_df <- tidylog::left_join(ids,omics_df,by="id")
omics_df <- omics_df %>%
  dplyr::select(-id)
metabolomics <- as.matrix(omics_df)
#Filter metabolites to only those selected from data reduction
# microbiome <- as.matrix(meta_filt[microbiome_features])
microbiome <- as.matrix(meta_filt[microbiome])
# omics_lst <- list(metabolomics = metabolomics,microbiome=microbiome)
omics_lst <- list(microbiome=microbiome,metabolomics = metabolomics)

#Select only PFAS associated with eGFR:
# exposure.scaled <- c("ma_pfos_scaled","ma_pfoa_scaled","ma_pfna_scaled","ma_pfhxs_scaled","ma_pfpes_scaled")
exposure.scaled <- c("ma_pfos_scaled","ma_pfoa_scaled","ma_pfna_scaled","ma_pfhxs_scaled","ma_pfpes_scaled","ma_pfhps_scaled","ma_pfda_scaled")


# eGFR <- "eGFR_log_scaled"
...
meta_filt_microbiome <- meta_filt[c("id",exposure_log_scaled,outcome_log_scaled,microbiome_scaled)]
mean(meta_filt$eGFR3)
sd(meta_filt$eGFR3)

mean(meta_filt$cystatin_c)
sd(meta_filt$cystatin_c)


log2(meta_filt$eap.pfas)
meta_filt$ma_pfna
```
