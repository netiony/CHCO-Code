...
#Filter to CROCODILE & Diabetes
dat_harm <- dat_harm %>% 
  filter(study=="CROCODILE" | study=="PENGUIN") 
PENGUIN <- dat_harm %>% 
  filter(study=="PENGUIN") 

#Metabolomics & proteomics
met.names <- c("record_id",colnames(dat_harm)[which(grepl("c000",colnames(dat_harm)))])
met <- dat_harm %>% 
  # filter(procedure == "metabolomics_blood") %>% 
  dplyr::select(all_of(met.names))
prot.names <- c("record_id",colnames(dat_harm)[which(grepl("seq.",colnames(dat_harm)))])
prot <- dat_harm %>% 
  # filter(procedure == "metabolomics_blood") %>% 
  dplyr::select(all_of(prot.names))
