...
df = df %>% 
  ...
  select(ID,visit,age,SEX,RACE,ETHN,HBA1C)
# Categorical levels
...
df = read.sas7bdat("/Volumes/Peds Endo/Petter Bjornstad/Teen Labs/Data_Cleaned/bjornstadancillary.sas7bdat")
df$albuminuria[df$UACRATIO>=30] <-1 
```
...
# Filter and select
df = df %>% select(record_id,co_enroll_id,study,visit,age,sex,race_ethnicity,hba1c)%>%
...
df$race_ethnicity=factor(df$race_ethnicity,levels = c("Black or African American, Not Hispanic or Latino",
       ...
       labels = c("Non-Hispanic Black","Other","Other","Other","Other","Other","Other","Non-Hispanic White"))
...
df %>% filter(`Somalogic Data Available`=="Yes") %>%
  select(age:`Olink Urine Data Available`,-`Somalogic Data Available`) %>% 
  tbl_summary(
    statistic = list(
      age ~ "{mean} ({sd})",
      hba1c ~ "{median} [{p25}, {p75}]"
