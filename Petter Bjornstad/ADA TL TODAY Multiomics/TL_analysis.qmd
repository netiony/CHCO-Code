...
setwd("/Volumes/Peds Endo/Petter Bjornstad")

# Clinical Data
clinical_dat <- read.sas7bdat("./Teen Labs/Data_Cleaned/bjornstad_03_15_2023.sas7bdat")
...
# Soma Data
soma_dat <- read_adat("./Teen Labs/Data_Raw/WUS_22_007_v4.1_EDTAPlasma.hybNorm.medNormInt.plateScale.calibrate.anmlQC.qcCheck.anmlSMP.adat")
...
dat$race <- factor(dat$race, levels = 1:8, labels = c("White or Caucasian", 
                                                      ...
                                                      "Unknown",
                                                      "More than one race"))
dat$surg <- factor(dat$surg, levels = 1:9, labels = c("Gastric bypass",
                                                      "Biliopancreatic diversion",
                                                      "Biliopancreatic diversion w/ Switch",
                                                      "Laparoscopic adjustable gastric band",
                                                      "Sleeve gastrectomy - initial stage",
                                                      "Sleeve gastrectomy - second stage",
                                                      "Other procedure",
...
dat$diab_resolved <- factor(dat$diab_resolved, levels = 0:1, labels = c("No", "Yes"))

# Data dictionary and labels
clin_dict <- read.csv("./Teen Labs/Data_Cleaned/DataDictionary_clean.csv") %>%
...
```{r}
table1(~ sex + age+ ethn + race + surg + diab + diab_resolved | visit, data = dat)
```
