...
## b. Load Gene Sets
```{r}
tca <- read.csv("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/RH_ROCKIES/Gene Sets/tca_cycle_genes.csv")$genesymbol
tca <- c(tca,c(
...
ox_phos <- read.csv("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/RH_ROCKIES/Gene Sets/ox_phos_genes.csv")$genesymbol
ox_phos <- c(ox_phos,c(
  "NDUFS6",  "SDHB", "SDHC", "SDHD",
  "UQCRC1", "UQCRC2", "COX4I1", "COX4I2", "ATP5PF"
)) #47 genes
ox_phos <- unique(ox_phos)
```
...
png(fs::path(dir.results, "Plot_Categorical_Covariates_T2D_only.png"),
    width = 3000, height = 3000, res = 300)
for (covariate in cat_covariates) {
  # p <- ggplot(dat, aes_string(x = "group", fill = covariate)) +
  #   geom_bar(position = "fill") +  # use "dodge" for absolute counts
...
# Display all plots together (adjust ncol/nrow as needed)
png(fs::path(dir.results, "Plot_Continuous_Covariates_T2D_only.png"),
    width = 3000, height = 3000, res = 300)
for (covariate in con_covariates) {
 p <- ggplot(dat, aes_string(x = "SGLT2", y = covariate, fill = "SGLT2")) +
    geom_boxplot(outlier.shape = NA, alpha = 0.7) +
...
png(fs::path(dir.results, "UMAP_by_SGLT2.png"),
...
nrow(so_celltype) #15103 genes
ncol(so_celltype) #10489 PT cells
...
so_celltype_path$SGLT2 <- factor(so_celltype_path$SGLT2)
#Make sure to set reference level
so_celltype_path$SGLT2  <- relevel(so_celltype_path$SGLT2 ,ref="No SGLT2i")

counts_path <- round(GetAssayData(so_celltype_path, layer = "counts")) # load counts and round
...
nebula_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    ...
    meta_gene <- so_celltype_path@meta.data
...
write.csv(full_results,fs::path(dir.results,"NEBULA_TCA_cycle_PT_cells_SGLT2_unadjusted.csv"))
...
dot_plot <- ggplot(full_results, aes(
  ...
  labs(
    title = "Differentially Expressed TCA Cycle Genes",
    subtitle = "SGLT2i vs. No SGLT2i in PT Cells, Unadjusted (REML, Log Normal, Library Size Offset)",
    x = "Log Fold Change",
    y = "Gene",
    caption = paste0(
...
png(fs::path(dir.results, "Plot_TCA_cycle_NEBULA_PT_SGLT2_unadjusted.png"), 
...
#Select TCA and Ox Phos Genes
# pathway_genes <- c(tca,ox_phos)
so_celltype_path <- subset(so_celltype, features = ox_phos)
DefaultAssay(so_celltype_path) <- "RNA"
...
write.csv(full_results,fs::path(dir.results,"NEBULA_Ox_Phos_PT_cells_SGLT2_unadjusted.csv"))
...
dot_plot <- ggplot(full_results, aes(
  ...
  theme_minimal() +  # Retains grid lines
  labs(
    title = "Differentially Expressed Oxidative Phosphorylation Genes",
    subtitle = "SGLT2i vs. No SGLT2i in PT Cells, Unadjusted (REML, Log Normal, Library Size Offset)",
    x = "Log Fold Change",
    y = "Gene",
    caption = paste0(
...
png(fs::path(dir.results, "Plot_Ox_Phos_NEBULA_PT_SGLT2_unadjusted.png"), 
    width = 2500, height = 2000, res = 300)
print(dot_plot)
dev.off()
...
write.csv(full_results,fs::path(dir.results,"NEBULA_TCA_cycle_PT_S1_S2_cells_SGLT2_unadjusted.csv"))
...
dot_plot <- ggplot(full_results, aes(
  ...
  labs(
    title = "Differentially Expressed TCA Cycle Genes",
    subtitle = "SGLT2i vs. No SGLT2i in PT-S1/S2 Cells, Unadjusted (REML, Log Normal, Library Size Offset)",
    x = "Log Fold Change",
    y = "Gene",
    caption = paste0(
      "FDR < 0.05, Genes = ", Genes,
...
png(fs::path(dir.results, "Plot_TCA_cycle_NEBULA_PT_S1_S2_SGLT2_unadjusted.png"), 
    width = 2500, height = 2000, res = 300)
print(dot_plot)
dev.off()
```
...
write.csv(full_results,fs::path(dir.results,"NEBULA_Ox_Phos_PT_S1_S2_cells_SGLT2_unadjusted.csv"))
...
png(fs::path(dir.results, "Plot_Ox_Phos_NEBULA_PT_S1_S2_SGLT2_unadjusted.png"), 
    width = 2500, height = 2000, res = 300)
print(dot_plot)
dev.off()
...
write.csv(full_results,fs::path(dir.results,"NEBULA_TCA_cycle_PT_S3_cells_SGLT2_unadjusted.csv"))
...
dot_plot <- ggplot(full_results, aes(
  ...
  labs(
    title = "Differentially Expressed TCA Cycle Genes",
    subtitle = "SGLT2i vs. No SGLT2i in PT-S3 Cells, Unadjusted (REML, Log Normal, Library Size Offset)",
    x = "Log Fold Change",
    y = "Gene",
    caption = paste0(
      "FDR < 0.05, Genes = ", Genes,
...
png(fs::path(dir.results, "Plot_TCA_cycle_NEBULA_PT_S3_SGLT2_unadjusted.png"), 
    width = 2500, height = 2000, res = 300)
print(dot_plot)
dev.off()
```
...
write.csv(full_results,fs::path(dir.results,"NEBULA_Ox_Phos_PT_S3_cells_SGLT2_unadjusted.csv"))
...
png(fs::path(dir.results, "Plot_Ox_Phos_NEBULA_PT_S3_SGLT2_unadjusted.png"), 
    width = 2500, height = 2000, res = 300)
print(dot_plot)
dev.off()
