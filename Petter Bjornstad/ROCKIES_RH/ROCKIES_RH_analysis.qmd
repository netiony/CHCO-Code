...
```{r echo = F, include = F}
full_dat <- read.csv("/Volumes/Peds Endo/Petter Bjornstad/Data Harmonization/Data Clean/harmonized_dataset.csv", na.strings = "")

full_dat <- full_dat %>%
  filter(study %in% c("RENAL-HEIR", "RENAL-HEIRitage") | 
           (study == "CROCODILE" & group == "Lean Control") | (study == "IMPROVE" & visit == "baseline")) %>%
  ...
  filter(record_id %nin% c("RH2-48-T", "RH2-52-T", "CRC-55")) # removing two RH2 without scRNA and one CRC control with IgAN

dict <- read.csv("/Volumes/Peds Endo/Petter Bjornstad/Data Harmonization/data_dictionary_master.csv") 
dict <- label_harmonized_dict(full_dat, dict) %>%
  dplyr::mutate(race_ethnicity_condensed = "Race/Ethnicity",
                ...
                group_t2ob = "Group")
full_dat <- label_harmonized(full_dat, dict[1,])
```
...
### Limited to subjects with PET

The tables below are limited to subjects with PET regardless of biopsy. 
...
```{r echo = F, results = 'asis'}
summary(tableby(as.formula(paste("group_sglt2i ~", 
                                 paste(c(clinical_vars, 
                                         'kwt(acr_u, "Nmiss", "medianq1q3", "range")', 
                                         'kwt(hba1c, "Nmiss", "medianq1q3", "range")'), 
                                       collapse = " + "))), 
...
```{r echo = F, results = 'asis'}
summary(tableby(as.formula(paste("group_sglt2i ~", paste(dexa_vars,
                                                  collapse = " + "))), 
                data = dat), 
        test = F)
```
...
```{r echo = F, results = 'asis'}
summary(tableby(as.formula(paste("group_sglt2i ~", paste(mri_vars,
                                                  collapse = " + "))), 
                data = dat), 
        test = F)
```
...
```{r echo = F, results = 'asis'}
summary(tableby(as.formula(paste("group_sglt2i ~", paste(pet_vars,
                                                  collapse = " + "))), 
                data = dat), 
        test = F)
```
...
```{r echo = F, results = 'asis'}
summary(tableby(as.formula(paste("group_sglt2i ~", paste(vw_vars,
                                                  collapse = " + "))), 
                data = dat), 
        test = F)
...
```{r echo = F, results = 'asis'}
rhrh2_dat <- dat %>% filter(group %in% c("Type 2 Diabetes", "Obese Control"))
...
###### 2c MRI
```{r echo = F, results = 'asis'}
mri_vars <- c("total_kidney_volume_ml", "left_kidney_volume_ml", "right_kidney_volume_ml", "pcasl3d_left", "pcasl3d_right", "avg_c_adc", "adc_left", "adc_right")
...
```{r echo = F, results = 'asis'}
summary(tableby(as.formula(paste("epic_sglti2_1 ~", paste(c(clinical_vars, 
                                                              'kwt(acr_u, "Nmiss", "medianq1q3", "range")', 
                                                              'kwt(hba1c, "Nmiss", "medianq1q3", "range")'), 
                                                            collapse = " + "))), 
                data = rhrh2_dat), 
...
```{r echo = F, results = 'asis'}
# summary(tableby(as.formula(paste("epic_sglti2_1 ~", paste(dexa_vars,
#                                                             collapse = " + "))),
#                 data = rhrh2_dat),
#         test = F)
```
...
###### 3c MRI
```{r echo = F, results = 'asis'}
summary(tableby(as.formula(paste("epic_sglti2_1 ~", paste(mri_vars,
                                                            collapse = " + "))),
                data = rhrh2_dat),
        test = F)
```
...
###### 3d PET
```{r echo = F, results = 'asis'}
summary(tableby(as.formula(paste("epic_sglti2_1 ~", paste(pet_vars,
                                                            collapse = " + "))), 
                data = rhrh2_dat), 
        test = F)
```
...
###### 3e Voxelwise
```{r echo = F, results = 'asis'}
summary(tableby(as.formula(paste("epic_sglti2_1 ~", paste(vw_vars,
                                                            collapse = " + "))), 
                data = rhrh2_dat), 
        test = F)
```
...
dat %>% ggplot(aes(x = group_sglt2i, y = avg_c_k1, color = group_sglt2i)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = label(dat$group_sglt2i),
       y = label(dat$avg_c_k1),
       color = label(dat$group_sglt2i)) +
  theme(panel.grid = element_blank(),
        text = element_text(size = 14)) +
  scale_color_manual(values = c("#84a59d", "#457b9d", "#f28482", "#ffa69e"))
...
dat %>% ggplot(aes(x = group_sglt2i, y = avg_c_k2, color = group_sglt2i)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = label(dat$group_sglt2i),
...
dat %>% ggplot(aes(x = group_sglt2i, y = avg_c_k2_f, color = group_sglt2i)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = label(dat$group_sglt2i),
...
###### K1 (Excluding Lean Control)
```{r echo = F, warning = F}
subset(dat, !is.na(epic_sglti2_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_sglti2_1, y = avg_c_k1, color = epic_sglti2_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
...
```{r echo = F, warning = F}
subset(dat, !is.na(epic_sglti2_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_sglti2_1, y = avg_c_k2, color = epic_sglti2_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "SGLT2i at visit",
...
```{r echo = F, warning = F}
subset(dat, !is.na(epic_sglti2_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_sglti2_1, y = avg_c_k2_f, color = epic_sglti2_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "SGLT2i at visit",
...
###### K1 (Excluding Lean Control)
```{r echo = F, warning = F}
subset(dat, !is.na(epic_ever_sglt2i_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_ever_sglt2i_1, y = avg_c_k1, color = epic_ever_sglt2i_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "GLP1RA ever",
...
```{r echo = F, warning = F}
subset(dat, !is.na(epic_ever_sglt2i_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_ever_sglt2i_1, y = avg_c_k2, color = epic_ever_sglt2i_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "GLP1RA ever",
...
```{r echo = F, warning = F}
subset(dat, !is.na(epic_ever_sglt2i_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_ever_sglt2i_1, y = avg_c_k2_f, color = epic_ever_sglt2i_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "GLP1RA ever",
...
##### GLP1RA timepoint status

###### K1 (Excluding Lean Control)
```{r echo = F, warning = F}
subset(dat, !is.na(epic_glp1ra_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_glp1ra_1, y = avg_c_k1, color = epic_glp1ra_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "GLP1RA at visit",
       y = label(dat$avg_c_k1),
       color = "GLP1RA at visit") +
  theme(panel.grid = element_blank(),
        text = element_text(size = 14)) +
  scale_color_manual(values = c("#457b9d", "#f28482"))
```
...
```{r echo = F, warning = F}
subset(dat, !is.na(epic_glp1ra_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_glp1ra_1, y = avg_c_k2, color = epic_glp1ra_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "GLP1RA at visit",
...
```{r echo = F, warning = F}
subset(dat, !is.na(epic_glp1ra_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_glp1ra_1, y = avg_c_k2_f, color = epic_glp1ra_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "GLP1RA at visit",
...
###### K1 (Excluding Lean Control)
```{r echo = F, warning = F}
subset(dat, !is.na(epic_ever_glp1ra_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_ever_glp1ra_1, y = avg_c_k1, color = epic_ever_glp1ra_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "GLP1RA ever",
...
```{r echo = F, warning = F}
subset(dat, !is.na(epic_ever_glp1ra_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_ever_glp1ra_1, y = avg_c_k2, color = epic_ever_glp1ra_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "GLP1RA ever",
...
```{r echo = F, warning = F}
subset(dat, !is.na(epic_ever_glp1ra_1) & group != "Lean Control") %>% 
  ggplot(aes(x = epic_ever_glp1ra_1, y = avg_c_k2_f, color = epic_ever_glp1ra_1)) +
  geom_boxplot(size = 1) +
  geom_jitter(alpha = 0.3) +
  theme_bw() +
  labs(x = "GLP1RA ever",
...
### Limited to subjects with biopsy

The tables below are limited to subjects with biopsy regardless of PET. 
...
### Limited to subjects with PET & biopsy

The tables below are limited to subjects with PET & biopsy. 
...
```{r echo = F, results = 'asis'}
dat <- full_dat %>%
  filter(!is.na(avg_c_f) & !is.na(kit_id))
dat$record_id
