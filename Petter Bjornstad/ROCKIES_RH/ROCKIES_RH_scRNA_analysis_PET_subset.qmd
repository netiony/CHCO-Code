...
AND/OR
	T2D/OB (-) vs. T2D/OB (+)
Q3: Interesting that SGLT2i is a/w mostly downregulation of transcripts of TCA here
Q4: SGLT2i (timepoint) data are missing
Q5: What is JCI (T2D(-) vs. HC) in this output report since this subset is limited to those with PET data, which are not the same participants?
(Perhaps these are only the same transcripts as those included in the JCI paper).
