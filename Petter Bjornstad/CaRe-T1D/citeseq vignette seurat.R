...
cbmc.rna <- as.sparse(read.csv(file = "/Volumes/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/Grants/CaRe-T1D/CITEseq vignette/GSE100866_CBMC_8K_13AB_10X-RNA_umi.csv.gz",
...
cbmc.adt <- as.sparse(read.csv(file = "/Volumes/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/Grants/CaRe-T1D/CITEseq vignette/GSE100866_CBMC_8K_13AB_10X-ADT_umi.csv.gz",
                               sep = ",", header = TRUE, row.names = 1))
...
# visualize RNA and protein side by side
# Normalize ADT data,
DefaultAssay(cbmc) <- "ADT"
...
# additional visualization of multimodal data
...
# as an example, we recreate the plots above using a dataset of 7,900 peripheral blood mononuclear cells (PBMC)
# freely available from 10X Genomics

# note: these files are huge so I didn't download them to test this code
# files are available: https://support.10xgenomics.com/single-cell-gene-expression/datasets/3.0.0/pbmc_10k_protein_v3?
# download them all and put in same directory
# then update directory in code below
