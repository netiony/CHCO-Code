...
phys.drop(["phys_age", "phys_normal", "phys_abnormal"], axis=1, inplace=True)
...
# MERGE
penguin = pd.merge(phys, screen, how="outer")
penguin = pd.merge(penguin, labs, how="outer")
penguin = pd.merge(penguin, dxa, how="outer")
penguin = pd.merge(penguin, clamp, how="outer")
penguin = pd.merge(penguin, rct, how="outer")
penguin = pd.merge(penguin, pet, how="outer")
penguin = pd.merge(penguin, mri, how="outer")
penguin = pd.merge(penguin, demo, how="outer")
# REORGANIZE
penguin["visit"] = "baseline"
penguin["study"] = "PENGUIN"
id_cols = ["record_id", "co_enroll_id", "study"] + \
    dem_cols[1:] + ["visit", "procedure", "date"]
other_cols = penguin.columns.difference(id_cols).tolist()
penguin = penguin[id_cols + other_cols]
# SORT
penguin.sort_values(["record_id", "date", "procedure"], inplace=True)
# Check for duplicated column names
dups = find_duplicate_columns(penguin)
dups.to_csv("~/penguin_duplicate_columns.csv", index=False)
# Print final data
penguin.to_csv("~/penguin.csv", index=False)
