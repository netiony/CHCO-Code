...
soma_harmonized$SampleDescription <- str_trim(soma_harmonized$SampleDescription)
# add labels
labels <- paste0("log(",analytes$EntrezGeneSymbol[match(colnames(soma_harmonized), analytes$AptName)],")")
labels[labels == "log(NA)"] <- ""
label(soma_harmonized) <- as.list(labels)

# write copy of the entire local SomaScan dataframe to the data harmonization folder
save(soma_harmonized, file = "/Volumes/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/Data Harmonization/Harmonized SomaScan/soma_harmonized_anml.RData")
...
soma2 <- NULL
panther <- NULL
soma_harmonized <- NULL

# read in data 
soma <- read.delim("./Local cohort Somalogic data/WUS-22-002/log2RFU_WUS-22-002_v4.1_EDTAPlasma.hybNorm.medNormInt.plateScale.calibrate.anmlQC.qcCheck.anmlSMP_quantile.txt")

## STOPPED HERE - COLNAMES ARE ODD AND DECIDED TO ASK KATERINA ABOUT NORMALIZATIN
