...
#                 skip = 1)
alb <- readxl::read_xls("/Users/hhampson/Documents/IPA/Pathways/PT_T2D_glp1_sglt2_vs_glp1.xls", skip = 1)

alb <- alb %>% arrange(desc("-log(p-value)")) 
alb_keep <- alb[1:20,]

alb_plot <- ipa_plot(alb_keep)
pdf(fs::path(dir.results,"PT_T2D_sglt2i_vs_glp1_Pathways.pdf"),width=12,height=10)
plot(alb_plot)
dev.off()
