...
gene_list <- gene_list_total[1:5000]
# gene_list2 <- gene_list_total[7314:14627]

# Get the total number of available cores
total_cores <- parallel::detectCores()-1

# Create the cluster using all available cores
...
process_gene <- function(gene_idx) {
  ...
  if (sum(data_subset[[gene]]) > 0) {
    ...
    # Fit the zero-inflated mixed-effects model
...
resdults <- foreach(i = seq_along(gene_list), .combine = rbind, .packages = c("glmmTMB", "lme4")) %dopar% {
  process_gene(i)
}
...
#     # m1 <- as.formula(paste0(gene, " ~ group + a1c + nih_ethnicity + (1 | Kit_Lot)"))
