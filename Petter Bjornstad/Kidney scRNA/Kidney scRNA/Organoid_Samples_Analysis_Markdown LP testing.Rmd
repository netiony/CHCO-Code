...
# LP file paths
dir.dat <- c("/Users/pylell/Library/CloudStorage/OneDrive-SharedLibraries-UW/Bjornstad/Biostatistics Core Shared Drive")
dir.dat2 <- c('/Users/pylell/Library/CloudStorage/OneDrive-SharedLibraries-UW/Bjornstad/Biostatistics Core Shared Drive/scRNA')
dir.results <- c("/Users/pylell/Documents/Temp")
...
p <- ggplot(df_selected_data_long, aes(x = variable, y = value, fill = variable)) +
  geom_boxplot() +
...
results <- foreach(gene = gene_names, .combine = rbind, .packages = c("glmmTMB", "dplyr","doParallel")) %dopar% {
  ...
  if (!is.null(model_full) && !is.null(model_full$sdr) && all(model_full$sdr$pdHess, na.rm = TRUE) &&
    ...
    #why is it bootstrapping the null model?
