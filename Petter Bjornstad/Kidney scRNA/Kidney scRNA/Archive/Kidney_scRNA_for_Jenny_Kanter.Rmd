...
#Filter to MAC and MON
unique(so_kpmp_sc$KPMP_celltype)
so_sub <- subset(so_kpmp_sc,KPMP_celltype=="MAC" | KPMP_celltype=="MON")
dim(so_sub) #9276 genes, 3265 cells
# Remove old UMAP embeddings
so_sub@reductions$umap <- NULL
so_sub@meta.data$umap_ <- NULL
...
DimPlot(so_sub, reduction = "umap", group.by = "KPMP_celltype", label = TRUE) +
  ggtitle("UMAP by Cell Type") +
  labs(x = "umap_1", y = "umap_2")


# Example: Visualize a specific marker gene using FeaturePlot
# FeaturePlot(so_sub, features = c("TREM2"))

# Visualize top marker genes using DotPlot
pbmc.markers <- FindAllMarkers(so_sub, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
pbmc.markers %>%
    group_by(cluster) %>%
...
DoHeatmap(so_sub, features = top10$gene) + NoLegend()
...
VlnPlot(so_sub, features = c(top10$gene), slot = "counts", log = TRUE)

FeaturePlot(so_sub, features = c(top10$gene[c(1,11,21,31,41,51,61,71,81,91,101,111,121)]),label=T)

# Plot UMAP and label by cell type stored in KPMP_celltype
DimPlot(so_sub, reduction = "umap")
FeaturePlot(so_sub,features = c(top10$gene[1]))

gene_expression <- FetchData(so_sub, vars = c(top10$gene[1]))

DimPlot(so_sub, reduction = "umap", group.by = "KPMP_celltype")
...
#Disease Groups
#1. T2D vs. OB
so_subgroup <- subset(so_sub,group=="Type_2_Diabetes" | group=="Lean_Control")
so_subgroup$DiseaseGroup <- so_subgroup$group
so_subgroup$DiseaseGroup <- factor(so_subgroup$DiseaseGroup)
degs_fxn(so=so_subgroup,cell=NULL,gene_set=all_genes,additional_group = NULL,exposure="DiseaseGroup",exp_group="Type_2_Diabetes",ref_group="Lean_Control",enrichment="No",top_gsea = 30)
length(unique(so_subgroup$kit_id)) #45 participants

#2. OB vs. LC
so_subgroup <- subset(so_sub,group=="Obese_Control" | group=="Lean_Control")
so_subgroup$DiseaseGroup <- so_subgroup$group
so_subgroup$DiseaseGroup <- factor(so_subgroup$DiseaseGroup)
degs_fxn(so=so_subgroup,cell=NULL,gene_set=all_genes,additional_group = NULL,exposure="DiseaseGroup",exp_group="Obese_Control",ref_group="Lean_Control",enrichment="No",top_gsea = 30)
length(unique(so_subgroup$kit_id)) #16 participants


#Filter to type 2 diabetes only since no other participants are on these main meds
so_diab <- subset(so_sub,group=="Type_2_Diabetes")
...
length(unique(so_subgroup$kit_id)) #20 participants
