...
```{r}
#EC-PTC
so_celltype <- subset(so_kpmp_sc,KPMP_celltype=="EC-PTC")
# Extract the gene expression data for all genes
gene_expression <- as.data.frame(GetAssayData(so_celltype, layer = "data"))
ncol(so_celltype) #3262 cells
nrow(so_celltype) #9276 genes
...
length(which(full_results$PValue=="NaN")) #4802 NaNs in Pvalue
Nonconvergence_Rate <- paste0(round(((length(which(is.na(full_results$PValue)))+length(which(full_results$PValue=="NaN")))/length(full_results$Gene))*100,0),"%")
write.csv(full_results2,fs::path(dir.results,"EC_PTC_T2D_HC_No_Med.csv"))
# full_results <- read.csv(fs::path(dir.results,"EC_PTC_T2D_HC_No_Med.csv"))
...
# write.csv(full_results,fs::path(dir.results,"EC_PTC_T2D_HC_No_Med.csv"))
# full_results <- read.csv(fs::path(dir.results,"EC_PTC_T2D_HC_No_Med.csv"))
...
pdf(fs::path(dir.results,"Plot_EC_PTC_T2D_HC_No_Med.pdf"),width=15,height=10)
print(volcano_plot)
dev.off()
```
#### v. EC-AEA
