...
pdf(fs::path(dir.results,"Normalization_gene_expression_histograms.pdf"), width = 8, height = 6)
...
# y <- estimateDisp(y)
# plotBCV(y)  # Dispersion vs mean
# Calculate library sizes
subset_counts <- counts[1:500, ]  # or use sample() for random subset
lib_sizes <- colSums(counts)

# Identify samples with zero library size
zero_libs <- which(lib_sizes == 0)

# Remove those samples
subset_counts_filtered <- counts[, lib_sizes > 0]

# Now re-run the analysis
y <- DGEList(counts = as.matrix(subset_counts_filtered))
y <- estimateDisp(y)

# subset_counts <- counts[1:100, ]  # or use sample() for random subset
# y <- DGEList(counts = as.matrix(subset_counts))
# y <- estimateDisp(y)
# plotBCV(y)
plotBCV(y)
...
#>0 = overdispersion
#>
#># Check for overdispersion based on common dispersion
if (exists("y") && !is.null(y$common.dispersion)) {
  if (y$common.dispersion > 0) {
    cat("There is overdispersion, use a Negative Binomial model.\n")
  } else {
    cat("There is not overdispersion, use a Gaussian model.\n")
  }
} else {
  cat("DGEList object 'y' or common dispersion not found. Run estimateDisp(y) first.\n")
}
