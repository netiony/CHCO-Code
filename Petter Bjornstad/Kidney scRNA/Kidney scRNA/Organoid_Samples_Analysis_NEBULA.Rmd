...
nebula_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    ...
    result <- nebula(count = count_gene, id = meta_gene$kit_id, pred = pred_gene, ncore = 1,output_re = T,reml=T,covariance=T,
                     offset=offset_hvg)
...
write.csv(full_results,fs::path(dir.results,"NEBULA_PT_cells_diabetes_unadjusted_2000_hvg_reml_offset.csv"))
...
volcano_plot <- ggplot(full_results, aes(x = `logFC_groupType_2_Diabetes`, y = PValue10, color = color)) +
  ...
  labs(
    ...
    subtitle = "PT, Unadjusted (REML, Offset)",
    x = "FC",
    y = "-log10(P-Value)",
...
# print(volcano_plot)
# dev.off()
png(fs::path(dir.results, "Plot_NEBULA_PT_Diabetes_unadjusted_2000_hvg_reml_offset.png"), 
...
write.csv(full_results,fs::path(dir.results,"NEBULA_aPT_cells_diabetes_unadjusted_2000_hvgreml.csv"))
