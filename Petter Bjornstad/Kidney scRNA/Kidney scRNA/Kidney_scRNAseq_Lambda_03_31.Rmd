...
FeaturePlot(so_kpmp_sc, features = "GLP1R", reduction = "umap.rpca",raster=F)
...
DimPlot(so_diab, reduction = "umap.rpca", group.by = "KPMP_celltype", label=T, raster = FALSE) +
  # geom_label(aes(label = KPMP_celltype), color = "black", size = 3, fill = "white", label.padding = 0.2) +
  ggtitle("UMAP by KPMP Cell Type among Type 2 Diabetes") +
  labs(x = "UMAP 1", y = "UMAP 2")

# Using geom_label instead of geom_text for labels with a box
DimPlot(so_diab, reduction = "umap.rpca", group.by = "celltype_rpca", label=T, raster = FALSE) +
  # geom_label(aes(label = KPMP_celltype), color = "black", size = 3, fill = "white", label.padding = 0.2) +
  ggtitle("UMAP by Cell Type among Type 2 Diabetes") +
  labs(x = "UMAP 1", y = "UMAP 2")

FeaturePlot(so_diab, features = "GLP1R", reduction = "umap.rpca",raster=F)
...
# Merge cell count with total cells per kit_id to get the proportions
proportion_cells <- cell_count_per_group %>%
  left_join(total_cells_per_kit, by = "medication") %>%
  mutate(proportion = cell_count / total_cells)

ggplot(proportion_cells, aes(x = medication, y = proportion, fill = celltype)) +
  geom_bar(stat = "identity", position = "stack") +
  theme_minimal() +
  labs(title = "Proportion of Cells by Medication Group in Type 2 Diabetes",
       x = "Medication Group",
       y = "Proportion of Cells") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
DimPlot(so_diab, reduction = "umap.rpca", group.by = "KPMP_celltype", label=T, raster = FALSE) +
  # geom_label(aes(label = KPMP_celltype), color = "black", size = 3, fill = "white", label.padding = 0.2) +
  ggtitle("UMAP by KPMP Cell Type among T2D + OB") +
  labs(x = "UMAP 1", y = "UMAP 2")

# Using geom_label instead of geom_text for labels with a box
DimPlot(so_diab, reduction = "umap.rpca", group.by = "celltype_rpca", label=T, raster = FALSE) +
  # geom_label(aes(label = KPMP_celltype), color = "black", size = 3, fill = "white", label.padding = 0.2) +
  ggtitle("UMAP by Cell Type among T2D + OB") +
  labs(x = "UMAP 1", y = "UMAP 2")
...
ggplot(proportion_cells, aes(x = medication, y = proportion, fill = celltype)) +
  geom_bar(stat = "identity", position = "stack") +
  theme_minimal() +
  labs(title = "Proportion of Cells by Medication Group in T2D + OB",
       x = "Medication Group",
       y = "Proportion of Cells") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
table1(~ age + sex + bmi + + hba1c +
         triglycerides + acr_u  | medication, data=dat)
table1(~ gfr_raw_plasma + eGFR_CKD_epi +
         erpf_raw_plasma+ total_kidney_volume_ml + ht_adj_tkv+
         raasi_1+mfm_1+insulin_1+medication | group, data=dat)
#Med table
...
# table1(~ gfr_raw_plasma + eGFR_CKD_epi +
#          erpf_raw_plasma+ total_kidney_volume_ml + ht_adj_tkv+
#          raasi_1+mfm_1+insulin_1+medication | group, data=dat_diab)
table1::table1(~ age + sex + bmi + + hba1c +
         triglycerides + acr_u + gfr_raw_plasma + eGFR_CKD_epi +
           total_kidney_volume_ml  | medication, data=dat_diab, render.missing = NULL) 
...
table1(~ age + sex + bmi + + hba1c +
         triglycerides + acr_u  | medication, data=dat)
table1(~ gfr_raw_plasma + eGFR_CKD_epi +
         erpf_raw_plasma+ total_kidney_volume_ml
         raasi_1+mfm_1+insulin_1+medication | group, data=dat_diab)
```
...
data_pb1 <- data_pb %>%
  filter(celltype=="PT") %>%
  filter(group=="Type 2 Diabetes") %>%
  filter(medication=="glp1" | medication=="sglt2") %>%
  mutate(across(all_of(genes),~log2(. + 1)))
title <- paste0("Differential Expression in GLP1 vs. SGLT2 in Type 2 Diabetes in PT Cells")
...
#Type 2 GLP-1 vs. No Med
data_pb1 <- data_pb %>%
  filter(celltype=="PT") %>%
  filter(group=="Type 2 Diabetes") %>%
  filter(medication=="glp1" | medication=="no_med") %>%
  mutate(across(all_of(genes),~log2(. + 1)))
title <- paste0("Differential Expression in GLP1 vs. No Medication in Type 2 Diabetes in PT Cells")
...
ggplot(total_results, aes(x = Log2FC, y = -log10(Pvalue))) +
  ...
  # geom_text(data = top_genes, aes(label = Gene), vjust = 1, hjust = 1, size = 3, color = "black")+
   geom_text_repel(data = top_genes, aes(label = Gene),
                  size = 3, color = "black", box.padding = 0.5, point.padding = 0.5,
                  segment.color = "grey50", max.overlaps = 10)
write.csv(total_results,fs::path(dir.ipa,"PT_T2D_GLP1_vs_No_Medication.csv"))
...
data_pb1 <- data_pb %>%
  ...
  filter(medication=="glp1" | medication=="no_med") %>%
  mutate(across(all_of(genes),~log2(. + 1)))
title <- paste0("Differential Expression in GLP1 vs. No Medication in T2D + OB in PT Cells")
...
hemo1 <- hemo1[which(hemo1 %in% rownames(so_kpmp_sc))]

#PT
so_pb <- subset(so_kpmp_sc,celltype=="PT")
so_diab <- subset(so_pb,group=="Type_2_Diabetes") #32
so_subgroup <- subset(so_diab,medication=="glp1_sglt2" | medication=="glp1")
so_subgroup$medication <- factor(so_subgroup$medication)
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo1))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo1,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Hemodynamics")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo1))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo1,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Hemodynamics")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo1))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo1,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Hemodynamics")

###########
infl <- c("IL6", "IL8", "CXCL1", "CXCL2", "CCL20", "ICAM1", "TNFAIP3", "NAF1", "TRAF1", "TNF")
#Check genes are in seurat object
infl<- infl[which(infl %in% rownames(so_kpmp_sc))]
...
#Run
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=infl,exposure="medication",additional_group="Typ_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="PT",pathway="Inflammation1")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), infl))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=infl,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Inflammation1")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), infl))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=infl,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Inflammation1")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), infl))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=infl,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Inflammation1")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), infl))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=infl,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Inflammation1")
#############
infl2 <-  c("C3", "C4", "CFB", "C5", "C6", "C7", "C8", "C9", "BMP2", "PTPRC", "CCL2")
#Check genes are in seurat object
infl2<- infl2[which(infl2 %in% rownames(so_kpmp_sc))]
...
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=infl2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="PT",pathway="infl2ammation2")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), infl2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=infl2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="infl2ammation2")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), infl2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=infl2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="infl2ammation2")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), infl2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=infl2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="infl2ammation2")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), infl2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=infl2,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="infl2ammation2")
##############
ros <- c("SOD1", "SOD2", "NOX4", "DAO", "NRF2", "FOXO1", "FOXO3", "FOXO4", "HMOX1")
#Check genes are in seurat object
ros<- ros[which(ros %in% rownames(so_kpmp_sc))]
...
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=ros,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species")
################
ros2 <- c("ALOX12", "ALOX15", "ALOX5", "ALOX5AP")
#Check genes are in seurat object
ros2<- ros2[which(ros2 %in% rownames(so_kpmp_sc))]
...
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=ros2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species2")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species2")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species2")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species2")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros2,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Reactive_Oxygen_Species2")
########
injury <- c("LRP2", "PPARA", "PPARGC1A", "SLC22A1", "SLC12A3")
#Check genes are in seurat object
injury<- injury[which(injury %in% rownames(so_kpmp_sc))]
...
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=injury,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), injury))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=injury,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), injury))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=injury,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), injury))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=injury,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), injury))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=injury,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers")

#############
injury2 <- c("CST3", "HAVCR1", "LCN2")
#Check genes are in seurat object
injury2<- injury2[which(injury2 %in% rownames(so_kpmp_sc))]
...
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=injury2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers22")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), injury2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=injury2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers22")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), injury2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=injury2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers22")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), injury2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=injury2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers22")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), injury2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=injury2,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="PT",pathway="Injury_Markers22")
```
...
data_pb1 <- data_pb %>%
  ...
  filter(medication=="glp1" | medication=="sglt2") %>%
  mutate(across(all_of(genes),~log2(. + 1)))
title <- paste0("Differential Expression in GLP1 vs. SGLT2 in Type 2 Diabetes in TAL Cells")
...
data_pb1 <- data_pb %>%
  ...
  filter(medication=="glp1" | medication=="no_med") %>%
  mutate(across(all_of(genes),~log2(. + 1)))
title <- paste0("Differential Expression in GLP1 vs. No Medication in T2D + OB in TAL Cells")
...
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=hemo2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="TAL",pathway="Hemodynamics")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="TAL",pathway="Hemodynamics")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="TAL",pathway="Hemodynamics")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="TAL",pathway="Hemodynamics")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo2,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="TAL",pathway="Hemodynamics")
...
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=hemo3,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics K+ Channels")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo3))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo3,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics K+ Channels")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo3))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo3,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics K+ Channels")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo3))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo3,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics K+ Channels")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo3))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo3,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics K+ Channels")
...
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=hemo4,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Signaling")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo4))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo4,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Signaling")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo4))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo4,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Signaling")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo4))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo4,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Signaling")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo4))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo4,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Signaling")
...
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=hemo5,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Vascular Remodeling")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo5))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo5,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Vascular Remodeling")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo5))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo5,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Vascular Remodeling")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo5))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo5,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Vascular Remodeling")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), hemo5))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=hemo5,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="EC",pathway="Hemodynamics Vascular Remodeling")
...
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros2))
#Run
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=ros2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="EC",pathway="ROS Regulation")
degs_fxn3(so=so_subgroup2,cell=NULL,gene_set=ros2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="glp1",enrichment="No",top_gsea = 30,cellname="EC",pathway="ROS Regulation")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1_sglt2",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="EC",pathway="ROS Regulation")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="EC",pathway="ROS Regulation")

#Glp1 vs. sglt2
so_subgroup <- subset(so_diab,medication=="glp1" | medication=="sglt2")
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros2,exposure="medication",additional_group="Type_2_Diabetes",exp_group="glp1",ref_group="sglt2",enrichment="No",top_gsea = 30,cellname="EC",pathway="ROS Regulation")
...
so_subgroup$medication <- factor(so_subgroup$medication)
so_subgroup2 <- subset(so_subgroup, features = intersect(rownames(so_subgroup), ros2))
degs_fxn3(so=so_subgroup,cell=NULL,gene_set=ros2,exposure="medication",additional_group="T2D + OB",exp_group="glp1",ref_group="no_med",enrichment="No",top_gsea = 30,cellname="EC",pathway="ROS Regulation")
...
#### b. PT, TAL & EC Pseudobulk
```{r,echo=F,warning=F,fig.width=15, fig.height=9}
#Pseuodbulk Tall and PT cells
# so_kpmp_sc$cell_type2 <- ifelse(grepl("PT-",so_kpmp_sc$celltype_rpca),"PT",
#                                   ifelse(grepl("TAL-",so_kpmp_sc$celltype_rpca),"TAL",NA))
# so_kpmp_sc$cell_type2 <- as.character(so_kpmp_sc$cell_type2)
so_kpmp_sc$celltype2 <- ifelse(so_kpmp_sc$celltype=="PT","PT",
                                  ifelse(so_kpmp_sc$celltype=="TAL","TAL",
                                         ifelse(so_kpmp_sc$celltype=="EC","EC",NA)))
so_kpmp_sc$celltype2 <- as.character(so_kpmp_sc$celltype2)

all_cell_types <- unique(so_kpmp_sc$celltype2)
all_cell_types <- all_cell_types[-1]
Idents(so_kpmp_sc) <- so_kpmp_sc$celltype2

#1. T2D vs. OB
so_subgroup <- subset(so_kpmp_sc,group=="Type 2 Diabetes" | group=="Obese Control")
so_subgroup$DiseaseGroup <- so_subgroup$group
so_subgroup$DiseaseGroup <- factor(so_subgroup$DiseaseGroup)
all_genes <- rownames(so_subgroup)
#Diabetes yes vs. no
...
#2. T2D vs. T1D
so_subgroup <- subset(so_kpmp_sc,group=="Type 2 Diabetes" | group=="Type 1 Diabetes")
so_subgroup$DiseaseGroup <- so_subgroup$group
so_subgroup$DiseaseGroup <- factor(so_subgroup$DiseaseGroup)
for (cell_type in all_cell_types) {
...
#3. T2D vs. LC
so_subgroup <- subset(so_kpmp_sc,group=="Type 2 Diabetes" | group=="Lean Control")
so_subgroup$DiseaseGroup <- so_subgroup$group
so_subgroup$DiseaseGroup <- factor(so_subgroup$DiseaseGroup)
for (cell_type in all_cell_types) {
...
#  a. T2D
so_subgroup <- subset(so_kpmp_sc,group=="Type 2 Diabetes")
so_subgroup$albuminuria_30 <- factor(so_subgroup$albuminuria_30)
for (cell_type in all_cell_types) {
...
#RAASI - type 2s only
#Filter out NAs for ace inhibitors
so_diab <- subset(so_kpmp_sc,group=="Type 2 Diabetes")
so_subgroup <- subset(so_diab,raasi_1=="Yes" | raasi_1=="No")
so_subgroup$raasi_1 <- factor(so_subgroup$raasi_1)
all_genes <- rownames(so_subgroup)
for (cell_type in all_cell_types) {
