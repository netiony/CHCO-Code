---
title: "Harmonized Data Manual Kidney Volume Analyses"
...
The aim of this report is to:
 ...
 1. Separately describe manual left and right kidney volume across all studies by age and disease type, using the harmonized dataset. 
...
```{r manual kv analysis data cleaning, include=FALSE}
### data cleaning for manual analyses
...
left_clean_dat <- clean_dat %>% filter(lkv_metric_type == 'Manual' | lkv_metric_type == 'Non-Manual and Manual')
right_clean_dat <- clean_dat %>% filter(rkv_metric_type == 'Manual' | rkv_metric_type == 'Non-Manual and Manual')
...
left_combined_clean <- coenroll_multivisit_cleaning(dat = left_clean_dat, kv_varname = volume_left_manual) %>% 
  rename('lkv_date' = 'date')

right_combined_clean <- coenroll_multivisit_cleaning(dat = right_clean_dat, kv_varname = volume_right_manual) %>% 
...
# add in missing height data, calculate ht. adj. kv - note: no missing height data as of 1/26/24 for manual subjects
...
left_combined_clean <- left_join(left_combined_clean, lkv_additional_heights, by = c('merged_id', 'study', 'visit')) %>% 
  mutate(height_meters =
           case_when(!is.na(height_meters) | !is.na(additional_height_meters) ~ rowSums(select(., height_meters, additional_height_meters),
                                                                                             na.rm = TRUE))) %>%
  mutate(ht_adj_left_kv = volume_left_manual/height_meters) %>% 
...
right_combined_clean <- left_join(right_combined_clean, rkv_additional_heights, by = c('merged_id', 'study', 'visit')) %>% 
  mutate(height_meters =
           case_when(!is.na(height_meters) | !is.na(additional_height_meters) ~ rowSums(select(., height_meters, additional_height_meters),
                                                                                             na.rm = TRUE))) %>%
  mutate(ht_adj_right_kv = volume_right_manual/height_meters) %>% 
  select(-additional_height_meters)
...
data_labeling <- function(dat, dictionary){
  ...
  label(dat$volume_left_manual) <- "Manually Measured Left Kidney Volume (mL)"
  label(dat$volume_right_manual) <- "Manually Measured Right Kidney Volume (mL)"
...
label(left_combined_clean$ht_adj_left_kv) <- "Height Adjusted Manual LKV (mL/m)"
label(right_combined_clean$ht_adj_right_kv) <- "Height Adjusted Manual RKV (mL/m)"
...
The primary outcome of interest for this report was manually measured left or right kidney volume (KV). Results were reported for this KV variable and additionally for a height-adjusted version of this variable $\bigl(\frac{\text{KV}}{\text{height (m)}}\bigr)$. In instances where subjects had multiple KV measurements or were enrolled on multiple studies, their earliest KV measurements were used for analyses; data from the same visit the KV measurement was taken was referenced for the covariates of interest (age, disease type, etc.) unless missing in which case covariate data from the closest visit in time was used.

## Data Analysis

**All Studies (RENAL-HEIR, RENAL-HEIRitage):**
...
table1::table1(~ sex + age + group + diabetes_duration + height_meters + volume_left_manual + ht_adj_left_kv + 
                 lkv_metric_type, 
               data = left_combined_clean, caption = 'Descriptive Statistics of all Participants for Left Kidney Analyses')
```
Note: while this report only summarizes results for the manually measured KV data, some subjects also had non-manually measured data. `Left Kidney Volume Measure Type` provides a breakdown of subjects who only had manual data (n = 56) versus those who had manual as well as non-manual data for a visit (n = 24).

### LKV by Age

The below results describe the association between left KV and age for subjects across all studies. Diagnostic plots for checking linear regression assumptions are also included. `volume_left_manual` is the manual KV measurement. `ht_adj_left_kv` is the height-adjusted version of `volume_left_manual`.

```{r lkv by age, echo=FALSE}
outcome_vars <- c('volume_left_manual','ht_adj_left_kv')
...
The below table reports the mean and standard deviation (SD) of the manual KV outcomes of interest by disease type across all subjects in the harmonized dataset. Kruskal-Wallis testing was used to evaluate whether there was a significant difference in the mean ranks between groups.
...
left_combined_clean %>% 
  select(merged_id, group, volume_left_manual, ht_adj_left_kv) %>% 
   tbl_summary(by = group, include = -merged_id,
              type = list(c(volume_left_manual, ht_adj_left_kv) ~ 'continuous', 
                        group ~ 'categorical'),
...
table1::table1(~ sex + age + group + diabetes_duration + height_meters + volume_right_manual + 
                 ht_adj_right_kv + rkv_metric_type, 
               data = right_combined_clean, caption = 'Descriptive Statistics of all Participants for Right Kidney Analyses')
```

### RKV by Age

The below results describe the association between right KV and age for subjects across all studies. Diagnostic plots for checking linear regression assumptions are also included. `volume_right_manual` is the manual KV measurement. `ht_adj_right_kv` is the height-adjusted version of `volume_right_manual`.

```{r rkv by age, echo=FALSE}
outcome_vars <- c('volume_right_manual','ht_adj_right_kv')
...
right_combined_clean %>%
  select(record_id, group, volume_right_manual, ht_adj_right_kv) %>%
   tbl_summary(by = group, include = -record_id,
              type = list(c(volume_right_manual, ht_adj_right_kv) ~ 'continuous',
                        group ~ 'categorical'),
    statistic=list(all_continuous() ~ '{mean} ({sd})'),
    digits=everything()~c(2,2),
    missing_text='N Missing') %>%
