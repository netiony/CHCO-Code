...
home_dir = ifelse(.Platform$OS.type != "unix","T:/",
                  "/mnt/UCD/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/Pima/Master data/Raw data")
...
important_vars = 
  c("nih","sacaton","pimc","date","testint","protocol","stratum","dob",
    ...
    "mono","eos","baso","stop_dt","med_metformin","med_ras","med_htn")
    ...
    met = apply(d,1,function(r){
      if(all(is.na(r))){
        NA
          ...
          ifelse(any(r == 70292,na.rm = T),1,0)
        }
    })
    return(met)
  } else {
...
ras = function(df){
  ...
  if (length(w) > 0){
    d = df[,w]
    ras = apply(d,1,function(r){
      if(all(is.na(r))){
        NA
      } else {
          ifelse(any((20320 <= r & r < 20340) | r %in% c(20371,20372,20374),na.rm = T),1,0)
        }
    })
    return(ras)
  } else {
...
htn = function(df){
  ...
  if (length(w) > 0){
    d = df[,w]
    htn = apply(d,1,function(r){
      if(all(is.na(r))){
        NA
      } else {
          ifelse(any((20080 <= r & r <= 20380) | (20420 <= r & r <= 20560),na.rm = T),1,0)
        }
    })
    return(htn)
  } else {
...
# Metformin
ficoll$med_metformin = metformin(ficoll)
# RAS
ficoll$med_ras = ras(ficoll)
```
...
# Metformin
ddn$med_metformin = metformin(ddn)
# RAS
ddn$med_ras = ras(ddn)
```
...
# Metformin
losartan$med_metformin = metformin(losartan)
# RAS
losartan$med_ras = ras(losartan)
```
...
```{r}
missing = c("med_arb","med_orl","med_lipid","med_nonras",
            "med_ace","med_ins","med_ald","med_statin","med_nsai",
