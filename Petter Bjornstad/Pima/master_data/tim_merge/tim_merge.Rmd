...
# Unite date columns
losartan = losartan %>% rename(date = f8visitdate)
losartan$date = lubridate::ymd(losartan$date)
# Unite test interval columns
losartan = losartan %>% rename(testint = clrnctestintvrl)
# Unite heart rate columns
...
losartan[,c("f4sys","f6sys")] = 
  lapply(losartan[,c("f4sys","f6sys")], function(c){
    c[which(is.nan(c))] = NA
    c
  })
losartan = losartan %>% unite(sbp,f4sys,f6sys,na.rm = T,remove = F)
losartan$sbp = as.numeric(sapply(strsplit(losartan$sbp,"_"),function(x){
  if (length(x)>0){x[1]}else{NA}
}))
...
losartan[,c("f4dia","f6dia")] = 
  lapply(losartan[,c("f4dia","f6dia")], function(c){
    c[which(is.nan(c))] = NA
    c
  })
losartan = losartan %>% unite(dbp,f4dia,f6dia,na.rm = T,remove = F)
...
missing = c("med_htn","med_arb","med_orl","med_lipid","med_ras","med_nonras",
            ...
            "smac_ast","fpg","fc_alb","fc_igg","gfrb","pahb","rpfb","ff","extract",
