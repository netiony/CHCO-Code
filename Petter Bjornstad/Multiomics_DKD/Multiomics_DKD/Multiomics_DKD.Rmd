...
Y <- dat$MIC.OR.MAC
X <- as.matrix(dat[c(met,prot)])
rownames(X) <- dat$releaseid
Z <- data.frame(predictors=colnames(X),metabolite=NA,protein=NA)
rownames(Z) <- Z$predictors
Z <- Z %>% 
  select(-predictors)
Z <- Z %>% 
  mutate(metabolite=ifelse(grepl("seq",rownames(Z)),0,1),
...
#univariate protein to outcome and metabolite to outcome 
#select top 1000 proteins & top 20 metabolites (largest effect estimates)
