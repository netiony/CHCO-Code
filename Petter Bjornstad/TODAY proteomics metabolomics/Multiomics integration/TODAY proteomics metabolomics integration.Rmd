...
colnames(proteins) <- c("releaseid",colnames(proteins[,2:ncol(proteins)]))

# make df with all omics variables for elastic net
df_elastic_net <- left_join(comorb, proteins, by = "releaseid")
df_elastic_net <- left_join(df_elastic_net, urine_final, by = "releaseid")
df_elastic_net <- left_join(df_elastic_net, plasma_final, by = "releaseid")
urine_final$releaseid <- NULL
plasma_final$releaseid <- NULL
proteins$releaseid <- NULL
...
list.keepX <- list(proteins = c(25,30),
...
# First just a quick model to see if this works 
# This is not a sparse model, no variable selection performed
diablo.glycemia.quick <- block.plsda(X, Y_glycemia, near.zero.var = TRUE)
...
# examine the number of components
# first fit a model without variable selection
diablo.glycemia.comp <- block.plsda(X, Y_glycemia, near.zero.var = TRUE, ncomp = 5, design = design)
...
test.keepX <- list(proteins = c(1:9, seq(10, 100, 5)),
                   urine_metabolites = c(1:9, seq(10, 25, 5)),
                    urine_metabolites = c(1:9, seq(10, 25, 5)))
#tune.diablo.glycemia <- tune.block.splsda(X, Y_glycemia, ncomp = 2,
#                                             test.keepX = test.keepX, design = design,
#                                             validation = "Mfold", folds = 5, nrepeat = 1,
#                                             dist = "max.dist", near.zero.var = TRUE)
#list.keepX <- tune.diablo.glycemia$choice.keepX
list.keepX <- list(proteins = c(70,85),
                   urine_metabolites = c(1,1),
                    plasma_metabolites = c(15,1))
list.keepX

# final model
diablo.glycemia <- block.splsda(X, Y_glycemia, ncomp = ncomp, keepX = list.keepX,
                                   design = design, near.zero.var = TRUE)
...
plotVar(diablo.glycemia, var.names= FALSE, style= 'graphics', 
        legend = TRUE, pch = c(16, 17, 15), cex = c(2,2,2), 
        col= c('darkorchid', 'brown1', 'lightgreen'), title= 'TCGA, DIABLO comp 1 - 2')
circosPlot(diablo.glycemia, cutoff = 0.7, line = TRUE, 
           color.blocks = c('darkorchid','brown1','lightgreen'),
           color.cor = c("chocolate3",'grey20'), size.labels = 1.5, size.variables = 0.3)
...
prot_matrix <- t(as.matrix(proteins))
...
```{r}
alb_nozero <- df_elastic_net[!df_elastic_net$DAYSTOMIC.OR.MAC == 0,]
preds <- c(colnames(proteins), colnames(urine_final), colnames(plasma_final))
