...
all$HYPER <- ifelse(all$HYPER==0,"NO_HYPER","HYPER")
...
is_seq <- function(.x) grepl("seq", .x)
seq <- is_seq(names(all))
model_data <- cbind(all[,c("releaseid","time","HYPER")], all[,seq])
model_data$time <- factor(model_data$time, levels=c("B","F"))
model_data$releaseid <- factor(model_data$releaseid)
model_data$HYPER <- factor(model_data$HYPER)
seq <- is_seq(names(model_data))
model_data[,seq] <- apply(model_data[,seq],2,as.numeric)
model_data$HYPER_by_time <- factor(paste(model_data$HYPER,model_data$time,sep="."))
design <- model.matrix(~0+HYPER_by_time, data = model_data)
colnames(design) <- levels(model_data$HYPER_by_time)
ymat <- t(model_data[,seq])
# estimate correlation between measurements on the same subject
corfit <- duplicateCorrelation(ymat,design,block=model_data$releaseid)
fit <- lmFit(ymat, design, block=model_data$releaseid, correlation=corfit$consensus.correlation)
cm <- makeContrasts(
      delta_HYPER_vs_delta_noHYPER    = (HYPER.F - HYPER.B) - (NO_HYPER.F - NO_HYPER.B), 
      levels=design
)
fit2 <- contrasts.fit(fit, cm)
fit2 <- eBayes(fit2)
results_HYPER <- topTable(fit2,coef="delta_HYPER_vs_delta_noHYPER",number = nrow(ymat))
results_HYPER$AptName <- row.names(results_HYPER)
results_HYPER <- merge(results_HYPER,analytes,by="AptName",all.x = T, all.y = F)
results_HYPER <- results_HYPER[order(results_HYPER$P.Value),] 
