...
# glycemic failure
# with A1c
glyc_a1c_contrast <- model.matrix(~GLYC+hba1c+log_trig+sbp+si_1_ins0,fup)
# moderated t-tests with and without FDR adjustment
...
# glycemic failure
glyc_contrast <- model.matrix(~GLYC+hba1c+log_trig+sbp+si_1_ins0,fup)
# moderated t-tests with and without FDR adjustment
y <- fup_log %>% filter(!is.na(fup_log$GLYC))
ymat <- t(y[,seq])
...
saveWorkbook(wb,"./Results/Metabolomics Linear and Cox models/TODAY urine metabolomics limma yr10 adjusted.xlsx",overwrite = TRUE)
