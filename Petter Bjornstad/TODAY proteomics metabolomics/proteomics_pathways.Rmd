...
```{r}
htmltools::includeHTML("./Results/Enrichment/Project_MAC_GSEA/Report_MAC_GSEA.html")
```
...
fit = lmFit(soma,m)
fit <- eBayes(fit)
gsea(fit,name = "MAC_adjusted")
```

```{r eval=FALSE}
htmltools::includeHTML("./Results/Enrichment/Project_MAC_adjusted_GSEA/Report_MAC_adjusted_GSEA.html")
```
