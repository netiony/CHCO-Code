...
tme <- tme %>% select(RELEASEID,TMETYPE)
colnames(tme) <- c("releaseid","TMETYPE")
...
# remove those who had bariatric surgery
base <- merge(base, tme, by="releaseid", all.x=T, all.y=F)
base <- base %>% filter(is.na(TMETYPE))
