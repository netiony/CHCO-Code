...
# Notes: for some reason the base_for_deltas df isn't behaving right
# won't allow me to calculate deltas
# my idea was to create a df of deltas plus binary variable for glycemic outcome and then compare changes in proteins in 2 groups
...
# ADJUSTED
delta_glyc_contrast_adj <- model.matrix(~ 1 + GLYC + bmi + si_1_ins0 + AGEBASE + sex, fup_log)
# moderated t-tests with and without FDR adjustment
ymat <- t(deltas)
fit <- lmFit(ymat,delta_glyc_contrast)
fit <- eBayes(fit)
...
saveWorkbook(wb,"/Users/pylell/Dropbox/TODAY glycemic manuscript [shared]/TODAY somalogic limma delta glyc.xlsx",overwrite = TRUE)
```
