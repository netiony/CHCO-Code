...
```{r, include=FALSE, comment=""}
form = as.formula(paste0("Surv(DAYSTOHTN, HTN)~",paste0(htn_select,collapse = "+"),paste0("+HbA1c+log_trig+sbp+si_1_ins0")))
mod = coxph(formula = form,data = htn_nozero)
kable(tidy(mod, exponentiate = TRUE, conf.int = TRUE),digits = 3)
...
moddf <- cbind(moddf$term,round(moddf$estimate, digits=3),round(moddf$std.error, digits=3),round(moddf$p.value, digits=3))
colnames(moddf) <- c("Variable","Hazard Ratio","SE","P-value")
concordance_df_temp_htn <- as.data.frame(cbind("Hypertension","Proteins + covariates",mod$concordance[[6]], mod$concordance[[7]]))
concordance_df_htn <- rbind(concordance_df_htn, concordance_df_temp_htn)
```
...
moddf <- cbind(moddf$term,round(moddf$estimate, digits=3),round(moddf$std.error, digits=3),round(moddf$p.value, digits=3))
colnames(moddf) <- c("Variable","Hazard Ratio","SE","P-value")
concordance_df_temp_htn <- as.data.frame(cbind("Hypertension","Covariates",mod$concordance[[6]], mod$concordance[[7]]))
concordance_df_htn <- rbind(concordance_df_htn, concordance_df_temp_htn)
```
...
# Forest plot of concordance and 95% CI
...
concordance_df_htn <- rbind(concordance_df_htn[3,],concordance_df_htn[1,],concordance_df_htn[2,])
