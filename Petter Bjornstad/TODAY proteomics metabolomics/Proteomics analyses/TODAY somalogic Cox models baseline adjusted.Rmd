...
if(Sys.info()["sysname"] == "Windows"){
  ...
  home_dir = "/Volumes/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/TODAY subward"
}
...
tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
contrast <- emmeans(mod, ~ seq.5590.11|sex, type="response")
emmip(mod, sex ~ seq.5590.11) 
# I don't think this comparison is working correctly, as there is only 1 observation per sex
# p = 0.03
...
# WFKN2
mod <- coxph(data = base_log_scale, formula = Surv(DAYSTOHYP, HYP)~ HbA1c+log_trig+sbp+si_1_ins0 + seq.13408.23*sex)
tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)

# SEZ6L
mod <- coxph(data = base_log_scale, formula = Surv(DAYSTOHYP, HYP)~ HbA1c+log_trig+sbp+si_1_ins0 + seq.19563.3*sex)
tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)

# SCG3
mod <- coxph(data = base_log_scale, formula = Surv(DAYSTOHYP, HYP)~ HbA1c+log_trig+sbp+si_1_ins0 + seq.7957.2*sex)
tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
```
