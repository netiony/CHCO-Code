...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOMAC, MAC)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOMIC, MIC)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOMIC.OR.MAC, MIC.OR.MAC)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHYP, HYP)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTORAPID, RAPID)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHTN, HTN)~",predictors[i],"+HbA1c+log_trig+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHTN, HTN)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHTN, HTN)~",predictors[i],"+HbA1c+log_trig+log_UAlbCreat+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHTN, HTN)~",predictors[i],"+HbA1c+log_trig+EstCreatClear+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTONEURO, NEURO)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTORETINO, RETINO)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOGLYC, GLYC)~",predictors[i],"+bmi+si_1_ins0+sex_char+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOGLYC, GLYC)~",predictors[i],"+HbA1c+bmi+si_1_ins0+sex_char+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
saveWorkbook(wb,"./Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted new covariates.xlsx",overwrite = TRUE)
saveWorkbook(wb,"/Users/pylell/Dropbox/TODAY HTN and DKD output/TODAY somalogic Cox models scaled baseline adjusted new covariates.xlsx",overwrite = TRUE)

# separate files for IPA
wb <- createWorkbook()
addWorksheet(wb,"MAC CPH")
writeData(wb,"MAC CPH",result_MAC,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted MAC new covariates.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"MIC CPH")
writeData(wb,"MIC CPH",result_MIC,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted MIC new covariates.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"MIC.OR.MAC CPH")
writeData(wb,"MIC.OR.MAC CPH",result_MIC.OR.MAC,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted MIC.OR.MAC new covariates.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"HYP CPH")
writeData(wb,"HYP CPH",result_HYP,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted HYP new covariates.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"RAPID CPH")
writeData(wb,"RAPID CPH",result_RAPID,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted RAPID new covariates.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"HTN with SBP CPH")
writeData(wb,"HTN with SBP CPH",results_htn_bp,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted HTN with SBP new covariates.xlsx",overwrite = TRUE)
...
addWorksheet(wb,"NEURO CPH")
writeData(wb,"NEURO CPH",result_NEURO,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted NEURO new covariates.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"RETINO CPH")
writeData(wb,"RETINO CPH",result_RETINO,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted RETINO new covariates.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"GLYC CPH")
writeData(wb,"GLYC CPH",result_GLYC,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted GLYC new covariates.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"GLYC with A1c CPH")
writeData(wb,"GLYC with A1c CPH",results_glyc_a1c,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted GLYC with A1c new covariates.xlsx",overwrite = TRUE)
...
#write.csv(result_intersect, "/Volumes/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/TODAY subaward//Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted intersection new covariates.csv",
#          row.names = F)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOMAC, MAC)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  result_MAC <- rbind(result_MAC,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOMIC, MIC)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  result_MIC <- rbind(result_MIC,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOMIC.OR.MAC, MIC.OR.MAC)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  result_MIC.OR.MAC <- rbind(result_MIC.OR.MAC,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHYP, HYP)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  result_HYP <- rbind(result_HYP,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTORAPID, RAPID)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  result_RAPID <- rbind(result_RAPID,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHTN, HTN)~",predictors[i],"+HbA1c+log_trig+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  result_HTN <- rbind(result_HTN,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHTN, HTN)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  results_htn_bp <- rbind(results_htn_bp,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHTN, HTN)~",predictors[i],"+HbA1c+log_trig+log_UAlbCreat+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  results_htn_uacr <- rbind(results_htn_uacr,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOHTN, HTN)~",predictors[i],"+HbA1c+log_trig+EstCreatClear+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  results_htn_egfr <- rbind(results_htn_egfr,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTONEURO, NEURO)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  result_NEURO <- rbind(result_NEURO,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTORETINO, RETINO)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0+tx+sex_char+bmi+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  result_RETINO <- rbind(result_RETINO,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOGLYC, GLYC)~",predictors[i],"+bmi+si_1_ins0+tx+sex_char+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  result_GLYC <- rbind(result_GLYC,tidymod)
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(DAYSTOGLYC, GLYC)~",predictors[i],"+HbA1c+bmi+si_1_ins0+tx+sex_char+AGEBASE+racedesc")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
  results_glyc_a1c <- rbind(results_glyc_a1c,tidymod)
...
addWorksheet(wb,"GLYC with A1c CPH")
writeData(wb,"GLYC with A1c CPH",results_glyc_a1c,rowNames = F)
saveWorkbook(wb,"/Volumes/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/TODAY subaward/Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted TX GROUP new covariates.xlsx",overwrite = TRUE)
saveWorkbook(wb,"/Users/pylell/Dropbox/TODAY HTN and DKD output/TODAY somalogic Cox models scaled baseline adjusted TX GROUP new covariates.xlsx",overwrite = TRUE)
