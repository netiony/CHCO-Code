...
f = "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/limma_MTF_results.csv"
f_keep <- read.csv(f)
f_keep <- arrange(f_keep,adj.P.Val)
f_drop <- f_keep[101:nrow(f_keep),"AptName"]
f_drop <- as.data.frame(f_drop)
rownames(f_drop) <- f_keep[101:nrow(f_keep),"AptName"]
f_drop <- as.data.frame(t(f_drop))
mtf_keep <- df_diff %>%
  filter(group == "MTF") %>%
  dplyr::select(contains("seq")) 
mtf_keep <- df_diff %>% select(-colnames(f_drop))  
```
...
```{r, echo = F}
# filter to top 100 proteins by effect size
f = "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/limma_FTM_results.csv"
```
