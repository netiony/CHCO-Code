...
load("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Somalogic data/knight_soma.Rdata")
load("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Somalogic data/analytes.Rdata")

# read in linkage file for Natalie's studies
link <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Somalogic data/BCF-23-091 Linker File 10.05.2023.csv")
link$Study.ID <- link$Sequence.Number
redcap <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Somalogic data/200572DataKNIGHT_DATA_LABELS_2023-12-10_1551.csv")
...
#nethlink <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Somalogic data/sample ID and visit labels SOMAscan WUS-23-004.csv")
...
sexhormones <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Data_raw/Excel KNIGHT - sex hormones.csv")
sexhormones <- sexhormones %>% select(study_nr, Visit, estradiol_CU, testosteron_CU)
extravars <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Data_raw/Excel KNIGHT extra data + variables.csv")
neth <- full_join(sexhormones, extravars, by=c("study_nr","Visit"))
gfr <-  read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Data_raw/Excel KNIGHT -GFR ERPF.csv")
...
crc_soma <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/Data Harmonization/Data Clean/soma_harmonized_dataset.csv", na.strings = "")
crc_soma <- crc_soma %>%
  filter(
...
write.csv(res, file = "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/limma_MTF_results.csv", row.names = F)
...
res$TargetFullName <- analytes$TargetFullName[match(rownames(res), analytes$AptName)]
res$UniProt <- analytes$UniProt[match(rownames(res), analytes$AptName)]
write.csv(res, file = "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/limma_FTM_results.csv", row.names = F)
...
corr_estradiol_CU <- corr.test(x = x, y = y, method = "spearman", adjust = "none")
corr_estradiol_CU_r <- data.frame(spearman_delta_estradiol = (corr_estradiol_CU$r %>% t() ))
corr_estradiol_CU_p <- data.frame(p.value = (corr_estradiol_CU$p %>% t() ))
corr_estradiol_CU_mat <- cbind(corr_estradiol_CU_r, corr_estradiol_CU_p)
colnames(corr_estradiol_CU_mat) <- c("spearman_delta_estradiol", "p.value")
...
corr_estradiol_CU_mat_save <- corr.test(x = x, y = y, method="spearman", adjust="none")
corr_estradiol_CU_mat_save_r <- data.frame(spearman_delta_estradiol = (corr_estradiol_CU_mat_save$r %>% t() ))
corr_estradiol_CU_mat_save_p <- data.frame(p.value = (corr_estradiol_CU_mat_save$p %>% t() ))
corr_estradiol_CU_mat_save <- cbind(corr_estradiol_CU_mat_save_r, corr_estradiol_CU_mat_save_p)
colnames(corr_estradiol_CU_mat_save) <- c("spearman_delta_estradiol", "p.value")
...
corr_M_value_corrected <- corr.test(x = x, y = y, method = "spearman", adjust = "none")
corr_M_value_corrected_r <- data.frame(spearman_delta_M_value_corrected = (corr_M_value_corrected$r %>% t() ))
corr_M_value_corrected_p <- data.frame(p.value = (corr_M_value_corrected$p %>% t() ))
corr_M_value_corrected_mat <- cbind(corr_M_value_corrected_r, corr_M_value_corrected_p)
colnames(corr_M_value_corrected_mat) <- c("spearman_delta_M_value_corrected", "p.value")

corr_M_value_corrected_mat <- corr_M_value_corrected_mat %>%
  rownames_to_column(var = "AptName") %>%
  left_join(analytes, by = "AptName") %>%
  # column_to_rownames("Target") %>%
  dplyr::select(spearman_delta_M_value_corrected, p.value, Target)

corr_M_value_corrected_mat_save <- corr.test(x = x, y = y, method="spearman", adjust="none")
corr_M_value_corrected_mat_save_r <- data.frame(spearman_delta_M_value_corrected = (corr_M_value_corrected_mat_save$r %>% t() ))
corr_M_value_corrected_mat_save_p <- data.frame(p.value = (corr_M_value_corrected_mat_save$p %>% t() ))
corr_M_value_corrected_mat_save <- cbind(corr_M_value_corrected_mat_save_r, corr_M_value_corrected_mat_save_p)
colnames(corr_M_value_corrected_mat_save) <- c("spearman_delta_M_value_corrected", "p.value")
...
corr_Plasma_uric_acid <- corr.test(x = x, y = y, method = "spearman", adjust = "none")
corr_Plasma_uric_acid_r <- data.frame(spearman_delta_Plasma_uric_acid = (corr_Plasma_uric_acid$r %>% t() ))
corr_Plasma_uric_acid_p <- data.frame(p.value = (corr_Plasma_uric_acid$p %>% t() ))
corr_Plasma_uric_acid_mat <- cbind(corr_Plasma_uric_acid_r, corr_Plasma_uric_acid_p)
colnames(corr_Plasma_uric_acid_mat) <- c("spearman_delta_Plasma_uric_acid", "p.value")

corr_Plasma_uric_acid_mat <- corr_Plasma_uric_acid_mat %>%
  rownames_to_column(var = "AptName") %>%
  left_join(analytes, by = "AptName") %>%
  # column_to_rownames("Target") %>%
  dplyr::select(spearman_delta_Plasma_uric_acid, p.value, Target)

corr_Plasma_uric_acid_mat_save <- corr.test(x = x, y = y, method="spearman", adjust="none")
corr_Plasma_uric_acid_mat_save_r <- data.frame(spearman_delta_Plasma_uric_acid = (corr_Plasma_uric_acid_mat_save$r %>% t() ))
corr_Plasma_uric_acid_mat_save_p <- data.frame(p.value = (corr_Plasma_uric_acid_mat_save$p %>% t() ))
corr_Plasma_uric_acid_mat_save <- cbind(corr_Plasma_uric_acid_mat_save_r, corr_Plasma_uric_acid_mat_save_p)
colnames(corr_Plasma_uric_acid_mat_save) <- c("spearman_delta_Plasma_uric_acid", "p.value")
...
corr_URklaring <- corr.test(x = x, y = y, method = "spearman", adjust = "none")
corr_URklaring_r <- data.frame(spearman_delta_URklaring = (corr_URklaring$r %>% t() ))
corr_URklaring_p <- data.frame(p.value = (corr_URklaring$p %>% t() ))
corr_URklaring_mat <- cbind(corr_URklaring_r, corr_URklaring_p)
colnames(corr_URklaring_mat) <- c("spearman_delta_URklaring", "p.value")

corr_URklaring_mat <- corr_URklaring_mat %>%
  rownames_to_column(var = "AptName") %>%
  left_join(analytes, by = "AptName") %>%
  # column_to_rownames("Target") %>%
  dplyr::select(spearman_delta_URklaring, p.value, Target)

corr_URklaring_mat_save <- corr.test(x = x, y = y, method="spearman", adjust="none")
corr_URklaring_mat_save_r <- data.frame(spearman_delta_URklaring = (corr_URklaring_mat_save$r %>% t() ))
corr_URklaring_mat_save_p <- data.frame(p.value = (corr_URklaring_mat_save$p %>% t() ))
corr_URklaring_mat_save <- cbind(corr_URklaring_mat_save_r, corr_URklaring_mat_save_p)
colnames(corr_URklaring_mat_save) <- c("spearman_delta_URklaring", "p.value")
...
corr_FEUA <- corr.test(x = x, y = y, method = "spearman", adjust = "none")
corr_FEUA_r <- data.frame(spearman_delta_FEUA = (corr_FEUA$r %>% t() ))
corr_FEUA_p <- data.frame(p.value = (corr_FEUA$p %>% t() ))
corr_FEUA_mat <- cbind(corr_FEUA_r, corr_FEUA_p)
colnames(corr_FEUA_mat) <- c("spearman_delta_FEUA", "p.value")

corr_FEUA_mat <- corr_FEUA_mat %>%
  rownames_to_column(var = "AptName") %>%
  left_join(analytes, by = "AptName") %>%
  # column_to_rownames("Target") %>%
  dplyr::select(spearman_delta_FEUA, p.value, Target)

corr_FEUA_mat_save <- corr.test(x = x, y = y, method="spearman", adjust="none")
corr_FEUA_mat_save_r <- data.frame(spearman_delta_FEUA = (corr_FEUA_mat_save$r %>% t() ))
corr_FEUA_mat_save_p <- data.frame(p.value = (corr_FEUA_mat_save$p %>% t() ))
corr_FEUA_mat_save <- cbind(corr_FEUA_mat_save_r, corr_FEUA_mat_save_p)
colnames(corr_FEUA_mat_save) <- c("spearman_delta_FEUA", "p.value")
...
corr_GFR..mL.min. <- corr.test(x = x, y = y, method = "spearman", adjust = "none")
corr_GFR..mL.min._r <- data.frame(spearman_delta_GFR..mL.min. = (corr_GFR..mL.min.$r %>% t() ))
corr_GFR..mL.min._p <- data.frame(p.value = (corr_GFR..mL.min.$p %>% t() ))
corr_GFR..mL.min._mat <- cbind(corr_GFR..mL.min._r, corr_GFR..mL.min._p)
colnames(corr_GFR..mL.min._mat) <- c("spearman_delta_GFR..mL.min.", "p.value")
...
corr_GFR..mL.min._mat_save <- corr.test(x = x, y = y, method="spearman", adjust="none")
corr_GFR..mL.min._mat_save_r <- data.frame(spearman_delta_GFR..mL.min. = (corr_GFR..mL.min._mat_save$r %>% t() ))
corr_GFR..mL.min._mat_save_p <- data.frame(p.value = (corr_GFR..mL.min._mat_save$p %>% t() ))
corr_GFR..mL.min._mat_save <- cbind(corr_GFR..mL.min._mat_save_r, corr_GFR..mL.min._mat_save_p)
colnames(corr_GFR..mL.min._mat_save) <- c("spearman_delta_GFR..mL.min.", "p.value")
...
corr_ERPF..mL.min. <- corr.test(x = x, y = y, method = "spearman", adjust = "none")
corr_ERPF..mL.min._r <- data.frame(spearman_delta_ERPF..mL.min. = (corr_ERPF..mL.min.$r %>% t() ))
corr_ERPF..mL.min._p <- data.frame(p.value = (corr_ERPF..mL.min.$p %>% t() ))
corr_ERPF..mL.min._mat <- cbind(corr_ERPF..mL.min._r, corr_ERPF..mL.min._p)
colnames(corr_ERPF..mL.min._mat) <- c("spearman_delta_ERPF..mL.min.", "p.value")
...
corr_ERPF..mL.min._mat_save <- corr.test(x = x, y = y, method="spearman", adjust="none")
corr_ERPF..mL.min._mat_save_r <- data.frame(spearman_delta_ERPF..mL.min. = (corr_ERPF..mL.min._mat_save$r %>% t() ))
corr_ERPF..mL.min._mat_save_p <- data.frame(p.value = (corr_ERPF..mL.min._mat_save$p %>% t() ))
corr_ERPF..mL.min._mat_save <- cbind(corr_ERPF..mL.min._mat_save_r, corr_ERPF..mL.min._mat_save_p)
colnames(corr_ERPF..mL.min._mat_save) <- c("spearman_delta_ERPF..mL.min.", "p.value")
...
saveWorkbook(wb,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/KNIGHT_correlations.xlsx",overwrite = TRUE)
...
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/KNIGHT_correlations_testosterone.xlsx",overwrite = TRUE)

ipa <- createWorkbook()
addWorksheet(ipa,"Change in estradiol")
writeData(ipa,"Change in estradiol",corr_estradiol_CU_mat_save,rowNames = F)
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/KNIGHT_correlations_estradiol.xlsx",overwrite = TRUE)
...
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/KNIGHT_correlations_M-value.xlsx",overwrite = TRUE)
...
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/KNIGHT_correlations_plasma_uric_acid.xlsx",overwrite = TRUE)

ipa <- createWorkbook()
addWorksheet(ipa,"Change in URklaring")
writeData(ipa,"Change in URklaring",corr_URklaring_mat_save,rowNames = F)
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/KNIGHT_correlations_URklaring.xlsx",overwrite = TRUE)

ipa <- createWorkbook()
addWorksheet(ipa,"Change in FEUA")
writeData(ipa,"Change in FEUA",corr_FEUA_mat_save,rowNames = F)
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/KNIGHT_correlations_FEUA.xlsx",overwrite = TRUE)

ipa <- createWorkbook()
addWorksheet(ipa,"Change in GFR")
writeData(ipa,"Change in GFR",corr_GFR..mL.min._mat_save,rowNames = F)
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/KNIGHT_correlations_GFR.xlsx",overwrite = TRUE)

ipa <- createWorkbook()
addWorksheet(ipa,"Change in ERPF")
writeData(ipa,"Change in ERPF",corr_ERPF..mL.min._mat_save,rowNames = F)
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/KNIGHT_correlations_ERPF.xlsx",overwrite = TRUE)
```
...
saveWorkbook(wb,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/KNIGHT_vs_CRC_comparisons.xlsx",overwrite = TRUE)
...
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/results_kMTF_CRCM.xlsx",overwrite = TRUE)
  
ipa <- createWorkbook()
addWorksheet(ipa,"results_kFTM_CRCF")
writeData(ipa,"results_kFTM_CRCF",results_kFTM_CRCF,rowNames = F)
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/results_kFTM_CRCF.xlsx",overwrite = TRUE)
  
ipa <- createWorkbook()
addWorksheet(ipa,"results_kMTF_CRCF")
writeData(ipa,"results_kMTF_CRCF",results_kMTF_CRCF,rowNames = F)
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/results_kMTF_CRCF.xlsx",overwrite = TRUE)  

ipa <- createWorkbook()
addWorksheet(ipa,"results_kFTM_CRCM")
writeData(ipa,"results_kFTM_CRCM",results_kFTM_CRCM,rowNames = F)
saveWorkbook(ipa,"/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/IPA/results_kFTM_CRCM.xlsx",overwrite = TRUE) 
...
res$TargetFullName <- analytes$TargetFullName[match(rownames(res), analytes$AptName)]
res$UniProt <- analytes$UniProt[match(rownames(res), analytes$AptName)]
write.csv(res, file = "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/Results/limma_MTF_vs_FTM_results.csv", row.names = F)
