  ...
  main = NULL, x.lab = NULL, sig_fc_lab = NULL, overlaps = 10, ...) 
{
  ...
  if (!is.null(identify_manual) & label_apt) {
    ...
    p <- p + geom_label_repel(
      data = dplyr::filter(plot_df, !!.label %in% identify_manual), aes(label = Target_apt), color = "black", min.segment.length = 0,
      max.overlaps = overlaps, nudge_x = -0.3, force = 4,
    ) 
  }
  if (!is.null(identify_manual) & !label_apt) {
...
corr_estradiol_CU_mat <- corr_estradiol_CU_mat %>%
  ...
  left_join(analytes, by = "AptName") %>%
  # column_to_rownames("Target") %>%
  dplyr::select(spearman_delta_estradiol, p.value, Target)
...
png(height = 1500, width = 2000, file = "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/KNIGHT/van Raalte talent grant/GFR_volcano_plot.png", res = 300)
  plotVolcano_mod_new(data = corr_GFR..mL.min._mat, 
            ...
            label_apt = T, 
            identify = F,
            identify_manual = c("SFRP4", "SCGF-beta", "FSTL1"),
            fc.cutoff = 0,
            cutoff = 0.05,
            x.lab = "Correlation",
            main = "",
...
corr_ERPF..mL.min._mat <- corr_ERPF..mL.min._mat %>%
  ...
  left_join(analytes, by = "AptName") %>%
  # column_to_rownames("Target") %>%
  dplyr::select(spearman_delta_ERPF..mL.min., p.value, Target)
