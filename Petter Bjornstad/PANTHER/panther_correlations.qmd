...
# Correlations between tanner stage & proteins
...
kendall_ts_mat %>% 
  ...
  group_map(~ ts_tbl(.x, columns = "tanner_stage_comp_ord")) %>%
  data.frame(.) %>%
...
plotVolcano_mod(kendall_partial_ts_df, 
                ...
                cutoff = 0.05,
                x.lab = "Correlation",
                main = "Kendall's rank sum multi-level correlations \n (adjusting for risk)",
                pt.size = 1.5,
                text.size = 3,
                fc_lab = "p > 0.05",
                fc_lab_col = "#dad7cd",
...
plotVolcano_mod(spearman_partial_ts_df, 
                ...
                cutoff = 0.05,
                x.lab = "Correlation",
                main = "Spearman's multi-level correlations \n (adjusting for risk)",
                pt.size = 1.5,
                text.size = 3,
                fc_lab = "p > 0.05",
                fc_lab_col = "#dad7cd",
...
# Correlations between MINMOD & proteins
...
# Spearman correlations
spearman_ts_airg <- correlation_table_minimal(data = label_harmonized(cleanData_panther, dict),
                                         relevant_vars = c("mm_airg", aov_tbl_ordinal_top),
                                         cor_method = "spearman",
                                         n_cols = 1,
                                         dict = dict,
                                         raw = T) %>%
  dplyr::rename(mm_airg = AIRg,
                p.value = `p-value`)

spearman_ts_airg_save <- correlation_table_minimal(data = label_harmonized(cleanData_panther, dict),
                                              relevant_vars = c("mm_airg", analytes$AptName),
                                              cor_method = "spearman",
                                              n_cols = 1,
                                              dict = dict,
                                              raw = T) %>%
...
spearman_ts_airg_save <- spearman_ts_airg_save %>%
  rownames_to_column(var = "Target_nodup") %>%
  left_join(analytes, by = "Target_nodup")

write.csv(spearman_ts_airg_save, 
          "/Volumes/Peds Endo/Petter Bjornstad/PANTHER/Results/spearman_correlation_airg.csv", 
          row.names = T)

# Heatmap tables (combined)
spearman_ts_airg %>% 
  dplyr::mutate(direction = ifelse(mm_airg < 0, "Negative", "Positive"), 
         Target_nodup = row.names(.))  %>% 
  left_join(analytes, by = "Target_nodup") %>% 
  filter(p.value < 0.05) %>%
  arrange(desc(abs(mm_airg))) %>% 
  group_by(relevel(factor(direction), "Negative")) %>% 
  slice_head(n=75) %>% 
  dplyr::select(Target, mm_airg) %>%
  group_map(~ ts_tbl(.x, columns = "mm_airg")) %>%
  data.frame(.) %>%
  setNames(., c("Negative\n correlation coefficient", "Positive\n correlation coefficient")) %>%
  gt() %>%
  fmt_markdown(columns = TRUE) 
...
Total number of correlations with p\<0.05: `r nrow(subset(spearman_ts_airg, p.value < 0.05))`

Total number of negative correlations with p\<0.05: `r nrow(subset(spearman_ts_airg, p.value < 0.05 & mm_airg <0))`

Total number of positive correlations with p\<0.05: `r nrow(subset(spearman_ts_airg, p.value < 0.05 & mm_airg >0))`

#### Volcano plot of correlation

```{r, echo = F}
spearman_ts_airg_cor_dat <- (spearman_ts_airg %>%
                  rownames_to_column(var = "Target_nodup") %>%
                  left_join(analytes, by = "Target_nodup"))

negative_correlations <- arrange(spearman_ts_airg_cor_dat, p.value) %>%
  filter(mm_airg < 0) 
positive_correlations <- arrange(spearman_ts_airg_cor_dat, p.value) %>%
  filter(mm_airg > 0) 
top_10_correlations <- c(negative_correlations$Target[1:10], 
                         positive_correlations$Target[1:10])

plotVolcano_mod(spearman_ts_airg_cor_dat, 
                FC = mm_airg, 
                p.value = p.value, 
                labels = Target, 
                identify = F,
...
```{r, warning=F, echo=F, message = F}
spearman_ts_airg_positive <- subset(spearman_ts_airg, mm_airg > 0 & p.value < 0.05) %>% 
  rownames_to_column("Target_nodup") %>%
  arrange(p.value)

spearman_ts_airg_positive_analytes <- 
  spearman_ts_airg_positive %>%
  left_join(analytes)
...
spearman_ts_airg_positive %>%
  ...
  tab_header(title = "Positive correlations")  %>%
  cols_label_with(fn = ~gsub("_\\d+$", "", .x)) %>%
  cols_label_with(fn = ~gsub("mm_airg" , "Correlation coefficient", .x)) %>%
  cols_label_with(fn = ~gsub("Target" , "Protein", .x)) %>%
  data_color(method = "numeric",
             palette = c("#eae2b7", "#d62828"),
             domain = c(0,0.7))
...
```{r, warning=F, echo=F, message = F}
spearman_ts_airg_negative <- subset(spearman_ts_airg, mm_airg < 0 & p.value < 0.05) %>% 
  rownames_to_column("Target_nodup") %>%
  arrange(p.value)

spearman_ts_airg_negative_analytes <- 
  spearman_ts_airg_negative %>%
  left_join(analytes)
...
spearman_ts_airg_negative %>%
  ...
  cols_label_with(fn = ~gsub("mm_airg" , "Correlation coefficient", .x)) %>%
  cols_label_with(fn = ~gsub("Target" , "Protein", .x)) %>%
  #cols_label(mm_airg = "correlation coefficient") %>%
  data_color(method = "numeric",
             palette = c("#023e8a", "#ade8f4"),
             domain = c(-1,0))
```

#### Partial Spearman's correlations adjusting for risk

```{r, warning=F, echo=F, message = F}
# Create an empty dataframe to store results
spearman_partial_ts_airg_df <- data.frame()

# Loop through each element of aov_tbl_ordinal_top
for (i in seq_along(aov_tbl_ordinal_top)) {
  
  # Calculate partial spearman correlation
  spearman_partial_ts_airg <- correlation(data = cleanData_panther,
                                    select = c("mm_airg", "group_risk"),
                                    select2 = aov_tbl_ordinal_top[i],
                                    method = "spearman", 
                                    partial = TRUE, 
                                    multilevel = TRUE)
  ...
  # Append the result to the main dataframe
  spearman_partial_ts_airg_df <- rbind(spearman_partial_ts_airg_df, spearman_partial_ts_airg)
}
spearman_partial_ts_airg_df<- as.data.frame(spearman_partial_ts_airg_df)
colnames(spearman_partial_ts_airg_df) <- c('param1','AptName','tau', "conf.level","lower", "upper","z", "p.value", "method", "n_obs")
spearman_partial_ts_airg_df <- left_join(spearman_partial_ts_airg_df,analytes)
...
Total number of correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_airg_df, p.value < 0.05))`

Total number of negative correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_airg_df, p.value < 0.05 & tau <0))`

Total number of positive correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_airg_df, p.value < 0.05 & tau >0))`

#### Volcano plot of correlation

```{r, warning=F, echo=F, message = F}
negative_correlations <- arrange(spearman_partial_ts_airg_df, p.value) %>%
  filter(tau < 0) 
positive_correlations <- arrange(spearman_partial_ts_airg_df, p.value) %>%
  filter(tau > 0) 
top_10_correlations <- c(negative_correlations$Target[1:10], 
                         positive_correlations$Target[1:10])

plotVolcano_mod(spearman_partial_ts_airg_df, 
                FC = tau, 
                p.value = p.value, 
                labels = Target, 
                identify = F,
...
```{r, warning=F, echo=F, message = F}
# Spearman correlations
spearman_ts_di <- correlation_table_minimal(data = label_harmonized(cleanData_panther, dict),
                                         relevant_vars = c("mm_di", aov_tbl_ordinal_top),
                                         cor_method = "spearman",
                                         n_cols = 1,
                                         dict = dict,
                                         raw = T) %>%
  dplyr::rename(mm_di = DI,
                p.value = `p-value`)

spearman_ts_di_save <- correlation_table_minimal(data = label_harmonized(cleanData_panther, dict),
                                              relevant_vars = c("mm_di", analytes$AptName),
                                              cor_method = "spearman",
                                              n_cols = 1,
                                              dict = dict,
                                              raw = T) %>%
...
spearman_ts_di_save <- spearman_ts_di_save %>%
  rownames_to_column(var = "Target_nodup") %>%
  left_join(analytes, by = "Target_nodup")

write.csv(spearman_ts_di_save, 
          "/Volumes/Peds Endo/Petter Bjornstad/PANTHER/Results/spearman_correlation_di.csv", 
          row.names = T)

# Heatmap tables (combined)
spearman_ts_di %>% 
  dplyr::mutate(direction = ifelse(mm_di < 0, "Negative", "Positive"), 
         Target_nodup = row.names(.))  %>% 
  left_join(analytes, by = "Target_nodup") %>% 
  filter(p.value < 0.05) %>%
  arrange(desc(abs(mm_di))) %>% 
  group_by(relevel(factor(direction), "Negative")) %>% 
  slice_head(n=75) %>% 
  dplyr::select(Target, mm_di) %>%
  group_map(~ ts_tbl(.x, columns = "mm_di")) %>%
  data.frame(.) %>%
  setNames(., c("Negative\n correlation coefficient", "Positive\n correlation coefficient")) %>%
  gt() %>%
  fmt_markdown(columns = TRUE) 
...
Total number of correlations with p\<0.05: `r nrow(subset(spearman_ts_di, p.value < 0.05))`

Total number of negative correlations with p\<0.05: `r nrow(subset(spearman_ts_di, p.value < 0.05 & mm_di <0))`

Total number of positive correlations with p\<0.05: `r nrow(subset(spearman_ts_di, p.value < 0.05 & mm_di >0))`

#### Volcano plot of correlation

```{r, echo = F}
spearman_ts_di_cor_dat <- (spearman_ts_di %>%
                  rownames_to_column(var = "Target_nodup") %>%
                  left_join(analytes, by = "Target_nodup"))

negative_correlations <- arrange(spearman_ts_di_cor_dat, p.value) %>%
  filter(mm_di < 0) 
positive_correlations <- arrange(spearman_ts_di_cor_dat, p.value) %>%
  filter(mm_di > 0) 
top_10_correlations <- c(negative_correlations$Target[1:10], 
                         positive_correlations$Target[1:10])

plotVolcano_mod(spearman_ts_di_cor_dat, 
                FC = mm_di, 
                p.value = p.value, 
                labels = Target, 
                identify = F,
...
```{r, warning=F, echo=F, message = F}
spearman_ts_di_positive <- subset(spearman_ts_di, mm_di > 0 & p.value < 0.05) %>% 
  rownames_to_column("Target_nodup") %>%
  arrange(p.value)

spearman_ts_di_positive_analytes <- 
  spearman_ts_di_positive %>%
  left_join(analytes)
...
spearman_ts_di_positive %>%
  ...
  tab_header(title = "Positive correlations")  %>%
  cols_label_with(fn = ~gsub("_\\d+$", "", .x)) %>%
  cols_label_with(fn = ~gsub("mm_di" , "Correlation coefficient", .x)) %>%
  cols_label_with(fn = ~gsub("Target" , "Protein", .x)) %>%
  data_color(method = "numeric",
             palette = c("#eae2b7", "#d62828"),
             domain = c(0,0.7))
...
```{r, warning=F, echo=F, message = F}
spearman_ts_di_negative <- subset(spearman_ts_di, mm_di < 0 & p.value < 0.05) %>% 
  rownames_to_column("Target_nodup") %>%
  arrange(p.value)

spearman_ts_di_negative_analytes <- 
  spearman_ts_di_negative %>%
  left_join(analytes)
...
spearman_ts_di_negative %>%
  ...
  cols_label_with(fn = ~gsub("mm_di" , "Correlation coefficient", .x)) %>%
  cols_label_with(fn = ~gsub("Target" , "Protein", .x)) %>%
  #cols_label(mm_di = "correlation coefficient") %>%
  data_color(method = "numeric",
             palette = c("#023e8a", "#ade8f4"),
             domain = c(-1,0))
```
...
```{r, warning=F, echo=F, message = F}
# Create an empty dataframe to store results
spearman_partial_ts_di_df <- data.frame()

# Loop through each element of aov_tbl_ordinal_top
for (i in seq_along(aov_tbl_ordinal_top)) {
  
  # Calculate partial spearman correlation
  spearman_partial_ts_di <- correlation(data = cleanData_panther,
                                    select = c("mm_di", "group_risk"),
                                    select2 = aov_tbl_ordinal_top[i],
                                    method = "spearman", 
                                    partial = TRUE, 
                                    multilevel = TRUE)
  ...
  # Append the result to the main dataframe
  spearman_partial_ts_di_df <- rbind(spearman_partial_ts_di_df, spearman_partial_ts_di)
}
spearman_partial_ts_di_df<- as.data.frame(spearman_partial_ts_di_df)
colnames(spearman_partial_ts_di_df) <- c('param1','AptName','tau', "conf.level","lower", "upper","z", "p.value", "method", "n_obs")
spearman_partial_ts_di_df <- left_join(spearman_partial_ts_di_df,analytes)
...
Total number of correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_di_df, p.value < 0.05))`

Total number of negative correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_di_df, p.value < 0.05 & tau <0))`

Total number of positive correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_di_df, p.value < 0.05 & tau >0))`

#### Volcano plot of correlation

```{r, warning=F, echo=F, message = F}
negative_correlations <- arrange(spearman_partial_ts_di_df, p.value) %>%
  filter(tau < 0) 
positive_correlations <- arrange(spearman_partial_ts_di_df, p.value) %>%
  filter(tau > 0) 
top_10_correlations <- c(negative_correlations$Target[1:10], 
                         positive_correlations$Target[1:10])

plotVolcano_mod(spearman_partial_ts_di_df, 
                FC = tau, 
                p.value = p.value, 
                labels = Target, 
                identify = F,
...
```{r, warning=F, echo=F, message = F}
# Spearman correlations
spearman_ts_si <- correlation_table_minimal(data = label_harmonized(cleanData_panther, dict),
                                         relevant_vars = c("mm_si", aov_tbl_ordinal_top),
                                         cor_method = "spearman",
                                         n_cols = 1,
                                         dict = dict,
                                         raw = T) %>%
  dplyr::rename(mm_si = `Insulin sensitivity`,
                p.value = `p-value`)

spearman_ts_si_save <- correlation_table_minimal(data = label_harmonized(cleanData_panther, dict),
                                              relevant_vars = c("mm_si", analytes$AptName),
                                              cor_method = "spearman",
                                              n_cols = 1,
                                              dict = dict,
                                              raw = T) %>%
...
spearman_ts_si_save <- spearman_ts_si_save %>%
  rownames_to_column(var = "Target_nodup") %>%
  left_join(analytes, by = "Target_nodup")

write.csv(spearman_ts_si_save, 
          "/Volumes/Peds Endo/Petter Bjornstad/PANTHER/Results/spearman_correlation_si.csv", 
          row.names = T)
...
spearman_ts_si %>% 
  ...
  group_by(relevel(factor(direction), "Negative")) %>% 
  slice_head(n=75) %>% 
  dplyr::select(Target, mm_si) %>%
  group_map(~ ts_tbl(.x, columns = "mm_si")) %>%
  data.frame(.) %>%
  setNames(., c("Negative\n correlation coefficient", "Positive\n correlation coefficient")) %>%
  gt() %>%
  fmt_markdown(columns = TRUE) 
...
Total number of correlations with p\<0.05: `r nrow(subset(spearman_ts_si, p.value < 0.05))`

Total number of negative correlations with p\<0.05: `r nrow(subset(spearman_ts_si, p.value < 0.05 & mm_si <0))`

Total number of positive correlations with p\<0.05: `r nrow(subset(spearman_ts_si, p.value < 0.05 & mm_si >0))`

#### Volcano plot of correlation

```{r, echo = F}
spearman_ts_si_cor_dat <- (spearman_ts_si %>%
                  rownames_to_column(var = "Target_nodup") %>%
                  left_join(analytes, by = "Target_nodup"))

negative_correlations <- arrange(spearman_ts_si_cor_dat, p.value) %>%
  filter(mm_si < 0) 
positive_correlations <- arrange(spearman_ts_si_cor_dat, p.value) %>%
  filter(mm_si > 0) 
top_10_correlations <- c(negative_correlations$Target[1:10], 
                         positive_correlations$Target[1:10])

plotVolcano_mod(spearman_ts_si_cor_dat, 
                FC = mm_si, 
                p.value = p.value, 
                labels = Target, 
                identify = F,
...
```{r, warning=F, echo=F, message = F}
spearman_ts_si_positive <- subset(spearman_ts_si, mm_si > 0 & p.value < 0.05) %>% 
  rownames_to_column("Target_nodup") %>%
  arrange(p.value)

spearman_ts_si_positive_analytes <- 
  spearman_ts_si_positive %>%
  left_join(analytes)
...
spearman_ts_si_positive %>%
  ...
  tab_header(title = "Positive correlations")  %>%
  cols_label_with(fn = ~gsub("_\\d+$", "", .x)) %>%
  cols_label_with(fn = ~gsub("mm_si" , "Correlation coefficient", .x)) %>%
  cols_label_with(fn = ~gsub("Target" , "Protein", .x)) %>%
  data_color(method = "numeric",
             palette = c("#eae2b7", "#d62828"),
             domain = c(0,0.7))
...
```{r, warning=F, echo=F, message = F}
spearman_ts_si_negative <- subset(spearman_ts_si, mm_si < 0 & p.value < 0.05) %>% 
  rownames_to_column("Target_nodup") %>%
  arrange(p.value)

spearman_ts_si_negative_analytes <- 
  spearman_ts_si_negative %>%
  left_join(analytes)
...
spearman_ts_si_negative %>%
  ...
  cols_label_with(fn = ~gsub("mm_si" , "Correlation coefficient", .x)) %>%
  cols_label_with(fn = ~gsub("Target" , "Protein", .x)) %>%
  #cols_label(mm_si = "correlation coefficient") %>%
  data_color(method = "numeric",
             palette = c("#023e8a", "#ade8f4"),
             domain = c(-1,0))
```
...
```{r, warning=F, echo=F, message = F}
# Create an empty dataframe to store results
spearman_partial_ts_si_df <- data.frame()

# Loop through each element of aov_tbl_ordinal_top
for (i in seq_along(aov_tbl_ordinal_top)) {
  
  # Calculate partial spearman correlation
  spearman_partial_ts_si <- correlation(data = cleanData_panther,
                                    select = c("mm_si", "group_risk"),
                                    select2 = aov_tbl_ordinal_top[i],
                                    method = "spearman", 
                                    partial = TRUE, 
                                    multilevel = TRUE)
  ...
  # Append the result to the main dataframe
  spearman_partial_ts_si_df <- rbind(spearman_partial_ts_si_df, spearman_partial_ts_si)
}
spearman_partial_ts_si_df<- as.data.frame(spearman_partial_ts_si_df)
colnames(spearman_partial_ts_si_df) <- c('param1','AptName','tau', "conf.level","lower", "upper","z", "p.value", "method", "n_obs")
spearman_partial_ts_si_df <- left_join(spearman_partial_ts_si_df,analytes)
...
Total number of correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_si_df, p.value < 0.05))`

Total number of negative correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_si_df, p.value < 0.05 & tau <0))`

Total number of positive correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_si_df, p.value < 0.05 & tau >0))`

#### Volcano plot of correlation

```{r, warning=F, echo=F, message = F}
negative_correlations <- arrange(spearman_partial_ts_si_df, p.value) %>%
  filter(tau < 0) 
positive_correlations <- arrange(spearman_partial_ts_si_df, p.value) %>%
  filter(tau > 0) 
top_10_correlations <- c(negative_correlations$Target[1:10], 
                         positive_correlations$Target[1:10])

plotVolcano_mod(spearman_partial_ts_si_df, 
                FC = tau, 
                p.value = p.value, 
                labels = Target, 
                identify = F,
...
```{r, warning=F, echo=F, message = F}
# Spearman correlations
spearman_ts_bcell <- correlation_table_minimal(data = label_harmonized(cleanData_panther, dict),
                                         relevant_vars = c("mm_bcell", aov_tbl_ordinal_top),
                                         cor_method = "spearman",
                                         n_cols = 1,
                                         dict = dict,
                                         raw = T) %>%
  dplyr::rename(mm_bcell = `Beta cell function`,
                p.value = `p-value`)

spearman_ts_bcell_save <- correlation_table_minimal(data = label_harmonized(cleanData_panther, dict),
                                              relevant_vars = c("mm_bcell", analytes$AptName),
                                              cor_method = "spearman",
                                              n_cols = 1,
                                              dict = dict,
                                              raw = T) %>%
...
spearman_ts_bcell_save <- spearman_ts_bcell_save %>%
  rownames_to_column(var = "Target_nodup") %>%
  left_join(analytes, by = "Target_nodup")

write.csv(spearman_ts_bcell_save, 
          "/Volumes/Peds Endo/Petter Bjornstad/PANTHER/Results/spearman_correlation_bcell.csv", 
          row.names = T)

# Heatmap tables (combined)
spearman_ts_bcell %>% 
  dplyr::mutate(direction = ifelse(mm_bcell < 0, "Negative", "Positive"), 
         Target_nodup = row.names(.))  %>% 
  left_join(analytes, by = "Target_nodup") %>% 
  filter(p.value < 0.05) %>%
  arrange(desc(abs(mm_bcell))) %>% 
  group_by(relevel(factor(direction), "Negative")) %>% 
  slice_head(n=75) %>% 
  dplyr::select(Target, mm_bcell) %>%
  group_map(~ ts_tbl(.x, columns = "mm_bcell")) %>%
  data.frame(.) %>%
  setNames(., c("Negative\n correlation coefficient", "Positive\n correlation coefficient")) %>%
  gt() %>%
  fmt_markdown(columns = TRUE) 
...
Total number of correlations with p\<0.05: `r nrow(subset(spearman_ts_bcell, p.value < 0.05))`

Total number of negative correlations with p\<0.05: `r nrow(subset(spearman_ts_bcell, p.value < 0.05 & mm_bcell <0))`

Total number of positive correlations with p\<0.05: `r nrow(subset(spearman_ts_bcell, p.value < 0.05 & mm_bcell >0))`

#### Volcano plot of correlation

```{r, echo = F}
spearman_ts_bcell_cor_dat <- (spearman_ts_bcell %>%
                  rownames_to_column(var = "Target_nodup") %>%
                  left_join(analytes, by = "Target_nodup"))

negative_correlations <- arrange(spearman_ts_bcell_cor_dat, p.value) %>%
  filter(mm_bcell < 0) 
positive_correlations <- arrange(spearman_ts_bcell_cor_dat, p.value) %>%
  filter(mm_bcell > 0) 
top_10_correlations <- c(negative_correlations$Target[1:10], 
                         positive_correlations$Target[1:10])

plotVolcano_mod(spearman_ts_bcell_cor_dat, 
                FC = mm_bcell, 
                p.value = p.value, 
                labels = Target, 
                identify = F,
...
```{r, warning=F, echo=F, message = F}
spearman_ts_bcell_positive <- subset(spearman_ts_bcell, mm_bcell > 0 & p.value < 0.05) %>% 
  rownames_to_column("Target_nodup") %>%
  arrange(p.value)

spearman_ts_bcell_positive_analytes <- 
  spearman_ts_bcell_positive %>%
  left_join(analytes)
...
spearman_ts_bcell_positive %>%
  ...
  tab_header(title = "Positive correlations")  %>%
  cols_label_with(fn = ~gsub("_\\d+$", "", .x)) %>%
  cols_label_with(fn = ~gsub("mm_bcell" , "Correlation coefficient", .x)) %>%
  cols_label_with(fn = ~gsub("Target" , "Protein", .x)) %>%
  data_color(method = "numeric",
             palette = c("#eae2b7", "#d62828"),
             domain = c(0,0.7))
...
```{r, warning=F, echo=F, message = F}
spearman_ts_bcell_negative <- subset(spearman_ts_bcell, mm_bcell < 0 & p.value < 0.05) %>% 
  rownames_to_column("Target_nodup") %>%
  arrange(p.value)

spearman_ts_bcell_negative_analytes <- 
  spearman_ts_bcell_negative %>%
  left_join(analytes)
...
spearman_ts_bcell_negative %>%
  ...
  cols_label_with(fn = ~gsub("mm_bcell" , "Correlation coefficient", .x)) %>%
  cols_label_with(fn = ~gsub("Target" , "Protein", .x)) %>%
  #cols_label(mm_bcell = "correlation coefficient") %>%
  data_color(method = "numeric",
             palette = c("#023e8a", "#ade8f4"),
             domain = c(-1,0))
```
...
```{r, warning=F, echo=F, message = F}
# Create an empty dataframe to store results
spearman_partial_ts_bcell_df <- data.frame()

# Loop through each element of aov_tbl_ordinal_top
for (i in seq_along(aov_tbl_ordinal_top)) {
  
  # Calculate partial spearman correlation
  spearman_partial_ts_bcell <- correlation(data = cleanData_panther,
                                    select = c("mm_bcell", "group_risk"),
                                    select2 = aov_tbl_ordinal_top[i],
                                    method = "spearman", 
                                    partial = TRUE, 
                                    multilevel = TRUE)
  ...
  # Append the result to the main dataframe
  spearman_partial_ts_bcell_df <- rbind(spearman_partial_ts_bcell_df, spearman_partial_ts_bcell)
}
spearman_partial_ts_bcell_df<- as.data.frame(spearman_partial_ts_bcell_df)
colnames(spearman_partial_ts_bcell_df) <- c('param1','AptName','tau', "conf.level","lower", "upper","z", "p.value", "method", "n_obs")
spearman_partial_ts_bcell_df <- left_join(spearman_partial_ts_bcell_df,analytes)
...
Total number of correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_bcell_df, p.value < 0.05))`

Total number of negative correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_bcell_df, p.value < 0.05 & tau <0))`

Total number of positive correlations with p\<0.05: `r nrow(subset(spearman_partial_ts_bcell_df, p.value < 0.05 & tau >0))`

#### Volcano plot of correlation

```{r, warning=F, echo=F, message = F}
negative_correlations <- arrange(spearman_partial_ts_bcell_df, p.value) %>%
  filter(tau < 0) 
positive_correlations <- arrange(spearman_partial_ts_bcell_df, p.value) %>%
  filter(tau > 0) 
top_10_correlations <- c(negative_correlations$Target[1:10], 
                         positive_correlations$Target[1:10])

plotVolcano_mod(spearman_partial_ts_bcell_df, 
                FC = tau, 
                p.value = p.value, 
                labels = Target, 
                identify = F,
