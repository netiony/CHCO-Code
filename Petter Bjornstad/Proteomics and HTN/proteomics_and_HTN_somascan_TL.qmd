...
means_res_bp$Target <- NA
means_res_bp$TargetFullName <- means_res_bp$a
means_res_bp$a <- NULL
means_res_bp$UniProt <- NA
...
# FOR PLOTS - WILL NEED TO COMBINE TARGET AND SEQNAME
```
...
```{r}
# NOTE: This might be pulling off the wrong coefficient, not for BP
...
means_res_prot_sbp <- means_res
lmm_res_prot_sbp$bp_var <- "sbp"
means_res_prot_sbp$bp_var <- "sbp"
lmm_res <- NULL
means_res <- NULL
```
...
means_res_prot_dbp <- means_res
lmm_res_prot_dbp$bp_var <- "dbp"
means_res_prot_dbp$bp_var <- "dbp"
lmm_res <- NULL
means_res <- NULL
