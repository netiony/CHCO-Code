...
load("/Users/pylell/Documents/GitHub/CHCO-Code/Petter Bjornstad/Proteomics and HTN/Data_Cleaned/analysis_dataset.RData")
top_htn <- top_htn_df %>%
...
df_long_keep <- df %>% select(ID, visit, sbp, dbp, all_of(seq_include))

load("/Users/pylell/Documents/GitHub/CHCO-Code/Petter Bjornstad/TODAY subaward/Somalogic data raw/analytes.Rdata")
...
means_res$TA <- ifelse(!(means_res$Target %in% c("sbp","dbp")), paste0(means_res$Target, " (", means_res$AptName, ")"), means_res$Target)
...
for (i in bpvars) {
  p[[i]] <- ggplot(means_res[means_res$Target == i,], aes(x=visit, y=emmean, colour=TA, group=TA)) +
    ...
    geom_errorbar(aes(ymin=emmean-SE, ymax=emmean+SE), width=.1, color = "black") +
    xlab("") +
    ylab(toupper(i)) + theme_bw() + theme(legend.position = "none") + scale_color_discrete(name="") 
}

panel_p <- ggarrange(p[[1]],p[[2]], ncol = 2, nrow = 2)
png('/Users/pylell/Dropbox/TODAY HTN and DKD output/Teen-LABS_BP.png', res = 300, width = 10, height = 8, units = "in")
panel_p
dev.off()
...
lmm_res <- rbind(lmm_res_prot_sbp, lmm_res_prot_dbp)
