...
prot_heatmap <- top_htn_df %>%
  filter(Target %in% c("SEZ6L","WFKN2","SCG3","H6ST3","Nr-CAM")) %>%
  pull(AptName)
...
prot <- top_htn_df %>%
  ...
  pull(Target)
p1 <- ggplot(df_t2d[!is.na(df_t2d$htn),],
       mapping = aes(x = htn,
                     y = seq.19563.3,
                     fill = htn))  +
  geom_boxplot() +  
...
prot <- top_htn_df %>%
  ...
  pull(Target)
p2 <- ggplot(df_t2d[!is.na(df_t2d$htn),],
       mapping = aes(x = htn,
                     y = seq.13408.23,
                     fill = htn))  +
  geom_boxplot() +  
...
prot <- top_htn_df %>%
  ...
  pull(Target)
p3 <- ggplot(df_t2d[!is.na(df_t2d$htn),],
       mapping = aes(x = htn,
                     y = seq.7957.2,
                     fill = htn))  +
  geom_boxplot() +  
...
# seq.18896.23
prot <- top_htn_df %>%
  filter(AptName %in% c("seq.18896.23")) %>%
  pull(Target)
...
# seq.3235.50
prot <- top_htn_df %>%
  filter(AptName %in% c("seq.3235.50")) %>%
  pull(Target)
p5 <- ggplot(df_t2d[!is.na(df_t2d$htn),],
       mapping = aes(x = htn,
                     y = seq.3235.50,
                     fill = htn))  +
  geom_boxplot() +  
