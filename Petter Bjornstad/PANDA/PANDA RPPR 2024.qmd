...
PANDA_rc <- redcap_read(redcap_uri = "https://redcap.ucdenver.edu/api/",
                        token = "FFB3E81F9B8EB62E96B05F6AACD728D2")$data
CROC_rc <- redcap_read(redcap_uri = "https://redcap.ucdenver.edu/api/",
                        token = "08235ADD045CB3CB6FEE0F8F4E819B6A")$data
...
ATTEMPT_mrn <- ATTEMPT_rc %>% select(subject_id, mrn, name_last, name_first) %>% distinct(subject_id, .keep_all = T) %>%
