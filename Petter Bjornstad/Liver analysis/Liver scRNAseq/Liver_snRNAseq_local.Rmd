...
nebula_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    ...
    #With offset
    result <- nebula(count = count_gene, id = meta_gene$Kit_Lot, pred = pred_gene, ncore = 1,reml=T,output_re = T,covariance=T,offset = offset_hvg)
    #With NBLMM
    # result <- nebula(count = count_gene, id = meta_gene$Kit_Lot, pred = pred_gene, ncore = 1,reml=T,output_re = T,covariance=T,model="NBLMM")
...
write.csv(full_results,fs::path(dir.results,"NEBULA_Hepatocytes_Steatosis_unadjusted_2000_hvg_reml_offset.csv"))
...
volcano_plot <- ggplot(full_results, aes(x = `logFC_steatosis_catHigh Steatosis (2+3)`, y = PValue10, color = color)) +
  ...
  labs(
    ...
    subtitle = "Hepatocytes, Unadjusted (REML, Offset)",
    x = "FC",
    y = "-log10(P-Value)",
...
# print(volcano_plot)
# dev.off()
png(fs::path(dir.results, "Plot_NEBULA_Hepatocytes_Steatosis_unadjusted_2000_hvg_reml_offset.png"),
...
### iii. GSEA
```{r echo = F}
full_results <- read.csv(fs::path(dir.results,"NEBULA_Hepatocytes_Steatosis_unadjusted_2000_hvg_reml_offset.csv")) %>%
  clean_names() %>% 
  dplyr::select(-x) %>% 
...
##### KEGG Legacy
plot_fgsea_transpose(kegg_legacy_res_Hepatocytes, title = "Hepatocytes Unadjusted Top 30 KEGG (REML, Offset)", xnudge = 0.03)

ggsave(fs::path(dir.results,"Hepatocytes_top30_kegg_pathways_unadjusted_reml_offset.jpeg"),
       width = 13, height = 10, scale = 1)

##### REACTOME
plot_fgsea_transpose(reactome_res_Hepatocytes, title = "Hepatocytes Unadjusted Top 30 REACTOME (REML, Offset)", xlimit = 5)

ggsave(fs::path(dir.results,"Hepatocytes_top30_reactome_pathways_unadjusted_reml_offset.jpeg"),
       width = 13, height = 10, scale = 1)
##### GO
plot_fgsea_transpose(go_res_Hepatocytes, title = "Hepatocytes Unadjusted Top 30 GO (REML, Offset)", xlimit = 8)
...
# count_gene <- counts_hvg[g, , drop = FALSE]
meta_gene_hvg <- so_celltype@meta.data
id <- meta_gene_hvg$Kit_Lot
pred_gene_hvg <- model.matrix(~steatosis_cat, data = meta_gene_hvg)
data_g_hvg = list(count = counts_hvg, id = id, pred = pred_gene_hvg)
offset_hvg = Matrix::colSums(data_g_hvg$count) 
...
nebula_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    ...
    result <- nebula(count = count_gene, id = meta_gene$Kit_Lot, pred = pred_gene, ncore = 1,output_re = T,covariance=T,reml=T,offset = offset_hvg)
    list(gene = g, result = result)  # return both gene name and result
...
write.csv(full_results,fs::path(dir.results,"Targeted_Genes_NEBULA_Hepatocytes_Steatosis_unadjusted_reml_offset.csv"))
...
dot_plot
ggsave(fs::path(dir.results, "NEBULA_Targeted_Hepatocytes_Steatosis_unadjusted_reml_offset.jpeg"), dot_plot, width = 7, height = 6)
