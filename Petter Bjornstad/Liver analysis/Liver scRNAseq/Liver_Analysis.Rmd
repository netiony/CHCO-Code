...
meta_liver_raw <- read.csv(fs::path(dir.dat,"scRNA","data_clean","liver_biopsy_metadata_PN.csv"))
unique(meta_liver_raw$Kit_Lot)
meta_liver_sc <-  so_liver_sc@meta.data[,1:11] %>%
  dplyr::mutate(RNAlater_ID = SampleID) %>%
  left_join(meta_liver_raw)
...
##c. Pathway Cells All Cells
```{r}
#TCA
all_genes <- tca
#Diabetes yes vs. no
degs_fxn_pathway(so=so_liver_sn,cell=NULL,gene_set=all_genes,exposure="diagnosis_of_diabetes",exp_group="Yes",ref_group="No",pathway="TCA_Cycle")
#Create diab so
so_diab <- subset(so_liver_sn,diagnosis_of_diabetes=="Yes")
#Either vs. None
so_diab$Medication <- so_diab$group3
so_diab$Medication <- factor(so_diab$Medication)
degs_fxn_pathway(so=so_sub,cell=NULL,gene_set=all_genes,exposure="group3",exp_group="either",ref_group="neither",pathway="TCA_Cycle")

#GLP-1 vs. None
so_sub <- subset(so_diab,group2=="glp1_exclusive" | group2=="neither")
so_sub$Medication <- so_sub$group2
so_sub$Medication <- factor(so_sub$Medication)
degs_fxn_pathway(so=so_sub,cell=NULL,gene_set=all_genes,exposure="group2",exp_group="glp1_exclusive",ref_group="neither",pathway="TCA_Cycle")

#Both vs. None
so_sub <- subset(so_diab,group2=="both" | group2=="neither")
so_sub$Medication <- so_sub$group2
so_sub$Medication <- factor(so_sub$Medication)
degs_fxn_pathway(so=so_sub,cell=NULL,gene_set=all_genes,exposure="group2",exp_group="both",ref_group="neither",pathway="TCA_Cycle")

#Ox Phos
all_genes <- intersect(ox_phos,rownames(so_liver_sn))
pathway <- "Ox_Phos"
#Diabetes yes vs. no
degs_fxn_pathway(so=so_liver_sn,cell=NULL,gene_set=all_genes,exposure="diagnosis_of_diabetes",exp_group="Yes",ref_group="No",pathway=pathway)
#Create diab so
so_diab <- subset(so_liver_sn,diagnosis_of_diabetes=="Yes")
#Either vs. None
so_diab$Medication <- so_diab$group3
so_diab$Medication <- factor(so_diab$Medication)
degs_fxn_pathway(so=so_diab,cell=NULL,gene_set=all_genes,exposure="Medication",exp_group="either",ref_group="neither",pathway=pathway)

#GLP-1 vs. None
so_sub <- subset(so_diab,group2=="glp1_exclusive" | group2=="neither")
so_sub$Medication <- so_sub$group2
so_sub$Medication <- factor(so_sub$Medication)
degs_fxn_pathway(so=so_sub,cell=NULL,gene_set=all_genes,exposure="Medication",exp_group="glp1_exclusive",ref_group="neither",pathway=pathway)

#Both vs. None
so_sub <- subset(so_diab,group2=="both" | group2=="neither")
so_sub$Medication <- so_sub$group2
so_sub$Medication <- factor(so_sub$Medication)
degs_fxn_pathway(so=so_sub,cell=NULL,gene_set=all_genes,exposure="Medication",exp_group="both",ref_group="neither",pathway=pathway)

#Beta Oxidation
all_genes <- intersect(beta_ox,rownames(so_liver_sn))
...
#Gluconeogenesis
all_genes <- intersect(gluco,rownames(so_liver_sn))
...
#Glycolysis
all_genes <- intersect(glyc,rownames(so_liver_sn))
pathway <- "Glycolysis"
#Diabetes yes vs. no
degs_fxn_pathway(so=so_liver_sn,cell=NULL,gene_set=all_genes,exposure="diagnosis_of_diabetes",exp_group="Yes",ref_group="No",pathway=pathway)
#Create diab so
...
#Energy Metabolism
all_genes <- intersect(energy_met,rownames(so_liver_sn))
...
#lipid metabolism
all_genes <- intersect(lipid_met,rownames(so_liver_sn))
...
#Metabolism
all_genes <- intersect(met,rownames(so_liver_sn))
pathway <- "Metabolism"
#Diabetes yes vs. no
degs_fxn_pathway(so=so_liver_sn,cell=NULL,gene_set=all_genes,exposure="diagnosis_of_diabetes",exp_group="Yes",ref_group="No",pathway=pathway)
#Create diab so
...
#Inflammation
all_genes <- intersect(inflamatory,rownames(so_liver_sn))
pathway <- "Inflamation"
#Diabetes yes vs. no
degs_fxn_pathway(so=so_liver_sn,cell=NULL,gene_set=all_genes,exposure="diagnosis_of_diabetes",exp_group="Yes",ref_group="No",pathway=pathway)
#Create diab so
...
#Cytokines
all_genes <- intersect(cytokines,rownames(so_liver_sn))
pathway <- "Cytokines"
#Diabetes yes vs. no
degs_fxn_pathway(so=so_liver_sn,cell=NULL,gene_set=all_genes,exposure="diagnosis_of_diabetes",exp_group="Yes",ref_group="No",pathway=pathway)
#Create diab so
...
#Chemokines
all_genes <- intersect(chemokines,rownames(so_liver_sn))
pathway <- "Chemokines"
#Diabetes yes vs. no
degs_fxn_pathway(so=so_liver_sn,cell=NULL,gene_set=all_genes,exposure="diagnosis_of_diabetes",exp_group="Yes",ref_group="No",pathway=pathway)
#Create diab so
...
so_diab$Medication <- so_diab$group3
so_diab$Medication <- factor(so_diab$Medication)
degs_fxn_pathway(so=so_sub,cell=NULL,gene_set=all_genes,exposure="group3",exp_group="either",ref_group="neither",pathway=pathway)

#GLP-1 vs. None
so_sub <- subset(so_diab,group2=="glp1_exclusive" | group2=="neither")
so_sub$Medication <- so_sub$group2
so_sub$Medication <- factor(so_sub$Medication)
degs_fxn_pathway(so=so_sub,cell=NULL,gene_set=all_genes,exposure="group2",exp_group="glp1_exclusive",ref_group="neither",pathway=pathway)

#Both vs. None
so_sub <- subset(so_diab,group2=="both" | group2=="neither")
so_sub$Medication <- so_sub$group2
so_sub$Medication <- factor(so_sub$Medication)
degs_fxn_pathway(so=so_sub,cell=NULL,gene_set=all_genes,exposure="group2",exp_group="both",ref_group="neither",pathway=pathway)
```

##d. Specefic Cells Pathway Cells
```{r}


```

##e. Senescence Pseudobulk
```{r}
#Among Type 2: No meds vs. GLP-1 Exclusive
##SGLT2i only -/+ #5 No, 0 yes
#GLP-1 only +/- # 2 Yes, 3 No
...
#Either vs. Neither
so_sub <- subset(so_diab,group3=="either" | group2=="neither")
so_sub$group2 <- factor(so_sub$group3)
degs_fxn_sens(so=so_sub,cell=NULL,gene_set=all_genes,exposure="group3",exp_group="either",ref_group="neither",enrichment="No",top_gsea = 30)
...
#Either vs. Neither
so_sub <- subset(so_diab,group3=="either" | group3 == "neither")
so_sub$group3 <- factor(so_sub$group3)
#Define genes
all_genes <- rownames(so_sub)
all_cell_types <- unique(so_sub$celltype2)[-c(6,11)]
for (cell_type in all_cell_types) {
  degs_fxn_sens(so=so_sub,cell=cell_type,gene_set=all_genes,exposure="group3",exp_group="either",ref_group="neither",enrichment="No",top_gsea = 30)
}
...
#sens_gene_in_data <- intersect(sens_genes, rownames(so_liver_sn))
#so_sens_all <- subset(so_liver_sn, features = sens_gene_in_data)
mast_sens_fxn(so=so_sens_all,cell=NULL,exposure="diagnosis_of_diabetes",gene_set=all_genes,exp_group="Yes",ref_group="No")
...
#AST
mast_sens_fxn(so=so_sens_all,cell=NULL,exposure="ast",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
#mast_sens_fxn(so=so_diab,cell=NULL,exposure="ast",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)

#ALT
mast_sens_fxn(so=so_sens_all,cell=NULL,exposure="alt",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
#mast_sens_fxn(so=so_diab,cell=NULL,exposure="alt",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)

#Fibrosis
mast_sens_fxn(so=so_sens_all,cell=NULL,exposure="fibrosis_stage",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
#mast_sens_fxn(so=so_diab,cell=NULL,exposure="fibrosis_stage",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)

#Steatosis
mast_sens_fxn(so=so_sens_all,cell=NULL,exposure="steatosis_grade",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
#mast_sens_fxn(so=so_diab,cell=NULL,exposure="steatosis_grade",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
...
mast_sens_fxn(so=so_subset,cell=NULL,exposure="Medication",covariate="alt",gene_set=gene_set,exp_group="neither",ref_group="obese_control")
...
so_subset$Medication <- factor(so_subset$Medication)
gene_set <- rownames(so_subset)
mast_sens_fxn(so=so_subset,cell=NULL,exposure="Medication",covariate="alt",gene_set=gene_set,exp_group="glp1_exclusive",ref_group="obese_control")
...
so_subset$Medication <- factor(so_subset$Medication)
gene_set <- rownames(so_subset)
mast_sens_fxn(so=so_subset,cell=NULL,covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="both",ref_group="obese_control")

#Either vs. Neither
so_subset <- subset(so_sens_all,group3=="neither" | group3=="either")
so_subset$Medication <- so_subset$group3
so_subset$Medication <- factor(so_subset$Medication)
gene_set <- rownames(so_subset)
mast_sens_fxn(so=so_subset,cell=NULL,covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="either",ref_group="neither")
...
so_subset$Medication <- factor(so_subset$Medication)
gene_set <- rownames(so_subset)
mast_sens_fxn(so=so_subset,cell=NULL,covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="glp1_exclusive",ref_group="neither")
...
so_subset$Medication <- factor(so_subset$Medication)
gene_set <- rownames(so_subset)
mast_sens_fxn(so=so_subset,cell=NULL,covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="both",ref_group="neither")
...
so_subset$Medication <- factor(so_subset$Medication)
gene_set <- rownames(so_subset)
mast_sens_fxn(so=so_subset,cell=NULL,covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="both",ref_group="glp1_exclusive")
...
for (cell in all_cell_types){
  mast_sens_fxn(so=so_sens_all,cell=cell,exposure="diagnosis_of_diabetes",gene_set=gene_set,exp_group="Yes",ref_group="No")
}
...
#AST
#for (cell in all_cell_types){
  mast_sens_fxn(so=so_diab,cell="Hepatocyte",exposure="ast",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
#} 

#ALT
#for (cell in all_cell_types){
  mast_sens_fxn(so=so_diab,cell="Hepatocyte",exposure="alt",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
#}

#Fibrosis
mast_sens_fxn(so=so_sens_all,cell="Hepatocyte",exposure="fibrosis_stage",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
#mast_sens_fxn(so=so_diab,cell=NULL,exposure="fibrosis_stage",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)

#Steatosis
mast_sens_fxn(so=so_sens_all,cell="Hepatocyte",exposure="steatosis_grade",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
#mast_sens_fxn(so=so_diab,cell=NULL,exposure="steatosis_grade",covariate=NULL,gene_set=gene_set,exp_group=NULL,ref_group=NULL)
...
for (cell in all_cell_types){
  mast_sens_fxn(so=so_subset,cell=cell,covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="neither",ref_group="obese_control")
} 
#GLP-1 vs. OC
so_subset <- subset(so_sens_all,group2=="obese_control" | group2=="glp1_exclusive")
so_subset$Medication <- so_subset$group2
...
gene_set <- rownames(so_subset)
for (cell in all_cell_types){
  mast_sens_fxn(so=so_subset,cell=cell,covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="glp1_exclusive",ref_group="obese_control")
} 
...
gene_set <- rownames(so_subset)
for (cell in all_cell_types){
  mast_sens_fxn(so=so_subset,cell=cell,covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="both",ref_group="obese_control")
}
...
gene_set <- rownames(so_subset)
#for (cell in all_cell_types){
  mast_sens_fxn(so=so_subset,cell="Hepatocyte",covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="glp1_exclusive",ref_group="neither")
#}
...
so_subset$Medication <- so_subset$group2
so_subset$Medication <- factor(so_subset$Medication)
so_subset$Medication <- relevel(so_subset$Medication, ref = "neither")
gene_set <- rownames(so_subset)
#for (cell in all_cell_types){
  mast_sens_fxn(so=so_subset,cell="Hepatocyte",covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="both",ref_group="neither")
#} 
...
gene_set <- rownames(so_subset)
for (cell in all_cell_types){
  mast_sens_fxn(so=so_subset,cell=cell,covariate="alt",exposure="Medication",gene_set=gene_set,exp_group="both",ref_group="glp1_exclusive")
}
...
gene_set <- rownames(so_subset)
#for (cell in all_cell_types){
  mast_sens_fxn(so=so_subset,cell="Hepatocyte",covariate="ast",exposure="Medication",gene_set=gene_set,exp_group="glp1_exclusive",ref_group="neither")
#}
...
gene_set <- rownames(so_subset)
#for (cell in all_cell_types){
  mast_sens_fxn(so=so_subset,cell="Hepatocyte",covariate="ast",exposure="Medication",gene_set=gene_set,exp_group="both",ref_group="neither")
#} 
```
...
#11. Pathway Scores
##a. Metabolism Pathways
```{r}
#TCA
sens_genes <- intersect(inflamatory, rownames(so_liver_sn))
...
#Diabetes Yes No
meta.data$diagnosis_of_diabetes <- factor(meta.data$diagnosis_of_diabetes)
lmm <- lmer(InflScore1 ~ diagnosis_of_diabetes + (1|Kit_Lot), data=meta.data)
summary(lmm)

meta.data$group2 <- factor(meta.data$group2)
lmm <- lmer(InflScore1 ~ group2 + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

meta.data$group3 <- factor(meta.data$group3)
lmm <- lmer(InflScore1 ~ group3 + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(TCAScore1 ~ diagnosis_of_MASLD + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(TCAScore1 ~ fibrosis_stage + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(TCAScore1 ~ steatosis_grade + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(TCAScore1 ~ alt + fibrosis_stage+ (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(TCAScore1 ~ alt + fibrosis_stage+ group2+(1|Kit_Lot), data=meta.data.diab)
summary(lmm)

lmm <- lmer(SenescenceScore1 ~ group2 + fibrosis_stage+ (1|Kit_Lot), data=meta.data.diab)
summary(lmm)

ggplot(meta.data, aes(x = alt, y = SenescenceScore1)) +
  geom_point(alpha = 0.6) +  # Scatter points
  geom_smooth(method = "lm", color = "blue", se = FALSE) +  # Linear regression line
  theme_minimal() +
  labs(title = paste0("Senescence Score vs. ALT (U/L)"),
       x = "ALT (U/L)",
       y = "Senescence Score") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

lmm <- lmer(SenescenceScore1 ~ ast + fibrosis_stage + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

ggplot(meta.data, aes(x = ast, y = SenescenceScore1)) +
  geom_point(alpha = 0.6) +  # Scatter points
  geom_smooth(method = "lm", color = "blue", se = FALSE) +  # Linear regression line
  theme_minimal() +
  labs(title = paste0("Senescence Score vs. AST (U/L)"),
       x = "AST (U/L)",
       y = "Senescence Score") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

meta.data$ALT_log <- log(meta.data$alt)
lmm <- lmer(SenescenceScore1 ~ ALT_log + group2 + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

meta.data$AST_log <- log(meta.data$ast)
lmm <- lmer(SenescenceScore1 ~ AST_log + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(SenescenceScore1 ~ ggt + fibrosis_stage + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(SenescenceScore1 ~ ggt + group3 + (1|Kit_Lot), data=meta.data.diab)
summary(lmm)


meta.data$GGT_log <- log(meta.data$ggt)
lmm <- lmer(SenescenceScore1 ~ GGT_log + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

ggplot(meta.data, aes(x = ggt, y = SenescenceScore1)) +
  geom_point(alpha = 0.6) +  # Scatter points
  geom_smooth(method = "lm", color = "blue", se = FALSE) +  # Linear regression line
  theme_minimal() +
  labs(title = paste0("Senescence Score vs. GGT (U/L)"),
       x = "GGT (U/L)",
       y = "Senescence Score") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
...
#Type 2 Diabetes
ggplot(so_liver_sn@meta.data, aes(x = diagnosis_of_diabetes, y = TCAScore1, fill = diagnosis_of_diabetes)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.2, outlier.shape = NA) +
  theme_minimal() +
  labs(title = "Senescence Score by Diabetes Status", x = "Diabetes (Yes vs. No)", y = "Senescence Score")

ggplot(so_liver_sn@meta.data, aes(x = group2, y = TCAScore1, fill = group2)) +
...
results <- data.frame()
for (oro in oro_vars){
  oro_var <- meta.data[[oro]]
  p <- ggplot(meta.data, aes(x = !!sym(oro), y = SenescenceScore1)) +
  geom_point(alpha = 0.6) +  # Scatter points
  geom_smooth(method = "lm", color = "blue", se = FALSE) +  # Linear regression line
  theme_minimal() +
  labs(title = paste0("Senescence Score vs. ",str_to_title(oro)),
       ...
       y = "Senescence Score") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
  pdf(fs::path(dir.results,paste0(oro,"_vs_SenescenceScore.pdf")))
  print(p)
  dev.off()
  
  lmm <- lmer(SenescenceScore1 ~ oro_var + (1|Kit_Lot), data=meta.data)
  coef <- summary(lmm)$coef[2,1]
  p <- summary(lmm)$coef[2,5]
  results_data <- data.frame(Oro=oro,Coef=coef,Pvalue=p)
  results <- rbind(results,results_data)
}
write.csv(results,fs::path(dir.results,paste0(oro,"_vs_SenescenceScore.csv")))
...
#Obese controls only
meta.data.oc <- meta.data %>%
  filter(diagnosis_of_diabetes=="No")
lmm <- lmer(SenescenceScore1 ~ steatosis_grade + (1|Kit_Lot), data=meta.data.oc)
summary(lmm)
lmm <- lmer(SenescenceScore1 ~ diagnosis_of_MASLD + (1|Kit_Lot), data=meta.data.oc)
summary(lmm)
lmm <- lmer(SenescenceScore1 ~ alt + (1|Kit_Lot), data=meta.data.oc)
summary(lmm)
lmm <- lmer(SenescenceScore1 ~ alt + fibrosis_stage + (1|Kit_Lot), data=meta.data.oc)
summary(lmm)

lmm <- lmer(SenescenceScore1 ~ ast + (1|Kit_Lot), data=meta.data.oc)
summary(lmm)
lmm <- lmer(SenescenceScore1 ~ ast + fibrosis_stage + (1|Kit_Lot), data=meta.data.oc)
summary(lmm)

lmm <- lmer(SenescenceScore1 ~ ggt + (1|Kit_Lot), data=meta.data.oc)
summary(lmm)
lmm <- lmer(SenescenceScore1 ~ ggt + fibrosis_stage + (1|Kit_Lot), data=meta.data.oc)
summary(lmm)

meta.data$group2 <- factor(meta.data$group2)
lmm <- lmer(SenescenceScore1 ~ group2 + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

meta.data$group3 <- factor(meta.data$group3)
lmm <- lmer(SenescenceScore1 ~ group3 + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(SenescenceScore1 ~ diagnosis_of_diabetes + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(SenescenceScore1 ~ diagnosis_of_MASLD + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(SenescenceScore1 ~ fibrosis_stage + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(SenescenceScore1 ~ steatosis_grade + (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(SenescenceScore1 ~ alt + fibrosis_stage+ (1|Kit_Lot), data=meta.data)
summary(lmm)
anova(lmm)

lmm <- lmer(SenescenceScore1 ~ alt + fibrosis_stage+ group2+(1|Kit_Lot), data=meta.data.diab)
summary(lmm)
...
#14. Compare Results 
````{r}
diab <- read.xlsx(fs::path(dir.results,"Senescence_Bulk_Results_for_Diagnosis Of Diabetes (Yes vs. No).xlsx"))
glp1 <- read.xlsx(fs::path(dir.results,"Senescence_Bulk_Results_for_Group2 (glp1_exclusive vs. neither).xlsx"))
sglt2_glp1 <- read.xlsx(fs::path(dir.results,"Senescence_Bulk_Results_for_Group2 (both vs. neither).xlsx"))

# Assuming the two dataframes are glp1 and diab
# Both dataframes have columns: Gene, p_val_adj, and avg_log2FC

# Filter significant genes (disregard p_val_adj since all are significant)
glp1_filtered <- glp1[, c("Gene", "avg_log2FC")]
glp1_filtered <- glp1_filtered %>%
  dplyr::rename(glp1_fc=avg_log2FC)
diab_filtered <- diab[, c("Gene", "avg_log2FC")]
diab_filtered <- diab_filtered %>%
  dplyr::rename(diab_fc=avg_log2FC)
sglt2_glp1_filtered <- sglt2_glp1[, c("Gene", "avg_log2FC")]
sglt2_glp1_filtered <- sglt2_glp1_filtered %>%
  dplyr::rename(both=avg_log2FC)

# Merge the two dataframes by Gene to identify common genes
merged_data <- tidylog::full_join(glp1_filtered,sglt2_glp1_filtered,by="Gene")
merged_data  <- tidylog::full_join(merged_data,diab_filtered,by="Gene")
merged_data <- merged_data %>%
  mutate(Direction1=ifelse(sign(diab_fc)==sign(glp1_fc),"Same","Change"),
         Direction2=ifelse(sign(diab_fc)==sign(both),"Same","Change"))%>%
  mutate(Change_GLP1=case_when(sign(diab_fc)==-1 & sign(glp1_fc)==1~"Downreg to Upreg",
                          sign(diab_fc)==1 & sign(glp1_fc)==-1~"Upreg to Downreg",
                          sign(diab_fc)==-1 & sign(glp1_fc)==-1~"Remains Downreg",
                          sign(diab_fc)==1 & sign(glp1_fc)==1~"Remains Upreg",
                          sign(diab_fc)==-1 & is.na(glp1_fc)~"Downreg to NS",
                          sign(diab_fc)==1 & is.na(glp1_fc)~"Downreg to NS"))
glp1_change_genes <- merged_data %>%
  dplyr::select(all_of(c("Gene","Change_GLP1"))) %>%
  filter(!grepl("Remains",Change_GLP1))

#Hepatocytes
diab <- read.xlsx(fs::path(dir.results,"Senescence_Results_Hepatocyte_cells_for_Diagnosis Of Diabetes (Yes vs. No).xlsx"))
glp1 <- read.xlsx(fs::path(dir.results,"Senescence_Results_Hepatocyte_cells_for_Group2 (glp1_exclusive vs. neither).xlsx"))
sglt2_glp1 <- read.xlsx(fs::path(dir.results,"Senescence_Results_Hepatocyte_cells_for_Group2 (both vs. neither).xlsx"))
# Filter significant genes (disregard p_val_adj since all are significant)
glp1_filtered <- glp1[, c("Gene", "avg_log2FC")]
glp1_filtered <- glp1_filtered %>%
  dplyr::rename(glp1_fc=avg_log2FC)
...
merged_data <- merged_data %>%
  ...
  mutate(Change_GLP1=case_when(sign(diab_fc)==-1 & sign(glp1_fc)==1~"Downreg to Upreg",
                          ...
                          sign(diab_fc)==-1 & is.na(glp1_fc)~"Downreg to NS",
                          sign(diab_fc)==1 & is.na(glp1_fc)~"Downreg to NS",
                          is.na(diab_fc) & sign(glp1_fc)==1~"NS to Upreg",
                          is.na(diab_fc) & sign(glp1_fc)==-1~"NS to Downreg")) %>%
    mutate(Change_Both=case_when(sign(diab_fc)==-1 & sign(both)==1~"Downreg to Upreg",
                          sign(diab_fc)==1 & sign(both)==-1~"Upreg to Downreg",
                          sign(diab_fc)==-1 & sign(both)==-1~"Remains Downreg",
                          sign(diab_fc)==1 & sign(both)==1~"Remains Upreg",
                          sign(diab_fc)==-1 & is.na(both)~"Downreg to NS",
                          sign(diab_fc)==1 & is.na(both)~"Downreg to NS",
                          is.na(diab_fc) & sign(both)==1~"NS to Upreg",
                          is.na(diab_fc) & sign(both)==-1~"NS to Downreg"))
glp1_change_genes <- merged_data %>%
  dplyr::select(all_of(c("Gene","Change_GLP1"))) %>%
  filter(!grepl("Remains",Change_GLP1)) %>%
  filter(!is.na(Change_GLP1))
both_change_genes <- merged_data %>%
  dplyr::select(all_of(c("Gene","Change_Both"))) %>%
  filter(!grepl("Remains",Change_Both)) %>%
    filter(!is.na(Change_Both))
change <- tidylog::full_join(both_change_genes,glp1_change_genes,by="Gene")

#Results with ALT/AST
#ast
ast <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Ast.csv"))
glp1 <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Medication (glp1_exclusive vs. neither).csv"))
both <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Medication (both vs. neither).csv"))
# Filter significant genes (disregard p_val_adj since all are significant)
glp1_filtered <- glp1[, c("Gene", "LogFoldChange")]
glp1_filtered <- glp1_filtered %>%
  dplyr::rename(glp1_fc=LogFoldChange)
ast_filtered <- ast[, c("Gene", "LogFoldChange")]
ast_filtered <- ast_filtered %>%
  dplyr::rename(ast_fc=LogFoldChange)
sglt2_glp1_filtered <- both[, c("Gene", "LogFoldChange")]
sglt2_glp1_filtered <- sglt2_glp1_filtered %>%
  dplyr::rename(both=LogFoldChange)

# Merge the two dataframes by Gene to identify common genes
merged_data <- tidylog::full_join(glp1_filtered,sglt2_glp1_filtered,by="Gene")
merged_data  <- tidylog::full_join(merged_data,ast_filtered,by="Gene")
merged_data <- merged_data %>%
  mutate(Direction1=ifelse(sign(ast_fc)==sign(glp1_fc),"Same","Change"),
         Direction2=ifelse(sign(ast_fc)==sign(both),"Same","Change"))%>%
  mutate(Change_GLP1=case_when(sign(ast_fc)==-1 & sign(glp1_fc)==1~"Downreg to Upreg",
                          sign(ast_fc)==1 & sign(glp1_fc)==-1~"Upreg to Downreg",
                          sign(ast_fc)==-1 & sign(glp1_fc)==-1~"Remains Downreg",
                          sign(ast_fc)==1 & sign(glp1_fc)==1~"Remains Upreg",
                          sign(ast_fc)==-1 & is.na(glp1_fc)~"Downreg to NS",
                          sign(ast_fc)==1 & is.na(glp1_fc)~"Downreg to NS",
                          is.na(ast_fc) & sign(glp1_fc)==1~"NS to Upreg",
                          is.na(ast_fc) & sign(glp1_fc)==-1~"NS to Downreg")) %>%
    mutate(Change_Both=case_when(sign(ast_fc)==-1 & sign(both)==1~"Downreg to Upreg",
                          sign(ast_fc)==1 & sign(both)==-1~"Upreg to Downreg",
                          sign(ast_fc)==-1 & sign(both)==-1~"Remains Downreg",
                          sign(ast_fc)==1 & sign(both)==1~"Remains Upreg",
                          sign(ast_fc)==-1 & is.na(both)~"Downreg to NS",
                          sign(ast_fc)==1 & is.na(both)~"Downreg to NS",
                          is.na(ast_fc) & sign(both)==1~"NS to Upreg",
                          is.na(ast_fc) & sign(both)==-1~"NS to Downreg"))
glp1_change_genes <- merged_data %>%
  dplyr::select(all_of(c("Gene","Change_GLP1"))) %>%
  filter(!grepl("Remains",Change_GLP1)) %>%
  filter(!is.na(Change_GLP1))
...
#alt
alt <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Alt.csv"))
glp1 <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Medication (glp1_exclusive vs. neither).csv"))
both <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Medication (both vs. neither).csv"))
# Filter significant genes (disregard p_val_adj since all are significant)
glp1_filtered <- glp1[, c("Gene", "LogFoldChange")]
glp1_filtered <- glp1_filtered %>%
  dplyr::rename(glp1_fc=LogFoldChange)
alt_filtered <- alt[, c("Gene", "LogFoldChange")]
alt_filtered <- alt_filtered %>%
  dplyr::rename(alt_fc=LogFoldChange)
sglt2_glp1_filtered <- both[, c("Gene", "LogFoldChange")]
sglt2_glp1_filtered <- sglt2_glp1_filtered %>%
  dplyr::rename(both=LogFoldChange)
...
# Merge the two dataframes by Gene to identify common genes
merged_data <- tidylog::full_join(glp1_filtered,sglt2_glp1_filtered,by="Gene")
merged_data  <- tidylog::full_join(merged_data,alt_filtered,by="Gene")
merged_data <- merged_data %>%
  mutate(Direction1=ifelse(sign(alt_fc)==sign(glp1_fc),"Same","Change"),
         Direction2=ifelse(sign(alt_fc)==sign(both),"Same","Change"))%>%
  mutate(Change_GLP1=case_when(sign(alt_fc)==-1 & sign(glp1_fc)==1~"Downreg to Upreg",
                          sign(alt_fc)==1 & sign(glp1_fc)==-1~"Upreg to Downreg",
                          sign(alt_fc)==-1 & sign(glp1_fc)==-1~"Remains Downreg",
                          sign(alt_fc)==1 & sign(glp1_fc)==1~"Remains Upreg",
                          sign(alt_fc)==-1 & is.na(glp1_fc)~"Downreg to NS",
                          sign(alt_fc)==1 & is.na(glp1_fc)~"Downreg to NS",
                          is.na(alt_fc) & sign(glp1_fc)==1~"NS to Upreg",
                          is.na(alt_fc) & sign(glp1_fc)==-1~"NS to Downreg")) %>%
    mutate(Change_Both=case_when(sign(alt_fc)==-1 & sign(both)==1~"Downreg to Upreg",
                          sign(alt_fc)==1 & sign(both)==-1~"Upreg to Downreg",
                          sign(alt_fc)==-1 & sign(both)==-1~"Remains Downreg",
                          sign(alt_fc)==1 & sign(both)==1~"Remains Upreg",
                          sign(alt_fc)==-1 & is.na(both)~"Downreg to NS",
                          sign(alt_fc)==1 & is.na(both)~"Downreg to NS",
                          is.na(alt_fc) & sign(both)==1~"NS to Upreg",
                          is.na(alt_fc) & sign(both)==-1~"NS to Downreg"))
glp1_change_genes <- merged_data %>%
  dplyr::select(all_of(c("Gene","Change_GLP1"))) %>%
  filter(!grepl("Remains",Change_GLP1)) %>%
  filter(!is.na(Change_GLP1))
...
#Fibrosis
fibrosis <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Fibrosis_stage.csv"))
glp1 <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Medication (glp1_exclusive vs. neither).csv"))
both <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Medication (both vs. neither).csv"))
# Filter significant genes (disregard p_val_adj since all are significant)
glp1_filtered <- glp1[, c("Gene", "LogFoldChange")]
glp1_filtered <- glp1_filtered %>%
  dplyr::rename(glp1_fc=LogFoldChange)
fibrosis_filtered <- fibrosis[, c("Gene", "LogFoldChange")]
fibrosis_filtered <- fibrosis_filtered %>%
  dplyr::rename(fibrosis_fc=LogFoldChange)
sglt2_glp1_filtered <- both[, c("Gene", "LogFoldChange")]
sglt2_glp1_filtered <- sglt2_glp1_filtered %>%
  dplyr::rename(both=LogFoldChange)
...
# Merge the two dataframes by Gene to identify common genes
merged_data <- tidylog::full_join(glp1_filtered,sglt2_glp1_filtered,by="Gene")
merged_data  <- tidylog::full_join(merged_data,fibrosis_filtered,by="Gene")
merged_data <- merged_data %>%
  mutate(Direction1=ifelse(sign(fibrosis_fc)==sign(glp1_fc),"Same","Change"),
         Direction2=ifelse(sign(fibrosis_fc)==sign(both),"Same","Change"))%>%
  mutate(Change_GLP1=case_when(sign(fibrosis_fc)==-1 & sign(glp1_fc)==1~"Downreg to Upreg",
                          sign(fibrosis_fc)==1 & sign(glp1_fc)==-1~"Upreg to Downreg",
                          sign(fibrosis_fc)==-1 & sign(glp1_fc)==-1~"Remains Downreg",
                          sign(fibrosis_fc)==1 & sign(glp1_fc)==1~"Remains Upreg",
                          sign(fibrosis_fc)==-1 & is.na(glp1_fc)~"Downreg to NS",
                          sign(fibrosis_fc)==1 & is.na(glp1_fc)~"Downreg to NS",
                          is.na(fibrosis_fc) & sign(glp1_fc)==1~"NS to Upreg",
                          is.na(fibrosis_fc) & sign(glp1_fc)==-1~"NS to Downreg")) %>%
    mutate(Change_Both=case_when(sign(fibrosis_fc)==-1 & sign(both)==1~"Downreg to Upreg",
                          sign(fibrosis_fc)==1 & sign(both)==-1~"Upreg to Downreg",
                          sign(fibrosis_fc)==-1 & sign(both)==-1~"Remains Downreg",
                          sign(fibrosis_fc)==1 & sign(both)==1~"Remains Upreg",
                          sign(fibrosis_fc)==-1 & is.na(both)~"Downreg to NS",
                          sign(fibrosis_fc)==1 & is.na(both)~"Downreg to NS",
                          is.na(fibrosis_fc) & sign(both)==1~"NS to Upreg",
                          is.na(fibrosis_fc) & sign(both)==-1~"NS to Downreg"))
glp1_change_genes <- merged_data %>%
  dplyr::select(all_of(c("Gene","Change_GLP1"))) %>%
  filter(!grepl("Remains",Change_GLP1)) %>%
  filter(!is.na(Change_GLP1))
...
change <- tidylog::full_join(both_change_genes,glp1_change_genes,by="Gene")
change <- change %>%
  mutate(Different=ifelse(Change_Both==Change_GLP1,"Same","Different"))

#Steatosis
steatosis <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Steatosis_Grade.csv"))
glp1 <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Medication (glp1_exclusive vs. neither).csv"))
both <- read.csv(fs::path(dir.results,"Results_Hepatocyte_cells_for_Medication (both vs. neither).csv"))
# Filter significant genes (disregard p_val_adj since all are significant)
glp1_filtered <- glp1[, c("Gene", "LogFoldChange")]
glp1_filtered <- glp1_filtered %>%
  dplyr::rename(glp1_fc=LogFoldChange)
steatosis_filtered <- steatosis[, c("Gene", "LogFoldChange")]
steatosis_filtered <- steatosis_filtered %>%
  dplyr::rename(steatosis_fc=LogFoldChange)
sglt2_glp1_filtered <- both[, c("Gene", "LogFoldChange")]
sglt2_glp1_filtered <- sglt2_glp1_filtered %>%
  dplyr::rename(both=LogFoldChange)
...
# Merge the two dataframes by Gene to identify common genes
merged_data <- tidylog::full_join(glp1_filtered,sglt2_glp1_filtered,by="Gene")
merged_data  <- tidylog::full_join(merged_data,steatosis_filtered,by="Gene")
merged_data <- merged_data %>%
  mutate(Direction1=ifelse(sign(steatosis_fc)==sign(glp1_fc),"Same","Change"),
         Direction2=ifelse(sign(steatosis_fc)==sign(both),"Same","Change"))%>%
  mutate(Change_GLP1=case_when(sign(steatosis_fc)==-1 & sign(glp1_fc)==1~"Downreg to Upreg",
                          sign(steatosis_fc)==1 & sign(glp1_fc)==-1~"Upreg to Downreg",
                          sign(steatosis_fc)==-1 & sign(glp1_fc)==-1~"Remains Downreg",
                          sign(steatosis_fc)==1 & sign(glp1_fc)==1~"Remains Upreg",
                          sign(steatosis_fc)==-1 & is.na(glp1_fc)~"Downreg to NS",
                          sign(steatosis_fc)==1 & is.na(glp1_fc)~"Downreg to NS",
                          is.na(steatosis_fc) & sign(glp1_fc)==1~"NS to Upreg",
                          is.na(steatosis_fc) & sign(glp1_fc)==-1~"NS to Downreg")) %>%
    mutate(Change_Both=case_when(sign(steatosis_fc)==-1 & sign(both)==1~"Downreg to Upreg",
                          sign(steatosis_fc)==1 & sign(both)==-1~"Upreg to Downreg",
                          sign(steatosis_fc)==-1 & sign(both)==-1~"Remains Downreg",
                          sign(steatosis_fc)==1 & sign(both)==1~"Remains Upreg",
                          sign(steatosis_fc)==-1 & is.na(both)~"Downreg to NS",
                          sign(steatosis_fc)==1 & is.na(both)~"Downreg to NS",
                          is.na(steatosis_fc) & sign(both)==1~"NS to Upreg",
                          is.na(steatosis_fc) & sign(both)==-1~"NS to Downreg"))
glp1_change_genes <- merged_data %>%
  dplyr::select(all_of(c("Gene","Change_GLP1"))) %>%
  filter(!grepl("Remains",Change_GLP1)) %>%
  filter(!is.na(Change_GLP1))
