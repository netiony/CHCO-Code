...
so_liver_sn@meta.data <- so_liver_sn@meta.data %>% 
  mutate(group=case_when(both == "Yes" ~ "both",
                         sglt2_exclusive == "Yes" ~ "sglt2_exclusive",
                         glp1_exclusive == "Yes" ~ "glp1_exclusive",
                         neither == "Yes" ~ "neither"))

so_liver_sn@meta.data$group <- factor(so_liver_sn@meta.data$group, levels = c("neither", "sglt2_exclusive", "glp1_exclusive","both"))
```
...
table(dat$exclusive_sglt2) #-/+
table(dat$exclusive_glp1) #+/-
table(dat$both) #+/+
table(dat$neither) #-/-
...
###i. Full Gene Set
```{r, Full Gene Set}
# Extract the count matrix (rows: genes, columns: cells)
count_matrix <- GetAssayData(so_liver_sn, assay = "RNA", layer = "count")
...
batch_genes <- gene_batches[[1]]
batch_matrix <- count_matrix[batch_genes, ]

# Check for any negative values
any(batch_matrix < 0)

# Check for non-integer values
any(batch_matrix != floor(batch_matrix))
############

# Extract cell metadata
meta_cell <- so_liver_sn@meta.data
meta_cell$cell_id <- rownames(meta_cell)  # Add cell IDs

# Ensure cell metadata includes a read depth column
# Example: Calculate total counts per cell as a proxy for read depth
meta_cell$cell_rd <- colSums(count_matrix)

# Create individual-level metadata (e.g., group information)
# Example: Assuming "individual" and "phenotype" are in meta_cell
meta_ind <- unique(meta_cell[, c("Kit_Lot", "diagnosis_of_diabetes")])

# Set variables
var2test <- "diagnosis_of_diabetes"  # Group variable to test
var2adjust <- NULL       # Adjusting variable (e.g., RIN, if applicable)
var2test_type <- "binary"  # "binary" or "continuous"
...
for (i in seq_along(gene_batches)) {
  ...
  # Subset the Seurat object to only include the current batch of genes
  so_batch <- subset(so_liver_sn, features = batch_genes)  # Adjust 'so_liver_sn' to your Seurat object name
  
  # Create the count matrix for this batch of genes
  batch_matrix <- as.matrix(GetAssayData(so_batch, assay = "RNA", slot = "counts"))
  
  # Ensure that the count matrix contains non-negative integers
  ...
  dist_batch <- ideas_dist(
    ...
    fit_method = "nb"  # Negative binomial fit
  )
