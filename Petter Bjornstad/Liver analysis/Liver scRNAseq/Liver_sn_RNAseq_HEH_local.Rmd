...
# Step 2: Extract metadata and add 'wellKey' column
cell_metadata <- so_sens@meta.data
cell_metadata$wellKey <- rownames(cell_metadata)

# Step 3: Filter out genes with zero variance
gene_variance <- apply(expression_matrix, 1, var)
expression_matrix_filtered <- expression_matrix[gene_variance > 0, ]

# Step 4: Identify the top 500 most variable genes
so_sens <- FindVariableFeatures(so_sens, selection.method = "vst", nfeatures = 500)

# Step 5: Extract the variable genes identified by Seurat
variable_genes <- VariableFeatures(so_sens)

# Step 6: Subset the expression matrix to include only the variable genes
expression_matrix_subset <- expression_matrix_filtered[variable_genes, ]

# Step 7: Create a SingleCellAssay object with the filtered and subsetted expression matrix
sca_subset <- FromMatrix(exprsArray = expression_matrix_subset, cData = cell_metadata)

# Step 8: Create the heatmap
aheatmap(assay(sca_subset), labRow='', annCol=as.data.frame(colData(sca_subset)), distfun='spearman')
...
expposures <- c("ast","alt","fibrosis_stage","steatosis_grade")
...
# write.csv(summary_dt,fs::path(dir.dat,"Adj_AST_Results.csv"))
