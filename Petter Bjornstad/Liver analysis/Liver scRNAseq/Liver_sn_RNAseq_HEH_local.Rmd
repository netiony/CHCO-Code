...
so_kidney_sc@meta.data <- so_kidney_sc@meta.data %>% 
  mutate(kit_id=ifelse(kit_id=="kl-0023998","KL-0023998",kit_id)) %>% 
    mutate(kit_id=ifelse(kit_id=="KI-0014643","KL-0014643",kit_id)) 
  
dat <- so_kidney_sc@meta.data

"kl-0023998"
"KI-0014643"
k_ids <- unique(dat$kit_id) #88 
record_ids <- unique(dat$record_id)
meta_raw <- read.csv("/Volumes/Peds Endo/Petter Bjornstad/Data Harmonization/Data Clean/harmonized_dataset.csv") %>% 
    dplyr::select(c(record_id,cryostor_id,mrn,kit_id,co_enroll_id,visit,procedure,hba1c,diabetes_duration,diabetes_dx_date,epic_ever_glp1ra_1,epic_glp1ra_1,epic_ever_sglt2i_1,epic_sglti2_1))

#Filter to baseline only
meta_raw1 <- meta_raw %>% 
  filter(visit=="baseline") 
#Summarize all data for each participant
meta_raw2 <- meta_raw1 %>% 
...
meta_raw3 <- meta_raw2 %>% 
  mutate(diagnosis_of_diabetes=ifelse(is.na(diabetes_dx_date),"No","Yes"))
#Remove coenrolled participants
meta_raw4 <- meta_raw3 %>% 
    summarise(across(where(negate(is.numeric)), ~ ifelse(all(is.na(.x)), NA_character_, last(na.omit(.x)))),
            across(where(is.numeric), ~ ifelse(all(is.na(.x)), NA_real_, mean(.x, na.rm = TRUE))),
            .by = c(kit_id, visit))
meta_raw5 <- meta_raw4 %>% 
  filter(kit_id %in% ids)
meta_raw5$record_id #81
meta_raw3$record_id #88
ids[which(!ids %in% meta_raw5$kit_id )] #"IT_07" "IT_10"

so_kidney_sc@meta.data <- so_kidney_sc@meta.data %>% 
    mutate(record_id=ifelse(record_id=="30051","PNDA-108",record_id)) %>% 
  
meta_kidney_sc <-  left_join(so_kidney_sc@meta.data,meta_raw4,by=c("record_id","kit_id"))
rownames(meta_kidney_sc) <- rownames(so_kidney_sc@meta.data)
so_kidney_sc <- AddMetaData(so_kidney_sc, meta_kidney_sc)
rm(meta_raw,meta_raw2,meta_raw3,meta_kidney_sc,meta_raw1,meta_raw4,dat)
...
so_sens_all <- FindVariableFeatures(object = so_sens_all)
so_sens_all <- RunPCA(so_sens_all, features = VariableFeatures(object = so_sens_all))
ElbowPlot(so_sens_all)
gc()
so_sens_all <- FindNeighbors(so_sens_all, dims = 1:15)
so_sens_all <- FindClusters(so_sens_all, resolution = 0.5)
so_sens_all <- RunUMAP(so_sens_all, dims = 1:15)
