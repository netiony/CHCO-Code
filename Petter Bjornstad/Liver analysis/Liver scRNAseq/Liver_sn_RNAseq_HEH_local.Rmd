...
umap_plot <- FeaturePlot(so_liver_sn, features = "Sens_Expression_Level", raster = TRUE,
  ...
  scale_color_gradientn(
    colors = c("gray", "lightblue1", "lightblue3","blue", "darkblue"),  # Red for 0, transitioning to dark blue
    values = scales::rescale(c(0,1,5, 20, 37.82061)),           # Control how the colors map to the expression values
    limits = c(0, 37.82061)                                    # Explicitly set the limits of the color scale
  ) +
  labs(color = "Expression Level") +
  ggtitle("UMAP with Gradient of Senescence Gene Expression by Cell Type") +
  theme_void()
...
hist(as.vector(so_liver_sn$Sens_Expression_Level), breaks = 50, main = "Distribution of Raw Expression for Sens Genes", xlab = "Expression Level")
...
# Calculate the mean expression of senescence genes by cell type
average_expression_by_celltype <- expression_with_celltype %>%
  dplyr::group_by(celltype) %>%
  dplyr::summarize(across(starts_with("sens_gene"), mean, na.rm = TRUE))

# If you want to calculate the mean of all senescence genes combined, you can do something like:
expression_with_celltype$average_sens_expression <- rowSums(expression_with_celltype[sens_genes]) / length(sens_genes)

# Now, calculate the average expression for each cell type
average_expression_by_celltype_combined <- expression_with_celltype %>%
  dplyr::group_by(celltype) %>%
  dplyr::summarize(mean_expression = mean(average_sens_expression, na.rm = TRUE))

# View the results
average_expression_by_celltype_combined
