...
model_formula <- ~ glp1agonist + alt
#Diabetes status, GLP1 status, age, sex, sbp, HbA1c
# Run the linear model with zlm
zlm_results <- zlm(formula = model_formula, sca = sca_gene_set)
# summary_zlm <- summary(zlm_results)
# Summarize results and perform likelihood ratio test (LRT) for outcome
summary_zlm <- summary(zlm_results, doLRT = "glp1agonistYes")
# summary_zlm <- summary(zlm_results)
# Convert the summary to a data table for easy manipulation
summary_dt <- summary_zlm$datatable
...
b <- ggplot(top_genes, aes(x = coef, y = Gene, fill = Direction)) +
  geom_bar(stat = "identity") +
  labs(x = "logFC", y = "Gene", title = "Overall Association of GLP-1 and Gene Expression, Among youth with T2D (Uunadjusted for ALT)") +
  scale_fill_manual(values = c("blue", "red"), labels = c("Lower Expression", "Higher Expression")) +
  theme_minimal()
b <- ggplot(top_genes, aes(x = coef, y = Gene, fill = coef)) +
  geom_bar(stat = "identity") +
  labs(
    x = "logFC",
    y = "Gene",
    title = "Overall Association of GLP-1 and Gene Expression, Among youth with T2D (Unadjusted for ALT)"
  ) +
  scale_fill_gradient2(
    low = "blue",      # Color for negative coefficients
    mid = "white",     # Neutral midpoint
    high = "red",      # Color for positive coefficients
    midpoint = 0,      # Center of the gradient at zero
    name = "Coefficient"
  ) +
  theme_minimal()

b <- ggplot(results_full, aes(x = coef, y = primerid, fill = coef)) +
  geom_tile(color = "white") +
  labs(
    x = NULL,
    y = "Gene",
    title = "Heatmap of GLP-1 and Gene Expression Coefficients",
    fill = "Coefficient"
  ) +
  scale_fill_gradient2(
    low = "blue",      # Negative coefficients
    mid = "white",     # Neutral midpoint
    high = "red",      # Positive coefficients
  ...
  facet_wrap(~Condition, scales = "free_y") +  # Facet by condition, with independent y-axis scales
  theme_minimal() +
  theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
...
# results2 <- top_genes %>%
#   dplyr::select(c(Gene,Direction)) %>%
#   dplyr::rename(Direction_GLP1=Direction)

# results3 <- top_genes %>%
#   dplyr::select(c(Gene,Direction)) %>%
#   dplyr::rename(Direction_ALT=Direction)
results4 <- top_genes %>%
  dplyr::select(c(Gene,Direction)) %>%
  dplyr::rename(Direction_GLP1_A1c=Direction)

# results_full <- tidylog::full_join(results,results2,by="Gene")
...
results_full <-results_full %>% 
  ...
  mutate(reverse=case_when(Direction_GLP1=="Positive" & Direction_ALT=="Negative" ~ "Reversed_Down",
                           Direction_GLP1=="Negative" & Direction_ALT=="Positive" ~ "Reversed_Up",
                           Direction_GLP1=="Positive" & Direction_ALT=="Positive" ~ "Maintained",
                           Direction_GLP1=="Negative" & Direction_ALT=="Negative" ~ "Maintained"))
...
# fcHurdle <- merge(summary_dt[contrast=='ast' & component=='D',.(primerid, `Pr(>Chisq)`)], #hurdle P values
#                   summary_dt[contrast=='ast' & component=='D', .(primerid, coef, ci.hi, ci.lo)], by='primerid') #logFC coefficients
# fcHurdle[,fdr:=p.adjust(`Pr(>Chisq)`, 'fdr')]
# fcHurdle <- fcHurdle %>% 
#   mutate(odds=exp(coef)) %>% 
#   mutate(odds=odds^10)
# write.csv(fcHurdle,fs::path(dir.results,"Senesence Results","AST_Senescence_Discrete.csv"))
# m <- fcHurdle[,c("primerid","odds","fdr")]
# # Filter top 10 positive and negative log2FoldChange
# top_pos <- as.data.frame(m) %>% 
#   filter(fdr<0.05) %>% 
#   filter(odds>1.0) %>% 
...
#   dplyr::rename(Gene=primerid)
# # top_pos$Gene <- rownames(top_pos)
# top_pos <- top_pos[c("Gene","odds","fdr")]
# rownames(top_pos) <- NULL
# top_pos$Direction <- "Positive"
# 
# top_neg <- as.data.frame(m) %>% 
...
#   dplyr::rename(Gene=primerid)
# # top_neg$Gene <- rownames(top_neg)
# # top_neg <- top_neg[c("Gene","odds","fdr")]
# # rownames(top_neg) <- NULL
# top_neg$Direction <- "Negative"
# 
# # Combine and prepare for plotting
...
# 
# # # Bar chart
# # ggplot(top_genes, aes(x = Gene, y = odds, fill = Direction)) +
# #   geom_bar(stat = "identity") +
# #   coord_flip() +
...
# # Set sorting order: all negative genes first, then all positive genes
# top_genes <- top_genes %>%
#   mutate(Gene = factor(Gene, levels = Gene[order(Direction, odds)]))  # Order by Direction and log2FC
# 
# # Bar chart with flipped x and y axes
# b <- ggplot(top_neg, aes(x = odds, y = Gene, fill = Direction)) +
#   geom_bar(stat = "identity") +
#   labs(x = "Odds Ratio", y = "Gene", title = "AST Levels and Gene Expression",
#        subtitle="") +
#   xlim(0,50)+
#   scale_fill_manual(values = c("blue", "red"), labels = c("Expression", "Higher Expression")) +
#   theme_minimal()
# b
# FCTHRESHOLD = log2(0.25)
# fcHurdleSig <- merge(fcHurdle[fdr<.05 & abs(coef)>FCTHRESHOLD], as.data.table(mcols(sca_gene_set)), by='primerid')
...
# coefAndCI <- summary(zlm_results, logFC=FALSE)$datatable
# coefAndCI <- coefAndCI[contrast != '(Intercept)',]
# coefAndCI[,contrast:=abbreviate(contrast)]
# plot <- ggplot(coefAndCI, aes(x=contrast, y=coef, ymin=ci.lo, ymax=ci.hi, col=component))+
...
#   filter(contrast=="alt")
# # coefAndCI[,contrast:=abbreviate(contrast)]
# plot <- ggplot(summary_dt, aes(x=contrast, y=coef, ymin=ci.lo, ymax=ci.hi, col=component))+
#     geom_pointrange(position=position_dodge(width=.5)) +facet_wrap(~primerid) +
#     theme(axis.text.x=element_text(angle=45, hjust=1)) + coord_cartesian(ylim=c(-3, 3))

# results_full <- results_full %>% 
#   mutate(coef=ifelse(coef=="NaN",NA,coef))
# results_full <- results_full %>% 
#   na.omit()
# pval <- results_full
# coefs <- results_full %>% 
...
results_full <- results_full %>%
  na.omit()
ggplot(results_full, aes(x = coef, y = primerid, fill = coef)) +
  geom_tile()+
  scale_fill_gradient2(
    low = "blue",      # Color for negative coefficients
    mid = "white",     # Neutral midpoint
