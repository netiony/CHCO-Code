...
# Prepare the data
m_top <- m_top %>%
  mutate(abs_log2FC = abs(avg_log2FC),         # Absolute log2 fold change for size
         log_pvalue = -log10(p_val_adj),       # Transform p-value for better visualization
         significant = p_val_adj < 0.05)      # Boolean for significance

# Create the bubble plot
p <- ggplot(m_top, aes(x = avg_log2FC, 
                       y = log_pvalue, 
                       size = abs_log2FC, 
                       color = avg_log2FC)) +
  geom_point(aes(fill = significant),          # Fill for significance
             shape = 21, alpha = 0.8) +        # Hollow circles for better visualization
  geom_text(aes(label = ifelse(significant, Gene, "")), # Labels only for significant genes
            hjust = 0, nudge_y = 0.5, size = 4, fontface = "bold") +
  scale_color_gradient2(low = "blue", mid = "gray", high = "red", midpoint = 0, 
                        name = "Log2 Fold Change") +  # Gradient for fold change
  scale_fill_manual(values = c("white", "black"), name = "Significance", 
                    labels = c("Not Significant", "Significant")) + # Black for significant
  scale_size_continuous(name = "|Log2 Fold Change|", range = c(2, 10)) + # Size scaling
  labs(title = "Bubble Plot of Differentially Expressed Genes",
       subtitle = "Black dots indicate statistical significance (P < 0.05)",
       x = "Log2 Fold Change",
...
model_formula <- ~ ast + age_surgery + nih_sex + a1c + diagnosis_of_diabetes + glp1agonist + diagnosis_of_MASLD
...
fcHurdle <- merge(summary_dt[contrast=='ast' & component=='C',.(primerid, `Pr(>Chisq)`)], #hurdle P values
                  summary_dt[contrast=='ast' & component=='C', .(primerid, coef, ci.hi, ci.lo)], by='primerid') #logFC coefficients
fcHurdle[,fdr:=p.adjust(`Pr(>Chisq)`, 'fdr')]
fcHurdle <- fcHurdle %>% 
  mutate(coef=10*coef)
write.csv(fcHurdle,fs::path(dir.results,"Senesence Results","AST_Senescence.csv"))
