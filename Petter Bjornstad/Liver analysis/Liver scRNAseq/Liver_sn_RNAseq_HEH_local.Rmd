...
expression_by_celltype_combined <- expression_with_celltype %>%
  ...
  dplyr::summarize(total_expression = sum(total_sens_expression, na.rm = TRUE),
                   total_cells = n())
expression_by_celltype_combined$average <- expression_by_celltype_combined$total_expression/expression_by_celltype_combined$total_cells

# View the results
average_expression_by_celltype_combined

# Extract expression data for senescence genes (assuming `sens_genes` is a character vector of gene names)
expression_data <- so_liver_sn@assays$RNA@data[sens_genes, , drop = FALSE]
