...
meta_raw1 <- meta_raw %>% 
  filter(record_id %in% unique(so_kidney_sc@meta.data$record_id))

# biopsy <- read.xlsx("/Volumes/Peds Endo/Petter Bjornstad/Data Harmonization/Biopsies/Biopsy master tracker.xlsx")
# biopsy <- biopsy[-1,]
# colnames(biopsy) <- biopsy[1,]
# biopsy <- biopsy[-1,]

# unique(so_kidney_sc@meta.data$cryostor_id)
# unique(meta_raw$cryostor_id)
# unique(so_kidney_sc@meta.data$record_id)
# unique(meta_raw$record_id)

meta_raw2 <- meta_raw1 %>% 
  filter(visit=="baseline") %>%
  dplyr::select(c(record_id,cryostor_id,kit_id,co_enroll_id,visit,procedure,hba1c,diabetes_duration,diabetes_dx_date,epic_ever_glp1ra_1,epic_glp1ra_1,sex,age,race,race_ethnicity,epic_ever_sglt2i_1,epic_sglti2_1,bmi,triglycerides,sbp,dbp))
...
meta_kidney_sc <-  left_join(so_kidney_sc@meta.data,meta_raw3,by=c("kit_id","record_id","cryostor_id"))
rownames(meta_kidney_sc) <- rownames(so_kidney_sc@meta.data)
so_kidney_sc <- AddMetaData(so_kidney_sc, meta_kidney_sc)
rm(meds,biopsy,meta_raw,meta_raw2,meta_raw3,meta_kidney_sc,meta_raw1)
