...
meta_liver_sn <-  so_sens@meta.data %>%
  dplyr::mutate(RNAlater_ID = SampleID) %>%
  left_join(meta_liver_raw)
...
col_data <- colData(sca)

# Check the structure of the column data to see if the covariates are included
str(col_data)

# Alternatively, you can check for specific covariates directly
all(c("a1c", "diagnosis_of_diabetes", "glp1agonist", "diagnosis_of_MASLD") %in% colnames(col_data))
...
model_formula <- ~ steatosis_grade + a1c + diagnosis_of_diabetes + glp1agonist + diagnosis_of_MASLD
