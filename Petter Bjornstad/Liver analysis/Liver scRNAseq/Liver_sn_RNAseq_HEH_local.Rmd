...
so_sens_PT@meta.data$group1 <- ifelse(so_sens_PT@meta.data$group=="Type 2 Diabetes" | so_sens_PT@meta.data$group=="Obese Control","Yes","No")
so_sens_PT1 <- subset(so_sens_PT, group1=="Yes")
so_sens_PT@meta.data$group2 <- ifelse(so_sens_PT@meta.data$group=="Type 2 Diabetes" | so_sens_PT@meta.data$group=="Lean Control","Yes","No")
...
so_sens_TAL@meta.data$group1 <- ifelse(so_sens_TAL@meta.data$group=="Type 2 Diabetes" | so_sens_TAL@meta.data$group=="Obese Control","Yes","No")
so_sens_TAL1 <- subset(so_sens_TAL, group1=="Yes")
so_sens_TAL@meta.data$group2 <- ifelse(so_sens_TAL@meta.data$group=="Type 2 Diabetes" | so_sens_TAL@meta.data$group=="Lean Control","Yes","No")
...
all_results <- mast_fxn(so=so_liver_sn,cell="Hepatocyte",exposure="diagnosis_of_diabetes",covariate=NULL,gene_set=genes,batch_size=1000,exp_group="Yes")
write.csv(all_results,dir.results)
...
#Filter to diabetes only
model_formula <- ~ epic_glp1ra_1
#Diabetes status, GLP1 status, age, sex, sbp, HbA1c
# Run the linear model with zlm
zlm_results <- zlm(formula = model_formula, sca = sca_gene_set)
# summary_zlm <- summary(zlm_results)
# Summarize results and perform likelihood ratio test (LRT) for outcome
summary_zlm <- summary(zlm_results, doLRT = "epic_glp1ra_1Yes")
# summary_zlm <- summary(zlm_results)
# Convert the summary to a data table for easy manipulation
summary_dt <- summary_zlm$datatable
...
=======
fcHurdle <- merge(summary_dt[contrast=='epic_glp1ra_1Yes' & component=='H',.(primerid, `Pr(>Chisq)`)], #hurdle P values
                  summary_dt[contrast=='epic_glp1ra_1Yes' & component=='logFC', .(primerid, coef, ci.hi, ci.lo)], by='primerid') #logFC coefficients
>>>>>>> Stashed changes
fcHurdle[,fdr:=p.adjust(`Pr(>Chisq)`, 'fdr')]
# write.csv(fcHurdle,fs::path(dir.results,"Senesence Results","ALT_Senescence_OverallResults.csv"))
m <- fcHurdle[,c("primerid","coef","fdr")]
...
=======
top_genes_glp_1 <- top_genes 
top_genes <- tidylog::left_join(top_genes_glp_1,top_genes_diab,by="Gene")
