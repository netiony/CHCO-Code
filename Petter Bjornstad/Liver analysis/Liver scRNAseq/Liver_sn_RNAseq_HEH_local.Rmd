...
#Sens
so_sens_all <- FindNeighbors(so_sens_all)
so_sens_all <- FindClusters(so_sense_all)
so_sens_all <- RunUMAP(so_sens_all, dims = 1:11)
DimPlot(so_liver_sn, reduction = "umap", raster = F) 
DimPlot(so_liver_sn, reduction = "umap", group.by = "celltype", raster = FALSE)
DimPlot(so_sens, reduction = "umap", group.by = "celltype", label = TRUE, label.size = 4)+
  ggtitle("UMAP Plot with Cell Type Labels")
