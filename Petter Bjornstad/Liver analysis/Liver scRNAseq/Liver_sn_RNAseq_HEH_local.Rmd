...
meta_raw2 <- meta_raw1 %>% 
  ...
  dplyr::select(c(record_id,cryostor_id,co_enroll_id,visit,procedure,hba1c,diabetes_duration,diabetes_dx_date,glp1_agonist_timepoint,sex,age,race,race_ethnicity,sglt2i_ever,bmi,triglycerides,sbp,dbp))


meta_raw3 <- meta_raw1 %>% 
  filter(procedure=="clamp") %>% 
  dplyr::select(c(record_id,cryostor_id,bmi,triglycerides,sbp,dbp))
meta_raw$hba1c
ids <- meta_raw %>% 
  dplyr::select(c(cryostor_id,record_id))
id.check <- so_kidney_sc@meta.data
