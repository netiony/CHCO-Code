---
title: "Liver Single Nuclei Analysis"
...
dir.results <- c("/Volumes/Peds Endo/Petter Bjornstad/Liver project/Results")
...
# sens_gene_in_data <- intersect(sens_genes, rownames(so_liver_sc))
# Subset the Seurat object to include only genes in sens_gene_in_data
# so_sens_all <- subset(so_liver_sc, features = sens_gene_in_data)
...
dat <- dat %>% 
  ...
  mutate(sglt2_exclusive=ifelse(sglt2=="Yes" & glp1agonist=="No","Yes","No")) %>% 
  mutate(glp1_exclusive=ifelse(sglt2=="No" & glp1agonist=="Yes","Yes","No")) %>% 
  mutate(either=ifelse(sglt2=="Yes" | glp1agonist=="Yes","Yes","No")) %>% 
  mutate(neither=ifelse(sglt2=="No" & glp1agonist=="No","Yes","No"))
...
label(dat$either) <- "SLGT2i or GLP1ra (Yes/No)"

table1(~ age + nih_sex + nih_race + nih_ethnicity + bmi + diagnosis_of_MASLD + sbp + dbp + tg + creatinine + sglt2_exclusive + glp1_exclusive + either + both | diagnosis_of_diabetes, data=dat)

table1(~ age + nih_sex + nih_race + nih_ethnicity + diagnosis_of_MASLD + bmi + sbp + dbp + tg + creatinine + sglt2_exclusive + glp1_exclusive + either + both | diagnosis_of_diabetes, data=dat)
...
sig_results <- results_total %>% 
  filter(sig=="*") %>% 
  dplyr::rename(Gene=EntrezGeneSymbol.x)

write.csv(sig_results,fs::path(dir.results,"TODAY_alt_ast_proteomics_adj.csv"))
# sig_pos <- sig_results %>% 
#   filter(LogHR>0)
# sig_neg <- sig_results %>% 
#   filter(LogHR<0)

#Compare today results to mast alt/ast results
#Read in mast alt/ast results
alt <- read.xlsx(fs::path(dir.results,"Results_LogFC_for_Alt.xlsx")) %>% 
  filter(fdr<0.05)
# alt_pos <- alt %>% 
#   filter(LogFoldChange>0)
# alt_neg <- alt %>% 
#   filter(LogFoldChange<0)
ast <- read.xlsx(fs::path(dir.results,"Results_LogFC_for_Ast.xlsx")) %>% 
  filter(fdr<0.05)
# ast_pos <- ast %>% 
#   filter(LogFoldChange>0)
# ast_neg <- ast %>% 
...
sig_comb <- tidylog::full_join(sig_results,alt,by="Gene")
sig_comb <- tidylog::full_join(sig_comb,ast,by="Gene")

write.csv(sig_comb,fs::path(dir.results,"Significant_Genes_ALT_AST_today_snRNA.csv"))
...
results <- results %>% 
  mutate(sig=ifelse(fdr<0.05,"*",""))
write.csv(results,fs::path(dir.results,"Diff_Results_Liver_Teenlabs.csv"))
...
write.csv(results,fs::path(dir.results,"LMEM_TeenLabs_Liver_Results.csv"))
...
#Teen labs lmem
lmem <- read.csv(fs::path(dir.results,"LMEM_TeenLabs_Liver_Results_Imputed.csv"))
lmem <- lmem %>% 
  dplyr::rename(Gene=EntrezGeneSymbol)
lmem <- lmem %>% 
  filter(Main_sig=="*" | Int_sig=="*")

results_comb <- tidylog::left_join(lmem,alt,by="Gene")
results_comb <- tidylog::left_join(results_comb,ast,by="Gene")


```

#9. Oroborous + Senescence Score  
```{r}
...
#10. Senescence Score + Diabetes & Medication Groups
```{r}
...
#11. Senescence Genes with Oroborous 
```{r}
#Read in results of analysis 
#Carbohydrate
oro_vars <- c("Carb_omy","Carbp_m","Carb_adp","Carb_cytoc","Carb_s","Carbfccp","Carbrcr_pm","Carbrcr_pmgs","Carbpe_pmgs",
              "Carb_ep","Carbrcr_state2","Carbrcr_state4_c1","Carbrcr_state4_c12","Carb_le")
results_total <- data.frame()
for (oro in oro_vars) {
  file <- paste0("Results_for_",oro,".csv")
  results <- read.csv(fs::path(dir.results,file))
  results_sig <- results %>% 
    # filter(fdr<0.05) %>% 
    dplyr::select(-X) 
  results_sig$Oro <- oro
  results_total <- rbind(results_total,results_sig)
}

#Visualize results
# Prepare data for labeling
df_labeled <- results_total %>%
  group_by(Oro) %>%
  arrange(desc(LogFoldChange)) %>%
  mutate(Rank_Pos = row_number()) %>%
  arrange(LogFoldChange) %>%
  mutate(Rank_Neg = row_number()) %>%
  mutate(Label = ifelse(Rank_Pos <= 10 | Rank_Neg <= 10, Gene, NA)) %>%
  ungroup() %>% 
  mutate(Significance = case_when(
    LogFoldChange > 0 & fdr < 0.05 ~ "Positive",
    LogFoldChange < 0 & fdr < 0.05 ~ "Negative",
    TRUE ~ "Non-Significant"
  ))

# Create the volcano plot
volc_carb <- ggplot(df_labeled, aes(x = LogFoldChange, y = -log10(fdr), color = Significance)) +
  geom_point(alpha = 0.7) +  # Dots colored by significance
  geom_text_repel(aes(label = Label), color = "black", max.overlaps = Inf) +  # Labels in black
  scale_color_manual(values = c("Positive" = "red", "Negative" = "blue", "Non-Significant" = "gray")) +  # Custom colors for points
  facet_wrap(~Oro, scales = "free", ncol = 2) +  # 2 columns, 7 rows
  labs(title = "Volcano Plots for Each Oro Variable",
       x = "Log Fold Change",
       y = "-log10(FDR)",
       color = "Significance") +
  theme_bw()
...
volc_carb <- ggplot(df_labeled, aes(x = LogFoldChange, y = -log10(fdr), color = Significance)) +
  ...
  geom_text_repel(aes(label = Label), color = "black", max.overlaps = Inf) +  # Labels in black
  scale_color_manual(values = c("Positive" = "red", "Negative" = "blue", "Non-Significant" = "gray")) +  # Custom colors for points
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +  # Vertical dotted line at x=0
  # geom_hline(yintercept = 0, linetype = "dotted", color = "black") +  # Horizontal dotted line at y=0
  facet_wrap(~Oro, scales = "free", ncol = 7) +  # 2 columns, 7 rows
  labs(title = "Volcano Plots for Each Oro Variable",
       x = "Log Fold Change",
       y = "-log10(FDR)",
       color = "Significance") +
...
pdf(fs::path(dir.results,"Carb_Volcano_plot_hor.pdf"),width=30,height=10)
print(volc_carb)
dev.off()

#Lipid
oro_vars <- c("Lipid_omy","Lipidpc_m","Lipid_adp","Lipid_cytoc","Lipid_s","Lipidfccp","Lipidrcr_pcm","Lipidrcr_pcmgs","Lipidpe_pcmgs","Lipid_ep","Lipidrcr_state2","Lipidrcr_state4_c1","Lipidrcr_state4_c12","Lipid_le")
results_total <- data.frame()
for (oro in oro_vars) {
  file <- paste0("Results_for_",oro,".csv")
  results <- read.csv(fs::path(dir.results,file))
...
# Create the volcano plot
volc_lipid <- ggplot(df_labeled, aes(x = LogFoldChange, y = -log10(fdr), color = Significance)) +
  geom_point(alpha = 0.7) +  # Dots colored by significance
  geom_text_repel(aes(label = Label), color = "black", max.overlaps = Inf) +  # Labels in black
  scale_color_manual(values = c("Positive" = "red", "Negative" = "blue", "Non-Significant" = "gray")) +  # Custom colors for points
  facet_wrap(~Oro, scales = "free", ncol = 2) +  # 2 columns, 7 rows
...
pdf(fs::path(dir.results,"Lipid_Volcano_plot.pdf"),width=15,height=25)
print(volc_lipid)
dev.off()
...
#12. Pseudotime Analysis - Hepatocytes
```{r}
