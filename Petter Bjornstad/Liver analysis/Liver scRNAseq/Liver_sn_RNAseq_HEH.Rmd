...
model_formula <- ~ fibrosis_stage

# Run the linear model with zlm
zlm_results <- zlm(formula = model_formula, sca = sca_gene_set)
#summary(zlm_results)
# Summarize results and perform likelihood ratio test (LRT) for outcome
summary_zlm <- summary(zlm_results, doLRT = "fibrosis_stage")
...
# Subset the data for components C and D
summary_dt_filtered <- summary_dt %>%
  filter(component %in% c("C", "D"))

# Create the bubble plot for components C and D
ggplot(summary_dt_filtered, aes(x = coef, y = `Pr(>Chisq)`, size = abs(coef), color = adj_p)) +
  geom_point(alpha = 0.6) +  # Create bubbles with transparency
  scale_size_continuous(range = c(2, 10)) +  # Adjust bubble size range
  scale_color_gradient(low = "blue", high = "red") +  # Color by adjusted p-value
  facet_wrap(~component) +  # Facet by component (C, D)
  theme_minimal() +  # Clean theme
  labs(
    title = "Bubble Plot of Coefficients vs P-values (Components C and D)",
    x = "Coefficient (Coef)",
    y = "P-value",
    size = "Coefficient Magnitude",
...
summary_dt_filtered <- summary_dt_filtered %>%
  mutate(log_pvalue = -log10(`Pr(>Chisq)`))  # Use the raw p-value or adjusted p-value if preferred

# Identify top 10 positive and negative coefficients for each component
top_genes <- summary_dt_filtered %>%
  group_by(component) %>%
  arrange(component, desc(coef)) %>%
  top_n(10, coef) %>%  # Top 10 positive coefficients
  bind_rows(
    summary_dt_filtered %>%
      group_by(component) %>%
      arrange(component, coef) %>%
      top_n(10, -coef)  # Top 10 negative coefficients
  )

# Create the volcano plot
ggplot(summary_dt_filtered, aes(x = coef, y = log_pvalue)) +
  geom_point(aes(color = `Pr(>Chisq)`), alpha = 0.6) +  # Points with transparency
  geom_text(data = top_genes, aes(label = primerid), size = 3, vjust = 1, hjust = 1) +  # Label top 10 genes
  facet_wrap(~component) +  # Facet by component (C, D)
  scale_color_gradient(low = "blue", high = "red") +  # Color by p-value (adjusted)
  theme_minimal() +  # Clean theme
  labs(
    title = "Volcano Plot of Coefficients vs -log10(P-value)",
    x = "Coefficient (Coef)",
    y = "-log10(P-value)",
    color = "P-value"
  ) +
...
top_genes <- summary_dt_filtered %>%
  ...
  filter(!is.na(coef) & !is.na(log_pvalue) & !is.na(primerid))  # Remove rows with NA values

# Create the volcano plot with the correct gene label column (primerid)
ggplot(summary_dt_filtered, aes(x = coef, y = log_pvalue)) +
  geom_point(aes(color = `Pr(>Chisq)`), alpha = 0.6) +  # Points with transparency
  geom_segment(data = top_genes, aes(x = coef, y = log_pvalue, xend = coef, yend = log_pvalue + 0.1), 
               color = "black", linetype = "dashed", linewidth = 0.5) +  # Draw lines from bubbles to labels
  geom_label(data = top_genes, aes(label = primerid), 
             size = 3, label.padding = 0.3, label.size = 0.5, color = "black", fill = "white") +  # Labels with border
  facet_wrap(~component) +  # Facet by component (C, D)
  scale_color_gradient(low = "blue", high = "red") +  # Color by p-value (adjusted)
  theme_minimal() +  # Clean theme
  labs(
