...
# 2. Load Full Data & Clean:## a. snRNA & MetaData
...
process_gene <- function(gene_idx) {
  ...
  if (sum(data_subset[[gene]]) > 0) {
    # 
    # # Fit the model with key covariates
    m1 <- as.formula(paste0(gene, " ~ steatosis_cat + (1 | Kit_Lot)")) #Crude Analysis
    # m1 <- as.formula(paste0(gene, " ~ steatosis_cat + a1c + nih_ethnicity + group2 + (1 | Kit_Lot)"))
...
write.csv(full_results,fs::path(dir.results,"Hep_1_Steatosis_crude_no_zi.csv"))
# write.csv(full_results,fs::path(dir.ipa %>% dirname(),"Liver Pathways","Hep_1_Steatosis_03_31.csv"))
# full_results <- read.csv(fs::path(dir.results,"Hep_1_Steatosis_crude_no_zi.csv")) %>%
#   dplyr::select(-X)
...
volcano_plot <- ggplot(full_results, aes(x = Beta, y = PValue10, color = color)) +
  ...
  labs(
    title = "High Steatosis (2+3) vs. Low Steatosis (0+1)",
    subtitle = "Hep-1 (Unadjusted Model)",
    x = "FC",
    y = "-log10(P-Value)",
    color = "FC Direction Direction",
    caption = paste0("FDR < 0.05, Genes = ",Genes,", Nuclei = ",Nuclei,", Non-Convergence Rate: ",Nonconvergence_Rate)
...
# pdf(fs::path(dir.results,"Plot_Hep_1_Steatosis_crude_no_zi.pdf"),width=10,height=7)
# print(volcano_plot)
# dev.off()
# Save as JPEG instead of PDF
png(fs::path(dir.results, "Plot_Hep_1_Steatosis_crude_no_zi.png"), 
...
Genes <- nrow(so_celltype) #11300 genes
Nuclei <- ncol(so_celltype) #22983 (Hepatocytes 2)
# Get Genes #
#############
so_celltype <- NormalizeData(so_celltype)
so_celltype <- FindVariableFeatures(so_celltype, selection.method = "vst", nfeatures = 2000)
# Extract HVGs
hvg_layer1 <- VariableFeatures(so_celltype, assay = "RNA", layer = "data.1")
hvg_layer2 <- VariableFeatures(so_celltype, assay = "RNA", layer = "data.2")
# Take the union which yields 2643 genes
top2000_union <- union(hvg_layer1, hvg_layer2)
# Subset the Seurat object to only the top 2000 union genes
so_celltype2 <- subset(so_celltype, features = top2000_union)

# Extract the gene expression data for all genes
gene_expression <- as.data.frame(GetAssayData(so_celltype2, layer = "data"))
#Assign gene names as rownames, cell names as colnames
#Transpose gene expression dataset to merge with metadata
gene_expression <- t(gene_expression)
gene_expression <- data.frame(gene_expression) #Make a dataframe again after transposing
...
# Extract the metadata
metadata <- so_celltype2@meta.data
metadata$cellname <- rownames(metadata)
rownames(metadata) <- NULL
...
process_gene <- function(gene_idx) {
  ...
  if (sum(data_subset[[gene]]) > 0) {
    ...
    zi <- summary(model1)$coef$zi[1,1]
    zi_se <- summary(model1)$coef$zi[1,2]
    zi_pval <- summary(model1)$coef$zi[1,4]
  ...
  # result <- data.frame(contrast = "Overall", estimate = beta1, SE = se, df = df,
  #                      lower.CL = lowerci, upper.CL = upperci, PValue = pval1, Gene = gene)
  # result <- data.frame(Gene=gene,Beta,SE,PValue)
  result <- data.frame(Gene=gene,Beta,SE,PValue,ZI_Intercept=zi,ZI_SE=zi_se,ZI_PValue=zi_pval)
  return(result)
}
...
write.csv(full_results,fs::path(dir.results,"Hep_2_Steatosis_crude_no_zi_top2000.csv"))
# write.csv(full_results,fs::path(dir.ipa %>% dirname(),"Liver Pathways","Hep_2_Steatosis_03_31.csv"))
# full_results <- read.csv(fs::path(dir.results,"Hep_2_Steatosis_crude_no_zi.csv")) %>%
#   dplyr::select(-X)
...
volcano_plot <- ggplot(full_results, aes(x = Beta, y = PValue10, color = color)) +
  ...
  labs(
    title = "High Steatosis (2+3) vs. Low Steatosis (0+1)",
    subtitle = "Hep-2 (Unadjusted Model)",
    x = "FC",
    y = "-log10(P-Value)",
    color = "FC Direction Direction",
    caption = paste0("FDR < 0.05, Genes = ",Genes,", Nuclei = ",Nuclei,", Non-Convergence Rate: ",Nonconvergence_Rate)
...
# pdf(fs::path(dir.results,"Plot_Hep_2_Steatosis_crude_no_zi.pdf"),width=10,height=7)
# print(volcano_plot)
# dev.off()
# Save as JPEG instead of PDF
png(fs::path(dir.results, "Plot_Hep_2_Steatosis_crude_no_zi_top2000.png"), 
    width = 3000, height = 2100, res = 300)
...
# hist(data_subset$NOC2L)
# hist(data_subset$FGGY)
# hist(data_subset$CP)
# hist(data_subset$MALAT1)
# hist(data_subset$METTL15)

volcano_plot <- ggplot(full_results, aes(x = ZI_Intercept, y = ZI_PValue, color = color)) +
  geom_point(alpha = 0.7) +  # Plot points with transparency
  scale_color_identity() +  # Use the color column directly
  theme_minimal() +  # Minimal theme
  labs(
...
## C. Mixed Models with Oroboros Analyses 
### a. Descriptives
```{r}
