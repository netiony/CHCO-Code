...
dat$steatosis_percent <- as.factor(dat$steatosis_percent)
...
# Render continuous variables with both mean (SD) and P-value
render.continuous.custom <- function(x, name, ...) {
    p <- attr(x, "pval")
    if (!is.null(p)) {
        sprintf("%s<br /><sub>P-value: %s</sub>", mean_sd(x), p)
    } else {
        mean_sd(x)
    }
}
