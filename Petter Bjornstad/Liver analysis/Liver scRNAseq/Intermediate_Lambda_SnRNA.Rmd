---
  title: "Intermediate Liver scRNA analysis HEH"
...
dir.results <- c("/run/user/1026/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad")
...
de.markers(so_liver_sn, genes, "diagnosis_of_diabetes", id2 = "No", id1 = "Yes", NULL, "",logfc.threshold=0.25,min.pct=0.1)
#write.csv(m,fs::path(dir.results,"DEG_Liver_Diabetes_All_Cell_Types.csv"))
m_top <- m
rm(m)
...
sce_sn_hep <- as.SingleCellExperiment(so_liver_sn)
# rm(so_liver_sn)
...
top_eaRes <- eaRes[order(eaRes$pval)][1:20, ]

# Create a dot plot
enrich.p <- ggplot(top_eaRes, aes(x = reorder(pathway, -NES), y = NES, size = -log10(pval), color = pval)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  coord_flip() +
  labs(
    title = "Top 20 Enriched Gene Sets (Diabetes vs. No Diabetes)",
    x = "Pathway",
...
#Barchart
ggplot(top_eaRes, aes(x = reorder(pathway, NES), y = NES, fill = -log10(pval))) +
  geom_bar(stat = "identity") +
  coord_flip() +
  scale_fill_gradient(low = "blue", high = "red") +
  labs(
    title = "Top 20 Enriched Gene Sets",
    x = "Pathway",
    y = "Normalized Enrichment Score (NES)",
    fill = "-log10(p-value)"
  ) +
...
top_eaRes <- eaRes[order(eaRes$pval)][1:20, ]
top_eaRes$Regulation <- ifelse(top_eaRes$NES > 0, "Upregulated", "Downregulated")

# Create a bar chart colored by regulation status
ggplot(top_eaRes, aes(x = reorder(pathway, NES), y = NES, fill = Regulation)) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
gc()
# Save to CSV
# write.csv(eaRes, fs::path(dir.results,"GSEA_Liver_Diabetes_All_Cell_Types.csv"))
gc()
...
genes <- unique(rownames(so_liver_sn))
Idents(so_liver_sn) <- so_liver_sn$celltype
de.markers(so_liver_sn, genes, "diagnosis_of_MASLD", id2 = "No", id1 = "Yes", NULL, "",logfc.threshold=0.25,min.pct=0.1)
# write.csv(m,fs::path(dir.results,"DEG_Liver_MASLD_All_Cell_Types.csv"))
m_top <- m
rm(m)
significant_genes <- m_top %>% filter(p_val_adj < 0.05)
...
p <- EnhancedVolcano(m_top,
                     ...
                     x = 'avg_log2FC',
                     y = 'p_val_adj',
                     title = paste0("Differentially Expressed Genes by MASLD Status (Pseudobulk All Cell Types)"),
...
enrich.p <- ggplot(top_eaRes, aes(x = reorder(pathway, -NES), y = NES, size = -log10(pval), color = pval)) +
  ...
  coord_flip() +
  labs(
    title = "Top 20 Enriched Gene Sets (MASLD vs. No MASLD)",
    x = "Pathway",
    y = "Normalized Enrichment Score (NES)",
    size = "-log10(p-value)"
  ) +
...
gc()
# Save to CSV
# write.csv(eaRes, fs::path(dir.results,"GSEA_Liver_MASLD_All_Cell_Types.csv"))
gc()
...
```{r}
#IPA Pathway Results
test_ipa <- readxl::read_xlsx(fs::path(dir.dat,"Liver project","IPA_MASLD.xlsx"))
colnames(test_ipa) <- c("pathway", "neglog_p", "zscore","ratio", "molecules")
...
masld <- readxl::read_xlsx(fs::path(dir.dat,"Liver project","IPA_MASLD.xlsx"))
masld <- masld %>% arrange(desc("-log(p-value)")) 
masld_keep <- masld[1:10,]

diab <- readxl::read_xlsx(fs::path(dir.dat,"Liver project","IPA_Diabetes.xlsx"))
diab <- diab %>% arrange(desc("-log(p-value)")) 
diab_keep <- diab[1:10,]

glp1 <- readxl::read_xls(fs::path(dir.dat,"Liver project","IPA_GLP1.xls"))
glp1 <- glp1 %>% arrange(desc("-log(p-value)")) 
glp1_keep <- glp1[1:10,]

masld_plot <- ipa_plot(masld_keep)
diab_plot <- ipa_plot(diab_keep)
glp1_plot <- ipa_plot(glp1_keep)

p <- ggarrange(masld_plot, diab_plot, glp1_plot, ncol = 1, nrow = 3, align = "hv", labels = c("A", "B", "C"))
...
fcHurdle <- merge(summary_dt[contrast=='alt' & component=='C',.(primerid, `Pr(>Chisq)`)], #hurdle P values
                      summary_dt[contrast=='alt' & component=='C', .(primerid, coef, ci.hi, ci.lo)], by='primerid') #logFC coefficients
fcHurdle[,fdr:=p.adjust(`Pr(>Chisq)`, 'fdr')]
fcHurdle <- fcHurdle %>% 
  mutate(coef=10*coef)
write.csv(fcHurdle,fs::path(dir.dat,"ALT_Senescence.csv"))
...
# Bar chart
ggplot(top_genes, aes(x = Gene, y = coef, fill = Direction)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(x = "Gene", y = "effect estimate", title = "Top 10 Positive and Negative DEGs") +
...
b <- ggplot(top_genes, aes(x = coef, y = Gene, fill = Direction)) +
  geom_bar(stat = "identity") +
  labs(x = "Coefficient", y = "Gene", title = "Top 10 Positive and Negative DEGs in Diabetes vs. No Diabetes") +
