...
data_subset <- data %>%
  filter(steatosis_cat=="0+1" | steatosis_cat=="2+3") 
...
for (i in seq(1, total_genes, by = batch_size)) {
  ...
  for (gene in gene_list[batch]) { #tester genes
    ...
    m1 <- as.formula(paste0(gene," ~ steatosis_cat + age_surgery + a1c + (1 | Kit_Lot)"))
...
volcano_plot <- ggplot(full_results, aes(x = Beta, y = -log10(fdr), color = color)) +
  ...
  labs(
    title = "Bulk Hepatocyte Cells: High Steatosis (3+2) vs. Low Steatosis (0+1)",
    subtitle = "Adjusted for Age & HbA1c",
    x = "Beta",
    y = "-log10(FDR)",
