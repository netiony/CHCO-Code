...
# Open JPEG device
jpeg(fs::path(dir.results,"umap_celltype_plot.jpeg"), width = 1500, height = 1000, res = 150)
...
proportion_table <- cellcount %>%
  group_by(steatosis_cat, celltype) %>%
  summarise(count = n()) %>%
  ungroup() %>% 
...
low_steat <- proportion_table %>% 
  filter(steatosis_cat=="Low Steatosis (0+1)") %>% 
  mutate(Percent=round(proportion*100,0))
high_steat <- proportion_table %>% 
  filter(steatosis_cat=="High Steatosis (2+3)") %>% 
  mutate(Percent=round(proportion*100,0))
...
volcano_plot <- ggplot(full_results, aes(x = Beta, y = PValue10, color = color)) +
  ...
  labs(
    ...
    caption = paste0("FDR < 0.05, Genes = ",Genes,", Nuclei = ",Nuclei,", Non-Convergence Rate: ",Nonconvergence_Rate)
  ) +
  theme(
...
pdf(fs::path(dir.results,"Plot_All_Celltypes_Steatosis_04_09.pdf"),width=10,height=7)
print(volcano_plot)
dev.off()
```
#####GSEA
...
process_gene <- function(gene_idx) {
  ...
  if (sum(data_subset[[gene]]) > 0) {
    ...
    # # Fit the model with key covariates
    # m1 <- as.formula(paste0(gene, " ~ fibrosis_cat + a1c + (1 | Kit_Lot)"))
    m1 <- as.formula(paste0(gene, " ~ steatosis_cat + a1c + nih_ethnicity + group2 + (1 | Kit_Lot)"))
    # model1 <- glmmTMB(Expression ~ Treatment * Visit + (1 | Subject / Visit),
    #         data = sc_dat_sub, family = gaussian, ziformula = ~1)
    # model1 <- glmmTMB(m1,data = data_subset, family = gaussian, ziformula = ~1)
        model1 <- glmmTMB(m1,data = data_subset, family = gaussian)
    # model1 <- lmer(m1, data = data_subset)
    # Beta2 <- summary(model1)$coef$cond[3,1]
    # PValue2 <- summary(model1)$coef$cond[3,4]
    # # SE2 <- summary(model1)$coef$cond[3,2]
  ...
  # result <- data.frame(contrast = "Overall", estimate = beta1, SE = se, df = df,
  #                      lower.CL = lowerci, upper.CL = upperci, PValue = pval1, Gene = gene)
    result <- data.frame(Gene=gene,Beta,PValue)
  # result <- data.frame(Gene=gene,Beta,PValue,ZI_Intercept=zi,ZI_PValue=zi_pval)
  return(result)
}
...
write.csv(full_results,fs::path(dir.results,"Hepatocytes_Steatosis_04_09_adj_group_no_zi.csv"))
# write.csv(full_results,fs::path(dir.ipa %>% dirname(),"Liver Pathways","Hep-5_Steatosis_03_31.csv"))
# full_results <- read.csv(fs::path(dir.results,"Hep_5_Steatosis_03_31.csv")) %>%
# dplyr::select(-X)
...
max <- max(full_results$Beta)
# max <- 3.1
min <- min(full_results$Beta)
# Create the volcano plot using ggplot
volcano_plot <- ggplot(full_results, aes(x = Beta, y = PValue10, color = color)) +
  geom_point(alpha = 0.7) +  # Plot points with transparency
  scale_color_identity() +  # Use the color column directly
  ...
  labs(
    title = "High Steatosis (2+3) vs. Low Steatosis (0+1)",
    subtitle = "Hepatocytes, Adjusted for HbA1c, Ethnicity & Medication Status",
    x = "FC",
    y = "-log10(P-Value)",
    color = "FC Direction Direction",
    caption = paste0("FDR < 0.05, Genes = ",Genes,", Nuclei = ",Nuclei,", Non-Convergence Rate: ",Nonconvergence_Rate)
...
pdf(fs::path(dir.results,"Plot_Hepatocytes_Steatosis_04_09_adj_group_no_zi.pdf"),width=10,height=7)
print(volcano_plot)
dev.off()
```
#####GSEA
```{r echo = F}
#Hepatocytes
full_results <- read.csv(fs::path(dir.results,"Hepatocytes_Steatosis_04_09_adj_group_no_zi.csv")) %>%
  dplyr::select(-X)
sig_pos <- full_results %>% 
  filter(Beta>0 & fdr<0.05)
sig_neg <- full_results %>% 
...
figure <- combined_plot + 
  plot_annotation(
    title = "Positive Gene Set Enrichment Analysis for Hepatocytes vs. Steatosis (High/Low)",
    theme = theme(
...
ggsave(fs::path(dir.results, "Positive_Hepatocytes_gsea_figure.jpeg"), figure, width = 18, height = 10)
...
figure <- combined_plot + 
  plot_annotation(
    title = "Negative Gene Set Enrichment Analysis for Hepatocytes vs. Steatosis (High/Low)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,         # Center title
        size = 18,           # Bigger font
...
ggsave(fs::path(dir.results, "Negative_Hepatocytes_gsea_figure.jpeg"), figure, width = 18, height = 10)
...
write.csv(full_results,fs::path(dir.results,"dHep_Steatosis_04_09_adj_group_no_zi.csv"))
# write.csv(full_results,fs::path(dir.ipa %>% dirname(),"Liver Pathways","dHep_Steatosis_03_31.csv"))
full_results <- read.csv(fs::path(dir.results,"dHep_Steatosis_04_09_adj_group_no_zi.csv")) %>%
dplyr::select(-X)
...
# max <- 3.1
min <- min(full_results$Beta)
Genes <- nrow(full_results) #11300 genes
Nuclei <- 4674
# Create the volcano plot using ggplot
volcano_plot <- ggplot(full_results, aes(x = Beta, y = PValue10, color = color)) +
  geom_point(alpha = 0.7) +  # Plot points with transparency
  ...
  labs(
    title = "High Steatosis (2+3) vs. Low Steatosis (0+1)",
    subtitle = "dHep, Adjusted for HbA1c, Ethnicity & Medication Status",
    x = "FC",
    y = "-log10(P-Value)",
    color = "FC Direction Direction",
    caption = paste0("FDR < 0.05, Genes = ",Genes,", Nuclei = ",Nuclei,", Non-Convergence Rate: ",Nonconvergence_Rate)
...
pdf(fs::path(dir.results,"Plot_dHep_Steatosis_04_09_adj_group_no_zi.pdf"),width=10,height=7)
print(volcano_plot)
dev.off()
```
...
```{r echo = F}
#Hep-1
full_results <- read.csv(fs::path(dir.results,"Hep-1_Steatosis_04_09_adj_group_no_zi.csv")) %>%
  dplyr::select(-X)
sig_pos <- full_results %>% 
  filter(Beta>0 & fdr<0.05)
sig_neg <- full_results %>% 
...
ggsave(fs::path(dir.results, "Negative_Hep-1_gsea_figure.jpeg"), figure, width = 18, height = 10)

```
####### b. Hep-2
```{r echo = F}
#Hep-2
full_results <- read.csv(fs::path(dir.results,"Hep-2_Steatosis_04_09_adj_group_no_zi.csv")) %>%
  dplyr::select(-X)
sig_pos <- full_results %>% 
  filter(Beta>0 & fdr<0.05)
sig_neg <- full_results %>% 
...
ggsave(fs::path(dir.results, "Negative_Hep-2_gsea_figure.jpeg"), figure, width = 18, height = 10)

```
####### c. Hep-3
```{r echo = F}
#Hep-3
full_results <- read.csv(fs::path(dir.results,"Hep-3_Steatosis_04_09_adj_group_no_zi.csv")) %>%
  dplyr::select(-X)
sig_pos <- full_results %>% 
  filter(Beta>0 & fdr<0.05)
sig_neg <- full_results %>% 
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-3 - GO")
p1 <- plotEnrich(filtered_results, title = "GO")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_go.jpeg"),width=10,height=7)
...
# Plot the filtered results
p2 <- plotEnrich(filtered_results, title = "KEGG")
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-3 - Kegg")
# plotEnrich(as.data.frame(sig_pos_en[[2]]), title = "Model 1 Positive Hep-3 - Kegg")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_kegg.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# # Plot the filtered results
# # plotEnrich(filtered_results, title = "Model 1 Positive Hep-3 - Reactome '22")
# p3 <- plotEnrich(filtered_results, title = "Positive Hep-3 - Reactome '22")
# # plotEnrich(as.data.frame(sig_pos_en[[3]]), title = "Model 1 Positive Hep-3 - Reactome '22")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome22.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-3 - Reactome '24")
p3 <- plotEnrich(filtered_results, title = "Reactome '24")
# plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive Hep-3 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# # Plot the filtered results
# p5 <- plotEnrich(filtered_results, title = "Positive Hep-3 - MsigDB Computational")
# # plotEnrich(filtered_results, title = "Model 1 Positive Hep-3 - MsigDB Computational")
# # plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive Hep-3 - Reactome '24")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-3 - MsigDB Hallmark '20")
# plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive Hep-3 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
p4 <- plotEnrich(filtered_results, title = "MsigDB '20")
...
figure <- combined_plot + 
  plot_annotation(
    title = "Positive Gene Set Enrichment Analysis for Hep-3 vs. Steatosis (High/Low)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,         # Center title
        size = 18,           # Bigger font
...
ggsave(fs::path(dir.results, "Positive_Hep-3_gsea_figure.jpeg"), figure, width = 18, height = 10)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-3 - GO")
p1 <- plotEnrich(filtered_results, title = "GO")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_go.jpeg"),width=10,height=7)
...
# Plot the filtered results
p2 <- plotEnrich(filtered_results, title = "KEGG")
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-3 - Kegg")
# plotEnrich(as.data.frame(sig_neg_en[[2]]), title = "Model 1 Negative Hep-3 - Kegg")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_kegg.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# # Plot the filtered results
# # plotEnrich(filtered_results, title = "Model 1 Negative Hep-3 - Reactome '22")
# p3 <- plotEnrich(filtered_results, title = "Negative Hep-3 - Reactome '22")
# # plotEnrich(as.data.frame(sig_neg_en[[3]]), title = "Model 1 Negative Hep-3 - Reactome '22")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome22.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-3 - Reactome '24")
p3 <- plotEnrich(filtered_results, title = "Reactome '24")
# plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative Hep-3 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# # Plot the filtered results
# p5 <- plotEnrich(filtered_results, title = "Negative Hep-3 - MsigDB Computational")
# # plotEnrich(filtered_results, title = "Model 1 Negative Hep-3 - MsigDB Computational")
# # plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative Hep-3 - Reactome '24")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-3 - MsigDB Hallmark '20")
# plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative Hep-3 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
p4 <- plotEnrich(filtered_results, title = "MsigDB '20")
...
figure <- combined_plot + 
  plot_annotation(
    title = "Negative Gene Set Enrichment Analysis for Hep-3 vs. Steatosis (High/Low)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,         # Center title
        size = 18,           # Bigger font
...
ggsave(fs::path(dir.results, "Negative_Hep-3_gsea_figure.jpeg"), figure, width = 18, height = 10)

```
####### d. Hep-4
```{r echo = F}
#Hep-4
full_results <- read.csv(fs::path(dir.results,"Hep-4_Steatosis_04_09_adj_group_no_zi.csv")) %>%
  dplyr::select(-X)
sig_pos <- full_results %>% 
  filter(Beta>0 & fdr<0.05)
sig_neg <- full_results %>% 
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-4 - GO")
p1 <- plotEnrich(filtered_results, title = "GO")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_go.jpeg"),width=10,height=7)
...
# Plot the filtered results
p2 <- plotEnrich(filtered_results, title = "KEGG")
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-4 - Kegg")
# plotEnrich(as.data.frame(sig_pos_en[[2]]), title = "Model 1 Positive Hep-4 - Kegg")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_kegg.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# # Plot the filtered results
# # plotEnrich(filtered_results, title = "Model 1 Positive Hep-4 - Reactome '22")
# p3 <- plotEnrich(filtered_results, title = "Positive Hep-4 - Reactome '22")
# # plotEnrich(as.data.frame(sig_pos_en[[3]]), title = "Model 1 Positive Hep-4 - Reactome '22")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome22.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-4 - Reactome '24")
p3 <- plotEnrich(filtered_results, title = "Reactome '24")
# plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive Hep-4 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# # Plot the filtered results
# p5 <- plotEnrich(filtered_results, title = "Positive Hep-4 - MsigDB Computational")
# # plotEnrich(filtered_results, title = "Model 1 Positive Hep-4 - MsigDB Computational")
# # plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive Hep-4 - Reactome '24")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-4 - MsigDB Hallmark '20")
# plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive Hep-4 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
p4 <- plotEnrich(filtered_results, title = "MsigDB '20")
...
figure <- combined_plot + 
  plot_annotation(
    title = "Positive Gene Set Enrichment Analysis for Hep-4 vs. Steatosis (High/Low)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,         # Center title
        size = 18,           # Bigger font
...
ggsave(fs::path(dir.results, "Positive_Hep-4_gsea_figure.jpeg"), figure, width = 18, height = 10)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-4 - GO")
p1 <- plotEnrich(filtered_results, title = "GO")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_go.jpeg"),width=10,height=7)
...
# Plot the filtered results
p2 <- plotEnrich(filtered_results, title = "KEGG")
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-4 - Kegg")
# plotEnrich(as.data.frame(sig_neg_en[[2]]), title = "Model 1 Negative Hep-4 - Kegg")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_kegg.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# # Plot the filtered results
# # plotEnrich(filtered_results, title = "Model 1 Negative Hep-4 - Reactome '22")
# p3 <- plotEnrich(filtered_results, title = "Negative Hep-4 - Reactome '22")
# # plotEnrich(as.data.frame(sig_neg_en[[3]]), title = "Model 1 Negative Hep-4 - Reactome '22")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome22.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-4 - Reactome '24")
p3 <- plotEnrich(filtered_results, title = "Reactome '24")
# plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative Hep-4 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# # Plot the filtered results
# p5 <- plotEnrich(filtered_results, title = "Negative Hep-4 - MsigDB Computational")
# # plotEnrich(filtered_results, title = "Model 1 Negative Hep-4 - MsigDB Computational")
# # plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative Hep-4 - Reactome '24")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-4 - MsigDB Hallmark '20")
# plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative Hep-4 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
p4 <- plotEnrich(filtered_results, title = "MsigDB '20")
...
figure <- combined_plot + 
  plot_annotation(
    title = "Negative Gene Set Enrichment Analysis for Hep-4 vs. Steatosis (High/Low)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,         # Center title
        size = 18,           # Bigger font
...
ggsave(fs::path(dir.results, "Negative_Hep-4_gsea_figure.jpeg"), figure, width = 18, height = 10)

```
####### e. Hep-5
```{r echo = F}
#Hep-5
full_results <- read.csv(fs::path(dir.results,"Hep-5_Steatosis_04_09_adj_group_no_zi.csv")) %>%
  dplyr::select(-X)
sig_pos <- full_results %>% 
  filter(Beta>0 & fdr<0.05)
sig_neg <- full_results %>% 
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-5 - GO")
p1 <- plotEnrich(filtered_results, title = "GO")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_go.jpeg"),width=10,height=7)
...
# Plot the filtered results
p2 <- plotEnrich(filtered_results, title = "KEGG")
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-5 - Kegg")
# plotEnrich(as.data.frame(sig_pos_en[[2]]), title = "Model 1 Positive Hep-5 - Kegg")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_kegg.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# # Plot the filtered results
# # plotEnrich(filtered_results, title = "Model 1 Positive Hep-5 - Reactome '22")
# p3 <- plotEnrich(filtered_results, title = "Positive Hep-5 - Reactome '22")
# # plotEnrich(as.data.frame(sig_pos_en[[3]]), title = "Model 1 Positive Hep-5 - Reactome '22")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome22.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-5 - Reactome '24")
p3 <- plotEnrich(filtered_results, title = "Reactome '24")
# plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive Hep-5 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# # Plot the filtered results
# p5 <- plotEnrich(filtered_results, title = "Positive Hep-5 - MsigDB Computational")
# # plotEnrich(filtered_results, title = "Model 1 Positive Hep-5 - MsigDB Computational")
# # plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive Hep-5 - Reactome '24")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive Hep-5 - MsigDB Hallmark '20")
# plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive Hep-5 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
p4 <- plotEnrich(filtered_results, title = "MsigDB '20")
...
figure <- combined_plot + 
  plot_annotation(
    title = "Positive Gene Set Enrichment Analysis for Hep-5 vs. Steatosis (High/Low)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,         # Center title
        size = 18,           # Bigger font
...
ggsave(fs::path(dir.results, "Positive_Hep-5_gsea_figure.jpeg"), figure, width = 18, height = 10)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-5 - GO")
p1 <- plotEnrich(filtered_results, title = "GO")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_go.jpeg"),width=10,height=7)
...
# Plot the filtered results
p2 <- plotEnrich(filtered_results, title = "KEGG")
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-5 - Kegg")
# plotEnrich(as.data.frame(sig_neg_en[[2]]), title = "Model 1 Negative Hep-5 - Kegg")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_kegg.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# # Plot the filtered results
# # plotEnrich(filtered_results, title = "Model 1 Negative Hep-5 - Reactome '22")
# p3 <- plotEnrich(filtered_results, title = "Negative Hep-5 - Reactome '22")
# # plotEnrich(as.data.frame(sig_neg_en[[3]]), title = "Model 1 Negative Hep-5 - Reactome '22")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome22.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-5 - Reactome '24")
p3 <- plotEnrich(filtered_results, title = "Reactome '24")
# plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative Hep-5 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# # Plot the filtered results
# p5 <- plotEnrich(filtered_results, title = "Negative Hep-5 - MsigDB Computational")
# # plotEnrich(filtered_results, title = "Model 1 Negative Hep-5 - MsigDB Computational")
# # plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative Hep-5 - Reactome '24")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative Hep-5 - MsigDB Hallmark '20")
# plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative Hep-5 - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
p4 <- plotEnrich(filtered_results, title = "MsigDB '20")
...
figure <- combined_plot + 
  plot_annotation(
    title = "Negative Gene Set Enrichment Analysis for Hep-5 vs. Steatosis (High/Low)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,         # Center title
        size = 18,           # Bigger font
...
ggsave(fs::path(dir.results, "Negative_Hep-5_gsea_figure.jpeg"), figure, width = 18, height = 10)
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive dHep - GO")
p1 <- plotEnrich(filtered_results, title = "GO")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_go.jpeg"),width=10,height=7)
...
# Plot the filtered results
p2 <- plotEnrich(filtered_results, title = "KEGG")
# plotEnrich(filtered_results, title = "Model 1 Positive dHep - Kegg")
# plotEnrich(as.data.frame(sig_pos_en[[2]]), title = "Model 1 Positive dHep - Kegg")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_kegg.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# # Plot the filtered results
# # plotEnrich(filtered_results, title = "Model 1 Positive dHep - Reactome '22")
# p3 <- plotEnrich(filtered_results, title = "Positive dHep - Reactome '22")
# # plotEnrich(as.data.frame(sig_pos_en[[3]]), title = "Model 1 Positive dHep - Reactome '22")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome22.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive dHep - Reactome '24")
p3 <- plotEnrich(filtered_results, title = "Reactome '24")
# plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive dHep - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# # Plot the filtered results
# p5 <- plotEnrich(filtered_results, title = "Positive dHep - MsigDB Computational")
# # plotEnrich(filtered_results, title = "Model 1 Positive dHep - MsigDB Computational")
# # plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive dHep - Reactome '24")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_pos_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Positive dHep - MsigDB Hallmark '20")
# plotEnrich(as.data.frame(sig_pos_en[[4]]), title = "Model 1 Positive dHep - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
p4 <- plotEnrich(filtered_results, title = "MsigDB '20")
...
figure <- combined_plot + 
  plot_annotation(
    title = "Positive Gene Set Enrichment Analysis for dHep vs. Steatosis (High/Low)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,         # Center title
        size = 18,           # Bigger font
...
figure
ggsave(fs::path(dir.results, "Positive_dHep_gsea_figure.jpeg"), figure, width = 18, height = 10)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative dHep - GO")
p1 <- plotEnrich(filtered_results, title = "GO")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_go.jpeg"),width=10,height=7)
...
# Plot the filtered results
p2 <- plotEnrich(filtered_results, title = "KEGG")
# plotEnrich(filtered_results, title = "Model 1 Negative dHep - Kegg")
# plotEnrich(as.data.frame(sig_neg_en[[2]]), title = "Model 1 Negative dHep - Kegg")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_kegg.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# # Plot the filtered results
# # plotEnrich(filtered_results, title = "Model 1 Negative dHep - Reactome '22")
# p3 <- plotEnrich(filtered_results, title = "Negative dHep - Reactome '22")
# # plotEnrich(as.data.frame(sig_neg_en[[3]]), title = "Model 1 Negative dHep - Reactome '22")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome22.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative dHep - Reactome '24")
p3 <- plotEnrich(filtered_results, title = "Reactome '24")
# plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative dHep - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
# filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# # Plot the filtered results
# p5 <- plotEnrich(filtered_results, title = "Negative dHep - MsigDB Computational")
# # plotEnrich(filtered_results, title = "Model 1 Negative dHep - MsigDB Computational")
# # plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative dHep - Reactome '24")
# # ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
...
filtered_results <- subset(sig_neg_en_df, Num_Genes >= 3)
# Plot the filtered results
# plotEnrich(filtered_results, title = "Model 1 Negative dHep - MsigDB Hallmark '20")
# plotEnrich(as.data.frame(sig_neg_en[[4]]), title = "Model 1 Negative dHep - Reactome '24")
# ggsave(fs::path(dir.results,"All_Cell_Types_pos_gsea_reactome24.jpeg"),width=10,height=7)
p4 <- plotEnrich(filtered_results, title = "MsigDB '20")
...
figure <- combined_plot + 
  plot_annotation(
    title = "Negative Gene Set Enrichment Analysis for dHep vs. Steatosis (High/Low)",
    theme = theme(
      plot.title = element_text(
        hjust = 0.5,         # Center title
        size = 18,           # Bigger font
...
figure
ggsave(fs::path(dir.results, "Negative_dHep_gsea_figure.jpeg"), figure, width = 18, height = 10)
...
write.csv(full_results,fs::path(dir.results,"Target_Genes_All_Cell_Types_High_vs_Low_steatosis_Adjusted_04_07.csv"))
# full_results <- read.csv(fs::path(dir.results,"dHep_High_vs_Low_steatosis.csv"))
...
dot_plot
pdf(fs::path(dir.results,"Plot_Targeted_Genes_All_Cell_Types_High_vs_Low_steatosis_Adjusted_04_07.pdf"),width=6,height=4)
print(dot_plot)
dev.off()
```
##### Fibrosis
...
write.csv(full_results,fs::path(dir.results,"Target_Genes_Hepatocytes_High_vs_Low_steatosis_Adjusted_04_07.csv"))
# full_results <- read.csv(fs::path(dir.results,"dHep_High_vs_Low_steatosis.csv"))
...
dot_plot <- ggplot(full_results, aes(x = Beta, y = Gene)) +
  ...
  theme(axis.text.y = element_text(size = 10),
        ...
        text =element_text(size=10)) 
dot_plot
pdf(fs::path(dir.results,"Plot_Targeted_Genes_Hepatocytes_High_vs_Low_steatosis_Adjusted_04_07.pdf"),width=6,height=4)
print(dot_plot)
dev.off()
...
write.csv(full_results,fs::path(dir.results,"Target_Genes_dHep_High_vs_Low_steatosis_Adjusted_04_07.csv"))
# full_results <- read.csv(fs::path(dir.results,"dHep_High_vs_Low_steatosis.csv"))
...
dot_plot <- ggplot(full_results, aes(x = Beta, y = Gene)) +
  ...
  geom_point(size = 3,color="purple") +  # Add points for the Beta (Log2FC)
  # geom_point(size = 3,aes(color=sig)) +  # Add points for the Beta (Log2FC)
  labs(title = "Steatosis (High vs. Low) in dHep",
       subtitle = "Adj. HbA1c, Ethnicity & Medication Status",
       x = "Beta", y = "Gene",
  ...
  theme(axis.text.y = element_text(size = 10),
        ...
        text =element_text(size=10)) 
dot_plot
pdf(fs::path(dir.results,"Plot_Targeted_Genes_dHep_High_vs_Low_steatosis_Adjusted_04_07.pdf"),width=6,height=4)
print(dot_plot)
dev.off()
```
##### Fibrosis
...
key_transcripts <- c(full_results$Gene)

dotplot <- DotPlot(object = so_liver_sn, 
                   features = key_transcripts, 
                   cols = c("lightgrey", "blue"),  
                   dot.scale = 5) + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_All_Celltypes_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()

#Hepatocytes
full_results <- read.csv(fs::path(dir.results,"Pseudobulk_Hepatocytes_Steatosis_03_20.csv")) %>%
  dplyr::select(-X) %>% 
  filter(fdr<0.05) %>% 
  mutate(Gene=str_replace(Gene,"RTCA.AS1","RTCA-AS1"))
key_transcripts <- c(full_results$Gene)

# so_hep <- subset(so_liver_sn,celltype2 == "Hep")
dotplot <- DotPlot(object = so_hep, 
                   features = key_transcripts, 
                   cols = c("lightgrey", "blue"),  
                   dot.scale = 5,
                   assay = "RNA") + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Transcripts Across Hepatocyte Cell Types")
dotplot
#RTCA.AS1 where is this gene in the dataset? 
pdf(fs::path(dir.results,"DotPlot_Hepatocytes_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
full_results <- read.csv(fs::path(dir.results,"dHep_Steatosis_03_31.csv")) %>%
  dplyr::select(-X) %>% 
  filter(fdr<0.001) %>% 
  mutate(Gene=str_replace_all(Gene,"HLX.","HLX-")) %>% 
  mutate(Gene=str_replace_all(Gene,"HLA.F.","HLA-F-"))
key_transcripts <- c(full_results$Gene)
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Transcripts for dHep Across Hepatocyte Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_dHep_Key_Transcripts.pdf"),width=15,height=5)
print(dotplot)
dev.off()

#Hep-1
full_results <- read.csv(fs::path(dir.results,"Hep_1_Steatosis_03_31.csv")) %>%
  dplyr::select(-X) %>% 
  filter(fdr<0.05) %>% 
  mutate(Gene=str_replace_all(Gene,"HLX.","HLX-")) %>% 
  mutate(Gene=str_replace_all(Gene,"HLA.","HLA-"))
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Transcripts for Hep-1 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Hep_1_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()

#Hep-2
full_results <- read.csv(fs::path(dir.results,"Hep_2_Steatosis_03_31.csv")) %>%
  dplyr::select(-X) %>% 
  filter(fdr<0.01) %>% 
  mutate(Gene=str_replace_all(Gene,"HLX.","HLX-")) %>% 
  mutate(Gene=str_replace_all(Gene,"HLA.","HLA-"))
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Transcripts for Hep-2 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Hep_2_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()

#Hep-3
full_results <- read.csv(fs::path(dir.results,"Hep_3_Steatosis_03_31.csv")) %>%
  dplyr::select(-X) %>% 
  filter(fdr<0.01) %>% 
  mutate(Gene=str_replace_all(Gene,"HLX.","HLX-")) %>% 
  mutate(Gene=str_replace_all(Gene,"HLA.","HLA-")) %>% 
  mutate(Gene=str_replace_all(Gene,"RBM5.AS1","RBM5-AS1"))
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Transcripts for Hep-3 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Hep_3_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()

#Hep-4
full_results <- read.csv(fs::path(dir.results,"Hep_4_Steatosis_03_31.csv")) %>%
  dplyr::select(-X) %>% 
  filter(fdr<0.05) %>% 
  mutate(Gene=str_replace_all(Gene,"HLX.","HLX-")) %>% 
  mutate(Gene=str_replace_all(Gene,"HLA.","HLA-")) %>% 
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Transcripts for Hep-4 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Hep_4_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Transcripts for Hep-5 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Hep_5_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
```
## b. Positive
...
dotplot <- DotPlot(object = so_liver_sn, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Positive Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_Positive_All_Celltypes_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Positive Transcripts Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Positive_Transcripts_All_Cells_Across_Hepatocytes_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
pdf(fs::path(dir.results,"DotPlot_Positive_Hepatocytes_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Positive Transcripts for dHep Across Hepatocyte Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_Positive_dHep_Key_Transcripts.pdf"),width=10,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Positive Transcripts for Hep-1 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Positive_Hep_1_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Positive Transcripts for Hep-2 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Positive_Hep_2_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Positive Transcripts for Hep-3 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Positive_Hep_3_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Positive Transcripts for Hep-4 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Positive_Hep_4_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Positive Transcripts for Hep-5 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Positive_Hep_5_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
```
## c. Negative
...
dotplot <- DotPlot(object = so_liver_sn, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Negative Transcripts Across All Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Negative_All_Celltypes_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Negative Transcripts Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Negative_Transcripts_All_Cells_Across_Hepatocytes_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
pdf(fs::path(dir.results,"DotPlot_Negative_Hepatocytes_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Negative Transcripts for dHep Across Hepatocyte Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_Negative_dHep_Key_Transcripts.pdf"),width=10,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Negative Transcripts for Hep-1 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Negative_Hep_1_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Negative Transcripts for Hep-2 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Negative_Hep_2_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Negative Transcripts for Hep-3 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Negative_Hep_3_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Negative Transcripts for Hep-4 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Negative_Hep_4_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Key Negative Transcripts for Hep-5 Across Hepatocyte Cell Types")
dotplot

pdf(fs::path(dir.results,"DotPlot_Negative_Hep_5_Key_Transcripts.pdf"),width=8,height=5)
print(dotplot)
dev.off()
```
...
key_transcripts1 <- c(full_results$Gene)
...
full_results <- read.csv(fs::path(dir.results,"Pseudobulk_Hepatocytes_Steatosis_03_20.csv")) %>%
  ...
  # filter(FoldChange<0) %>% 
  mutate(Gene=str_replace(Gene,"RTCA.AS1","RTCA-AS1"))
key_transcripts2 <- c(full_results$Gene)
...
full_results <- read.csv(fs::path(dir.results,"dHep_Steatosis_03_31.csv")) %>%
  ...
  mutate(Gene=str_replace_all(Gene,"HLX.","HLX-")) %>% 
  mutate(Gene=str_replace_all(Gene,"HLA.F.","HLA-F-"))
key_transcripts3 <- c(full_results$Gene)
...
key_transcripts4 <- c(full_results$Gene)
...
key_transcripts5 <- c(full_results$Gene)
...
key_transcripts6 <- c(full_results$Gene)
...
key_transcripts7 <- c(full_results$Gene)
...
key_transcripts8 <- c(full_results$Gene)

unique_genes <- unique(unlist(c(key_transcripts1, key_transcripts2, key_transcripts3, 
                                key_transcripts4, key_transcripts5, key_transcripts6, 
                                key_transcripts7, key_transcripts8)))
...
dotplot <- DotPlot(object = so_liver_sn, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("All Key Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"All_DotPlot_All_Celltypes_Key_Transcripts.pdf"),width=30,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("All Key Transcripts Across Hepatocyte Cell Types")
dotplot
pdf(fs::path(dir.results,"All_DotPlot_Hepatocytes_Key_Transcripts.pdf"),width=30,height=5)
print(dotplot)
dev.off()
...
unique_pos_genes <- unique(unlist(c(key_transcripts1, key_transcripts2, key_transcripts3, 
                                    key_transcripts4, key_transcripts5, key_transcripts6, 
                                    key_transcripts7, key_transcripts8)))

dotplot <- DotPlot(object = so_liver_sn, 
                   features = unique_pos_genes, 
                   cols = c("lightgrey", "blue"),  
                   dot.scale = 5) + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("All Key Positive Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"All_Positive_DotPlot_All_Celltypes_Key_Transcripts.pdf"),width=30,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("All Key Positive Transcripts Across Hepatocyte Cell Types")
dotplot
pdf(fs::path(dir.results,"All_Positive_DotPlot_Hepatocytes_Key_Transcripts.pdf"),width=30,height=5)
print(dotplot)
dev.off()
...
unique_neg_genes <- unique(unlist(c(key_transcripts1, key_transcripts2, key_transcripts3, 
                                    key_transcripts4, key_transcripts5, key_transcripts6, 
                                    key_transcripts7, key_transcripts8)))

dotplot <- DotPlot(object = so_liver_sn, 
                   features = unique_neg_genes, 
                   cols = c("lightgrey", "blue"),  
                   dot.scale = 5) + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("All Key Negative Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"All_Negative_DotPlot_All_Celltypes_Key_Transcripts.pdf"),width=30,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("All Key Negative Transcripts Across Hepatocyte Cell Types")
dotplot
pdf(fs::path(dir.results,"All_Negative_DotPlot_Hepatocytes_Key_Transcripts.pdf"),width=30,height=5)
print(dotplot)
dev.off()
```
...
### a. Descriptives
```{r}
#Descriptive Statistics for Orobors Variables by Cell types & Steatosis Grade
oro <- colnames(so_liver_sn@meta.data[,c(85:114)])
meta_liver_raw <- read.csv(fs::path(dir.dat,"scRNA","data_clean","liver_biopsy_metadata_PN.csv"))
# dat <- so_liver_sn@meta.data %>% 
#   dplyr::select(c("Kit_Lot","StudyID",oro,"steatosis_grade","age_surgery","nih_sex","nih_race","nih_ethnicity","bmi","tg","a1c",
#          "diagnosis_of_diabetes","diagnosis_of_MASLD","sglt2","glp1agonist")) %>% 
#   group_by(Kit_Lot) %>% 
#   summarise(across(everything(), first)) %>%
#   ungroup() 
dat <- meta_liver_raw %>% 
...
dat <- dat %>% 
  mutate(steatosis_cat = ifelse((steatosis_grade==0 | steatosis_grade==1),"Low Steatosis (0+1)","High Steatosis (2+3)"))
dat <- dat %>% 
  filter(!is.na(carbp_m))
#Table 1. 
table1(~ carbp_m + carb_adp + carb_g + carb_cytoc + carb_s + carb_omy + carbfccp + 
         carbrcr_pm + carbrcr_pmgs + carbpe_pmgs| steatosis_cat, data=dat)
table1(~ carb_ep + carbrcr_state2 + 
         carbrcr_state4_c1 + carbrcr_state4_c12 + carb_le | steatosis_cat, data=dat)

table1(~ lipidpc_m + lipid_adp + 
         lipid_cytoc + lipid_g + lipid_s + lipid_omy + lipidfccp + lipidrcr_pcm | steatosis_cat, data=dat)


table1(~ lipidrcr_pcmgs + lipidpe_pcmgs + lipid_ep + lipidrcr_state2 + 
         lipidrcr_state4_c1 + lipidrcr_state4_c12 + lipid_le | steatosis_cat, data=dat)



# for (var in oro){
#   formula <- as.formula(paste0(var,"~ steatosis_cat"))
#   print(t.test(formula,data=dat))
#   }
```
...
```{r}
#TCA Cycle
tca #IDH3G, KGD4, ISCA2, SDHAF1, CSKMT
dotplot <- DotPlot(object = so_liver_sn, 
                   features = tca, 
                   cols = c("lightgrey", "blue"),  
                   dot.scale = 5) + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("TCA Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_TCA_Cycle_All_Celltypes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("TCA Transcripts Across Hepatocytes Types")
dotplot 
pdf(fs::path(dir.results,"DotPlot_TCA_Cycle_Hepatocytes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()

#Ox Phos
ox_phos #GZMB, ATP6, ND1, ND2, ND3, ND4, ND4L, ND5, ND6, NDUFA4L2
dotplot <- DotPlot(object = so_liver_sn, 
                   features = ox_phos, 
                   cols = c("lightgrey", "blue"),  
                   dot.scale = 5) + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Ox Phos Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_Ox_Phos_All_Celltypes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Ox Phos Transcripts Across Hepatocytes Types")
dotplot 
pdf(fs::path(dir.results,"DotPlot_Ox_Phos_Hepatocytes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()

#Beta Metabolism
beta_ox #ACSBG1, ACSL6, ACSBG2
dotplot <- DotPlot(object = so_liver_sn, 
                   features = beta_ox, 
                   cols = c("lightgrey", "blue"),  
                   dot.scale = 5) + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Beta Oxidation Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_Beta_Ox_All_Celltypes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Beta Oxidation Transcripts Across Hepatocytes Types")
dotplot 
pdf(fs::path(dir.results,"DotPlot_Beta_Ox_Hepatocytes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_liver_sn, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Fatty Acid Oxidation Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_FA_Ox_All_Celltypes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Fatty Acid Oxidation Transcripts Across Hepatocytes Types")
dotplot 
pdf(fs::path(dir.results,"DotPlot_FA_Ox_Hepatocytes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_liver_sn, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Glycolysis Transcripts Across All Cell Types")
dotplot
pdf(fs::path(dir.results,"DotPlot_Glycolysis_All_Celltypes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()
...
dotplot <- DotPlot(object = so_hep, 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +  
  ggtitle("Glycolysis Transcripts Across Hepatocytes Types")
dotplot 
pdf(fs::path(dir.results,"DotPlot_Glycolysis_Hepatocytes_Transcripts.pdf"),width=12,height=5)
print(dotplot)
dev.off()
```
...
#Hepatocytes
# oro <- colnames(so_liver_sn@meta.data[,c(85:114)])
oro <- c("carb_cytoc","carb_s","carb_omy","carbfccp","lipid_cytoc","lipid_s","lipid_omy","lipidfccp")
# hep_celltypes <-c("Hep-1","Hep-2","Hep-3","Hep-4","Hep-5","dHep")
# for (celltype in hep_celltypes) {
# oro2 <- oro[15:30]
celltype <- "dHep"
for(oro_var in oro) {
  #Filter to celltype
  # so_celltype <- subset(so_liver_sn,celltype=="dHep")
  so_celltype <- subset(so_liver_sn,subset = celltype==!!celltype)
  so_celltype$celltype <- factor(so_celltype$celltype)
  Genes <- nrow(so_celltype) #11300 genes
  Nuclei <- ncol(so_celltype) #24808 (Hepatocytes 1),22983 (Hepatocytes 2), 19994 (Hepatocytes 3), 19,319 (Hepatocytes 4),4674 nuclei (dHep)
  gene_expression <- as.data.frame(GetAssayData(so_celltype, layer = "data"))
  gene_expression <- t(gene_expression)
  ...
  process_gene <- function(gene_idx) {
    ...
    if (sum(data_subset[[gene]]) > 0) {
      ...
      # m1 <- as.formula(paste0(gene, " ~ fibrosis_cat + a1c + (1 | Kit_Lot)"))
      # m1 <- as.formula(paste0(gene, " ~ steatosis_cat + a1c + nih_ethnicity + (1 | Kit_Lot)"))
      # m1 <- as.formula(paste0(gene, " ~ ", oro_var,"+ a1c + nih_ethnicity + group2 + (1 | Kit_Lot)"))
      m1 <- as.formula(paste0(gene, " ~ ", oro_var," + (1 | Kit_Lot)"))
      # model1 <- glmmTMB(Expression ~ Treatment * Visit + (1 | Subject / Visit),
      #         data = sc_dat_sub, family = gaussian, ziformula = ~1)
      model1 <- glmmTMB(m1,data = data_subset, family = gaussian, ziformula = ~1)
      # model1 <- lmer(m1, data = data_subset)
  ...
  full_results <- full_results %>%
    mutate(fdr=p.adjust(PValue,method="fdr"))  
  full_results$PValue10 <- -log10(pmax(as.numeric(full_results$PValue), 1e-10))  # Avoid log(0)
  
  write.csv(full_results,fs::path(dir.results,paste0(celltype,"_Oroboros_",oro_var,"_crude.csv")))
  # full_results <- read.csv(fs::path(dir.results,"dHep_Steatosis_03_31.csv")) %>%
  # dplyr::select(-X)
  ...
  #Figure Range
  max <- max(significant_df$Beta,na.rm=T)+3
  min <- min(significant_df$Beta,na.rm=T)-3
  ...
  volcano_plot <- ggplot(full_results, aes(x = Beta, y = PValue10, color = color)) +
    ...
    theme_minimal() +  # Minimal theme
    labs(
      title = str_to_title(str_replace_all(paste0("Oroboros ", oro_var," vs. Gene Expression"),"_"," ")),
      subtitle = paste0(celltype," Cells, adjusting for HbA1c, Ethnicity & Medication Status"),
      x = "Beta",
      y = "-log10(P-Value)",
      color = "FC Direction Direction",
      caption = paste0("FDR < 0.05, Genes = ",Genes,", Nuclei = ",Nuclei,", Non-Convergence Rate: ",Nonconvergence_Rate)
  ...
  pdf(fs::path(dir.results,paste0("Plot_",celltype,"_Oroboros_",oro_var,"_crude.pdf")),width=10,height=7)
  print(volcano_plot)
  dev.off()
}
# }
