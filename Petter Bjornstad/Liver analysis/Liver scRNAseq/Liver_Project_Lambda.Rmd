...
# 1. Set up Libraries & Directores & Python
...
# gene_list <- gene_list_total[1:500]
...
# genes <- rownames(so_celltype)
# genes2 <- genes
# so_celltype <- subset(so_celltype,features=genes2)
# so_celltype <- subset(so_liver_sn,celltype2=="Hep")
#START HERE
# so_celltype <- subset(so_liver_sn,celltype=="Hep-5")
# so_celltype <- so_liver_sn
Genes <- nrow(so_celltype) #11300 genes
Nuclei <- ncol(so_celltype) #24808 nuclei
# Extract the gene expression data for all genes
gene_expression <- as.data.frame(GetAssayData(so_celltype, layer = "data"))
...
#Batch genes
gene_list_total <- gene_list #Run this line then comment out so you dont overwrite the full gene list
gene_list <- gene_list_total[1:10] #Select 10 genes to test, then do bigger batches 
# gene_list <- gene_list_total[1:20]
# gene <- gene_list[1]
process_gene <- function(gene_idx) {
...
# write.csv(full_results,fs::path(dir.results,"Hep_1_Steatosis_03_20.csv"))
# write.csv(full_results,fs::path(dir.ipa %>% dirname(),"Liver Pathways","Hep_1_Steatosis_03_20.csv"))
# full_results <- read.csv(fs::path(dir.results,"Hep_5_Steatosis.csv"))
...
# pdf(fs::path(dir.results,"Plot_Hep_1_Steatosis_03_17.pdf"),width=10,height=7)
# print(volcano_plot)
# dev.off()
...
# Detect the number of available cores (minus 1 for safety)
num_cores <- detectCores() - 1

# Example mclapply code with the number of cores set
results_list <- mclapply(gene_batches, function(batch) {
  lapply(batch, process_gene)
}, mc.cores = num_cores)
