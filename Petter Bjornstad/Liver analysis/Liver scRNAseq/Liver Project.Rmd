...
write.csv(full_results,fs::path(dir.results,"Hep_1_High_vs_Low_steatosis.csv"))
# full_results <- read.csv(fs::path(dir.results,"Pseudobulk_Hepatocytes_High_vs_Low_steatosis.csv"))
...
pdf(fs::path(dir.results,"Hep_1_High_vs_Low_steatosis.pdf"),width=10,height=7)
print(volcano_plot)
dev.off()
...
#   mutate(FoldChange=2^Beta-1)
# 
# write.csv(full_results,fs::path(dir.results,"Hep_2_High_vs_Low_steatosis.csv"))
# # full_results <- read.csv(fs::path(dir.results,"Pseudobulk_Hepatocytes_High_vs_Low_steatosis.csv"))
# 
# # }
# # Add a column to determine color based on the sign of the beta
...
#   ylim(0,12.5)
# 
# pdf(fs::path(dir.results,"Hep_2_High_vs_Low_steatosis.pdf"),width=10,height=7)
# print(volcano_plot)
# dev.off()
...
write.csv(full_results,fs::path(dir.results,"Hep_3_High_vs_Low_steatosis.csv"))
# full_results <- read.csv(fs::path(dir.results,"Pseudobulk_Hepatocytes_High_vs_Low_steatosis.csv"))
...
pdf(fs::path(dir.results,"Hep_3_High_vs_Low_steatosis.pdf"),width=10,height=7)
print(volcano_plot)
dev.off()
...
write.csv(full_results,fs::path(dir.results,"Hep_4_High_vs_Low_steatosis.csv"))
# full_results <- read.csv(fs::path(dir.results,"Pseudobulk_Hepatocytes_High_vs_Low_steatosis.csv"))
...
volcano_plot <- ggplot(full_results, aes(x = Beta, y = PValue10, color = color)) +
  ...
  theme_minimal() +  # Minimal theme
  labs(
    title = "Hepatocytes Suptype 4 Cells (Hep-4): High vs. Low Steatosis",
    subtitle = "Adjusted for HbA1c",
    x = "Log2FC",
    y = "-log10(P-Value)",
    color = "Beta Direction",
...
pdf(fs::path(dir.results,"Hep_4_High_vs_Low_steatosis.pdf"),width=10,height=7)
print(volcano_plot)
dev.off()
...
write.csv(full_results,fs::path(dir.results,"Hep_5_High_vs_Low_steatosis.csv"))
# full_results <- read.csv(fs::path(dir.results,"Pseudobulk_Hepatocytes_High_vs_Low_steatosis.csv"))
...
volcano_plot <- ggplot(full_results, aes(x = Beta, y = PValue10, color = color)) +
  ...
  theme_minimal() +  # Minimal theme
  labs(
    title = "Hepatocytes Suptype 5 Cells (Hep-5): High vs. Low Steatosis",
    subtitle = "Adjusted for HbA1c",
    x = "Log2FC",
    y = "-log10(P-Value)",
    color = "Beta Direction",
...
pdf(fs::path(dir.results,"Hep_5_High_vs_Low_steatosis.pdf"),width=10,height=7)
print(volcano_plot)
dev.off()
...
#First half of genes 
gene_list_total <- gene_list
gene_list <- gene_list_total[2001:2500]
# gene_list2 <- gene_list_total[2246:4489]
# gene <- gene_list[1]
# Define the function to process each gene
process_gene <- function(gene_idx) {
  ...
  if (sum(data_subset[[gene]]) > 0) {
    ...
    m1 <- as.formula(paste0(gene, " ~ fibrosis_cat + a1c + (1 | Kit_Lot)"))
    model1 <- lmer(m1, data = data_subset)
    Beta2 <- summary(model1)$coef[3,1]
    PValue2 <- summary(model1)$coef[3,5]
    # SE2 <- summary(model1)$coef[3,2]
    Beta3 <- summary(model1)$coef[2,1]
    PValue3 <- summary(model1)$coef[2,5]
    # SE3 <- summary(model1)$coef[3,2]
  ...
  # result <- data.frame(contrast = "Overall", estimate = beta1, SE = se, df = df,
  #                      lower.CL = lowerci, upper.CL = upperci, PValue = pval1, Gene = gene)
  result <- data.frame(Gene=gene,Beta2,PValue2,Beta3,PValue3)
  return(result)
}
...
# Print the final results
view(full_results)
full_results6 <- full_results
full_results_total <- rbind(full_results_total,full_results6)
