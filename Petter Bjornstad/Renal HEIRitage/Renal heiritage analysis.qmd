...
summary(tableby(group ~ fe(egfr_hyper) + fe(mgfr_hyper) , data = rh2, test=F, total = F))

summary(tableby(group ~ fe(metformin_timepoint) + fe(insulin_med_timepoint) +  fe(sglti_timepoint) + fe(tzd_timepoint) + fe(raasi_timepoint) + fe(statin) + fe(fibrates_timepoint) + fe(glp1_agonist_timepoint)  , data = rh2, test=F, total = F))

summary(tableby(group ~ ff +  glomerular_pressure +  ra +  kwt(re, "Nmiss", "median", "q1q3", "range") +  rvr, data = rh2, test=F, total = F))

 
# Multiparametric kidney MRI
 
summary(tableby(group ~ avg_pcasl3d + avg_bl_cortex + avg_bl_medulla + avg_fsoc_medulla, data = rh2, test=F, total = F))
