...
CO_clean <- left_join(CO_raw, coenroll_id, by = 'record_id') %>% 
  ...
  mutate(bsa = (0.007184 * (height^0.725))*(weight^0.425)) %>% # Du Bois and Du Bois BSA calculation
...
Neth_clean <- cbind(site='Netherlands', study='RED', Neth_clean, ethnicity='Non-Hispanic or White')

# subset to only include variables of interest
Neth_clean <- Neth_clean %>% 
  select(site, study, record_id, ethnicity, INUorIOH, G_V2_Age.c, G_Sex.c, Anthro_Length.c, Anthro_V2_BMI.a, Lab_2_1_Blood_LDL.c,
...
Neth_clean <- Neth_clean %>% 
  mutate(AER = (L_2_0_24u_urine_mircoalb_quant.u/1440)*1000) %>% # times 1000 per Daniel
...
# merge datasets
combined_dat <- full_join(CO_clean, Neth_clean, by=c('record_id', 'site', 'study', 'age', 'sex', 'ethnicity', 'height', 'bmi',
         ...
         'raasi_timepoint', 'statin', 'steady_state_insulin', 'eGFR_fas_cr_cysc', 'gir_190', 'gir_200', 'rvr', 'diabetes_duration', 
...
combined_dat <- combined_dat %>% 
  select(c('record_id', 'site', 'study', 'group', 'age', 'diabetes_duration', 'sex', 'ethnicity', 'height', 
           ...
           'gir_190', 'gir_200', 'steady_state_ffa', 'ffa_suppression', 'steady_state_insulin',
           'NEFA_V2.1_Fasting', 'NEFA_delta', 'INUorIOH', 'metformin_timepoint', 'raasi_timepoint','statin',
...
# labels - these labels work as desired; don't need them for this rmd, but left this chunk for reference code
...
# label(combined_dat$site) <- "Site"
# label(combined_dat$`Body_Fat_Percentage`) <- "Body Fat Percentage"
# label(combined_dat$cholesterol) <- "Cholesterol (mg/dL)"
# label(combined_dat$AER_sex_adjusted) <- "Sex-Adjusted Estimate of AER (mg/24hr)"
# label(combined_dat$AER_covariate_adjusted) <- "Covariate-Adjusted Estimate of AER (mg/24hr)"
...
# label(combined_dat$bsa) <- "BSA (m^2)"
```
