...
This file includes all code used for cleaning and merging the RED Netherlands data with the Colorado harmonized dataset.
...
# add data source variable for differentiating between CO and Neth dfs
...
Neth_clean <- cbind(study='RED', Neth_clean)

# subset to only include variables of interest
Neth_clean <- Neth_clean %>% 
  select(data_source, study, record_id, INUorIOH, G_V2_Age.c, G_Sex.c, Anthro_Length.c, Anthro_V2_BMI.a, Lab_2_1_Blood_LDL.c,
...
Neth_clean <- Neth_clean %>% 
  rename('age'='G_V2_Age.c', 'sex'='G_Sex.c', 'height'='Anthro_Length.c', 'bmi'='Anthro_V2_BMI.a',
         ...
         'gir'='Euclamp_V2_Mvalue_corr_minusglycosuria', 'di'='DI_V2_secondphase', 'rvr'='Renal_ERVR_V2_Hyper',
...
# merge datasets
combined_dat <- full_join(CO_clean, Neth_clean, by=c('record_id', 'data_source', 'study', 'age', 'sex', 'height', 'bmi',
         'hba1c', 'gfr_raw_plasma', 'erpf_raw_plasma', 'ff', 'AER_sex_adjusted', 'AER_covariate_adjusted', 
         'glomerular_pressure', 'ra', 're', 'sbp', 'dbp', 'map', 'weight', 'waistcm', 'dexa_body_fat', 'cholesterol', 
         'raasi_timepoint', 'statin', 'steady_state_insulin', 'eGFR_fas_cr_cysc', 'gir_190', 'gir_200', 'di', 'rvr', 'diabetes_duration', 
         'bsa', 'metformin_timepoint', 'creatinine_s', 'ldl', 'hdl', 'ffa_suppression', 'gfr_bsa_plasma', 'erpf_bsa_plasma'))
```
...
combined_dat <- combined_dat %>% 
  select(c('record_id', 'data_source', 'study', 'group', 'age', 'diabetes_duration', 'sex', 'ethnicity', 'height', 
           ...
           'airg', 'di', 'gir_190', 'gir_200', 'steady_state_cpeptide', 'steady_state_ffa', 'ffa_suppression', 'steady_state_insulin',
           'NEFA_V2.1_Fasting', 'NEFA_V2.7_Eu', 'NEFA_delta', 'INUorIOH', 'metformin_timepoint', 'raasi_timepoint','statin',
           'Med_betablocker', 'Med_calciumantagonist', 'Med_anticoagulant', 'Medication')) %>% 
...
rownames(dict) <- "label"
dict <- dict[intersect(names(combined_dat), names(dict))]
dict[setdiff(names(combined_dat), names(dict))] <- " "
label(combined_dat) = as.list(dict[match(names(combined_dat), names(dict))])

label(combined_dat$data_source) <- "Site"
label(combined_dat$`Body_Fat_Percentage`) <- "Body Fat Percentage"
label(combined_dat$cholesterol) <- "Cholesterol (mg/dL)"
label(combined_dat$AER_sex_adjusted) <- "Sex-Adjusted Estimate of AER"
label(combined_dat$AER_covariate_adjusted) <- "Covariate-Adjusted Estimate of AER"
label(combined_dat$NEFA_V2.1_Fasting) <- "Fasting NEFA level 100 minutes after iohexol/PAH"
label(combined_dat$NEFA_V2.7_Eu) <- "NEFA level 180 minutes after initiation of euglycemic clamp"
label(combined_dat$NEFA_delta) <- "Difference in NEFA level (fasting NEFA – NEFA during euglycemic clamp)"
label(combined_dat$INUorIOH) <- "Inulin/Iohexol Indicator for GFR Measurement"
label(combined_dat$Med_betablocker) <- "Betablocker use"
label(combined_dat$Med_calciumantagonist) <- "Calcium channel blocker use"
label(combined_dat$Med_anticoagulant) <- "Anticoagulant use"
label(combined_dat$Medication) <- "Current medication use and dose"
```
