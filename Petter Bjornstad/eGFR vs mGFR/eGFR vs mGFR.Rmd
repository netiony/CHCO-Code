...
# fix one creatinine value
renal_heir$serum_creatinine <- ifelse(renal_heir$serum_creatinine==145,1,renal_heir$serum_creatinine)
# fix one ACR value
renal_heir$clamp_acr_baseline <- ifelse(renal_heir$clamp_acr_baseline<0,NA,renal_heir$clamp_acr_baseline)
# one RH ppt has strange data
renal_heir <- renal_heir %>% filter(!subject_id=="RH-07-O")
...
# write file with missing ACR
write.csv(alldata[is.na(alldata$clamp_acr_baseline),],"./missing baseline ACR.csv",row.names = F)
...
mod_noint <- function(data, predictor) {
  df = data
  p <- ggplot(data=df, aes_string(x = predictor,y = "gfr_bsa")) + geom_point() + geom_smooth(method="lm") +
...
mod_int <- function(data, predictor) {
  df = data
  p <- ggplot(data=df, aes_string(x = predictor,y = "gfr_bsa",group="group", color="group")) + geom_point() + 
...
The purpose of this analysis is to compare the accuracy and precision of several eGFR equations to gold standard measured GFR (mGFR).  Data from 6 groups of participants (T1D, T2D, lean control, obese control, PKD, BMT) were analyzed together and separately.

# Methods

mGFR was normalized to body surface area (BSA). eGFR was calculated using the following equations: FAS creatinine, FAS combined creatinine and cystatin-C, Zappitelli, bedside Schwartz, CKiD age- and sex-adjusted creatinine, CKiD age- and sex-adjusted cystatin-C.

The distributions of all variables were examined prior to analysis. Descriptive statistics include mean and standard deviation or median (25th percentile, 75th percentile) for continuous variables, and frequencies and percentages for categorical variables.  Groups were compared using ANOVA for normally distributed continuous variables, the Wilcoxon test for non-normally distributed continuous variables, or the chi-square test for categorical variables.

The relationship between each eGFR equation and mGFR was examined using scatterplots and linear models for all participant groups combined. Next, evidence of differential association between eGFR and mGFR by participant group was assessed using linear models with an interaction term between group and eGFR. Spearman's correlation coefficient was calculated between each eGFR equation and mGFR, for all participants combined and after stratifying by group. Finally, Bland-Altman plots were used to examine the bias between each eGFR equation and mGFR, for all participants combined and after stratifying by group.
...
How to interpret these results: in the table labeled "Coefficients," the estimate for the eGFR equation represents the change in mGFR for a 1 unit change in eGFR. The p-value for the slope is given in the column labeled "Pr(>|t|)." Diagnostic plots for the linear models follow the model results.
...
How to interpret these results: in the table labeled "Anova table (Type III tests)," the column labeled "Pr(>F)" gives the p-value for each term in the model.  The model term "eGFR:group" is the interaction between the eGFR equation and group (for example, eGFR.fas_cr:group is the interaction between the FAS creatinine eGFR equation and group). If the interaction p-value is significant, this indicates that the relationship between eGFR and mGFR is different by group. Diagnostic plots for the linear models follow the model results.
...
```{r include=T, echo=FALSE, comment="", warning=F}
mod_int(data=alldata,predictor = "eGFR.fas_cr_cysc")
```

### Zappitelli

```{r include=T, echo=FALSE, comment="", warning=F}
mod_int(data=alldata,predictor = "eGFR.Zap" )
```

### Bedside Schwartz

```{r include=T, echo=FALSE, comment="", warning=F}
mod_int(data=alldata,predictor = "eGFR.bedside_Schwartz" )
```

### CKiD creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
mod_int(data=alldata,predictor = "eGFR.CKiD_scr" )
```

### CKiD cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
mod_int(data=alldata,predictor = "eGFR.CKiD_cysc" )
```

## Correlation coefficients - all participants

```{r include=T, echo=FALSE, comment="", warning=F}
corr_all <- rcorr(as.matrix(alldata[, c("gfr_bsa","eGFR.fas_cr","eGFR.fas_cr_cysc","eGFR.Zap","eGFR.bedside_Schwartz",
                                        "eGFR.CKiD_scr","eGFR.CKiD_cysc")]),type = "spearman")
corr_all <- flattenCorrMatrix(corr_all$r, corr_all$P)
corr_all <- corr_all[corr_all$row=="gfr_bsa",]

kable(corr_all,digits = 3)
```
...
```{r include=T, echo=FALSE, comment="", warning=F}
corr_t1d <- rcorr(as.matrix(alldata[alldata$group=="T1D", c("gfr_bsa","eGFR.fas_cr","eGFR.fas_cr_cysc","eGFR.Zap","eGFR.bedside_Schwartz",
                                        "eGFR.CKiD_scr","eGFR.CKiD_cysc")]),type = "spearman")
corr_t1d <- flattenCorrMatrix(corr_t1d$r, corr_t1d$P)
corr_t1d <- corr_t1d[corr_t1d$row=="gfr_bsa",]

kable(corr_t1d,digits = 3)
```

### T2D

```{r include=T, echo=FALSE, comment="", warning=F}
corr_t2d <- rcorr(as.matrix(alldata[alldata$group=="T2D", c("gfr_bsa","eGFR.fas_cr","eGFR.fas_cr_cysc","eGFR.Zap","eGFR.bedside_Schwartz",
                                        "eGFR.CKiD_scr","eGFR.CKiD_cysc")]),type = "spearman")
corr_t2d <- flattenCorrMatrix(corr_t2d$r, corr_t2d$P)
corr_t2d <- corr_t2d[corr_t2d$row=="gfr_bsa",]

kable(corr_t2d,digits = 3)
```

### Lean controls

```{r include=T, echo=FALSE, comment="", warning=F}
corr_lean <- rcorr(as.matrix(alldata[alldata$group=="Lean control", c("gfr_bsa","eGFR.fas_cr","eGFR.fas_cr_cysc","eGFR.Zap","eGFR.bedside_Schwartz",
                                        "eGFR.CKiD_scr","eGFR.CKiD_cysc")]),type = "spearman")
corr_lean <- flattenCorrMatrix(corr_lean$r, corr_lean$P)
corr_lean <- corr_lean[corr_lean$row=="gfr_bsa",]

kable(corr_lean,digits = 3)
```

### Obese controls

```{r include=T, echo=FALSE, comment="", warning=F}
corr_obese <- rcorr(as.matrix(alldata[alldata$group=="Obese control", c("gfr_bsa","eGFR.fas_cr","eGFR.fas_cr_cysc","eGFR.Zap","eGFR.bedside_Schwartz",
                                        "eGFR.CKiD_scr","eGFR.CKiD_cysc")]),type = "spearman")
corr_obese <- flattenCorrMatrix(corr_obese$r, corr_obese$P)
corr_obese <- corr_obese[corr_obese$row=="gfr_bsa",]

kable(corr_obese,digits = 3)
```

### PKD

Measured GFR values are missing.

### BMT

```{r include=T, echo=FALSE, comment="", warning=F}
corr_BMT <- rcorr(as.matrix(alldata[alldata$group=="BMT", c("gfr_bsa","eGFR.fas_cr","eGFR.fas_cr_cysc","eGFR.Zap","eGFR.bedside_Schwartz",
                                        "eGFR.CKiD_scr","eGFR.CKiD_cysc")]),type = "spearman")
corr_BMT <- flattenCorrMatrix(corr_BMT$r, corr_BMT$P)
corr_BMT <- corr_BMT[corr_BMT$row=="gfr_bsa",]

kable(corr_BMT,digits = 3)
```
...
```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata$gfr_bsa,alldata$eGFR.fas_cr, sig.level = 0.95)

blandr::blandr.draw(alldata$gfr_bsa,alldata$eGFR.fas_cr, sig.level = 0.95)
```

### FAS combined creatinine and cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata$gfr_bsa,alldata$eGFR.fas_cr_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata$gfr_bsa,alldata$eGFR.fas_cr_cysc, sig.level = 0.95)
```

### Zappitelli

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata$gfr_bsa,alldata$eGFR.Zap, sig.level = 0.95)

blandr::blandr.draw(alldata$gfr_bsa,alldata$eGFR.Zap, sig.level = 0.95)
```

### Bedside Schwartz

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata$gfr_bsa,alldata$eGFR.bedside_Schwartz, sig.level = 0.95)

blandr::blandr.draw(alldata$gfr_bsa,alldata$eGFR.bedside_Schwartz, sig.level = 0.95)
```

### CKiD creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata$gfr_bsa,alldata$eGFR.CKiD_scr, sig.level = 0.95)

blandr::blandr.draw(alldata$gfr_bsa,alldata$eGFR.CKiD_scr, sig.level = 0.95)
```

### CKiD cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata$gfr_bsa,alldata$eGFR.CKiD_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata$gfr_bsa,alldata$eGFR.CKiD_cysc, sig.level = 0.95)
```

## Bland-Altman plots - T1D

### FAS creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.fas_cr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.fas_cr, sig.level = 0.95)
```

### FAS combined creatinine and cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.fas_cr_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.fas_cr_cysc, sig.level = 0.95)
```

### Zappitelli

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.Zap, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.Zap, sig.level = 0.95)
```

### Bedside Schwartz

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.bedside_Schwartz, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.bedside_Schwartz, sig.level = 0.95)
```

### CKiD creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.CKiD_scr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.CKiD_scr, sig.level = 0.95)
```

### CKiD cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.CKiD_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T1D",]$gfr_bsa,alldata[alldata$group=="T1D",]$eGFR.CKiD_cysc, sig.level = 0.95)
```

## Bland-Altman plots - T2D

### FAS creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.fas_cr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.fas_cr, sig.level = 0.95)
```

### FAS combined creatinine and cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.fas_cr_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.fas_cr_cysc, sig.level = 0.95)
```

### Zappitelli

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.Zap, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.Zap, sig.level = 0.95)
```

### Bedside Schwartz

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.bedside_Schwartz, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.bedside_Schwartz, sig.level = 0.95)
```

### CKiD creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.CKiD_scr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.CKiD_scr, sig.level = 0.95)
```

### CKiD cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.CKiD_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="T2D",]$gfr_bsa,alldata[alldata$group=="T2D",]$eGFR.CKiD_cysc, sig.level = 0.95)
```

## Bland-Altman plots - Lean controls

### FAS creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.fas_cr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.fas_cr, sig.level = 0.95)
```

### FAS combined creatinine and cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.fas_cr_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.fas_cr_cysc, sig.level = 0.95)
```

### Zappitelli

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.Zap, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.Zap, sig.level = 0.95)
```

### Bedside Schwartz

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.bedside_Schwartz, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.bedside_Schwartz, sig.level = 0.95)
```

### CKiD creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.CKiD_scr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.CKiD_scr, sig.level = 0.95)
```

### CKiD cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.CKiD_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Lean control",]$gfr_bsa,alldata[alldata$group=="Lean control",]$eGFR.CKiD_cysc, sig.level = 0.95)
```

## Bland-Altman plots - Obese controls

### FAS creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.fas_cr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.fas_cr, sig.level = 0.95)
```

### FAS combined creatinine and cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.fas_cr_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.fas_cr_cysc, sig.level = 0.95)
```

### Zappitelli

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.Zap, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.Zap, sig.level = 0.95)
```

### Bedside Schwartz

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.bedside_Schwartz, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.bedside_Schwartz, sig.level = 0.95)
```

### CKiD creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.CKiD_scr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.CKiD_scr, sig.level = 0.95)
```

### CKiD cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.CKiD_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="Obese control",]$gfr_bsa,alldata[alldata$group=="Obese control",]$eGFR.CKiD_cysc, sig.level = 0.95)
```

## Bland-Altman plots - PKD

Measured GFR values are missing.

## Bland-Altman plots - BMT

### FAS creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.fas_cr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.fas_cr, sig.level = 0.95)
```

### FAS combined creatinine and cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.fas_cr_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.fas_cr_cysc, sig.level = 0.95)
```

### Zappitelli

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.Zap, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.Zap, sig.level = 0.95)
```

### Bedside Schwartz

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.bedside_Schwartz, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.bedside_Schwartz, sig.level = 0.95)
```

### CKiD creatinine

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.CKiD_scr, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.CKiD_scr, sig.level = 0.95)
```

### CKiD cystatin-C

```{r include=T, echo=FALSE, comment="", warning=F}
blandr::blandr.output.text(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.CKiD_cysc, sig.level = 0.95)

blandr::blandr.draw(alldata[alldata$group=="BMT",]$gfr_bsa,alldata[alldata$group=="BMT",]$eGFR.CKiD_cysc, sig.level = 0.95)
```
