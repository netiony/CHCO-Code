...
# eGFR revised Schwartz - going to hold off on this because it requires BUN and I didn't pull that from REDCap

# Schwartz-Lyon - not frequently used so do this later?

# eGFR CKiD U25 age and sex-dependent sCR
# old equation was gfr = 41.3 x (height in m/sCR)
# here we replace 41.3 with different values of k
alldata$k_CKiD_scr[alldata$sex_MF=="M" & alldata$age<12] <- 39*(1.008^(alldata$age-12))
alldata$k_CKiD_scr[alldata$sex_MF=="M" & alldata$age>=12 & alldata$age<18] <- 39*(1.045^(alldata$age-12))
alldata$k_CKiD_scr[alldata$sex_MF=="M" & alldata$age>=18] <- 50.8
alldata$k_CKiD_scr[alldata$sex_MF=="F" & alldata$age<12] <- 36.1*(1.008^(alldata$age-12))
alldata$k_CKiD_scr[alldata$sex_MF=="F" & alldata$age>=12 & alldata$age<18] <- 36.1*(1.023^(alldata$age-12))
alldata$k_CKiD_scr[alldata$sex_MF=="F" & alldata$age>=18] <- 41.4

# Tim's apply function for above
