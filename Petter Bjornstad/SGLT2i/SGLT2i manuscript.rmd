...
```{r results='asis'}
vars = c("age","sex","bmi","sbp","dbp","acr_u","hba1c","creatinine_s","gfr_bsa_plasma")
df = read.csv("/Volumes/Peds Endo/Petter Bjornstad/Data Harmonization/Data Clean/harmonized_dataset.csv",na.strings = "")
t = df %>% 
  filter(study %in% c("CROCODILE","IMPROVE","RENAL-HEIR"),!is.na(cryostor_id),
         group != "Type 1 Diabetes" & group != "Obese Control",
         visit=="baseline",!grepl("IT2D",co_enroll_id)) %>%
  select(record_id,co_enroll_id,study,visit,procedure,sglt2i_ever,group,all_of(vars)) %>%
