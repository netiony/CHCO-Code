...
df$eGFR_schwartz_unadj = df$eGFR_schwartz * df$bsa
egfr_sch_plot2 = ggplot(df,aes(y = igfr_ab,x = eGFR_schwartz_unadj)) + 
  geom_point() + geom_abline(intercept = 0,slope = 1) +
  annotate("text",x=225, y=100, label=expression(paste(R^2," = 0.128",sep = ""))) +
...
# Calculate eGFR not adjusted for BSA
df$eGFR_cr_unadj = df$eGFR_cr * df$bsa
egfr_pot_plot2 = ggplot(df,aes(y = igfr_ab,x = eGFR_cr_unadj)) + 
  geom_point() + geom_abline(intercept = 0,slope = 1) +
  annotate("text",x=250, y=100, label=expression(paste(R^2," = 0.368",sep = ""))) +
  theme_bw() + theme(axis.text.y = element_blank(),
                     axis.ticks.y = element_blank(),
