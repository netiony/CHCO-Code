...
```{r results='asis'}
t1_vars = c("age","sex","bsa","hr","sbp","dbp",
            "igfr","igfr_ab","eGFR_schwartz","eGFR_cr",
            "screat","scystc","ucreat","sglu","umalbcreat")
f = as.formula(paste0("~",paste0(t1_vars,collapse = "+")))
newlabels = list(age = "Age (years)",sex = "Sex",bsa = "Body Surface Area (m^2)",
...
pred = c('hr','a2_peak_flow_sr','rbf_avg','rbf_avg_vein','rbf_peak',
...
igfr_plot = ggplot(plot_df,aes(y = igfr,x = pred)) + 
  geom_point() + geom_smooth(method = "lm",se = F,color = "black",size = 0.5) +
  annotate("text",x=115, y=175, label=expression(paste("Multiple ",R^2," = 0.538",sep = ""))) +
  xlab("GFR Estimated by MRI Model (ml/min/1.73 sqm)") + theme_bw() + 
...
igfr_ab_plot = ggplot(plot_df,aes(y = igfr_ab,x = pred)) + 
  geom_point() + geom_smooth(method = "lm",se = F,color = "black",size = 0.5) +
  annotate("text",x=130, y=95, label=expression(paste("Multiple ",R^2," = 0.654",sep = ""))) +
  xlab("GFR Estimated by MRI Model (ml/min)") + theme_bw() + 
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank())
```
...
```{r results='asis'}
easly_elnet(outcome = "eGFR_schwartz",predictors = pred,data = df,print = "min")
mod = lm(eGFR_schwartz ~ hr + rbf_avg + rbf_avg_vein + rbf_peak + rbv_peak + rbv_peak_vein,data = df)
plot_df = mod$model
plot_df$pred = predict(mod)
egfr_sch_plot = ggplot(df,aes(y = igfr,x = eGFR_schwartz)) + 
  geom_point() + geom_smooth(method = "lm",se = F,color = "black",size = 0.5) +
  annotate("text",x=225, y=100, label=expression(paste(R^2," = 0.169",sep = ""))) +
  theme_bw() + ylab(expression(paste("BSA indexed GFR by ",` `^125,"I iothalamate clearance (ml/min/1.73 sqm)"))) + 
  xlab("eGFR Schwartz (ml/min/1.73 sqm)")
# Calculate eGFR not adjusted for BSA
df$eGFR_schwartz_unadj = df$eGFR_schwartz * (df$bsa / 1.73)
egfr_sch_plot2 = ggplot(df,aes(y = igfr_ab,x = eGFR_schwartz_unadj)) + 
  geom_point() + geom_smooth(method = "lm",se = F,color = "black",size = 0.5) +
  annotate("text",x=150, y=100, label=expression(paste(R^2," = 0.128",sep = ""))) +
  theme_bw() + ylab(expression(paste("Absolute GFR by ",` `^125,"I iothalamate clearance (ml/min)"))) + 
  xlab("eGFR Schwartz (ml/min)")
```
...
egfr_pot_plot = ggplot(df,aes(y = igfr,x = eGFR_cr)) + 
  geom_point() + geom_smooth(method = "lm",se = F,color = "black",size = 0.5) +
  annotate("text",x=250, y=100, label=expression(paste(R^2," = 0.299",sep = ""))) +
  theme_bw() + theme(axis.text.y = element_blank(),
                     axis.ticks.y = element_blank(),
                     axis.title.y = element_blank()) + xlab("eGFR by Full Age Spectrum (ml/min/1.73 sqm)")
# Calculate eGFR not adjusted for BSA
df$eGFR_cr_unadj = df$eGFR_cr * (df$bsa / 1.73)
egfr_pot_plot2 = ggplot(df,aes(y = igfr_ab,x = eGFR_cr_unadj)) + 
  geom_point() + geom_smooth(method = "lm",se = F,color = "black",size = 0.5) +
  annotate("text",x=160, y=100, label=expression(paste(R^2," = 0.368",sep = ""))) +
  theme_bw() + theme(axis.text.y = element_blank(),
                     axis.ticks.y = element_blank(),
                     axis.title.y = element_blank()) + xlab("eGFR by Full Age Spectrum (ml/min)")
```

# Model performance plot

```{r warning=FALSE}
(egfr_sch_plot + egfr_pot_plot + igfr_plot)/(egfr_sch_plot2 + egfr_pot_plot2 + igfr_ab_plot) + 
...
write.csv(df[,c(outcomes,pred,colnames(df)[grep("_sr",colnames(df))])],
          "./Data_Clean/renal_imaging_data.csv",row.names = F)
```
