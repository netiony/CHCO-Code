...
boxplot_adpkd_combined_dat <- dat %>%
  filter(!is.na(group_w_class)) %>%
  dplyr::select(record_id, group_w_class, bmi, avg_f, avg_k1, avg_k2_w_cyst, avg_k2_wo_cyst) %>%
  reshape(varying = c("bmi", "avg_f", "avg_k1", "avg_k2_w_cyst", "avg_k2_wo_cyst"),
          ...
          direction = "long")
# BMI
bmi_grp <- ggplot(subset(boxplot_adpkd_combined_dat, pet_param == "bmi"),
       mapping = aes(x = group_w_class,
                     y = value,
                     fill = group_w_class))  +
  ...
  geom_jitter(aes(fill = group_w_class),
              color = "black",
              alpha = 0.7, 
              shape = 21) +
  scale_fill_manual(values = c("#eaac8b", "#e56b6f",
                               ...
                               "#eaac8b", "#e56b6f",
                                "#b56576", "#6d597a")) +
  scale_color_manual(values = c("#eaac8b", "#e56b6f",
                              "#b56576", "#6d597a"))

boxplot1 <-
  ggplot(data = subset(boxplot_adpkd_combined_dat, pet_param == "avg_f"|pet_param == "avg_k1"), 
       aes(x = group_w_class,
           y = value,
           fill = group_w_class)) + 
  geom_boxplot(outlier.shape = NA) +
...
boxplot2 <-
  ggplot(data = subset(boxplot_adpkd_combined_dat, pet_param == "avg_k2_w_cyst"|pet_param == "avg_k2_wo_cyst"), 
       aes(x = group_w_class,
           y = value,
           fill = group_w_class)) + 
  geom_boxplot(outlier.shape = NA) +
...
(bmi_grp)/(boxplot1|boxplot2) + plot_layout(heights = c(1,2.2),
                                              guides = "collect") & theme(legend.position = "top")
ggsave("/Users/choiyej/GitHub/YC_CHCO/PENGUIN/boxplots.png")
