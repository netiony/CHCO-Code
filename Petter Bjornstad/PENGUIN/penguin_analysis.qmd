...
dat <- dat %>% mutate(group = case_when(group == "Lean Control" ~ "Lean",
                                   group == "PKD" ~ "PKD",
...
```{r}
table1(~ age +  sex + race + height + weight +  adjusted_bmi +  bmi + sbp + dbp + pulse + creatinine_s + eGFR_CKD_epi + acr_u + aer_24 +  aer_24_coltime  +   m_i | group, data=subset(dat, group != "Lean"))

table1(~ total_kidney_volume_ml + height_adj_total_kidney_volume + right_kidney_volume_ml + height_adj_right_kidney_volume + left_kidney_volume_ml + height_adj_left_kidney_volume + total_liver_volume_ml + total_cyst_volume_ml + total_number_of_cysts + lc_f + rc_f +  lc_k1 +  rc_k1 +  lc_k2 +  rc_k2 +  lc_vb + rc_vb +  rtot_k1_w_cyst + 	rtot_k1_wo_cyst + 	ltot_k1_w_cyst + 	ltot_k1_wo_cyst + 	rtot_k2_w_cyst + 	rtot_k2_wo_cyst + 	ltot_k2_w_cyst + 	ltot_k2_wo_cyst +  avg_f +  avg_k1 +  avg_k2 +  avg_vb +   avg_k1_w_cyst + 	avg_k1_wo_cyst + 		avg_k2_w_cyst + 	avg_k2_wo_cyst +  k2_f +  k2_k1 | group,  data=subset(dat, group != "Lean"))
```
...
pcor.plot(data = subset(dat, group=="PKD"), 
                   Y = c("height_adj_total_kidney_volume",
                         "total_liver_volume_ml", 
                         "total_cyst_volume_ml", 
                         "total_number_of_cysts",
...
f_dot <- ggplot(dat, aes(x = avg_f, fill = group)) +
  geom_histogram(stackgroups = T, binpositions = "all", binaxis = "x", stackdir = "up", aes(fill=group),
                 show.legend = F, alpha = 0.8, bins = 30) +
  labs(x=label(dat$avg_f), y="Count") +
  theme_bw() +
  scale_fill_manual(values = c("#4f772d", "#e56b6f","#b56576"))
...
k2_k1_dot <- ggplot(dat, aes(x = k2_k1)) +
  geom_histogram(stackgroups = T, binpositions = "all", binaxis = "x", stackdir = "up", aes(fill=group), alpha = 0.8, bins = 30) +
...
dat %<>%
  mutate(group_w_class = case_when(is.na(adpkd_classification) ~ group,
                                   as.character(adpkd_classification) == "1A" |
                                     ...
                                     as.character(adpkd_classification) == "1C" ~ "Mild ADPKD",
...
dat$group_w_class <- factor(dat$group_w_class,
       level = c("Lean", "Obese", "Mild ADPKD", "Severe ADPKD"))

comparison_groups = list(c("Lean", "Obese"),
                         c("Lean", "Mild ADPKD"),
                         c("Lean", "Severe ADPKD"),
                         c("Obese", "Mild ADPKD"),
                         c("Obese", "Severe ADPKD"),
                         c("Mild ADPKD", "Severe ADPKD"))

# BMI
bmi_grp <- ggplot(subset(dat, !is.na(group_w_class)),
...
# K2 w/ cyst
k2cyst <- ggplot(subset(dat, !is.na(group_w_class)),
       mapping = aes(x = group_w_class,
                     y = avg_k2_w_cyst,
                     fill = group_w_class))  +
  geom_boxplot() +  
...
# K2 w/o cyst
k2nocyst <- ggplot(subset(dat, !is.na(group_w_class)),
       mapping = aes(x = group_w_class,
                     y = avg_k2_wo_cyst,
                     fill = group_w_class))  +
  geom_boxplot() +  
...
# Avg F
f_grp <- ggplot(subset(dat, !is.na(group_w_class)),
       mapping = aes(x = group_w_class,
                     y = avg_f,
                     fill = group_w_class))  +
  geom_boxplot() +  
...
# Avg K1
k1_grp <- 
  ggplot(subset(dat, !is.na(group_w_class)),
       mapping = aes(x = group_w_class,
                     y = avg_k1,
                     fill = group_w_class))  +
  geom_boxplot() +  
...
ggsave("/Users/choiyej/GitHub/YC_CHCO/PENGUIN/bmiboxplots.png", bmi_grp)
...
dat$group <- factor(dat$group,
       level = c("PKD", "Obese", "Lean"))

# Height adjusted total kidney volume
## Adjusting for age and sex
summary(lm(ht_adj_tkv ~ group + age + sex + bmi, subset(dat, group != "Obese")))
summary(lm(ht_adj_tkv ~ group + age + sex + bmi, data = dat))
lsmeans(lm(ht_adj_tkv ~ group + age + sex + bmi, subset(dat, group != "Obese")), specs = "group")

# Oxidative biomarkers (F, K1, K2)
summary(lm(avg_f ~ group + age + sex + bmi, dat))
f_adj <- lsmeans(lm(avg_f ~ group + age + sex + bmi, dat), specs = "group")
contrast(f_adj, method = "pairwise", alpha=0.05)
f_adj_plot <- emmip(lm(avg_f ~ group + age + sex + bmi, dat),  ~ group, CIs = T, plotit = F) %>%
  ...
  scale_color_manual(values = c("#b56576","#e56b6f","#4f772d")) +
  xlim(0.5, 2.2) +
  theme_bw()

summary(lm(avg_k1 ~ group + age + sex + bmi, dat))
k1_adj <- lsmeans(lm(avg_k1 ~ group + age + sex + bmi, dat), specs = "group")
contrast(k1_adj, method = "pairwise", alpha=0.05)
k1_adj_plot <- emmip(lm(avg_k1 ~ group + age + sex + bmi, dat),  ~ group, CIs = T, plotit = F) %>%
  ggplot(aes(y = group, x = yvar, color = group)) + 
...
summary(lm(avg_k2_w_cyst ~ group + age + sex + bmi, dat))
k2_w_cyst_adj <- lsmeans(lm(avg_k2_w_cyst ~ group + age + sex + bmi, dat), specs = "group")
contrast(k2_w_cyst_adj, method = "pairwise", alpha=0.05)
k2_cyst_adj_plot <- emmip(lm(avg_k2_w_cyst ~ group + age + sex + bmi, dat),  ~ group, CIs = T, plotit = F) %>%
  ggplot(aes(y = group, x = yvar, color = group)) + 
...
summary(lm(avg_k2_wo_cyst ~ group + age + sex + bmi, dat))
k2_wo_cyst_adj <- lsmeans(lm(avg_k2_wo_cyst ~ group + age + sex + bmi, dat), specs = "group")
contrast(k2_wo_cyst_adj, method = "pairwise", alpha=0.05)
k2_wo_cyst_adj_plot <- emmip(lm(avg_k2_wo_cyst ~ group + age + sex + bmi, dat),  ~ group, CIs = T, plotit = F) %>%
  ggplot(aes(y = group, x = yvar, color = group)) + 
...
vw_activity_low <- dat %>% 
  mutate(avg_k1_w_cyst = rowMeans(select(., rtot_k1_w_cyst_percent_low,ltot_k1_w_cyst_percent_low), na.rm=T),
         ...
         level = "Below") %>%
  select(record_id, group, level, avg_k1_w_cyst, avg_k1_wo_cyst, avg_k2_w_cyst, avg_k2_wo_cyst)
...
rects <- data.frame(xstart = c(0, (count(vw_activity$group)$freq)[1], (count(vw_activity$group)$freq)[2]), 
                    xend = c(count(vw_activity$group)$freq), 
         col = unique(vw_activity$group))

vw_activity$level<- factor(vw_activity$level, levels = c("Below", "Above"))
vw_activity$group<- factor(vw_activity$group, levels = c("Lean", "PKD", "Obese"))
```

### Plots

```{r}
x1 = count(subset(vw_activity, !is.na(avg_k1_w_cyst))$group)$freq[1]
x2 = count(subset(vw_activity, !is.na(avg_k1_w_cyst))$group)$freq[2]
x3 = count(subset(vw_activity, !is.na(avg_k1_w_cyst))$group)$freq[3]
k1_1<- vw_activity %>%
  ...
  annotate("rect",
           ...
           ymin = -Inf, ymax = Inf,
           alpha = 0.5,
           fill = c("#adc178", "#f7a072", "#fcbf49")) +
  annotate("label", x = c(x1/4, (2*x1+x2)/4, (2*x1+2*x2+x3)/4),
           y = -100,
           label = c("Lean","ADPKD", "Obese"),
           fill = c("#adc178", "#f7a072", "#fcbf49"),
           alpha = 0.8,
           size = 5,
           label.size=0) +
  geom_bar(stat = "identity", 
           position = position_stacknudge(
             y = subset(vw_activity, level == "Above" & !is.na(avg_k1_w_cyst))$avg_k1_w_cyst)) +
...
x1 = count(subset(vw_activity, !is.na(avg_k1_wo_cyst))$group)$freq[1]
x2 = count(subset(vw_activity, !is.na(avg_k1_wo_cyst))$group)$freq[2]
x3 = count(subset(vw_activity, !is.na(avg_k1_wo_cyst))$group)$freq[3]
k1_2 <- vw_activity %>%
  ...
  geom_bar(stat = "identity", 
           position = position_stacknudge(
             y = subset(vw_activity, level == "Above" & !is.na(avg_k1_wo_cyst))$avg_k1_wo_cyst)) +
  scale_y_reverse(breaks = c(-100, -75, -50, -25, 0, 25, 50, 75, 100),
                     labels = abs(c(-100, -75, -50, -25, 0, 25, 50, 75, 100)),
                     limits = c(100, -100)) +
  scale_x_discrete(label = NULL) +
...
x1 = count(subset(vw_activity, !is.na(avg_k2_w_cyst))$group)$freq[1]
x2 = count(subset(vw_activity, !is.na(avg_k2_w_cyst))$group)$freq[2]
x3 = count(subset(vw_activity, !is.na(avg_k2_w_cyst))$group)$freq[3]
k2_1 <- vw_activity %>%
  ...
  geom_bar(stat = "identity", 
           position = position_stacknudge(
             y = subset(vw_activity, level == "Above" & !is.na(avg_k2_w_cyst))$avg_k2_w_cyst)) +
  scale_y_reverse(breaks = c(-100, -75, -50, -25, 0, 25, 50, 75, 100),
                     labels = abs(c(-100, -75, -50, -25, 0, 25, 50, 75, 100)),
                     limits = c(100, -100)) +
  scale_x_discrete(label = NULL) +
...
x1 = count(subset(vw_activity, !is.na(avg_k2_wo_cyst))$group)$freq[1]
x2 = count(subset(vw_activity, !is.na(avg_k2_wo_cyst))$group)$freq[2]
x3 = count(subset(vw_activity, !is.na(avg_k2_wo_cyst))$group)$freq[3]
k2_2 <- vw_activity %>%
  ...
  geom_bar(stat = "identity", 
           position = position_stacknudge(
             y = subset(vw_activity, level == "Above" & !is.na(avg_k2_wo_cyst))$avg_k2_wo_cyst)) +
  scale_y_reverse(breaks = c(-100, -75, -50, -25, 0, 25, 50, 75, 100),
                     labels = abs(c(-100, -75, -50, -25, 0, 25, 50, 75, 100)),
                     limits = c(100, -100)) +
  scale_x_discrete(label = NULL) +
