...
## 1. Simulate Microbiome Data ----
# sim.fxn <- function(i) {
#   j <- sim.par$P.s.causal[i]
...
#   
#   # Simulate exposures
#   X <- do.call(cbind, lapply(1:P.e, FUN=function(v) { rnorm(current_n, mean=0, sd=1)}))
#   # P <- ncol(X)
...
ZING_Model <- function(r,dat,exposure) {
  ...
  if(min(dat[[r]])==0){
    tryCatch({
      ...
      # sink("temp_sink.txt")
      sink(paste0("temp_sink_",scenario,"_scenario_",k,"_03_31.txt"))
