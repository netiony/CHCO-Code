...
analytes_keep <- analytes %>% filter(UniProt %in% c("P10912", "P41159", "P60568", "P05231", "P29459|P29460", "P29460|Q9NPF7","P01567"))
...
colnames(improve_means_fc) <- c("Target", "AptName", "UniProt", "FC (IMPROVE)", "p.value_IMP", "p.value.adj_IMP")
tl_means_fc <- tl_means_fc %>% select(AptName, fc, p.value, p.value.adj)
colnames(tl_means_fc) <- c("AptName", "FC (Teen-LABS)", "p.value_TL", "p.value.adj_TL")
summary <- left_join(improve_means_fc, tl_means_fc, by = "AptName")
summary$tl_sig <- ifelse(summary$p.value_TL < 0.05, "TRUE", "FALSE")
summary$improve_sig <- ifelse(summary$p.value_IMP < 0.05, "TRUE", "FALSE")
...
colored_table <- summary %>%
  select(Target, `FC (IMPROVE)`, `FC (Teen-LABS)`) %>%  # Select only these columns
  kable(format = "html", 
        escape = FALSE, 
  ...
  column_spec(2, width = "1cm",
              color = "black", 
              background = generate_color_vector(summary, "FC (IMPROVE)", "improve_sig")) %>%
...
create_custom_html_table <- function(summary) {
  ...
  improve_colors <- sapply(1:nrow(summary), function(i) 
    get_cell_color(summary$`FC (IMPROVE)`[i], summary$improve_sig[i]))
  ...
  htmlTable(
    x = cbind(
      summary$Target,
      sprintf('<div style="background-color:%s">%0.2f</div>', improve_colors, summary$`FC (IMPROVE)`),
      sprintf('<div style="background-color:%s">%0.2f</div>', tl_colors, summary$`FC (Teen-LABS)`)
    ),
    header = c("Target", "FC (IMPROVE)", "FC (Teen-LABS)"),
