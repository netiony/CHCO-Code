...
df_no_biopsy <- df %>% filter(is.na(kit_id))
df_no_biopsy <- df_no_biopsy %>% filter(visit %in% c("Baseline", "12 Months Post Surgery"))

# THESE NUMBERS ARE NOT ADDING UP...IN THE BIOPSY VS. NO BIOPSY
# NEED TO CHECK TABLES 
...
#| tbl-cap: Comparisons by Visit (Participants without biopsy)
t1_no_biopsy <- paired(visit ~ sex + age + race + ethnicity + eGFR_CKD_epi +
                      weight + bmi + bod_pod_body_fat +
                      gfr_raw_plasma + gfr_bsa_plasma +
                      ff + signed.rank(acr_u,"Nmiss", "median", "q1q3", "range") + 
                      albuminuria_cat + elevated_albuminuria +
                      ...
                      mes_volume_weibel + mes_volume_wiggins + glom_nuc_count +
                      volume_left_comb + volume_right_comb + tkv_comb, 
                    data = df_no_biopsy, id = record_id)
