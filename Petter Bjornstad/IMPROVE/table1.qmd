...
df = df[['record_id', 'co_enroll_id', 'study','visit',"group", 'age','sex','race','hba1c']]
df = df[df["visit"].isin(["baseline","pre_surgery"])]
df = df[df["group"]=="Type 2 Diabetes"]
# Aggregate
...
cat = ['sex', 'race']
group = "study"
TableOne(data=df, columns=cols, categorical=cat, groupby=group,nonnormal=["hba1c"])
```
