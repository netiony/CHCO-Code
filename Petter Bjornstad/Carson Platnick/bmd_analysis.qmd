...
dat <- dat %>%
  dplyr::mutate(group = case_when(group == "HWC" ~ "HWC",
                                  group == "OC" ~ "OC",
                                  group == "T1D" ~ "T1D",
                                  group == "T2D" ~ "T2D"))
...
```{r, include=F}
table_one <- tableby(group ~ diabetes_duration + study + sex + age + race_ethnicity_condensed + ethnicity + kwt(hba1c, "Nmiss", "median", "q1q3", "range") + height + weight + kwt(bmi, "Nmiss", "median", "q1q3", "range") + dexa_body_fat + dexa_fat_kg + dexa_lean_mass + dexa_lean_kg + dexa_bone_mineral_density + map + smr, data = dat)
```

```{r results='asis'}
#| echo: false
table_one <- summary(table_one, pfootnote = T, digits=1, digits.pct=0, total = F)
table_one
```
...
```{r,include = F}
#| echo: false
adj_hba1c_diab <- lm(dexa_bone_mineral_density ~ hba1c + age + sex + bmi, subset(dat, group == "T1D"|group == "T2D"))
