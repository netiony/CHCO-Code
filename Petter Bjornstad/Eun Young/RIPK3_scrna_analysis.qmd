...
## DEGs with SGLT2i in TAL

### SGLT2i (ever)

```{r echo = F}
FindMarkers(so_tal, group.by = "sglt2i_ever", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
...
```{r echo = F}
FindMarkers(so_tal, group.by = "sglt2i_timepoint", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
            logfc.threshold = 0, min.pct = 0)
```
...
### SGLT2i (ever, subject random effects)

```{r echo = F}
sc_dat_ripk3_ever <- sc_dat_ripk3 %>%
  filter(sglt2i_ever %in% c("Yes", "No"))
mod_ever <- glmmTMB(Expression ~ sglt2i_ever + (1 | Subject), 
            data = sc_dat_ripk3_ever, family = gaussian, ziformula = ~ 1)
summary(mod_ever)
```

### SGLT2i (at visit, subject random effects)
...
mod_timepoint <- glmmTMB(Expression ~ sglt2i_timepoint + (1 | Subject), 
            data = sc_dat_ripk3_timepoint, family = gaussian, ziformula = ~ 1)
...
## DEGs with SGLT2i in Immune cells

### SGLT2i (ever)

```{r echo = F}
FindMarkers(so_immune, group.by = "sglt2i_ever", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
            logfc.threshold = 0, min.pct = 0)
```
...
```{r echo = F}
FindMarkers(so_immune, group.by = "sglt2i_timepoint", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
            logfc.threshold = 0, min.pct = 0)
```
...
```{r echo = F, include = F}
so$celltype_pc <- ifelse(grepl("PC", so$LR_Cluster_harmony),
                                 "PC", as.character(so$LR_Cluster_harmony))
so_pc <- subset(so, celltype_pc == "PC")
so_pc <- NormalizeData(so_pc)
so_pc <- ScaleData(so_pc)
ElbowPlot(so_pc)
so_pc <- RunPCA(so_pc, ncomponents = 10, features = VariableFeatures(object = so_pc))
so_pc <- FindNeighbors(so_pc)
so_pc <- FindClusters(so_pc)
so_pc <- RunUMAP(so_pc, dims = 1:30, reduction.key = "UMAP_")
```

```{r echo = F}
expr_matrix <- as.matrix(GetAssayData(so_pc, layer = "data"))
expr_df <- as.data.frame(as.table(as.matrix(expr_matrix)))
colnames(expr_df) <- c("Gene", "Cell", "Expression")

sc_data <- data.frame(Gene = expr_df$Gene,
                      Cell = expr_df$Cell,
                      sglt2i_ever = so_pc@meta.data$sglt2i_ever,
                      sglt2i_timepoint = so_pc@meta.data$sglt2i_timepoint,
                      Subject = so_pc@meta.data$record_id,
                      Expression = as.vector(expr_df$Expression))
remove(expr_df)
remove(expr_matrix)
```
...
```{r echo = F}
DimPlot(so_pc, reduction = "umap", group.by = "sglt2i_ever", raster = F, alpha = 0.2)
DimPlot(so_pc, reduction = "umap", group.by = "sglt2i_timepoint", raster = F, alpha = 0.2)
FeaturePlot(so_pc, reduction = "umap", features = "RIPK3", cols = c("lightgray", "blue"), raster = F)
```

## DEGs with SGLT2i in PC cells

### SGLT2i (ever)

```{r echo = F}
FindMarkers(so_pc, group.by = "sglt2i_ever", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
            logfc.threshold = 0, min.pct = 0)
```
...
```{r echo = F}
FindMarkers(so_pc, group.by = "sglt2i_timepoint", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
            logfc.threshold = 0, min.pct = 0)
```
...
## DEGs with SGLT2i in CNT cells

### SGLT2i (ever)

```{r echo = F}
FindMarkers(so_cnt, group.by = "sglt2i_ever", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
            logfc.threshold = 0, min.pct = 0)
```
...
```{r echo = F}
FindMarkers(so_cnt, group.by = "sglt2i_timepoint", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
            logfc.threshold = 0, min.pct = 0)
```
...
## DEGs with SGLT2i in DCT cells

### SGLT2i (ever)

```{r echo = F}
FindMarkers(so_dct, group.by = "sglt2i_ever", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
            logfc.threshold = 0, min.pct = 0)
```
...
```{r echo = F}
FindMarkers(so_dct, group.by = "sglt2i_timepoint", ident.1 = "Yes", ident.2 = "No", features = "RIPK3", 
            logfc.threshold = 0, min.pct = 0)
```
...
```{r echo = F, eval = F}
s3$upload_file("/mmfs1/gscratch/scrubbed/yejichoi/Bjornstad/CHCO-Code/Petter Bjornstad/Eun Young/RIPK3_scrna_analysis.html", "test", 'RIPK3_scrna_analysis.html')
```
