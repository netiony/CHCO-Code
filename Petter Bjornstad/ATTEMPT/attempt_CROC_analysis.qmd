...
croc_nebula_pt_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
...
croc_nebula_pt_results_list <- Filter(Negate(is.null), croc_nebula_pt_results_list)  # remove NULLs first
names(croc_nebula_pt_results_list) <- sapply(croc_nebula_pt_results_list, function(x) x$gene)  # set names
croc_nebula_pt_results_list <- lapply(croc_nebula_pt_results_list, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_pt_results_list))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_pt_results_list, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/PT/nebula/pt_croc_hvg_nebula_res.rds')
```
...
croc_nebula_pt_results_list_reml <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_pt[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_pt, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
    ...
    result <- nebula(count = data_g_gene$count, id = data_g_gene$id, pred = data_g_gene$pred, ncore = 1, output_re = T, covariance = T, reml = 1)
    list(gene = g, result = result)  # return both gene name and result
...
# set the names of results based on gene names
croc_nebula_pt_results_list_reml <- Filter(Negate(is.null), croc_nebula_pt_results_list_reml)  # remove NULLs first
names(croc_nebula_pt_results_list_reml) <- sapply(croc_nebula_pt_results_list_reml, function(x) x$gene)  # set names
croc_nebula_pt_results_list_reml <- lapply(croc_nebula_pt_results_list_reml, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_pt_results_list_reml))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_pt_results_list_reml, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/PT/nebula/pt_croc_hvg_nebula_res_reml.rds')
```
...
croc_nebula_pt_results_list_reml_offset <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_pt[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_pt, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
    ...
    result <- nebula(count = data_g_gene$count, id = data_g_gene$id, pred = data_g_gene$pred, 
                     ncore = 1, output_re = T, covariance = T, reml = 1, 
                     offset = offset_hvg_pt)
    list(gene = g, result = result)  # return both gene name and result
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/PT/nebula/pt_croc_hvg_nebula_res_reml_offset.rds')
```

### Targeted in ATTEMPT

```{r echo = F}
# exclude non-converged genes
...
# identify p<0.05 genes in group comparison in CROCODILE
croc_pt_nebula_res_combined_reml_offset_sig <- croc_pt_nebula_res_combined_reml_offset %>%
  filter(p_groupType_1_Diabetes < 0.05)

croc_pt_nebula_sig_gene_list <- croc_pt_nebula_res_combined_reml_offset$Gene
```

```{r echo = F}
# subset ATTEMPT to same HVGs as CROCODILE
attempt_so_croc_subset <- subset(attempt_so_pt, features = hvgs_pt)

# run nebula REML + offset
attempt_croc_counts_hvg_pt <- round(GetAssayData(attempt_so_croc_subset, layer = "counts")) # load counts and round
genes_list <- rownames(attempt_croc_counts_hvg_pt)

attempt_croc_meta_hvg_pt <- attempt_so_croc_subset@meta.data
attempt_croc_pred_pt = model.matrix(~treatment*visit, data = attempt_croc_meta_hvg_pt)
attempt_croc_data_g_hvg_pt = group_cell(count=attempt_croc_counts_hvg_pt, id=attempt_croc_meta_hvg_pt$subject, pred=attempt_croc_pred_pt)
attempt_croc_offset_hvg_pt = Matrix::colSums(attempt_croc_data_g_hvg_pt$count) 
...
attempt_croc_nebula_pt_results_list_reml_offset <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- counts_hvg_pt[g, , drop = FALSE]
    meta_gene <- subset(attempt_so_croc_subset, features = g)@meta.data
    ...
    result <- nebula(count = data_g_gene$count, id = data_g_gene$id, pred = data_g_gene$pred, 
                     ncore = 1, output_re = T, covariance = T, reml = 1, 
                     offset = attempt_croc_offset_hvg_pt)
    list(gene = g, result = result)  # return both gene name and result
...
# set the names of results based on gene names
attempt_croc_nebula_pt_results_list_reml_offset <- Filter(Negate(is.null), attempt_croc_nebula_pt_results_list_reml_offset)  # remove NULLs first
names(attempt_croc_nebula_pt_results_list_reml_offset) <- sapply(attempt_croc_nebula_pt_results_list_reml_offset, function(x) x$gene)  # set names
attempt_croc_nebula_pt_results_list_reml_offset <- lapply(attempt_croc_nebula_pt_results_list_reml_offset, function(x) x$result)  # clean list back to just results
...
nebula_nonconverged_percent <- (length(genes_list)-length(attempt_croc_nebula_pt_results_list_reml_offset))/length(genes_list)
print(paste0(nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(attempt_croc_nebula_pt_results_list_reml_offset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/PT/nebula/pt_attempt_hvg_nebula_res_reml_offset.rds')
```

```{r echo = F}
# exclude non-converged genes from ATTEMPT
attempt_croc_pt_nebula_convergence_reml_offset <- map_dfr(
  names(attempt_croc_nebula_pt_results_list_reml_offset),
  function(gene_name) {
    converged <- attempt_croc_nebula_pt_results_list_reml_offset[[gene_name]]$convergence
    df <- data.frame(Gene = gene_name,
                     Convergence_Code = converged)
    return(df)
  }
...
attempt_croc_pt_nebula_converged_reml_offset <- attempt_croc_pt_nebula_convergence_reml_offset %>%
  filter(Convergence_Code >=-10)

attempt_croc_pt_nebula_res_combined_reml_offset <- map_dfr(
  names(attempt_croc_nebula_pt_results_list_reml_offset),
  function(gene_name) {
    df <- attempt_croc_nebula_pt_results_list_reml_offset[[gene_name]]$summary
    df <- df %>% filter(gene_name %in% attempt_croc_pt_nebula_converged_reml_offset$Gene) %>%
      dplyr::ungroup() %>%
      dplyr::mutate(Gene = gene_name)
    return(df)
  }
)
attempt_croc_pt_nebula_res_combined_reml_offset <- attempt_croc_pt_nebula_res_combined_reml_offset %>%
  mutate(fdr_interaction = p.adjust(`p_treatmentDapagliflozin:visitPOST`, method = "fdr"))

# identify p<0.05 genes in group comparison in CROCODILE
attempt_croc_pt_nebula_res_combined_reml_offset_sig <- attempt_croc_pt_nebula_res_combined_reml_offset %>%
  filter(`p_treatmentDapagliflozin:visitPOST` < 0.05)

attempt_croc_pt_nebula_sig_gene_list <- attempt_croc_pt_nebula_res_combined_reml_offset$Gene
```

```{r echo = F}
sum(attempt_croc_pt_nebula_sig_gene_list %in% croc_pt_nebula_sig_gene_list)
sum(attempt_croc_pt_nebula_sig_gene_list %nin% croc_pt_nebula_sig_gene_list)

sum(croc_pt_nebula_sig_gene_list %in% attempt_croc_pt_nebula_sig_gene_list)
sum(croc_pt_nebula_sig_gene_list %nin% attempt_croc_pt_nebula_sig_gene_list)
```
...
croc_nebula_tal_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
...
# set the names of results based on gene names
croc_nebula_tal_results_list <- Filter(Negate(is.null), croc_nebula_tal_results_list)  # remove NULLs first
names(croc_nebula_tal_results_list) <- sapply(croc_nebula_tal_results_list, function(x) x$gene)  # set names
croc_nebula_tal_results_list <- lapply(croc_nebula_tal_results_list, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_tal_results_list))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_tal_results_list, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/TAL/nebula/tal_croc_hvg_nebula_res.rds')
```
###### REML with rounded counts
...
croc_nebula_tal_results_list_reml <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_tal[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_tal, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
# set the names of results based on gene names
croc_nebula_tal_results_list_reml <- Filter(Negate(is.null), croc_nebula_tal_results_list_reml)  # remove NULLs first
names(croc_nebula_tal_results_list_reml) <- sapply(croc_nebula_tal_results_list_reml, function(x) x$gene)  # set names
croc_nebula_tal_results_list_reml <- lapply(croc_nebula_tal_results_list_reml, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_tal_results_list_reml))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_tal_results_list_reml, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/TAL/nebula/tal_croc_hvg_nebula_res_reml.rds')
```
###### REML with rounded counts & offset
...
croc_nebula_tal_results_list_reml_offset <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_tal[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_tal, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/TAL/nebula/tal_croc_hvg_nebula_res_reml_offset.rds')
```
...
```{r echo = F}
croc_so_immune <- FindVariableFeatures(croc_so_immune, selection.method = "vst", nfeatures = 2000)
hvgs_immune <- VariableFeatures(croc_so_immune)

# Perform remaining steps on top 2000 hvgs
# Subset Seurat object to only HVGs
croc_so_hvg_immune <- subset(croc_so_immune, features = hvgs_immune)
```
...
```{r echo = F, eval = F}
croc_counts_hvg_immune <- round(GetAssayData(croc_so_hvg_immune, layer = "counts")) # load counts and round
# meta_hvg_immune <- croc_so_hvg_immune@meta.data
# pred = model.matrix(~group, data = meta_hvg_immune)
# data_g_hvg_immune = list(count=croc_counts_hvg_immune, id=meta_hvg_immune$subject, pred=pred)

# With parallelization
# List of genes
genes_list <- rownames(croc_counts_hvg_immune)
...
croc_nebula_immune_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_immune[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_immune, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
    data_g_gene <- list(count = count_gene, id = meta_gene$record_id, pred = pred_gene)
...
# set the names of results based on gene names
croc_nebula_immune_results_list <- Filter(Negate(is.null), croc_nebula_immune_results_list)  # remove NULLs first
names(croc_nebula_immune_results_list) <- sapply(croc_nebula_immune_results_list, function(x) x$gene)  # set names
croc_nebula_immune_results_list <- lapply(croc_nebula_immune_results_list, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_immune_results_list))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_immune_results_list, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/Immune/nebula/immune_croc_hvg_nebula_res.rds')
```
...
croc_nebula_immune_results_list_reml <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_immune[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_immune, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
# set the names of results based on gene names
croc_nebula_immune_results_list_reml <- Filter(Negate(is.null), croc_nebula_immune_results_list_reml)  # remove NULLs first
names(croc_nebula_immune_results_list_reml) <- sapply(croc_nebula_immune_results_list_reml, function(x) x$gene)  # set names
croc_nebula_immune_results_list_reml <- lapply(croc_nebula_immune_results_list_reml, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_immune_results_list_reml))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_immune_results_list_reml, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/Immune/nebula/immune_croc_hvg_nebula_res_reml.rds')
```
...
croc_nebula_immune_results_list_reml_offset <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_immune[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_immune, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/Immune/nebula/immune_croc_hvg_nebula_res_reml_offset.rds')
```
...
croc_nebula_pc_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
...
# set the names of results based on gene names
croc_nebula_pc_results_list <- Filter(Negate(is.null), croc_nebula_pc_results_list)  # remove NULLs first
names(croc_nebula_pc_results_list) <- sapply(croc_nebula_pc_results_list, function(x) x$gene)  # set names
croc_nebula_pc_results_list <- lapply(croc_nebula_pc_results_list, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_pc_results_list))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_pc_results_list, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/PC/nebula/pc_croc_hvg_nebula_res.rds')
```
###### REML with rounded counts
...
croc_nebula_pc_results_list_reml <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_pc[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_pc, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
# set the names of results based on gene names
croc_nebula_pc_results_list_reml <- Filter(Negate(is.null), croc_nebula_pc_results_list_reml)  # remove NULLs first
names(croc_nebula_pc_results_list_reml) <- sapply(croc_nebula_pc_results_list_reml, function(x) x$gene)  # set names
croc_nebula_pc_results_list_reml <- lapply(croc_nebula_pc_results_list_reml, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_pc_results_list_reml))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_pc_results_list_reml, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/PC/nebula/pc_croc_hvg_nebula_res_reml.rds')
```
...
croc_nebula_pc_results_list_reml_offset <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_pc[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_pc, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/PC/nebula/pc_croc_hvg_nebula_res_reml_offset.rds')
```
...
croc_nebula_ic_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
...
# set the names of results based on gene names
croc_nebula_ic_results_list <- Filter(Negate(is.null), croc_nebula_ic_results_list)  # remove NULLs first
names(croc_nebula_ic_results_list) <- sapply(croc_nebula_ic_results_list, function(x) x$gene)  # set names
croc_nebula_ic_results_list <- lapply(croc_nebula_ic_results_list, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_ic_results_list))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_ic_results_list, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/IC/nebula/ic_croc_hvg_nebula_res.rds')
```
###### REML with rounded counts
...
croc_nebula_ic_results_list_reml <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_ic[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_ic, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
# set the names of results based on gene names
croc_nebula_ic_results_list_reml <- Filter(Negate(is.null), croc_nebula_ic_results_list_reml)  # remove NULLs first
names(croc_nebula_ic_results_list_reml) <- sapply(croc_nebula_ic_results_list_reml, function(x) x$gene)  # set names
croc_nebula_ic_results_list_reml <- lapply(croc_nebula_ic_results_list_reml, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_ic_results_list_reml))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_ic_results_list_reml, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/IC/nebula/ic_croc_hvg_nebula_res_reml.rds')
```
...
croc_nebula_ic_results_list_reml_offset <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_ic[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_ic, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/IC/nebula/ic_croc_hvg_nebula_res_reml_offset.rds')
```
# EC
...
croc_nebula_ec_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
...
# set the names of results based on gene names
croc_nebula_ec_results_list <- Filter(Negate(is.null), croc_nebula_ec_results_list)  # remove NULLs first
names(croc_nebula_ec_results_list) <- sapply(croc_nebula_ec_results_list, function(x) x$gene)  # set names
croc_nebula_ec_results_list <- lapply(croc_nebula_ec_results_list, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_ec_results_list))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_ec_results_list, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/EC/nebula/ec_croc_hvg_nebula_res.rds')
```
###### REML with rounded counts
...
croc_nebula_ec_results_list_reml <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_ec[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_ec, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
# set the names of results based on gene names
croc_nebula_ec_results_list_reml <- Filter(Negate(is.null), croc_nebula_ec_results_list_reml)  # remove NULLs first
names(croc_nebula_ec_results_list_reml) <- sapply(croc_nebula_ec_results_list_reml, function(x) x$gene)  # set names
croc_nebula_ec_results_list_reml <- lapply(croc_nebula_ec_results_list_reml, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_ec_results_list_reml))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_ec_results_list_reml, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/EC/nebula/ec_croc_hvg_nebula_res_reml.rds')
```
...
croc_nebula_ec_results_list_reml_offset <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_ec[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_ec, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/EC/nebula/ec_croc_hvg_nebula_res_reml_offset.rds')
```
...
croc_nebula_fibvsmc_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
...
# set the names of results based on gene names
croc_nebula_fibvsmc_results_list <- Filter(Negate(is.null), croc_nebula_fibvsmc_results_list)  # remove NULLs first
names(croc_nebula_fibvsmc_results_list) <- sapply(croc_nebula_fibvsmc_results_list, function(x) x$gene)  # set names
croc_nebula_fibvsmc_results_list <- lapply(croc_nebula_fibvsmc_results_list, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_fibvsmc_results_list))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_fibvsmc_results_list, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/FIBVSMCP/nebula/fibvsmc_croc_hvg_nebula_res.rds')
```
###### REML with rounded counts
...
croc_nebula_fibvsmc_results_list_reml <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_fibvsmc[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_fibvsmc, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
# set the names of results based on gene names
croc_nebula_fibvsmc_results_list_reml <- Filter(Negate(is.null), croc_nebula_fibvsmc_results_list_reml)  # remove NULLs first
names(croc_nebula_fibvsmc_results_list_reml) <- sapply(croc_nebula_fibvsmc_results_list_reml, function(x) x$gene)  # set names
croc_nebula_fibvsmc_results_list_reml <- lapply(croc_nebula_fibvsmc_results_list_reml, function(x) x$result)  # clean list back to just results
...
croc_nebula_nonconverged_percent <- (length(genes_list)-length(croc_nebula_fibvsmc_results_list_reml))/length(genes_list)
print(paste0(croc_nebula_nonconverged_percent*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(croc_nebula_fibvsmc_results_list_reml, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/FIBVSMCP/nebula/fibvsmc_croc_hvg_nebula_res_reml.rds')
```
###### REML with rounded counts & offset
...
croc_nebula_fibvsmc_results_list_reml_offset <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- croc_counts_hvg_fibvsmc[g, , drop = FALSE]
    meta_gene <- subset(croc_so_hvg_fibvsmc, features = g)@meta.data
    pred_gene <- model.matrix(~group, data = meta_gene)
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/FIBVSMCP/nebula/fibvsmc_croc_hvg_nebula_res_reml_offset.rds')
```
...
pt_expr_matrix <- as.matrix(GetAssayData(croc_so_pt, layer = "data"))
...
pt_croc_sc_data <- data.frame(Gene = pt_expr_df$Gene,
                              ...
                              Group = croc_so_pt@meta.data$group,
                              Subject = croc_so_pt@meta.data$record_id,
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_scrna_mm_model_combined.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pt_croc_emmeans_df_combined, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_scrna_mm_emmeans_combined.rds')
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_croc_sig_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pt_attempt_sig_emmeans_mod3, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_croc_sig_emmeans_mod3.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_croc_sig_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pt_attempt_sig_emmeans_mod4, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_croc_sig_emmeans_mod4.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_croc_sig_target_subset_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pt_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_croc_sig_emmeans_target_subset_mod3.rds')
...
#### Dot Plots
```{r echo = F}
combined_so_pt <- merge(so_attempt_pt, croc_so_pt)
Seurat::DotPlot(so)
```
...
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pt_croc_sig_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_croc_sig_target_subset_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pt_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PT/pt_attempt_croc_sig_emmeans_target_subset_mod4.rds')
...
```{r echo = F}
# Subset Seurat object to retain only these genes
tal_expr_matrix <- as.matrix(GetAssayData(croc_so_tal, layer = "data"))
...
tal_croc_sc_data <- data.frame(Gene = tal_expr_df$Gene,
                              ...
                              Group = croc_so_tal@meta.data$group,
                              Subject = croc_so_tal@meta.data$record_id,
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_scrna_mm_model_combined.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(tal_croc_emmeans_df_combined, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_scrna_mm_emmeans_combined.rds')
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_croc_sig_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(tal_attempt_sig_emmeans_mod3, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_croc_sig_emmeans_mod3.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_croc_sig_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(tal_attempt_sig_emmeans_mod4, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_croc_sig_emmeans_mod4.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_croc_sig_target_subset_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(tal_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_croc_sig_emmeans_target_subset_mod3.rds')
...
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(tal_croc_sig_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_croc_sig_target_subset_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(tal_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/TAL/tal_attempt_croc_sig_emmeans_target_subset_mod4.rds')
...
```{r echo = F}
# Subset Seurat object to retain only these genes
pc_expr_matrix <- as.matrix(GetAssayData(croc_so_pc, layer = "data"))
...
pc_croc_sc_data <- data.frame(Gene = pc_expr_df$Gene,
                              ...
                              Group = croc_so_pc@meta.data$group,
                              Subject = croc_so_pc@meta.data$record_id,
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_scrna_mm_model_combined.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pc_croc_emmeans_df_combined, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_scrna_mm_emmeans_combined.rds')
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_croc_sig_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pc_attempt_sig_emmeans_mod3, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_croc_sig_emmeans_mod3.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_croc_sig_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pc_attempt_sig_emmeans_mod4, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_croc_sig_emmeans_mod4.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_croc_sig_target_subset_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pc_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_croc_sig_emmeans_target_subset_mod3.rds')
...
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pc_croc_sig_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_croc_sig_target_subset_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pc_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/PC/pc_attempt_croc_sig_emmeans_target_subset_mod4.rds')
...
```{r echo = F}
# Subset Seurat object to retain only these genes
ec_expr_matrix <- as.matrix(GetAssayData(croc_so_ec, layer = "data"))
...
ec_croc_sc_data <- data.frame(Gene = ec_expr_df$Gene,
                              ...
                              Group = croc_so_ec@meta.data$group,
                              Subject = croc_so_ec@meta.data$record_id,
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_scrna_mm_model_combined.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ec_croc_emmeans_df_combined, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_scrna_mm_emmeans_combined.rds')
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_croc_sig_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ec_attempt_sig_emmeans_mod3, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_croc_sig_emmeans_mod3.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_croc_sig_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ec_attempt_sig_emmeans_mod4, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_croc_sig_emmeans_mod4.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_croc_sig_target_subset_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ec_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_croc_sig_emmeans_target_subset_mod3.rds')
...
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ec_croc_sig_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_croc_sig_target_subset_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ec_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/EC/ec_attempt_croc_sig_emmeans_target_subset_mod4.rds')
...
```{r echo = F}
# Subset Seurat object to retain only these genes
ic_expr_matrix <- as.matrix(GetAssayData(croc_so_ic, layer = "data"))
...
ic_croc_sc_data <- data.frame(Gene = ic_expr_df$Gene,
                              ...
                              Group = croc_so_ic@meta.data$group,
                              Subject = croc_so_ic@meta.data$record_id,
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_scrna_mm_model_combined.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ic_croc_emmeans_df_combined, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_scrna_mm_emmeans_combined.rds')
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_croc_sig_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ic_attempt_sig_emmeans_mod3, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_croc_sig_emmeans_mod3.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_croc_sig_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ic_attempt_sig_emmeans_mod4, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_croc_sig_emmeans_mod4.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_croc_sig_target_subset_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ic_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_croc_sig_emmeans_target_subset_mod3.rds')
...
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ic_croc_sig_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_croc_sig_target_subset_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ic_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/IC/ic_attempt_croc_sig_emmeans_target_subset_mod4.rds')
...
```{r echo = F}
# Subset Seurat object to retain only these genes
immune_expr_matrix <- as.matrix(GetAssayData(croc_so_immune, layer = "data"))
...
immune_croc_sc_data <- data.frame(Gene = immune_expr_df$Gene,
                              ...
                              Group = croc_so_immune@meta.data$group,
                              Subject = croc_so_immune@meta.data$record_id,
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_scrna_mm_model_combined.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_croc_emmeans_df_combined, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_scrna_mm_emmeans_combined.rds')
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_croc_sig_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_attempt_sig_emmeans_mod3, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_croc_sig_emmeans_mod3.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_croc_sig_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_attempt_sig_emmeans_mod4, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_croc_sig_emmeans_mod4.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_croc_sig_target_subset_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_croc_sig_emmeans_target_subset_mod3.rds')
...
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_croc_sig_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_croc_sig_target_subset_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/Immune/immune_attempt_croc_sig_emmeans_target_subset_mod4.rds')
...
croc_so$celltype_fibvsmc <- ifelse(croc_so$celltype_harmony %in% c("VSMC/MC/FIB"),
                                     "FIBVSMC", as.character(croc_so$celltype_harmony))
croc_so_fibvsmcp <- subset(croc_so, celltype_fibvsmc == "FIBVSMC")
```

```{r echo = F}
# Subset Seurat object to retain only these genes
fibvsmcp_expr_matrix <- as.matrix(GetAssayData(croc_so_fibvsmcp, layer = "data"))
...
fibvsmcp_croc_sc_data <- data.frame(Gene = fibvsmcp_expr_df$Gene,
                              ...
                              Group = croc_so_fibvsmcp@meta.data$group,
                              Subject = croc_so_fibvsmcp@meta.data$record_id,
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_scrna_mm_model_combined.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_croc_emmeans_df_combined, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_scrna_mm_emmeans_combined.rds')
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_croc_sig_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_attempt_sig_emmeans_mod3, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_croc_sig_emmeans_mod3.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_croc_sig_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_attempt_sig_emmeans_mod4, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_croc_sig_emmeans_mod4.rds')
```
...
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_croc_sig_target_subset_mod3.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_croc_sig_emmeans_target_subset_mod3.rds')
...
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_croc_sig_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_croc_sig_target_subset_mod4.rds')

temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_croc_sig_emmeans_target_subset, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'CROCODILE comparison analysis/CROCODILE comparison analysis/FIBVSMCP/fibvsmcp_attempt_croc_sig_emmeans_target_subset_mod4.rds')
