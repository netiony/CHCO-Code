...
save.image(file = "/home/choiyej/Documents/CHCO-Code/Petter Bjornstad/ATTEMPT/attempt_workspace.RData")
```
...
# no MT
summary_dt1_nomt <- summary_dt1 %>%
  filter(!startsWith(primerid, "MT"))

#Format results for barchart
fcHurdle <- merge(summary_dt1_nomt[contrast=='treatmentDapagliflozin:visitPOST' & component=='H',.(primerid, `Pr(>Chisq)`)], #hurdle P values
                  summary_dt1_nomt[contrast=='treatmentDapagliflozin:visitPOST' & component=='logFC', .(primerid, coef, ci.hi, ci.lo)], by='primerid') #logFC coefficients
fcHurdle[,fdr:=p.adjust(`Pr(>Chisq)`, 'fdr')]
m <- fcHurdle[,c("primerid","coef","fdr")]
...
save.image(file = "/home/choiyej/Documents/CHCO-Code/Petter Bjornstad/ATTEMPT/attempt_workspace.RData")
# load("/home/choiyej/Documents/CHCO-Code/Petter Bjornstad/ATTEMPT/attempt_workspace.RData")
```
...
kegg_sets_indices <- limma::ids2indices(kegg_list, mcols(sca_attempt_pt)$primerid)
...
go_sets_indices <- limma::ids2indices(go_list, mcols(sca_attempt_pt)$primerid)
...
```{r echo = F}
# marginal means
MM_int_subj <- model.matrix(~treatment*visit,unique(colData(sca_attempt_pt)[,c("visit", "treatment"),drop=FALSE]))
predicted_int_subj <- predict(zlm_res1_subj, modelmatrix=MM_int_subj) %>%
  ...
  mutate(mm = case_when(sample == "S-2107-023529-PB_AAACCAAAGCATGTCA-1_1" ~ "PRE, Dapagliflozin",
                        ...
                        sample == "S-2107-023682-PB_AAACGAATCCTGTACT-1_4" ~ "PRE, Placebo",
                        sample == "S-2203-000892-PB_AAACCATTCAATGGCC-1_8" ~ "POST, Placebo"))
...
sum(abs(dapa_wide_diff$dapa_diff) > .1 & dapa_wide_diff$dapa_q_value < 0.1, na.rm = T)
sum(abs(placebo_wide_diff$placebo_diff) > .15 & placebo_wide_diff$placebo_q_value < 0.1, na.rm = T)
...
# write.csv(dapa_wide_diff, "/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/ATTEMPT_dapa_mm.csv", row.names = F)
# write.csv(placebo_wide_diff, "/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/ATTEMPT_placebo_mm.csv", row.names = F)
# write.csv(dapa_placebo_wide_diff, "/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/ATTEMPT_dapa_placebo_mm.csv", row.names = F)
```
...
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/neg1_geneset.jpeg", neg1, scale = 1, width = 12, height = 5)
neg2 <- plot_pathway_genes(2, mm_delta_long, pathways_genes_neg, pathways_neg, "#2c7da0")
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/neg2_geneset.jpeg", neg2, scale = 1, width = 12, height = 5)
neg3 <- plot_pathway_genes(3, mm_delta_long, pathways_genes_neg, pathways_neg, "#2c7da0")
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/neg3_geneset.jpeg", neg3, scale = 1, width = 12, height = 5)
neg4 <- plot_pathway_genes(4, mm_delta_long, pathways_genes_neg, pathways_neg, "#2c7da0")
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/neg4_geneset.jpeg", neg4, scale = 1, width = 12, height = 5)
neg5 <- plot_pathway_genes(5, mm_delta_long, pathways_genes_neg, pathways_neg, "#2c7da0")
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/neg5_geneset.jpeg", neg5, scale = 1, width = 12, height = 5)

# top 5 activated pathways
pos1 <- plot_pathway_genes(1, mm_delta_long, pathways_genes_pos, pathways_pos, "#c75146")
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/pos1_geneset.jpeg", pos1, scale = 1, width = 12, height = 5)
pos2 <- plot_pathway_genes(2, mm_delta_long, pathways_genes_pos, pathways_pos, "#c75146")
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/pos2_geneset.jpeg", pos2, scale = 1, width = 12, height = 5)
pos3 <- plot_pathway_genes(3, mm_delta_long, pathways_genes_pos, pathways_pos, "#c75146")
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/pos3_geneset.jpeg", pos3, scale = 1, width = 12, height = 5)
pos4 <- plot_pathway_genes(4, mm_delta_long, pathways_genes_pos, pathways_pos, "#c75146")
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/pos4_geneset.jpeg", pos4, scale = 1, width = 12, height = 5)
pos5 <- plot_pathway_genes(5, mm_delta_long, pathways_genes_pos, pathways_pos, "#c75146")
ggsave("/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Results/pos5_geneset.jpeg", pos5, scale = 1, width = 12, height = 5)
...
```{r echo = F}
# denoising method (or DCA), computationally extensive
# so_attempt_pt_nomt <- so_attempt_pt[grep("^MT", rownames(so_attempt_pt), invert = TRUE),] # 97 removed
# dim(so_attempt_pt_nomt@assays$RNA$counts)
# 
# count_matrix = so_attempt_pt_nomt@assays$RNA$counts
# 
# saver1 <- saver(count_matrix[1:10000,])
# saver2 <- saver(count_matrix[10001:20000,])
```
...
(sce_attempt_pt_muscat <- prepSCE(sce_attempt_pt, 
    kid = "treatment", # subpopulation assignments (clusters)
    gid = "visit",  # group IDs
    sid = "id",     # sample IDs (pre1234,post1234,...)
    drop = TRUE))   # drop all other colData columns
nk <- length(kids <- levels(sce_attempt_pt$cluster_id))
ns <- length(sids <- levels(sce_attempt_pt_muscat$sample_id))
names(kids) <- kids; names(sids) <- sids

t(table(sce_attempt_pt_muscat$cluster_id, sce_attempt_pt_muscat$sample_id))

# UMAP
sce_attempt_pt_muscat <- runUMAP(sce_attempt_pt_muscat, pca = 20)

plotReducedDim(sce_attempt_pt_muscat, dimred = "UMAP", colour_by = "cluster_id") +
    guides(fill = guide_legend(override.aes = list(alpha = 1, size = 3))) +
    theme_minimal() + theme(aspect.ratio = 1)
plotReducedDim(sce_attempt_pt_muscat, dimred = "UMAP", colour_by = "group_id") +
    guides(fill = guide_legend(override.aes = list(alpha = 1, size = 3))) +
    theme_minimal() + theme(aspect.ratio = 1)
```
...
```{r echo = F}
# pseudobulk
pb <- aggregateData(sce_attempt_pt_muscat,
...
ei <- metadata(sce_attempt_pt_muscat)$experiment_info
...
p_de <- format(n_de / nrow(sce_attempt_pt_muscat) * 100, digits = 3)
data.frame("#DS" = n_de, "%DS" = p_de, check.names = FALSE)
```

```{r echo = F}
mm <- mmDS(sce_attempt_pt_muscat, method = "dream",
