...
# re = nebula(count=sample_data$count,id=sample_data$sid,pred=pred, ncore = 1)
```
...
nebula_pt_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    ...
    result <- nebula(count = data_g_gene$count, id = data_g_gene$id, pred = data_g_gene$pred, ncore = 1)
