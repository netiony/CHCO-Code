---
title: "ATTEMPT scRNA other cell type analysis"
author: "Ye Ji Choi"
...
so_attempt_tal <- readRDS(file = "/run/user/778527649/gvfs/smb-share:server=ucdenver.pvt,share=som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/ATTEMPT/Data Clean/attempt_so_tal.RDS")
sce_attempt_tal <- as.SingleCellExperiment(so_attempt_tal)
sce_attempt_tal$id <- paste0(sce_attempt_tal$visit, sce_attempt_tal$subject_id)
# remove undetected genes
sce_attempt_tal <- sce_attempt_tal[rowSums(counts(sce_attempt_tal) > 0) > 0, ]
dim(sce_attempt_tal)
qc <- perCellQCMetrics(sce_attempt_tal)
# remove cells with few detected genes
ol <- isOutlier(metric = qc$detected, nmads = 2, log = TRUE)
sce_attempt_tal <- sce_attempt_tal[, !ol]
dim(sce_attempt_tal)
# remove lowly expressed genes
sce_attempt_tal <- sce_attempt_tal[rowSums(counts(sce_attempt_tal) > 1) >= 10, ]
dim(sce_attempt_tal)
# compute sum-factors & normalize
sce_attempt_tal <- computeLibraryFactors(sce_attempt_tal)
sce_attempt_tal <- logNormCounts(sce_attempt_tal)

sca_attempt_tal <- SceToSingleCellAssay(sce_attempt_tal)
...
gene_names <- rownames(so_attempt_tal)
...
zlm_res1_subj <- zlm(~ treatment*visit + (1|subject_id), 
                     sca_attempt_tal, method = "glmer", ebayes = F,
...
# 
# kegg_sets_indices <- limma::ids2indices(kegg_list, mcols(sca_attempt_tal)$primerid)
...
# 
# go_sets_indices <- limma::ids2indices(go_list, mcols(sca_attempt_tal)$primerid)
