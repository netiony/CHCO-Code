...
knitr::opts_knit$set(root.dir = "/Users/timvigers/Metabolomics")
library(arsenal)
library(skimr)
library(knitr)
library(mixOmics)
...
```{r pca,eval=FALSE}
# 3 PCs
pca.res <- pca(X, ncomp =2, center = TRUE, scale = FALSE)
# Plot
...
```{r plsda results}
plotIndiv(plsda_true,comp = c(1,2),ellipse = T,legend = T,ind.names = F)
```
...
```{r}
vars = c("GIR","GIR_per_kg","M_I_kg_GC","M_I_leankg_GC","body_fat","screen_bmi_percentile","rpf","acr_mean","kidney_oxygenation","gfr","kidney_GFR","medullary_GFR","cortex_GFR","FOSC_Cortex","FOSC_Medullary","FOSC_Kidney","KIM1","NGAL","MCP1","logYKL40","logIL18" )
varnames = c("GIR","GIR per kg","M_I_kg_GC","M_I_leankg_GC","Body fat","BMI %ile","RPF","Mean ACR","Kidney Oxy.","GFR","Kidney GFR","Medullary GFR","Cortex GFR","Cortex FOSC","Medullary FOSC","Kidney FOSC","KIM1","NGAL","MCP1","logYKL40","logIL18")
...
```{r}
c = corr.test(X[,intersection],df[,vars],method = "spearman",adjust = "none")
