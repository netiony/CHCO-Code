...
b = boot(df_boot,plsda_fun,1,parallel = "multicore",ncpus = round(detectCores()*0.75))
# CIs
...
easy_lasso = function(outcome){
  # CV elasticnet
  elnet = train(x = X,y = Y[,outcome],
                method = "glmnet",
                trControl = cv
  )
  # Get best model
  best = which(rownames(elnet$results) == rownames(elnet$bestTune))
  best_result = elnet$results[best, ]
  # Fit
  mod = glmnet(x = X,y =  Y[,outcome],alpha = best_result$alpha,lambda = best_result$lambda)
  # Get selected
  coefs = coef(mod)
