...
knitr::opts_knit$set(root.dir = "/Users/timvigers/Dropbox/Work/Petter Bjornstad/Metabolomics")
library(arsenal)
library(skimr)
library(knitr)
library(mixOmics)
...
b = boot(df_boot,plsda_fun,10,parallel = "multicore",ncpus = 8)
# CIs
...
```{r}
vars = c("rpf","acr_mean","gfr","kidney_GFR","medullary_GFR","cortex_GFR","FOSC_Cortex","FOSC_Medullary","FOSC_Kidney")
...
```{r}
c = corr.test(X[df$group == "Control",intersection],df[df$group == "Control",vars],
...
```{r}
c = corr.test(X[df$group == "T1D",intersection],df[df$group == "T1D",vars],
              method = "spearman",adjust = "fdr")
rownames(c$r) <- raw_data$Name[match(rownames(c$r),compounds)]
colnames(c$r) <- varnames
