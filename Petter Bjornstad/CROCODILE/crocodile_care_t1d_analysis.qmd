...
gene_list <- c("S100A4", "GZMA", "CCL5", "GZMB", "CD8A", "CD8B")
...
t.table <- ggtexttable(t.table,
      cols = c("   T1D     ", "     HC   "),
      theme = ttheme("blank", base_size = 16)
    ) %>%
      tab_add_hline(at.row = 1:2, row.side = "top", linewidth = 1) %>%
      tab_add_title(paste("% Expressed in T-cells", sep = ""), size = 17) 
...
plot_table <- ggarrange(t.barplot, NULL, t.table,
                                 nrow = 1, widths = c(2.5,0,1), common.legend = F)
plot_table
...
t.barplot <- ggplot(m_tcell_up, aes(x = gene, y = avg_log2FC, fill = direction)) +
  geom_col(show.legend = T) +
  scale_fill_manual(values = direction_colors,
                    labels = c("Upregulated    ")) +
  theme_bw() +
