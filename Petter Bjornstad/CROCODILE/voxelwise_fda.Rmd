...
# API import
tokens <- read.csv("/home/tim/UCD/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/Data Harmonization/api_tokens.csv")
...
df <- read.csv("/home/tim/UCD/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/CROCODILE/Data_Cleaned/voxelwise_rows.csv")
...
fda_df %>% 
  modelr::add_predictions(fpca) %>% 
  ggplot(aes(y = pred)) + 
  geom_spaghetti(alpha = .4) + 
  geom_spaghetti(aes(y = lcortex_k1), color = "blue", alpha = .3) + 
  facet_grid(~group)
```
