...
de.markers <- function(seurat_object, genes, group.by, id1, id2, celltype, extension){
  m = FindMarkers(seurat_object, features = genes,group.by = group.by,ident.1 = id1, 
                  ident.2 = id2,subset.ident = celltype,verbose = T,logfc.threshold=0.1,
                  min.pct = .1)
...
# TCA cycle genes
de.markers(so, genes, "Group", "HC", "T1D", "PT", "")

# Acetyl CoA genes
de.markers(so, acetylcoa_genes, "Group", "HC", "T1D", "PT", "_acetylcoa")

# CKD genes
de.markers(so, ckd_genes, "Group", "HC", "T1D", "PT", "_ckd")

# IR genes
de.markers(so, ir_genes, "Group", "HC", "T1D", "PT", "_ir")

# SGLT2i genes
de.markers(so, sglt2_genes, "Group", "HC", "T1D", "PT", "_sglt2")

# Oxygen phophorylation genes
de.markers(so, oxy_phos_genes, "Group", "HC", "T1D", "PT", "_oxy_phos")
...
```{r}
## PT-1
de.markers(subset(so, celltype == "PT-1"), genes, "Group", "HC", "T1D", "PT", ".1")

## PT-2
de.markers(subset(so, celltype == "PT-2"), genes, "Group", "HC", "T1D", "PT", ".2")

## PT-3
de.markers(subset(so, celltype == "PT-3"), genes, "Group", "HC", "T1D", "PT", ".3")

## PT-4
de.markers(subset(so, celltype == "PT-4"), genes, "Group", "HC", "T1D", "PT", ".4")

## PT-5
de.markers(subset(so, celltype == "PT-5"), genes, "Group", "HC", "T1D", "PT", ".5")
```
# UMAP
...
dp.formatted <- function(seurat_object, genes, celltype, group.by, m,
  ...
  pt.table <- m %>%
    ...
    mutate(p_val_adj = p_format(p_val_adj)) %>% 
    select(pct.1, pct.2, avg_log2FC, p_val_adj) 
  gg.pt.table <- ggtexttable(format(pt.table, digits = 3, scientific = F),
              cols = c("HC", "T1D", "Log2FC", "q-value"),
              theme = ttheme("blank")) %>%
...
m1.label <- m.1 %>% mutate(m1.label = case_when(p_val_adj <= 0.05 & pct.1 > pct.2 ~ "<",
                                                p_val_adj <= 0.05 & pct.1 < pct.2 ~ ">"))%>%
  select(m1.label) %>%
  mutate(genes_subset = rownames(m.1))
genes_subset.2 <- ""
match_2 <- (genes_subset %in% genes_subset.2) * 7.5
m2.label <- m.2 %>% mutate(m2.label = case_when(p_val_adj <= 0.05 & pct.1 > pct.2 ~ "<",
                                                p_val_adj <= 0.05 & pct.1 < pct.2 ~ ">")) %>%
  select(m2.label) %>%
  mutate(genes_subset = rownames(m.2))
match_3 <- (genes_subset %in% genes_subset.3) * 5.5
m3.label <- m.3 %>% mutate(m3.label = case_when(p_val_adj <= 0.05 & pct.1 > pct.2 ~ "<",
                                                p_val_adj <= 0.05 & pct.1 < pct.2 ~ ">")) %>%
  select(m3.label) %>%
  mutate(genes_subset = rownames(m.3))
match_4 <- (genes_subset %in% genes_subset.4) * 3.5
m4.label <- m.4 %>% mutate(m4.label = case_when(p_val_adj <= 0.05 & pct.1 > pct.2 ~ "<",
                                                p_val_adj <= 0.05 & pct.1 < pct.2 ~ ">")) %>%
  select(m4.label) %>%
  mutate(genes_subset = rownames(m.4))
match_5 <- (genes_subset %in% genes_subset.5) * 1.5
m5.label <- m.5 %>% mutate(m5.label = case_when(p_val_adj <= 0.05 & pct.1 > pct.2 ~ "<",
                                                p_val_adj <= 0.05 & pct.1 < pct.2 ~ ">")) %>%
  select(m5.label) %>%
  mutate(genes_subset = rownames(m.5))

genes_subset_df <- as.data.frame(genes_subset) %>%
  mutate(x = seq_along(genes_subset)) %>% 
...
plot_gsva_heatmap(gsva_result, pathway_ids = pathways_tca$Pathway,
                  scale = "row",
                  dendrogram = "col",
                  truncate_names = T)
...
gsva_full_annotated <- gsva_full_annotated[-1,] %>%
  as.data.frame() %>%
  mutate(delta = as.numeric(T1D) - as.numeric(HC),
         T1D = as.numeric(T1D),
         HC = as.numeric(HC)) %>%
  arrange(delta)
