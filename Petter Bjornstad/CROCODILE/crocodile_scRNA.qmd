...
# Save image
ggsave(filename = "/home/tim/UCD/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/CROCODILE/Dissemination/Figures/UMAP_1200x900.jpeg",plot = gumap,scale = 3,
...
```{r}
FeaturePlot(so,
  features = genes, split.by = "Group", order = T,
...
```{r warning=FALSE}
plot_ly(x = so[["tsne"]]@cell.embeddings[,1],
        y = so[["tsne"]]@cell.embeddings[,2],
        z =so[["tsne"]]@cell.embeddings[,3],
        color = so$LR_clusters,
...
```{r}
VlnPlot(so, features = genes, group.by = "Group", idents = "PT")
```
...
```{r}
#| message: false
RidgePlot(so, features = genes, group.by = "Group", idents = "PT")
```
...
```{r}
DotPlot(so,
  features = genes, idents = "PT", split.by = "Group",
  cols = "RdGy", scale = F
)
```
