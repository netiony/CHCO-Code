...
pah <- read.csv("/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive/Data Harmonization/Data Clean/harmonized_dataset.csv")
pah <- pah %>% 
...
# data <- merged_data
pdf(fs::path(dir.results,"gene_expression_plots_abs_adj_02_28.pdf"), width = 20, height = 10)  # You can adjust the width and height
...
for (celltype in unique(data$KPMP_celltype)) {
  ...
  for (gene in oat_genes) {
    ...
    # results <- data.frame(Gene = gene, Beta = foldchange,SE=se, LowerCI=lower_CI,UpperCI=upper_CI,PValue = pval,CellType=celltype,Membrane=membrane,N=N)
