...
# 
# # First, make sure the correct cluster variable is set in Idents()
# Idents(so_kpmp_sc) <- "KPMP_celltype"  # or your custom cluster variable
# 
# # Fetch the expression data for the genes of interest
# expression_data <- FetchData(so_kpmp_sc, vars = oat_genes)
# 
# # Get the cluster labels
# clusters <- Idents(so_kpmp_sc)
# 
# # Calculate the percentage of cells expressing each gene by cluster
# percent_expression_by_cluster <- sapply(oat_genes, function(gene) {
#   # Get expression data for the gene
#   gene_expr <- expression_data[[gene]]
#   
#   # For each cluster, calculate the percentage of cells expressing the gene
#   tapply(gene_expr > 0, clusters, function(x) mean(x) * 100)  # Percentage of cells expressing the gene
# })
# 
# # Convert to a data frame for easier viewing
# percent_expression_by_cluster_df <- as.data.frame(percent_expression_by_cluster)
# colnames(percent_expression_by_cluster_df) <- oat_genes
# 
# # View the result
# percent_expression_by_cluster_df
...
data_subgroup <- data %>% 
  ...
  for (gene in genes) {
    ...
    results <- data.frame(Gene = gene, FoldChange = foldchange, PValue = pval,CellType=celltype)
