...
length(unique(harm_meta_data))
# pah <- pah %>% 
#   filter(!is.na(pah_clear_abs)) #65 with pah and gene expression
pah <- pah %>% 
  dplyr::select(all_of(c("mrn","pah_clear_abs","pah_clear_bsa")))
# meta_kidney_sc <- so_kidney_sc@meta.data
# meta_kidney_sc <- meta_kidney_sc %>%
#   dplyr::select(all_of(c("mrn"))) 
...
pah <- pah %>% 
  ...
  filter(mrn %in% meta_kidney_sc$mrn)
meta_kidney_sc2 <- tidylog::left_join(meta_kidney_sc,pah,by="mrn")
rownames(meta_kidney_sc) <- rownames(so_kidney_sc@meta.data)
ids <- pah$mrn
