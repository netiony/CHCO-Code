...
harm_data <- harm_data %>% 
  ...
  dplyr::select(record_id,visit,pah_clear_bsa,diabetes)
...
de.markers(so_kidney_sc, oat.genes, "diabetes", id2 = "Yes", id1 = "No", "PT", "")
m_top <- m 
significant_genes <- m_top %>% filter(p_val_adj < 0.05)
...
p <- EnhancedVolcano(m_top,
                     ...
                     x = 'avg_log2FC',
                     y = 'p_val_adj',
                     title = paste0("Differentially Expressed OAT Genes by Diabetes (Pseudobulk PT)"),
                     subtitle = paste0("Positive Log2 FC = Greater Expression in Diabetes Yes vs. No\n",
                                       "(Significant at FDR-P<0.05, FC Threshold = 0.5)"),
                     pCutoff = 0.05,
                     FCcutoff = 0.1,
                     labFace = 'bold',
...
de.markers(so_kidney_sc, oat.genes, "pah_norm", id2 = "Normal", id1 = "Low", "PT", "")
m_top <- m 
significant_genes <- m_top %>% filter(p_val_adj < 0.05)
