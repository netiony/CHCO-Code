...
so_PT <- subset(so_kidney_sc,celltype=="PT")

# Extract the gene expression data for all genes
gene_expression <- as.data.frame(so_PT@assays$RNA$data)

# Assign the gene names (row names of Seurat object) as the column names
rownames(gene_expression) <- rownames(so_PT)
colnames(gene_expression) <- colnames(so_PT)
...
ref <- "Lean Control"
results_output <- lmm_fxn(genes = gene_list,exposure="pah_clear_abs")
