...
binary_expr <- as.matrix(so_PT@assays$RNA@layers$counts) > 0
...
binary_expr <- matrix(binary_expr, nrow = nrow(so_PT@assays$RNA@layers$counts), ncol = ncol(so_PT@assays$RNA@layers$counts))

# Add group information to the metadata
so_PT$category <- paste(so_PT$KPMP_celltype, so_PT$kit_id, sep = "_")
