...
so_kpmp_sc <- subset(so_kpmp_sc,kit_id!="KL-0014624")
# so_kpmp_sc <- subset(so_kpmp_sc,group!="Obese_Control")
```
...
#Step 1: Count number of cells per individual
group_data <- so_kpmp_sc@meta.data %>%
  filter(KPMP_celltype=="aPT" | KPMP_celltype=="PT-S1/S2" | KPMP_celltype=="PT-S3") %>% 
  group_by(kit_id,group) %>% 
  tally() %>% 
  ungroup() %>% 
  dplyr::rename(Total_Cells=n)

# Step 2: Count number of cells for each KPMP_celltype within each individual (kit_id)
celltype_data <- so_kpmp_sc@meta.data %>%
  filter(KPMP_celltype %in% c("aPT", "PT-S1/S2", "PT-S3")) %>%
  group_by(kit_id, KPMP_celltype,group) %>%
  tally() %>%
  ungroup() %>%
  dplyr::rename(Cell_Count = n)

# Step 3: Merge the total cell counts with the celltype counts to calculate proportions
merged_data <- celltype_data %>%
  left_join(group_data, by = c("kit_id","group"))%>%
  mutate(Proportion = Cell_Count / Total_Cells)  # Calculate proportion of each cell type

merged_data2 <- merged_data %>% 
group_by(group, KPMP_celltype) %>% 
summarise(mean_pct = mean(Proportion)) 

# group_ID <- so_kpmp_sc@meta.data %>% 
#   dplyr::select(c("kit_id","group"))
# merged_data <- tidylog::left_join(merged_data,group_ID,by=c("kit_id"))

# Plot the proportions using ggplot
ggplot(merged_data2, aes(x = group, y = mean_pct, fill = group)) +
  geom_bar(stat = "identity") +
  labs(title = "Proportion of Cells in Different Groups",
       x = "Group", y = "Proportion") +
  theme_minimal() +
  facet_wrap(~KPMP_celltype)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

group_data_pt <- group_data %>% 
  filter(KPMP_celltype=="aPT")
# Plot the proportions using ggplot
ggplot(group_data_pt, aes(x = group, y = proportion, fill = group)) +
  geom_bar(stat = "identity") +
  labs(title = "Proportion of aPT Cells in by Disease Status",
       x = "Group", y = "Proportion") +
  theme_minimal() +
  # facet_wrap(~KPMP_celltype)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
data <- merged_data %>% 
  dplyr::select(all_of(c("kit_id","KPMP_celltype","acr_u","age","sex","height","weight","medication","gfr_bsa_plasma","eGFR_CKD_epi","group","pah_clear_abs","pah_clear_bsa",oat_genes))) 
...
prop_data <- data %>% 
  filter(KPMP_celltype=="aPT")
prop_data2 <- merged_data %>% 
  filter(KPMP_celltype=="aPT") %>% 
  dplyr::select(c("kit_id","Proportion"))
prop_data <- tidylog::left_join(prop_data,prop_data2,by=c("kit_id"))
prop_data$acr_u_cat <- ifelse(prop_data$acr_u<30,"Low","High")
summary(lm(Proportion~group,data=prop_data))
summary(lm(Proportion~group*acr_u_cat+age+sex+medication,data=prop_data))
summary(lm(Proportion~group+acr_u_cat+age+sex+medication,data=prop_data))

#PT total
data2 <- merged_data2 %>% 
  dplyr::select(all_of(c("kit_id","age","sex","height","weight","medication","eGFR_CKD_epi","gfr_bsa_plasma","group","pah_clear_abs","pah_clear_bsa",oat_genes))) 
# dplyr::select(-c("SLC22A31"))
...
data$eGFR_CKD_epi <- as.numeric(data$eGFR_CKD_epi)
data$gfr_bsa_plasma
# #Groups
# #1. Type 2 Diabetes
...
for (celltype in unique(data$KPMP_celltype)) {
  for (exposure in pah_exposures) {
    for (gene in oat_genes) {
      ...
      m0 <- as.formula(paste0(exposure," ~ ",gene,"+group+height+weight+sex+gfr_bsa_plasma"))
...
ggplot(data, aes(x=SLC22A25,y=pah_clear_abs_scaled))+
  geom_point()+
  geom_smooth(method="lm")
