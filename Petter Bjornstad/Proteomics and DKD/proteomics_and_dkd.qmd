...
df <- df %>%
  ...
  select(
    record_id, co_enroll_id, visit, group, age, sex, race_ethnicity,
...
```{r message = FALSE}
ids = unique(sub("_BL|_12M","",c(improve_olink_plasma$record_id,improve_olink_urine$record_id,
               soma$record_id)))
df %>% filter(record_id %in% ids) %>%
  select(group,age,sex,race_ethnicity,diabetes_duration,hba1c,bmi) %>%
...
neuro_p_comp <- pathway_comparison(list(
  Baseline = de_genes_neuro,
...
glyc_p_comp <- pathway_comparison(list(
  Baseline = de_genes_glyc,
...
glyc_a1c_p_comp <- pathway_comparison(list(
  Baseline = de_genes_glyc_a1c,
...
```{r}
# Average scRNA for correlations
