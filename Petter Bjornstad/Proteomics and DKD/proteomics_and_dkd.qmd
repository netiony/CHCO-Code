...
de = function(seurat_object = so,outcome,ref_group,vars){
  ...
  cells = FetchData(so,vars = entrez)
  cells$type = so$generaltype
  ...
  # DE testing
  m = FindMarkers(so,features = "NELL1",ident.1 = ref_group,
                  group.by = outcome,subset.ident = max[2])
  p = VlnPlot(so,features = "NELL1",group.by = outcome,
              idents = max[2],pt.size=0.01,log = F)
}
```
...
1. To determine which cell types are expressing the genes of interest the most, do we want to look at average expression or proportion of cells with detectable expression? These may usually be the same thing, but not necessarily if one groups of cells all express at a low level and another expresses in a few cells at a very high level.
    - Do we want to look at cells expressing over a certain threshold?