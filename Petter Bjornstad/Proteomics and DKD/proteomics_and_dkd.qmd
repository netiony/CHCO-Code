...
de <- function(seurat_object = so, outcome, ref_group, vars) {
  ...
  de_tables <- lapply(names(max), function(n) {
    m <- 
        tryCatch(FindMarkers(seurat_object,features = n, ident.1 = ref_group,
...
df %>%
  ...
  select(
    group, age, sex, race_ethnicity_condensed, diabetes_duration, hba1c, bmi, acr_u, albuminuria_cat, soma, olink_plasma, olink_urine, scRNA
  ) %>%
