...
# remove people with <4 clamp measures (based on insulin and glucose?)
...
data <- data %>% filter(nmissgluc==0)  
data <- data %>% filter(nmissins==0)  
...
calc_predicted <- function(data, outcome, stage) {
  ...
  df = data
  stage = stage
  predmean = df %>% filter(time==stage) %>% summarise(log(mean(Insulin)))
  ...
  y_hat <- data.frame(cbind(unique(df$StudyID),y_hat))
  colnames(y_hat) <- c("StudyID","y_hat")
  return(y_hat)
}

test <- calc_predicted(data = long, outcome = "Glucose", stage=10)
```
