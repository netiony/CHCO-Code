...
# Get time since last visit on EC
df <- df %>%
  group_by(record_number) %>%
  mutate(
    last_ec = ifelse(ec == "Yes", cv_monthssincepcosdx, NA)
  ) %>%
  fill(last_ec, .direction = "down") %>%
  mutate(
    months_since_last_ec =
      cv_monthssincepcosdx - last_ec
  )
# For those who go off EC, if the visit is within 6 months of last EC,
# include that value in the post-LARC analysis.
df$ec[df$ec == "No" & df$months_since_last_ec < 6] <- "Yes"
# Find visits where they are off EC after first starting
df <- df %>%
  group_by(record_number) %>%
  mutate(
    pre_post_ec =
      first(cv_monthssincepcosdx[ec == "No" & !is.na(last_ec)], na_rm = T)
  )
df$pre_post_ec[is.na(df$pre_post_ec)] <- Inf
# Get time since last visit on Metformin
df <- df %>%
  group_by(record_number) %>%
  mutate(
    last_metformin = ifelse(metformin == "Yes", cv_monthssincepcosdx, NA)
  ) %>%
  fill(last_metformin, .direction = "down") %>%
  mutate(
    months_since_last_metformin =
      cv_monthssincepcosdx - last_metformin
  )
# For those who go off Metformin, if the visit is within 6 months of last
# Metformin, include that value in the post-Metformin analysis.
df$metformin[df$metformin == "No" & df$months_since_last_metformin < 6] <- "Yes"
# Find visits where they are off Metformin after first starting
df <- df %>%
  group_by(record_number) %>%
  mutate(
    pre_post_metformin =
      first(cv_monthssincepcosdx[metformin == "No" & !is.na(last_metformin)],
        na_rm = T
      )
  )
df$pre_post_metformin[is.na(df$pre_post_metformin)] <- Inf
# Get time since last visit on lifestyle
df <- df %>%
  group_by(record_number) %>%
  mutate(
    last_lifestyle = ifelse(lifestyle == "Yes", cv_monthssincepcosdx, NA)
  ) %>%
  fill(last_lifestyle, .direction = "down") %>%
  mutate(
    months_since_last_lifestyle =
      cv_monthssincepcosdx - last_lifestyle
  )
# Find visits where they are off lifestyle after first starting
df <- df %>%
  group_by(record_number) %>%
  mutate(
    pre_post_lifestyle =
      first(cv_monthssincepcosdx[lifestyle == "No" & !is.na(last_lifestyle)],
        na_rm = T
      )
  )
df$pre_post_lifestyle[is.na(df$pre_post_lifestyle)] <- Inf
...
# Write data for Melanie et al. to check
write.csv(
  df %>% select(
    record_number, redcap_repeat_instance, cv_visittype,
    cv_newmeds___1, cv_medications___5, cv_medications___6, cv_medications___7,
    ec, metformin, lifestyle,
    cv_monthssincepcosdx, months_since_last_ec, months_since_last_metformin,
    months_since_last_lifestyle,
    cv_weight, cv_bmi, cv_a1c, cv_tg, cv_hdl, cv_alt, cv_sbp
  ),
  file = "./Data_Clean/aim_2_cleaned_data.csv",
  row.names = F, na = ""
)
```
...
ec_only <- df %>%
  filter(ec_ever == "EC Ever") %>%
  filter(cv_monthssincepcosdx < pre_post_ec | ec_ever == "No") %>%
  mutate(
    time_from_first_ec = (cv_monthssincepcosdx - time_first_ec),
...
write.csv(
  ec_only %>% select(
    record_number, redcap_repeat_instance, cv_visittype,
    cv_medications___5, cv_medications___6, cv_medications___7, ec,
    cv_monthssincepcosdx,months_since_last_ec, time_from_first_ec,
    cv_weight, cv_bmi, cv_a1c, cv_tg, cv_hdl, cv_alt, cv_sbp
  ),
  file = "./Data_Clean/aim_2_ec_only_cleaned_data.csv",
  row.names = F, na = ""
)
```
