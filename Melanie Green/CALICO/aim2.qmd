...
label(df$time_from_first_larc) <- "Months from First Visit on LARC"
...
df <- df %>%
  select(
    ...
    ec, metformin, lifestyle, larc, time_from_first_ec,
    time_from_first_metformin, time_from_first_lifestyle, time_from_first_larc,
    cv_weight, cv_bmi, cv_a1c, cv_tg, cv_hdl,
    cv_alt, cv_sbp
  ) %>%
  arrange(record_number, cv_monthssincepcosdx)
...
ec_remove <- which(df$time_from_first_ec > 0 & df$ec == "No")
ec_df <- df[-ec_remove, ]
