...
# Fit mixed model
mod <- lmer(
  cv_weight ~ cv_monthssincepcosdx * ec + baseline_weight + cv_age +
    combined_race + insur_type + metformin + (1 | record_number),
  data = df
)
# Model plot
mod_df <- mod@frame
mod_df$baseline_weight <- mean(mod_df$baseline_weight, na.rm = T)
...
The model above was fit using `r nrow(mod_df)` observations. In the table, the row labeled "Time From First EC" is the slope prior to starting medication, and the "Slope Change After EC" row shows the average change in slope after starting the medication.
...
# Fit mixed model
mod <- lmer(
  cv_bmi ~ cv_monthssincepcosdx * ec + baseline_bmi + cv_age +
    combined_race + insur_type + metformin + (1 | record_number),
  data = df
)
# Model plot
mod_df <- mod@frame
mod_df$baseline_bmi <- mean(mod_df$baseline_bmi, na.rm = T)
mod_df$cv_age <- mean(mod_df$cv_age, na.rm = T)
mod_df$combined_race <- "Caucasian"
mod_df$insur_type <- "Private"
mod_df$metformin <- "No"
...
metformin_only <- df %>%
  ...
  mutate(
    time_from_first_metformin = cv_age - age_first_metformin,
    x = (time_from_first_metformin > 0) * time_from_first_metformin
  )
label(metformin_only$time_from_first_metformin) <- "Time From First Metformin"
...
# Fit mixed model
mod <- lmer(
  cv_weight ~ cv_monthssincepcosdx * metformin + baseline_weight + cv_age +
...
change_mod <- lmer(cv_weight ~ time_from_first_metformin + x + baseline_weight +
  ...
  (1 + time_from_first_metformin | record_number), data = metformin_only)
# Model plot
mod_df <- change_mod@frame
mod_df$baseline_weight <- mean(mod_df$baseline_weight, na.rm = T)
mod_df$cv_age <- mean(mod_df$cv_age, na.rm = T)
...
p <- ggplot(mod_df, aes(x = time_from_first_metformin)) +
  ...
  geom_line(aes(y = pop_pred)) +
  xlab("Years From Metformin Start") +
  theme_classic()
ggplotly(p)
# Model summary
change_mod %>% gtsummary::tbl_regression()
...
The model above was fit using `r nrow(mod_df)` observations. In the table, the row labeled "Time From First Metformin" is the slope prior to starting medication, and the "Slope Change After Metformin" row shows the average change in slope after starting the medication.
...
# Fit mixed model
mod <- lmer(
  cv_bmi ~ cv_monthssincepcosdx * metformin + baseline_bmi + cv_age +
    combined_race + insur_type + ec + (1 | record_number),
  data = df
)
# Model plot
...
lifestyle_only <- df %>%
  ...
  mutate(
    time_from_first_lifestyle = cv_age - age_first_lifestyle,
    x = (time_from_first_lifestyle > 0) * time_from_first_lifestyle
  )
label(lifestyle_only$time_from_first_lifestyle) <- "Time From First Lifestyle Medicine"
...
# Fit mixed model
mod <- lmer(
  cv_weight ~ cv_monthssincepcosdx * lifestyle + baseline_weight + cv_age +
...
change_mod <- lmer(cv_weight ~ time_from_first_lifestyle + x + baseline_weight +
  ...
  (1 + time_from_first_lifestyle | record_number), data = lifestyle_only)
# Model plot
mod_df <- change_mod@frame
mod_df$baseline_weight <- mean(mod_df$baseline_weight, na.rm = T)
mod_df$cv_age <- mean(mod_df$cv_age, na.rm = T)
...
p <- ggplot(mod_df, aes(x = time_from_first_lifestyle)) +
  ...
  geom_line(aes(y = pop_pred)) +
  xlab("Years From Lifestyle Medicine Start") +
  theme_classic()
ggplotly(p)
# Model summary
change_mod %>% gtsummary::tbl_regression()
...
The model above was fit using `r nrow(mod_df)` observations. In the table, the row labeled "Time From First Lifestyle Medicine" is the slope prior to starting medication, and the "Slope Change After Lifestyle Medicine" row shows the average change in slope after starting the medication.
...
# Fit mixed model
mod <- lmer(
  cv_bmi ~ cv_monthssincepcosdx * lifestyle + baseline_bmi + cv_age +
    combined_race + insur_type + (1 | record_number),
  data = df
)
# Model plot
