...
plot_df <- m %>%
  ...
  tidy_add_term_labels(
    interaction_sep = " x ",
    categorical_terms_pattern = "{var_label}\n{level} vs. {reference_level}"
  )
p <- forestplot(plot_df[-1, ],
  name = label, se = std.error, pvalue = p.value, logodds = T
) + xlab("HR") + ggtitle("Pre-diabetes")
p
```
...
p <- forestplot(plot_df[-1, ],
  name = label, se = std.error, pvalue = p.value, logodds = T
) + xlab("HR") + ggtitle("Type 2 Diabetes")
p
```
...
p <- forestplot(plot_df[-1, ],
  name = label, se = std.error, pvalue = p.value, logodds = T
) + xlab("HR") + ggtitle("Free Testosterone Percentage of ULN")
p
```
