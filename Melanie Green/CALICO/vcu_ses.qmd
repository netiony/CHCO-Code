...
There were only 2 people were newly prescribed Topiramate for weight loss and there were 0 people without insurance who were prescribed it, so only people with public or private insurance were included in this model.

```{r}
m <- glm(cv_newmeds___25 ~ insur_type,
  data = cv1[cv1$insur_type != "None", ], family = "binomial"
)
m %>% tbl_regression(exponentiate = T)
```
...
There were only 4 people with a new Semaglutide (Wygovy/Ozempic) for weight loss prescription at clinic visit 1, so we probably cannot include additional variables in this model. `cv_newmeds___29` was not associated with insurance type. There were no people without insurance who were prescribed Semaglutide, so only private and public insuarance was included in the model.
...
```{r}
naniar::vis_miss(cv1[, predictors])
```

Birth weight is the main issue in terms of missing data, so I removed it in the model below.

```{r}
m <- lm(
  update(f, log(cv_ft_perc) ~ .
    - cv_tt_perc - cv_ft_perc - pcosdx_birthweight_calc),
  data = cv1
)
m %>%
  tbl_regression() %>%
