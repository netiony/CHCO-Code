...
mod_df <- df %>%
  ...
  slice_min(redcap_repeat_instance) %>%
  select(
    record_number, pcosdx_age, cv_tt_perc,
    insur_type, combined_race, ethnicity,
    pcosdx_birthweight_calc, contains("pcosdx_famhx"), cv_bmi_percentile,
    cv_sbp, cv_dbp, cv_tc, cv_tg, cv_ast, cv_alt, pcosdx_irregular_menses,
    pcosdx_pmh___6, contains("cv_acne"), cv_hirsutism_cat, pcosdx_specialty
  )
mod_df <- mod_df[, -caret::nearZeroVar(mod_df)]
mod_df <- as.data.frame(mod_df)
xvars <- colnames(mod_df)[which(colnames(mod_df) == "insur_type"):ncol(mod_df)]
# Plot missing
vis_miss(mod_df)
gg_miss_var(mod_df, show_pct = T)
# Basic formula
...
Models were selected using 5 fold cross-validation to determine the optimal $\lambda$ value for LASSO regression. Variables with near zero variability were excluded from model selection (for example, parent history of reproductive cancer).
