...
There were very people with new ADHD medication at clinic visit 1 (n=8), so we need to think more about what variables to include in this model. `cv_newmeds___19` was not associated with insurance type, but there were 0 people without insurance who were prescribed new ADHD medication, so only people with public or private insurance were included in the model.
...
There weren't a huge number of people participating in group therapy at diagnosis (n=10), so we need to think more about what variables to include in this model. `pcosdx_pmh___7` was not associated with insurance type.

```{r}
m <- glm(pcosdx_mentalhealthcounseling___1 ~ insur_type,
  data = cv1,
  family = "binomial"
)
m %>% tbl_regression(exponentiate = T)
...
## `pcosdx_mentalhealthcounseling___2`

There were a decent number of people participating in individual therapy at diagnosis (n=158), but `pcosdx_mentalhealthcounseling___2` was not associated with insurance type.

```{r}
m <- glm(pcosdx_mentalhealthcounseling___2 ~ insur_type, data = cv1, family = "binomial")
m %>% tbl_regression(exponentiate = T)
```

## `pcosdx_mentalhealthcounseling___3`

There were 20 people participating in inpatient therapy at diagnosis, but `pcosdx_mentalhealthcounseling___3` was not associated with insurance type.

```{r}
m <- glm(pcosdx_mentalhealthcounseling___3 ~ insur_type, data = cv1, family = "binomial")
m %>% tbl_regression(exponentiate = T)
```
