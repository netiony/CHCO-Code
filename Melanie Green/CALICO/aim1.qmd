...
```{r results='asis'}
#| label: tbl-table-1-obese
#| tbl-cap: Stratified by obesity
f <- as.formula(paste0("Obesity ~ ", paste0(aim1_vars, collapse = "+")))
t1_bmi <- tableby(f,
  data = df[df$redcap_repeat_instance == 1, ], control = mycontrols
)
...
For those with a BMI percentile included in the dataset, participants with a BMI percentile $\geq$ 95 were considered have obesity. For those with a missing value for BMI percentile, those with a raw BMI value $\geq$ 30 were considered to have obesity. 
