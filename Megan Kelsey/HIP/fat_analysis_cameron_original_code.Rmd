...
The following models explore the effect of puberty and Obesity on a number of fat measures. Mixed effects models were used with a random intercept by patient yielding a model which effectively controls for baseline values of each measure and estimates the change experienced by a patient by Tanner 5. 

Example Interpretation: Liver Fat Percentage

Figure: Raw data (points) plotted with model estimated means with error bars representing 95% Confidence Intervals. 

Table 1: T-table

(Intercept) - The estimated mean liver fat percentage for a normal weight patient at Tanner 2/3 is 1.339 (NS)

study_visit_number_svlTanner 5 - The difference between Tanner 2/3 and Tanner 5 is 0.869 for a normal weight patient (NS)

study_phaseObese - The difference between a normal weight and obese patient at Tanner 2/3 is 6.292 (p = 0.003)

study_visit_number_svlTanner 5:study_phaseObese - The difference in delta liver fat between normal weight and obese patients is -0.599 (More negative or less positive) (NS). Simple interpretation: Difference in slope or trajectory. 

Table 2: Model Means

This one should be pretty self-explanatory. Group Means as estimated by the model.

Table 3: Contrasts by Group

Using the same model, contrast statements are used to test differences in means between discrete groups of patients. Here we see differences between Obese and Normal Weight patients at each Tanner Stage

Table 4: Contrasts by Tanner Stage

Similar to table 3, but looking at differences between Tanner 5 and Tanner 2/3 by Obesity Group. 
...
load("./data/dat_analysis_2.RData")
```

```{r}
dat_1 <- dat_analysis_1 %>% filter(!is.na(liver_fat_perc))
...
write.csv(fat_measures_baseline,"fat_measures_baseline.csv", row.names = F)
```
...
write.csv(fat_measures_tanner5,"fat_measures_tanner5.csv", row.names = F)
```
...
write.csv(fat_measures_delta,"fat_measures_delta.csv", row.names = F)
```
...
write.csv(fat_measures_tanner5_w_baseline,"fat_measures_tanner5_w_baseline.csv", row.names = F)
```
...
## Predict Tanner 5 Liver Fat with Tanner 5 Covariates
...
## Predict Tanner 2/3 Liver Fat with Tanner 2/3 Covariates
...
## Predict Tanner 5 Liver Fat with Tanner 2/3 Covariates
...
model1 <- t.test(liver_fat_perc ~ study_visit_number_svl, data = healthy, paired = T)

p1 <- ggplot(data = healthy, aes(y = liver_fat_perc, x = study_visit_number_svl)) +
  ...
  labs(y = "Hepatic Fat (%)",  x = "") +
  ylim(range(healthy$liver_fat_perc)[1],
       range(healthy$liver_fat_perc)[2] + 
         bound_extend*(range(healthy$liver_fat_perc)[2] - 
                         range(healthy$liver_fat_perc)[1]))
...
scalar <- 0.9
png(filename = "healthy_measures.png", res = 300, width = scalar*3000, height = scalar*2000)
...
model1 <- t.test(liver_fat_perc ~ study_visit_number_svl, data = obese, paired = T)

p1 <- ggplot(data = obese, aes(y = liver_fat_perc, x = study_visit_number_svl)) +
  geom_boxplot(outlier.alpha = 0) +
  geom_point(alpha = point_alpha) +
  geom_line(alpha = line_alpha, aes(group = hip_id_screen)) +
  theme_light() +
  geom_label(aes(x=Inf,y=Inf,hjust=h_just,vjust=y_just,
             ...
             label.r = label_radius) +
  labs(y = "Hepatic Fat (%)",  x = "") +
  ylim(range(obese$liver_fat_perc)[1],
       range(obese$liver_fat_perc)[2] + 
         bound_extend*(range(obese$liver_fat_perc)[2] - 
                         range(obese$liver_fat_perc)[1]))
...
scalar <- 0.9
png(filename = "obese_measures.png", res = 300, width = scalar*3000, height = scalar*2000)
ggarrange(p1,p2,p3,p4,p5,p6,p7,p8,p9,
          nrow = 3, ncol = 3)
dev.off()
