...
model_and_plot = function(outcome_name,ylab,group_var,pred,df,pois = F,plot){
  ...
  # Fit a mixed model. 
  mod <- lme(as.formula(paste0(outcome_name,"~as.numeric(time)|",pred)),random=~1|HIP.ID,df,na.action=na.omit)
  # Plot
...
```{r include=TRUE, warning=FALSE, message=F}
model_and_plot(outcome_name = "LH.Corrected.by.SG",pred="Obese.Lean",df=data[data$LH.Corrected.by.SG<3 & data$time<1500 & !is.na(data$Obese.Lean),],group_var = "HIP.ID",ylab="LH.Corrected.by.SG",plot="1")
```
