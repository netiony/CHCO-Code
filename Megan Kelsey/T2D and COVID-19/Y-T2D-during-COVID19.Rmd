...
mercy <- read.csv("E:\\Megan Kelsey\\T2D and COVID\\Data raw\\T2D COVID - Children's Mercy Final Deidentified - Updates 11-12-21.csv")
mercy$site <- "Mercy"
mercy$ptid <- NA
...
vanderbilt <- read.csv("E:\\Megan Kelsey\\T2D and COVID\\Data raw\\T2D COVID - Shoemaker - VUMC 9-2021.csv")
...
# read in CHOP
chop <- read.csv("E:\\Megan Kelsey\\T2D and COVID\\Data raw\\Finalized Nov 1 T2D Covid CHOP All Years.csv")
chop$site <- "CHOP"
chop$X <- NULL
chop$X.1 <- NULL
...
temp <- alldata %>% select(gad_check,ICA_check,Insulin_check,zn_check) %>% mutate(num_ab_check=gad_check+ICA_check+Insulin_check+zn_check,.keep="all")
...
write.csv(abproblem_cchmc,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_cchmc.csv",row.names = F)
abproblem_chla <- abproblem %>% filter(site=="CHLA")
write.csv(abproblem_chla,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_chla.csv",row.names = F)
abproblem_columbia <- abproblem %>% filter(site=="Columbia")
write.csv(abproblem_columbia,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_columbia.csv",row.names = F)
abproblem_duke <- abproblem %>% filter(site=="Duke")
write.csv(abproblem_duke,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_duke.csv",row.names = F)
abproblem_michigan <- abproblem %>% filter(site=="Michigan")
write.csv(abproblem_michigan,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_michigan.csv",row.names = F)
abproblem_pennstate <- abproblem %>% filter(site=="Penn State")
write.csv(abproblem_pennstate,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_pennstate.csv",row.names = F)
abproblem_pennington <- abproblem %>% filter(site=="Pennington")
write.csv(abproblem_pennington,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_pennington.csv",row.names = F)
abproblem_tch <- abproblem %>% filter(site=="TCH")
write.csv(abproblem_tch,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_tch.csv",row.names = F)
abproblem_ucsd <- abproblem %>% filter(site=="UCSD")
write.csv(abproblem_ucsd,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_UCSD.csv",row.names = F)
abproblem_ucsf <- abproblem %>% filter(site=="UCSF")
write.csv(abproblem_ucsf,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_UCSF.csv",row.names = F)
abproblem_vanderbilt <- abproblem %>% filter(site=="Vanderbilt")
write.csv(abproblem_vanderbilt,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\abcheck_vanderbilt.csv",row.names = F)

# filter out those with fewer than 2 ab tested for now - can add back individually as sites write back 
alldata <- alldata %>% filter(!num_ab_tested<2)
...
write.csv(bmicheck_chop,"E:\\Megan Kelsey\\T2D and COVID\\Data clean\\bmicheck_chop.csv",row.names = F)
...
# then double check all data values
# then fix analysis stuff
...
- Any patients with <2 ab tested have been excluded from this report for now, but queries were sent to the sites.
