...
ptab <- ptab[,c("sex","age","perc_95_0")]

# the problem is that now ptab has multiples of sex and age_merge combination
# but if I don't round to 1 digit some of the values in alldata won't match up
test <- merge(alldata,ptab,by=c("sex","age_merge"),all.x=T,all.y = T)

test <- alldata %>% left_join(ptab,by=c())
