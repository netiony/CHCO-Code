...
test_m_join <- difference_left_join(males,ptab_m,by="age_dx",distance_col="dist_bmi")
# this won't work because some records don't have unique ID
test_m_join_keep <- test_m_join %>% group_by(ptid) %>% slice_min(dist_bmi)

# there are people in the dataset who did not get matched to a BMI percentile
bad_id <- males$ptid[!(males$ptid %in% test_m_join_keep$ptid)]
checkdata <- males %>% filter(test_m$ptid %in% bad_id)

test <- cbind(test_m_join_keep,test_f_join_keep
...
vars <- c("site","region","mo_dx","yr_dx","period","num_ab_tested","GAD65.Ab","ICA.Ab","Insulin.Ab","ZnT8.Ab",
          "age_dx","Race","Ethnicity","race_eth","race_eth_black","sex","insurance","Location.Diagnosis","COVID.19.at.diagnosis","BMI","bmip","HbA1c",
