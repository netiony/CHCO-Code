...
- Queries were sent to the sites regarding patients with <2 antibodies tested.  The 10 sites were CCHMC, CHLA, Duke, Michigan, Penn State, Pennington, TCH, UCSD, UCSF, and Vanderbilt.  Duke, TCH, CHLA, Vanderbilt, UCSF confirmed these patients should be excluded. CCHMC and Pennington provided antibody results for some patients and confirmed that others should be excluded.  Other sites are pending (data are included for now).
...
vars <- c("site","region","mo_dx","yr_dx","period","num_ab_tested","GAD65.Ab","ICA.Ab","Insulin.Ab","ZnT8.Ab",
          "age_dx","Race","Ethnicity","race_eth","sex","insurance","Location.Diagnosis","COVID.19.at.diagnosis","BMI","bmip","HbA1c",
...
x <- xtabs( ~ pandemic+sex+race_eth, alldata)
loglinmod <- loglm(~pandemic+sex+race_eth,data=x)
...
```{r echo=FALSE}
ggplot(alldata, aes(x=dx_date, fill=met_decomp)) + geom_bar() + geom_vline(xintercept = as.Date("03-01-2020",format="%m-%d-%Y"),linetype="dotted")+ ylab("New diagnoses of T2D") + xlab("") + labs(fill="") + scale_fill_discrete(labels=c("No metabolic decompensation","Metabolic decompensation")) + scale_x_date(date_labels = "%m-%Y",date_breaks = "1 month") + theme(axis.text.x = element_text(angle = 90)) 
```
