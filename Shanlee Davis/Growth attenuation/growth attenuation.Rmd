...
```{r echo=FALSE, include=FALSE}
mod1 <- lme(ba_pah_calc ~ time_gat_ba*sex,random=~1|record_id,data = bone_age,na.action = na.omit)
...
```{r echo=FALSE, include=FALSE}
mod1 <- lme(ba_pah_calc ~ time_gat_ba*sex,random=~1|record_id,data = bone_age_noae,na.action = na.omit)
# estimation at mean value of time
mod1_pred <- matrix(predict(ref_grid(mod1)))
# estimation at baseline
mod1_pred.0yr <- ref_grid(mod1, at = list(time_gat_ba = 0))
