...
#setwd(home_dir)
alldata <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Nadeau/IMPROVE-T2D/Data raw/IMPROVET2D-MMTTVariables_DATA_2024-04-29_1548.csv")
...
# read in OMM data
omm <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Nadeau/IMPROVE-T2D/Data raw/IMPROVE_OMM_ISR_results.csv")
omm <- omm %>% filter(Visit %in% c("BL", "3m"))
...
mixed_omm <- function(data, outcome){
 ...
 # prs <-  pairs(m,adjust="tukey",by="Visit")
  return(list(anova,m))
}
...
p <- plot_mixed(mod, ylab = "C-peptide")
p
```
...
p <- plot_mixed(mod, ylab = "Glucagon")
p
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_omm(data = omm, outcome = "OMM_EFF")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_adj_omm(data = omm, outcome = "OMM_EFF")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_omm(data = omm, outcome = "ISR_AUC_30min")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_adj_omm(data = omm, outcome = "ISR_AUC_30min")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_omm(data = omm, outcome = "ISR_AUC_4h")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_adj_omm(data = omm, outcome = "ISR_AUC_4h")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_omm(data = omm, outcome = "B_Cell_Resp_30m")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_adj_omm(data = omm, outcome = "B_Cell_Resp_30m")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_omm(data = omm, outcome = "DI_30_glu")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_adj_omm(data = omm, outcome = "DI_30_glu")
```
