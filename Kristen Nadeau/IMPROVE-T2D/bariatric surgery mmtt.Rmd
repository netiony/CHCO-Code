...
# read in OMM data
omm <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Nadeau/IMPROVE-T2D/Data raw/IMPROVE_OMM_ISR_results 1.csv")
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_omm(data = omm, outcome = "beta_cell_resp")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_adj_omm(data = omm, outcome = "beta_cell_resp")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_omm(data = omm, outcome = "disposition_ind")
```
...
```{r echo=FALSE, include=TRUE}
mod <- mixed_adj_omm(data = omm, outcome = "disposition_ind")
```
