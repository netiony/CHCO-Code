...
mixed <- function(data, outcome){
  ...
  m <- as.data.frame(m)
  p <- ggplot(m, aes(time, emmean, color = study_visit_mttt)) +
  geom_point(aes(colour = study_visit_mttt)) + # You can also add `geom_point(aes(group = variable)) + ` if you want to see the actual points
    geom_line(aes(group = "study_visit_mttt")) +
...
plot_mixed <- function(mixed_output, data) {
  emmeans_result <- mixed_output[[2]]
  emmeans_result <- as.data.frame(emmeans_result)
  # Extract relevant data
  emmeans_data <- emmeans_result$emmean
  emmeans_data$time <- as.numeric(as.character(emmeans_result$time))
  emmeans_data <- as.data.frame(emmeans_data)
  
  # Create line plot
 p <-  ggplot(emmeans_result, aes(x = time, y = emmean, color = "study_visit_mmtt", group = "study_visit_mmtt")) +
    geom_line() +
    geom_point() +
    labs(x = "Time", y = "Estimated Marginal Mean", color = "study_visit_mmtt") +
    theme_bw()
}
