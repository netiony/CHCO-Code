...
write.csv(data2[!(is.na(data2$lab_ogtt_fasting) & is.na(data2$lab_ogtt_1_hour_glucose) & is.na(data2$lab_ogtt_2_hour_glucose)),
                c("MRN","Encounter_date","lab_ogtt_fasting","lab_ogtt_1_hour_glucose","lab_ogtt_2_hour_glucose")],
          "/Volumes/Peds Endo/Christine Chan/Prospective analysis/Data clean/REDCap OGTT data.csv")
...
write.csv(ogtt_keep,"/Volumes/Peds Endo/Christine Chan/Prospective analysis/Data clean/Elin OGTT data prior to merge.csv")
...
# then do a fuzzy join allowing OGTTs up to 90 days prior, 14 days after
data3 <- fuzzy_left_join(data3, ogtt_keep,
                                 by = c("MRN" = "MRN",
                                        'date_cgm_placement_minus_90' = "Date of Test",
                                        'date_cgm_placement_plus_14' = 'Date of Test'),
                                 match_fun = list(`==`, `<=`,`>=`))
data3$MRN <- data3$MRN.x
data3$MRN.x <- NULL
data3$MRN.y <- NULL
data3$Encounter_date <- data3$Encounter_date.x
data3$Encounter_date.x <- NULL
data3$Encounter_date.y <- NULL

# why are there now 85 people not on insulin at baseline?
# are there less missing data now?
# not really, still only 25 with data
