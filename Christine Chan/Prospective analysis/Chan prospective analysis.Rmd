...
t1vars <- c("age","gender.factor","race_ethnicity.factor","genoRisk","bmiZ","bmi_percentile","g_tube_feeds.factor","cf_pancreatic.factor" )
...
## Demographics and clinical characteristics at baseline (time of CGM wear)

```{r}
t1 <- CreateTableOne(vars=t1vars, data=t1data)
kable(print(t1,varLabels=TRUE,showAllLevels=TRUE))
```
<br>
...
### Time to CFRD vs. average glucose
...
```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ standard_deviation,data = t1data)
summary(mod)
```

### Time to CFRD vs. min_sensor

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ min_sensor,data = t1data)
summary(mod)
```

### Time to CFRD vs. max_sensor

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ max_sensor,data = t1data)
summary(mod)
```

### Time to CFRD vs. % time >120

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ percent_time_over_120,data = t1data)
summary(mod)
```

### Time to CFRD vs. % time >140

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ percent_time_over_140,data = t1data)
summary(mod)
```

### Time to CFRD vs. % time >180

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ percent_time_over_180,data = t1data)
summary(mod)
```

### Time to CFRD vs. % time >200

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ percent_time_over_200,data = t1data)
summary(mod)
```

### Time to CFRD vs. % time >250

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ percent_time_over_250,data = t1data)
summary(mod)
```

### Time to CFRD vs. average excursions >140/day

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ avg_excur_over_140_per_day,data = t1data)
summary(mod)
```

### Time to CFRD vs. average excursions >200/day

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ avg_excur_over_200_per_day,data = t1data)
summary(mod)
```

### Time to CFRD vs. % time <54

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ percent_time_under_54,data = t1data)
summary(mod)
```

### Time to CFRD vs. % time <60

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ percent_time_under_60,data = t1data)
summary(mod)
```

### Time to CFRD vs. % time <70

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ percent_time_under_70,data = t1data)
summary(mod)
```

### Time to CFRD vs. average AUC>180

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ auc_over_180,data = t1data)
summary(mod)
```

### Time to CFRD vs. MAGE

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ mean_amp_glycemic_ex,data = t1data)
summary(mod)
```
...
```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ cov,data = t1data)
summary(mod)
```

## CGM vs. time to insulin

### Time to insulin vs. average glucose

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ average_sensor,data = t1data)
summary(mod)
```

### Time to insulin vs. SD

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ standard_deviation,data = t1data)
summary(mod)
```

### Time to insulin vs. min_sensor

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ min_sensor,data = t1data)
summary(mod)
```

### Time to insulin vs. max_sensor

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ max_sensor,data = t1data)
summary(mod)
```

### Time to insulin vs. % time >120

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ percent_time_over_120,data = t1data)
summary(mod)
```

### Time to insulin vs. % time >140

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ percent_time_over_140,data = t1data)
summary(mod)
```

### Time to insulin vs. % time >180

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ percent_time_over_180,data = t1data)
summary(mod)
```

### Time to insulin vs. % time >200

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ percent_time_over_200,data = t1data)
summary(mod)
```

### Time to insulin vs. % time >250

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ percent_time_over_250,data = t1data)
summary(mod)
```

### Time to insulin vs. average excursions >140/day

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ avg_excur_over_140_per_day,data = t1data)
summary(mod)
```

### Time to insulin vs. average excursions >200/day

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ avg_excur_over_200_per_day,data = t1data)
summary(mod)
```

### Time to insulin vs. % time <54

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ percent_time_under_54,data = t1data)
summary(mod)
```

### Time to insulin vs. % time <60

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ percent_time_under_60,data = t1data)
summary(mod)
```

### Time to insulin vs. % time <70

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ percent_time_under_70,data = t1data)
summary(mod)
```

### Time to insulin vs. average AUC>180

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ auc_over_180,data = t1data)
summary(mod)
```

### Time to insulin vs. MAGE

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ mean_amp_glycemic_ex,data = t1data)
summary(mod)
```

### Time to insulin vs. CV

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ cov,data = t1data)
summary(mod)
```

## Time to CFRD vs. OGTT

### Time to CFRD vs. 1 hour OGTT glucose

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ lab_ogtt_1_hour_glucose,data = t1data)
summary(mod)
```

### Time to CFRD vs. 2 hour OGTT glucose

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_CFRD,has_CFRD) ~ lab_ogtt_2_hour_glucose,data = t1data)
summary(mod)
```

## Time to insulin vs. OGTT

### Time to insulin vs. 1 hour OGTT glucose

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ lab_ogtt_1_hour_glucose,data = t1data)
summary(mod)
```

### Time to insulin vs. 2 hour OGTT glucose

```{r, echo=FALSE,comment=""}
mod <- coxph(Surv(time_to_ins,on_insulin) ~ lab_ogtt_2_hour_glucose,data = t1data)
summary(mod)
```