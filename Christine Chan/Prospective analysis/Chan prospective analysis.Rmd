...
final_alltime_withins <- data8 
final_1yr_withins <- data8 %>% filter(days<=365)
final_3yr_withins <- data8 %>% filter(days<=365*3)
final_alltime_noins <- data8 %>% filter(on_insulin==0)
final_1yr_noins <- data8 %>% filter(on_insulin==0) %>% filter(days<=365)
final_3yr_noins <- data8 %>% filter(on_insulin==0) %>% filter(days<=365*3)
...
# TURN MODELS INTO FUNCTIONS!!!  
# are CGM variables predictive of BMI and PFT change?
mod <- lme( ~ days + average_sensor,random=~1|MRN,data = final_alltime_noins,na.action = na.omit)
