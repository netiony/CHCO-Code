...
CFRD <- data7[,c("MRN","Encounter_date","Hbg A1C","poc_a1c","OGTT 2 Hour","lab_ogtt_2_hour_glucose","poc_fasting_glucose","OGTT Fasting")]
...
CFRD$dia_visit <- ifelse(CFRD$`Hbg A1C`>=6.5 | CFRD$poc_a1c >=6.5 | CFRD$`OGTT 2 Hour`>= 200 | CFRD$lab_ogtt_2_hour_glucose >= 200
                         | CFRD$poc_fasting_glucose>=126 | CFRD$`OGTT Fasting`>=126, 1, NA)
...
# find date of IGT (2 hour glucose >=140)
igt <- data7[,c("MRN","Encounter_date","OGTT 2 Hour","lab_ogtt_2_hour_glucose")]
igt$igt_visit <- ifelse((igt$`OGTT 2 Hour`>= 140 & igt$`OGTT 2 Hour`<200) | (igt$lab_ogtt_2_hour_glucose >= 140 & igt$lab_ogtt_2_hour_glucose <200), 1, NA)
igt <- igt %>% filter(igt$igt_visit==1)
igt_date <- igt %>% arrange(MRN,Encounter_date) %>% group_by(MRN) %>% filter(row_number()==1)
igt_date$igt_date <- as.Date("")
igt_date$igt_date <- igt_date$Encounter_date
igt_date <- igt_date[,c("MRN","igt_date")]
data8 <- merge(data8,igt_date,by="MRN",all.x = T,all.y = T)
data8$flag_igt_cfrd <- ifelse(!is.na(data8$CFRD_date) & !is.na(data8$igt_date) & data8$igt_date>data8$CFRD_date,1,NA)
data8$igt_date <- ifelse(is.na(data8$flag_igt_cfrd),data8$igt_date,NA)
...
data8$has_igt <-  ifelse(is.na(data8$igt_date) | data8$Encounter_date < data8$igt_date,0,1) 
...
ever_igt <- data8[,c("MRN","has_igt")] %>% filter(data8$has_igt==1)
ever_igt <- unique(ever_igt)
colnames(ever_igt) <- c("MRN","ever_igt")
data8 <- merge(data8,ever_igt,by="MRN",all.x = T,all.y = T)
...
write.csv(data9,'E:\\Christine Chan\\Prospective analysis\\Data clean\\final_export_prospective.csv')
...
t1data$ever_igt <- ifelse(is.na(t1data$ever_igt),0,t1data$ever_igt)
t1data$ever_cfrd <- as.factor(t1data$ever_cfrd)
t1data$ever_igt <- as.factor(t1data$ever_igt)
label(t1data$ever_cfrd)="Developed CFRD during f/up"
label(t1data$ever_igt)="Developed IGT during f/up"

t1data$checking_CFRD <- as.factor(t1data$has_CFRD)
```
...
check <- CreateTableOne(vars="checking_CFRD", data=t1data, strata="group_category.factor",test = F)
...
```{r, include=FALSE, echo=FALSE, results='hide'}
t2 <- CreateTableOne(vars=c("ever_cfrd","ever_igt"), data=t1data)
