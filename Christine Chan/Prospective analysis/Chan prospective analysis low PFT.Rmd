...
# read in PFTs
pfts <- read_xlsx("E:\\Christine Chan\\Prospective analysis\\Data raw\\Updated Full List for Elin_Data_Updated2022_07_22.xlsx",sheet="PFTs")
...
ogtt <- read_xlsx("E:\\Christine Chan\\Prospective analysis\\Data raw\\Updated Full List for Elin_Data_Updated2022_07_22.xlsx",sheet="OGTTandA1C")
...
# read in sputum data
sputum <- read_xlsx("E:\\Christine Chan\\Prospective analysis\\Data raw\\Updated Full List for Elin_Data_Updated2022_07_22.xlsx",sheet="Sputum")
...
mods <- read_xlsx("E:\\Christine Chan\\Prospective analysis\\Data raw\\Updated Full List for Elin_Data_Updated2022_07_22.xlsx",sheet="Modulators")
...
# read in admits
admits <- read_xlsx("E:\\Christine Chan\\Prospective analysis\\Data raw\\Updated Full List for Elin_Data_Updated2022_07_22.xlsx",sheet="Admits_List")
...
newcgm <- read.csv("E:\\Christine Chan\\Prospective analysis\\Data raw\\Corrected outcome data\\Extra CGM upload\\REDCap Upload.csv")
...
write.csv(check_poc,"E:\\Christine Chan\\Prospective analysis\\Data clean\\checking_POC_glucose.csv",row.names = F)
...
# for this analysis, keep only lowest tertile of baseline BMI
data9 <- data9[!is.na(data9$low_pft),]
...
write.csv(checkoutcomes,'E:\\Christine Chan\\Prospective analysis\\Data clean\\check_outcomes.csv',row.names = F)
...
write.csv(checkfsogtt,'E:\\Christine Chan\\Prospective analysis\\Data clean\\check_fsogtt.csv',row.names = F)
...
model_and_plot = function(outcome_name,ylab,group_var,pred,df,pois = F,plot){
  ...
  # Fit a mixed model. 
  try(mod <- lme(as.formula(paste0(outcome_name,"~days+",pred)),random=~1|MRN,df,na.action=na.omit))
  # Plot
  ...
  if(plot==1){print(p)}
  try(print(kable(anova.lme(mod,type="marginal"),digits = 3,caption = "Type III ANOVA")))
  try(print(kable(summary(mod)$tTable,digits = 3,caption = "Fixed Effects")))
}
...
model_and_plot_ins = function(outcome_name,ylab,group_var,pred,df,pois = F,plot){
  ...
  df[,outcome_name] = as.numeric(df[,outcome_name])
  # Fit a mixed model. 
  try(mod <- lme(as.formula(paste0(outcome_name,"~days + on_insulin + ",pred)),random=~1|MRN,df,na.action=na.omit))
  # Plot
  p = ggplot(df,aes_string(x = "days",y = outcome_name, group = group_var, color=as.factor(df$CFRD_after_base))) +
    geom_line(alpha=0.5) + geom_smooth(aes(group=1)) +
    theme_bw() +xlab("Days") + ylab(ylab) +  scale_color_discrete(name ="Developed CFRD", labels=c("No","Yes"))
...
model_and_plot_admit = function(outcome_name,ylab,xlab,pred,df) {
  ...
  df[,outcome_name] = as.numeric(df[,outcome_name])
  # Fit a linear model 
  try(mod <- lm(as.formula(paste0(outcome_name,"~",pred)),df))
  # Plot
  ...
  try(print(kable(summary(mod)$coef,digits = 3,caption = "Fixed Effects")))
}
```
...
model_cox = function(timevar,statusvar,pred,df) {
  ...
  # Fit Cox model 
  try(mod <- coxph(Surv(paste0("(",timevar,",",statusvar,")~",pred),df)))
  # print results
  try(print(kable(summary(mod),digits = 3,caption = "")))
}
...
This version of the analysis includes only those participants with FEV% or FVC% <80 were included.
...
### Checking results of baseline OGTT vs. group variable in REDCap

```{r, include=FALSE, echo=FALSE, results='hide'}
check <- CreateTableOne(vars="has_CFRD_base.factor", data=t1data, strata="group_category.factor",test = F)
check <- print(check,varLabels=TRUE,showAllLevels=TRUE)
```
