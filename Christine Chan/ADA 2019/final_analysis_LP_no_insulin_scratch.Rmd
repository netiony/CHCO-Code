...
# this gives almost the same result as the pROC package
test <- optimal.cutpoints(X="percent_time_over_140",data=alldata[alldata$group_category.factor != "Healthy Control",],status="diabetes",
                          methods="Youden",tag.healthy="0")
...
# trying with Optimal.Cutpoints
test1 <- optimal.cutpoints(X="percent_time_over_140",data=alldata[alldata$group_category.factor != "Healthy Control",],status="diabetes",
...
# trying with Optimat.Cutpoints
# this gives almost the same result as the pROC package
test3 <- optimal.cutpoints(X="percent_time_over_200",data=alldata[alldata$group_category.factor != "Healthy Control",],status="diabetes",
                          methods="MinValueSe",tag.healthy="0",control=control.cutpoints(valueSe = 0.9) )
...
test4 <- optimal.cutpoints(X="percent_time_over_200",data=alldata[alldata$group_category.factor != "Healthy Control",],status="diabetes",
                          methods="MinValueSe",tag.healthy="0",control=control.cutpoints(valueSe = 0.9) )
...
test5 <- optimal.cutpoints(X="average_sensor",data=alldata[alldata$group_category.factor != "Healthy Control",],status="diabetes",
                          methods="MinValueSe",tag.healthy="0",control=control.cutpoints(valueSe = 0.9) )
...
No cutpoint was identified which maximized specificity while maintaining sensitivity at 90%.
...
No cutpoint was identified which maximized specificity while maintaining sensitivity above 90%.
...
plot(pct140_roc)
plot(pct200_roc,add=TRUE,lty=2)
plot(average_roc,add=TRUE,lty=3)
plot(max_roc,add=TRUE,lty=4)
plot(sd_roc,add=TRUE,lty=5)
plot(cv_roc,add=TRUE,lty=6)

legend("bottomright", 
       legend = c("% time>140", "% time>200", "Mean sensor glucose", "Max glucose","SD","CV"), 
       lty=c(1,2,3,4,5,6),
...
plot(pct140_roc_pre)
plot(pct200_roc_pre,add=TRUE,lty=2)
plot(average_roc_pre,add=TRUE,lty=3)
plot(max_roc_pre,add=TRUE,lty=4)
plot(sd_roc_pre,add=TRUE,lty=5)
plot(cv_roc_pre,add=TRUE,lty=6)
