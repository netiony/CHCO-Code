...
# pairwise comparisons for table 2 - CV
a17 <- aov(alldata$cov ~alldata$group_category.factor)
a17t <- tidy(TukeyHSD(a17))
a17t <- a17t[,-1]

# pairwise comparisons for table 2 - TIR
a18 <- aov(alldata$tir_70_140 ~alldata$group_category.factor)
a18t <- tidy(TukeyHSD(a18))
a18t <- a18t[,-1]

# pairwise comparisons for table 2 - matsuda
a19 <- aov(alldata$matsuda ~alldata$group_category.factor)
a19t <- tidy(TukeyHSD(a19))
a19t <- a19t[,-1]
...
corr_red <- flattenCorrMatrix(corr_red$r, corr_red$P)
...
sigvars <- c("poc_a1c","isi","isi_cpep","odi","odi_cpep","iaucbase_glucose","iaucbase_cpep","iAUCcpep_iAUCgluc","iAUCins_iAUC_gluc","average_sensor","standard_deviation","max_sensor","average_auc_per_day","excursions_over_200","avg_excur_over_200_per_day","min_spent_over_200","excursions_over_140","avg_excur_over_140_per_day","percent_time_over_140","min_spent_over_140","mean_amp_glycemic_ex","percent_time_over_180","percent_time_over_200","cov","tir_70_140","matsuda")
l <- list()
for (var in sigvars) {
...
```{r echo=FALSE}
# Pairwise comparisons
sigvars_odi <- c("max_sensor","average_sensor","average_auc_per_day","avg_excur_over_140_per_day","avg_excur_over_200_per_day",
                 "excursions_over_140","percent_time_over_140","percent_time_over_180","min_spent_over_140","standard_deviation",
...
pct140_roc <- pROC::roc(diabetes~percent_time_over_140,data=alldata,quiet=T)
...
# trying pct140 with cutpointr - max sensitivity given spec 50%
# here we are saying that the cost of a false negative is 30x that of a false positive
pct140_sens <- cutpointr(data=alldata,class=diabetes,x=percent_time_over_140,
...
pct180_roc <- pROC::roc(diabetes~percent_time_over_180,data=alldata,quiet=T)
...
pct180_sens <- cutpointr(data=alldata,class=diabetes,x=percent_time_over_180,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
pct180_sens_ppv <- pct180_sens %>% add_metric(list(ppv)) %>% select(ppv)

pct200_roc <- pROC::roc(diabetes~percent_time_over_200,data=alldata,quiet=T)
...
pct200_sens <- cutpointr(data=alldata,class=diabetes,x=percent_time_over_200,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
pct200_sens_ppv <- pct200_sens %>% add_metric(list(ppv)) %>% select(ppv)

ex200_roc <- pROC::roc(diabetes~excursions_over_200,data=alldata,quiet=T)
...
ex200_sens <- cutpointr(data=alldata,class=diabetes,x=excursions_over_200,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
ex200_sens_ppv <- ex200_sens %>% add_metric(list(ppv)) %>% select(ppv)

average_roc <- pROC::roc(diabetes~average_sensor,data=alldata,quiet=T)
...
average_sens <- cutpointr(data=alldata,class=diabetes,x=average_sensor,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
average_sens_ppv <- average_sens %>% add_metric(list(ppv)) %>% select(ppv)

max_roc <- pROC::roc(diabetes~max_sensor,data=alldata,quiet=T)
...
max_sens <- cutpointr(data=alldata,class=diabetes,x=max_sensor,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
max_sens_ppv <- max_sens %>% add_metric(list(ppv)) %>% select(ppv)

sd_roc <- pROC::roc(diabetes~standard_deviation,data=alldata,quiet=T)
...
sd_sens <- cutpointr(data=alldata,class=diabetes,x=standard_deviation,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
sd_sens_ppv <- sd_sens %>% add_metric(list(ppv)) %>% select(ppv)

cv_roc <- pROC::roc(diabetes~cov,data=alldata,quiet=T)
...
cv_sens <- cutpointr(data=alldata,class=diabetes,x=cov,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
cv_sens_ppv <- cv_sens %>% add_metric(list(ppv)) %>% select(ppv)

mage_roc <- pROC::roc(diabetes~r_mage,data=alldata,quiet=T)
...
mage_sens <- cutpointr(data=alldata,class=diabetes,x=r_mage,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
...
# use these variables: percent_time_over_140 percent_time_over_180 percent_time_over_200 excursions_over_200 average_sensor
# max_sensor standard_deviation cv r_mage
pct140_roc_pre <- pROC::roc(prediabetes~percent_time_over_140,data=alldata,quiet=T)
...
pct140_sens_pre <- cutpointr(data=alldata,class=prediabetes,x=percent_time_over_140,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
pct140_sens_pre_ppv <- pct140_sens_pre %>% add_metric(list(ppv)) %>% select(ppv)

pct180_roc_pre <- pROC::roc(prediabetes~percent_time_over_180,data=alldata,quiet=T)
...
pct180_sens_pre <- cutpointr(data=alldata,class=prediabetes,x=percent_time_over_180,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
pct180_sens_pre_ppv <- pct180_sens_pre %>% add_metric(list(ppv)) %>% select(ppv)

pct200_roc_pre <- pROC::roc(prediabetes~percent_time_over_200,data=alldata,quiet=T)
...
pct200_sens_pre <- cutpointr(data=alldata,class=prediabetes,x=percent_time_over_200,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
pct200_sens_pre_ppv <- pct200_sens_pre %>% add_metric(list(ppv)) %>% select(ppv)

ex200_roc_pre <- pROC::roc(prediabetes~excursions_over_200,data=alldata,quiet=T)
...
ex200_sens_pre <- cutpointr(data=alldata,class=prediabetes,x=excursions_over_200,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
ex200_sens_pre_ppv <- ex200_sens_pre %>% add_metric(list(ppv)) %>% select(ppv)

average_roc_pre <- pROC::roc(prediabetes~average_sensor,data=alldata,quiet=T)
...
average_sens_pre <- cutpointr(data=alldata,class=prediabetes,x=average_sensor,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
average_sens_pre_ppv <- average_sens_pre %>% add_metric(list(ppv)) %>% select(ppv)

max_roc_pre <- pROC::roc(prediabetes~max_sensor,data=alldata,quiet=T)
...
max_sens_pre <- cutpointr(data=alldata,class=prediabetes,x=max_sensor,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
max_sens_pre_ppv <- max_sens_pre %>% add_metric(list(ppv)) %>% select(ppv)

sd_roc_pre <- pROC::roc(prediabetes~standard_deviation,data=alldata,quiet=T)
...
sd_sens_pre <- cutpointr(data=alldata,class=prediabetes,x=standard_deviation,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
sd_sens_pre_ppv <- sd_sens_pre %>% add_metric(list(ppv)) %>% select(ppv)

cv_roc_pre <- pROC::roc(prediabetes~cov,data=alldata,quiet=T)
...
cv_sens_pre <- cutpointr(data=alldata,class=prediabetes,x=cov,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
cv_sens_pre_ppv <- cv_sens_pre %>% add_metric(list(ppv)) %>% select(ppv)

mage_roc_pre <- pROC::roc(prediabetes~r_mage,data=alldata,quiet=T)
...
mage_sens_pre <- cutpointr(data=alldata,class=prediabetes,x=r_mage,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
...
# use these variables: percent_time_over_140 percent_time_over_180 percent_time_over_200 excursions_over_200 average_sensor
# max_sensor standard_deviation cv r_mage
pct140_roc_pd <- pROC::roc(prediabetes_or_diabetes~percent_time_over_140,data=alldata,quiet=T)
...
pct140_sens_pd <- cutpointr(data=alldata,class=prediabetes_or_diabetes,x=percent_time_over_140,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
pct140_sens_pd_ppv <- pct140_sens_pd %>% add_metric(list(ppv)) %>% select(ppv)

pct180_roc_pd <- pROC::roc(prediabetes_or_diabetes~percent_time_over_180,data=alldata,quiet=T)
...
pct180_sens_pd <- cutpointr(data=alldata,class=prediabetes_or_diabetes,x=percent_time_over_180,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
pct180_sens_pd_ppv <- pct180_sens_pd %>% add_metric(list(ppv)) %>% select(ppv)

pct200_roc_pd <- pROC::roc(prediabetes_or_diabetes~percent_time_over_200,data=alldata,quiet=T)
...
pct200_sens_pd <- cutpointr(data=alldata,class=prediabetes_or_diabetes,x=percent_time_over_200,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
pct200_sens_pd_ppv <- pct200_sens_pd %>% add_metric(list(ppv)) %>% select(ppv)

ex200_roc_pd <- pROC::roc(prediabetes_or_diabetes~excursions_over_200,data=alldata,quiet=T)
...
ex200_sens_pd <- cutpointr(data=alldata,class=prediabetes_or_diabetes,x=excursions_over_200,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
ex200_sens_pd_ppv <- ex200_sens_pd %>% add_metric(list(ppv)) %>% select(ppv)

average_roc_pd <- pROC::roc(prediabetes_or_diabetes~average_sensor,data=alldata,quiet=T)
...
average_sens_pd <- cutpointr(data=alldata,class=prediabetes_or_diabetes,x=average_sensor,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
average_sens_pd_ppv <- average_sens_pd %>% add_metric(list(ppv)) %>% select(ppv)

max_roc_pd <- pROC::roc(prediabetes_or_diabetes~max_sensor,data=alldata,quiet=T)
...
max_sens_pd <- cutpointr(data=alldata,class=prediabetes_or_diabetes,x=max_sensor,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
max_sens_pd_ppv <- max_sens_pd %>% add_metric(list(ppv)) %>% select(ppv)

sd_roc_pd <- pROC::roc(prediabetes_or_diabetes~standard_deviation,data=alldata,quiet=T)
...
sd_sens_pd <- cutpointr(data=alldata,class=prediabetes_or_diabetes,x=standard_deviation,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
sd_sens_pd_ppv <- sd_sens_pd %>% add_metric(list(ppv)) %>% select(ppv)

cv_roc_pd <- pROC::roc(prediabetes_or_diabetes~cov,data=alldata,quiet=T)
...
cv_sens_pd <- cutpointr(data=alldata,class=prediabetes_or_diabetes,x=cov,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
cv_sens_pd_ppv <- cv_sens_pd %>% add_metric(list(ppv)) %>% select(ppv)

mage_roc_pd <- pROC::roc(prediabetes_or_diabetes~r_mage,data=alldata,quiet=T)
...
mage_sens_pd <- cutpointr(data=alldata,class=prediabetes_or_diabetes,x=r_mage,
                  method=minimize_metric,metric=misclassification_cost,
                  cost_fp=1,cost_fn=30,na.rm = T,pos_class = 1,direction=">=")
...
LASSO regression was used to determine which CGM variables best predict diabetes or a 2 hour OGTT>=140.  The data were randomly divided 1:1 into training and test sets.  The training set was used to fit the LASSO regression model, with cross-validation used to determine the optimal value of the lambda parameter.  The model was refit to the test set in order to examine model performance.  Finally, because the parameter estimates from LASSO regression are biased, the model was refit to the entire dataset using logistic regression.
...
Pairwise Comparisons for Tables 2a and 2b using Tukey's Honestly Significant Difference
```{r echo=FALSE}
...
kable(l[[24]],caption = sigvars[24])
kable(l[[25]],caption = sigvars[25])
kable(l[[26]],caption = sigvars[26])
```
...
Pairwise Comparisons for Table 2c using Tukey's Honestly Significant Difference
```{r echo=FALSE}
# Kable does not appear to work in a for loop for some reason.
...
kable(l_odi[[10]],caption = sigvars_odi[10])
kable(l_odi[[11]],caption = sigvars_odi[11])
kable(l_odi[[12]],caption = sigvars_odi[12])
kable(l_odi[[13]],caption = sigvars_odi[13])
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$percent_time_over_140<pct140_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$percent_time_over_140>=pct140_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$percent_time_over_200<pct200_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$percent_time_over_200>=pct200_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$excursions_over_200<ex200_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$excursions_over_200>=ex200_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$average_sensor<average_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$average_sensor>=average_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$max_sensor<max_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$max_sensor>=max_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$standard_deviation<sd_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$standard_deviation>=sd_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$cov<cv_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$cov>=cv_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$r_mage<mage_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$r_mage>=mage_sens$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$percent_time_over_140<pct140_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$percent_time_over_140>=pct140_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$percent_time_over_180<pct180_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$percent_time_over_180>=pct180_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$percent_time_over_200<pct200_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$percent_time_over_200>=pct200_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$excursions_over_200<ex200_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$excursions_over_200>=ex200_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$average_sensor<average_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$average_sensor>=average_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$max_sensor<max_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$max_sensor>=max_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$standard_deviation<sd_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$standard_deviation>=sd_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$cov<cv_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$cov>=cv_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$r_mage<mage_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$r_mage>=mage_sens_pre$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$percent_time_over_140<pct140_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$percent_time_over_140>=pct140_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$percent_time_over_180<pct180_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$percent_time_over_180>=pct180_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$percent_time_over_200<pct200_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$percent_time_over_200>=pct200_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$excursions_over_200<ex200_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$excursions_over_200>=ex200_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$average_sensor<average_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$average_sensor>=average_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$max_sensor<max_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$max_sensor>=max_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$standard_deviation<sd_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$standard_deviation>=sd_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$cov<cv_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$cov>=cv_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
```{r, results='asis',tidy=TRUE, echo=FALSE}
boxplot(alldata[alldata$r_mage<mage_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        alldata[alldata$r_mage>=mage_sens_pd$optimal_cutpoint,]$lab_ogtt_2_hour_glucose,
        names = c("< cutpoint",">= cutpoint"),ylab="2 hour glucose")
```
<br>
...
LASSO regression was used to determine which CGM variables best predict diabetes.  The final model was refit to the entire dataset using regular logistic regression in order to obtain unbiased parameter estimates.

```{r, results='asis',tidy=TRUE, echo=FALSE}
kable(mod_diabetes_table)
```
<br>

The proportion of participants who were correctly classified in the test set was `r round(mean(predicted_classes==observed_classes),4)`.  The table below provides a cross-tabulation of the true ("observed") diabetes status compared to the predicted diabetes status in the test set.

```{r, results='asis',tidy=TRUE, echo=FALSE}
kable(diabetes_crosstab, caption = "Cross-tabulation of observed diabetes status (rows) vs. predicted diabetes status (columns).")
```
<br>

\newpage

LASSO regression was used to determine which CGM variables best predict a 2 hour glucose >=140.  The final model was refit to the entire dataset using regular logistic regression in order to obtain unbiased parameter estimates.
...
The proportion of participants who were correctly classified in the test set was `r round(mean(predicted_classes_2hr==observed_classes_2hr),4)`.  The table below provides a cross-tabulation of the true ("observed") 2 hour glucose status (1 corresponds to glucose >=140) compared to the predicted 2 hour glucose status in the test set.

```{r, results='asis',tidy=TRUE, echo=FALSE}
kable(twohr_crosstab, caption = "Cross-tabulation of observed 2 hour glucose status (rows) vs. predicted 2 hour glucose status (columns).")
```
<br>
