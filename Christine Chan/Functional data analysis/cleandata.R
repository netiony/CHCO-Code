...
# Format columns.
    if (cgmtype == "diasend") {
      if (id_filename == F) {
        ...
        } else {id <- sub("_.*$","",basename(files[f]))}
...
# Trim end of data so it is in 24 hour chunks.
    ...
    for (i in 1:nrow(table)){
      table$minutes[i] <- floor(as.numeric(difftime(table$timestamp[i],table$timestamp[1],units="mins")))
    }
    for (i in 1:nrow(table)){
      table$minutes_scaled[i] <- as.numeric(table$minutes[i]) / as.numeric(table$minutes[nrow(table)])
    }
    table <-table[,c("subjectid","timestamp","sensorglucose","minutes","minutes_scaled")]
    #master <- ifelse(f==1,table,rbind(master,table))
    #master <- rbind(master,table)
    if (f==1) {master <- table}
    else {master <- rbind(master,table[-1,])}
    filename <- 
  ...
  masterfilename <- 
    base::paste(outputdirectory,"/","master.csv",sep = "")
  utils::write.csv(as.data.frame(master),file = masterfilename,row.names = FALSE)
  }