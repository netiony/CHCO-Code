...
demographics, pulmonary function, BMI, pulmonary exacerbations, microbiology, medications, comorbidities
```{r ttests data manip, include=FALSE}
# combine data into one set
test = full_join(cfrd_prevalence, CFF19_Annualized, by = c("eDWID", "reviewyear" = "ReviewYear")) %>% distinct()
test = full_join(CFF19_Demog, test) %>% distinct
# test = full_join(test, CFF19_CareEpisodes) %>% distinct()

# demographics:
# interested in birth/death year?, cause of death? race/ethn, socioeconimic, smoking, parental heights, preg status
test_set = test %>% select(eDWID. reviewyear, Race1:Race6, Hispanicrace, Primarycauseofdeath, patient_education:pregnancy_outcome, smoking:smoking_household,family_income, A_bmivalue, MotherHeight, FatherHeight, Diagnosis_year, MutClass, # diagnosis
  A_FEV1, A_FVC,  A_bmipercentile, NumPulmExacerbation,  # pulm function
  A_mycobacterials1:A_mycobacterials10,A_medscurrentepisode1:A_medscurrentepisode7, # meds?
  )
```
```{r list of variables}
# diagnosis: year/lenght of cf diagnosis?, genotype/class

# clinic visits: siteid, # clinic visits/yr, pulm exacerbation freq, PE severity?

# clinical measurements:
#anthropometric measures, pulm function tests, cbc, lfts, hba1c, fecal elastase, random glucose, fasting glucose, ogtt glucose, dxa scan

# microbiology: bacterial culture, myco bacteria culture, aspergillus or other fungal

# treatments: 
# antibiotics, pulm meds(inc oral and inhaled steroids), cftr mods, pancreatic enzymes(inc lipase units/day), supp feedings, oxygen, airway cleaning status, organ transplant status

# complications: cfrd treatment(oral agent, insulin), microvascular complc, liver disease, arthritis, bone frac, osteoporosis, osteopenia, allergic bronchopulmonary aspergilliosis, asthma, distal intestinal obs syndrome, pancreatitis, reflux, dpression, sinus disease, hypertension, kidneystones, renal failure

# care episodes: hosps, duration hosps, reason
